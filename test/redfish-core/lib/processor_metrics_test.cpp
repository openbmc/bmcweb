...
// handleCoreVoltageSensors' success path dispatches async D-Bus calls through
// the null unit-test systemBus, so only its error/early-return branches are
// covered here; the terminal success is in the afterGet* test above.
