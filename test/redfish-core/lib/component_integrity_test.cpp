...
{
    auto result = dbusVerificationStatusToRedfish("xyz.openbmc_project.Unknown");
    EXPECT_FALSE(result.has_value());
}
...
TEST(FillComponentIntegrityProperties, AllPropertiesPopulated)
{
    ...
    properties.emplace_back(
        "TypeVersion",
        dbus::utility::DbusVariantType(std::string("1.2.0")));
    properties.emplace_back("LastUpdated",
                            dbus::utility::DbusVariantType(uint64_t(1700000000000)));
...
TEST(FillComponentIntegrityProperties, UnknownTypeNotPopulated)
{
    ...
    properties.emplace_back(
        "Type",
        dbus::utility::DbusVariantType(std::string("xyz.invalid.Type")));
