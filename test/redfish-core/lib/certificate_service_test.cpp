...
TEST(CertificateServiceTest, isValidEmail)
{
    ...
    EXPECT_FALSE(isValidEmail(
        "test@example.com#")); // special character after domain name
    EXPECT_FALSE(
        isValidEmail("test@example..com")); // consecutive dots in domain part
    EXPECT_FALSE(
        isValidEmail("test@example.com.")); // trailing dot after domain name
}
