...
TEST(LogServicesBMCJouralTest, LogServicesBMCJouralGetReturnsError)
{
    ...
    EXPECT_FALSE(getTimestampFromID(shareAsyncResp, "", bootIDOut, timestampOut,
                                    indexOut));
    EXPECT_FALSE(getTimestampFromID(shareAsyncResp, "1701604800002075",
                                    bootIDOut, timestampOut, indexOut));
    EXPECT_FALSE(getTimestampFromID(shareAsyncResp, "1701604800002075_1",
                                    bootIDOut, timestampOut, indexOut));
    ...
    // obtain a goodBootID
    EXPECT_GE(sd_id128_from_string(bootIDStr.c_str(), &goodBootID));
    ...
    EXPECT_TRUE(getTimestampFromID(
        ...
        bootIDOut, timestampOut, indexOut));
    EXPECT_NE(sd_id128_equal(goodBootID, bootIDOut));
