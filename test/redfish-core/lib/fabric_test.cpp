...
TEST(AfterGetSwitchPowerState, ErrorOmitsPowerState)
{
...
TEST(AfterGetSwitchPowerState, MappedTokenSetsPowerState)
{
    auto response = std::make_shared<bmcweb::AsyncResp>();
    boost::system::error_code ec;
    ...
    EXPECT_EQ(response->res.result(), boost::beast::http::status::ok);
    EXPECT_EQ(response->res.jsonValue["PowerState"], "On");
}
