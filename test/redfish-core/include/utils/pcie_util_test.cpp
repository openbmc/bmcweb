...
TEST(PcieUtil, InterfacePropertiesUnknownMaxGenerationOmitted)
{
    auto resp = std::make_shared<bmcweb::AsyncResp>();
    dbus::utility::DBusPropertiesMap properties = {
        {"GenerationSupported",
    ...
    EXPECT_TRUE(addPcieInterfaceProperties(
        resp, nlohmann::json::json_pointer("/PCIeInterface"), properties));
    EXPECT_EQ(resp->res.result(), boost::beast::http::status::ok);
    EXPECT_FALSE(resp->res.jsonValue["PCIeInterface"].contains("MaxPCIeType"));
}

TEST(PcieUtil, InterfacePropertiesInvalidMaxGenerationFails)
{
    auto resp = std::make_shared<bmcweb::AsyncResp>();
    dbus::utility::DBusPropertiesMap properties = {
        {"GenerationSupported", std::string("garbage")},
    };
...
TEST(PcieUtil, PortPropertiesInvalidPortTypeFails)
{
    auto resp = std::make_shared<bmcweb::AsyncResp>();
    boost::system::error_code ec;
    dbus::utility::DBusPropertiesMap properties = {
        {"PortType", std::string("garbage")},
    };
...
TEST(PcieUtil, PortPropertiesInvalidPortProtocolFails)
{
    auto resp = std::make_shared<bmcweb::AsyncResp>();
    boost::system::error_code ec;
    dbus::utility::DBusPropertiesMap properties = {
        {"PortProtocol", std::string("garbage")},
    };
