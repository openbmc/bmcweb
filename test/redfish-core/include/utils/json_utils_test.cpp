...
{
    crow::Response res;
    nlohmann::json jsonRequest = {{"integer", "wrong_type"}, // Wrong type
                                  {"string", "hello"},
    ...
    ASSERT_FALSE(
        readJsonSub(jsonRequest, res, "integer", integer, "string", str));
    EXPECT_EQ(res.result(), boost::beast::http::status::bad_request);
...
{
    crow::Response res;
    nlohmann::json jsonRequest = {{"present_field", 42},
                                  {"extra_field", "ignored"}};
