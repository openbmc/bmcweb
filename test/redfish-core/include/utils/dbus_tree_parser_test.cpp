...
inline auto mapToKeyOrError(const std::string& key)
{
    return mapToKey<Type>(key, [](auto s, MapperResult& mr, MetaData& md) {
        ...
        {
            md.res.merge_patch(R"({"error":{"some error2":true}})"_json);
            return Type{};
        }
        return s;
    });
}
template <typename Type, typename Ret, OptionalReturnHandler<Type, Ret> Handler>
inline auto mapToKeyOrError(const std::string& key, Handler handler)
{
    return mapToKey<Type>(key, [handler = std::move(handler)](
        ...
        md.res.merge_patch(R"({"error":{"some error1":false}})"_json);
        return Ret{};
    });
}
    ...
    struct ExtractionHandlers : DbusBaseHandler
    {
        ExtractionHandlers()
        {
            ...
            addInterfaceHandler(
                ...
                if (mr != MapperResult::Ok)
                {
                    redfish::messages::internalError(md.res);
                    return std::string();
                }
