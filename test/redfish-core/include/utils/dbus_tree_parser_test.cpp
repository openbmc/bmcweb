    ...
    struct ExtractionHandlers : DbusBaseHandler
    {
        ExtractionHandlers()
        {
            ...
            addInterfaceHandler(
                ...
                "AuthDeviceNumber",
                mapToKey<uint32_t>("AuthDeviceNumber",
                                   [](auto s, MapperResult& mr, MetaData& md) {
                if (mr != MapperResult::Ok)
                {
                    md.res.merge_patch(R"({"error":["some error2"]})"_json);
                    return s;
                }
            ...
            addInterfaceHandler(
                ...
                if (mr != MapperResult::Ok)
                {
                    md.res.merge_patch(R"({"error":["some error1"]})"_json);
                    return std::string();
                }
...
TEST(TestParserNegative, ParsetWithErrorMessage)
{
    ...
    bool errorfound = false;
    parser.parse([&errorfound](DbusParserStatus status, auto&&) {
