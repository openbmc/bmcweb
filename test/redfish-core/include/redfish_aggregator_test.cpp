...
// Fixing should only occur for 2XX and 404 codes.
void assertProcessResponse(unsigned result)
{
    ...
    if (((result >= 200) && (result < 300)) || (result == 404))
    {
    ...
    {
        EXPECT_EQ(asyncResp->res.jsonValue.size(), 0);
        EXPECT_EQ(asyncResp->res.body(),
...
TEST(processResponse, InvalidResponseCodes)
{
    assertProcessResponse(199);
