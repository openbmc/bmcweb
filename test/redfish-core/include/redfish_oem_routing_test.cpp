...
TEST(OemRouter, PatchHandlerWithJsonObject)
{
    ...
    router.newOemRule<crow::utility::getParameterTag(fragment1)>(std::string(fragment1))
        .setPatchHandler(patchCallback1);
    constexpr std::string_view fragment2 = "/foo/<str>#Oem/OemBar";
    router.newOemRule<crow::utility::getParameterTag(fragment2)>(std::string(fragment2))
