...
// Fake-HMC completion handler: on a successful accept, sends a garbage
// header, closes the connection once the write completes, and re-arms
// doAccept for the next connection.
void handleGarbageAcceptCompletion(
    boost::system::error_code ec,
    const std::shared_ptr<boost::asio::ip::tcp::socket>& sock,
    int& connectionCount, const std::function<void()>& doAccept)
{
    if (ec)
    {
        return;
    ...
    boost::asio::async_write(*sock, boost::asio::buffer(*garbage),
                             [sock, garbage](boost::system::error_code,
                                             std::size_t) { sock->close(); });
    doAccept();
}
...
TEST(HttpClientBug1, MalformedHmcHeader_CallbackOnceAtRetryExhaustion)
{
    ...
    doAccept = [&ioc, &acceptor, &connectionCount, &doAccept]() {
        auto sock = std::make_shared<boost::asio::ip::tcp::socket>(ioc);
        acceptor.async_accept(*sock, [&connectionCount, &doAccept,
                                      sock](boost::system::error_code ec) {
            handleGarbageAcceptCompletion(ec, sock, connectionCount, doAccept);
        });
    };
    doAccept();
    ...
    client.sendDataWithCallback(
        ...
        [&callbackCount, &callbackStatus, &acceptor, &ioc](Response& res) {
            ++callbackCount;
...
// Writes the fake-HMC body then drops the connection (closes both the
// socket and the acceptor), simulating a mid-transfer TCP drop.
void writeHmcBodyThenDropConnection(
    const std::shared_ptr<boost::asio::ip::tcp::socket>& sock,
    boost::asio::ip::tcp::acceptor& acceptor, size_t sentSize)
{
    auto body = std::make_shared<std::string>(sentSize, 'X');
    boost::asio::async_write(
        *sock, boost::asio::buffer(*body),
        [&acceptor, sock, body](boost::system::error_code, std::size_t) {
            sock->close();    // TCP drop mid-transfer
            acceptor.close(); // no further connections
        });
}

void handleTruncationAccept(
    boost::system::error_code ec,
    const std::shared_ptr<boost::asio::ip::tcp::socket>& sock,
    boost::asio::ip::tcp::acceptor& acceptor, size_t claimedSize,
    size_t sentSize)
{
    if (ec)
    {
    ...
    boost::asio::async_write(
        *sock, boost::asio::buffer(*hdr),
        [&acceptor, sock, hdr,
         sentSize](boost::system::error_code, std::size_t) {
            writeHmcBodyThenDropConnection(sock, acceptor, sentSize);
        });
}
...
TEST(HmcTruncation, TcpDropMidStream_PipeGivesShortEof)
{
    ...
    auto doHmcSession = [&ioc, &acceptor, claimedSize, sentSize]() {
        auto sock = std::make_shared<boost::asio::ip::tcp::socket>(ioc);
        acceptor.async_accept(*sock, [&acceptor, sock, claimedSize,
                                      sentSize](boost::system::error_code ec) {
            handleTruncationAccept(ec, sock, acceptor, claimedSize, sentSize);
        });
    };
    doHmcSession();
    ...
    client.sendDataWithCallback(
        ...
        boost::beast::http::verb::get,
        [&capturedFd](Response& res) { capturedFd = dupStreamFd(res); });
...
// Writes the fake-HMC body then closes only the acceptor, deliberately
// keeping the socket open to simulate an HMC that stalls mid-transfer.
void writeHmcBodyThenStall(
    const std::shared_ptr<boost::asio::ip::tcp::socket>& stallSock,
    boost::asio::ip::tcp::acceptor& acceptor, size_t sentSize)
{
    auto body = std::make_shared<std::string>(sentSize, 'X');
    boost::asio::async_write(
        *stallSock, boost::asio::buffer(*body),
        [&acceptor, stallSock, body](boost::system::error_code, std::size_t) {
            acceptor.close();
        });
}

void handleStallAccept(
    boost::system::error_code ec,
    const std::shared_ptr<boost::asio::ip::tcp::socket>& stallSock,
    boost::asio::ip::tcp::acceptor& acceptor, size_t claimedSize,
    size_t sentSize)
{
    ...
    boost::asio::async_write(
        *stallSock, boost::asio::buffer(*hdr),
        [&acceptor, stallSock, hdr,
         sentSize](boost::system::error_code, std::size_t) {
            writeHmcBodyThenStall(stallSock, acceptor, sentSize);
        });
}
...
TEST(HmcTruncation, DISABLED_StallTimer120s_Integration)
{
    ...
    acceptor.async_accept(*stallSock, [&acceptor, stallSock, claimedSize,
                                       sentSize](boost::system::error_code ec) {
        handleStallAccept(ec, stallSock, acceptor, claimedSize, sentSize);
    });
...
void handleBufferedPathAccept(
    boost::system::error_code ec,
    const std::shared_ptr<boost::asio::ip::tcp::socket>& sock,
    boost::asio::ip::tcp::acceptor& acceptor, const std::string& jsonBody)
{
    if (ec)
    {
        return;
...
TEST(HmcBufferedPath, Http404Json_RoutedToBufferedPath_NoPipe)
{
    ...
    auto sock = std::make_shared<boost::asio::ip::tcp::socket>(ioc);
    acceptor.async_accept(*sock, [&acceptor, sock,
                                  &jsonBody](boost::system::error_code ec) {
        handleBufferedPathAccept(ec, sock, acceptor, jsonBody);
    });
    ...
    client.sendDataWithCallback(
        "", destUrl, ensuressl::VerifyCertificate::NoVerify, hdrs,
        boost::beast::http::verb::get,
        [&callbackCount, &callbackStatus, &callbackBody, &pipeOpened, &acceptor,
         &ioc](Response& res) {
            ++callbackCount;
            callbackStatus = res.result();
