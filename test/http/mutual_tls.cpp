...
class MtlsHandshake
{
  ...
  public:
    ...
    void init(X509* clientCert)
    {
        ...
        ASSERT_THAT(sn, NotNull());
        std::array<unsigned char, 7> serverCN = {'s', 'e', 'r', 'v',
                                                 'e', 'r', '\0'};
        ASSERT_EQ(X509_NAME_add_entry_by_txt(sn, "CN", MBSTRING_ASC,
                                             serverCN.data(), -1, -1, 0),
                  1);
        ASSERT_EQ(X509_set_issuer_name(serverCert.get(), sn), 1);
        ASSERT_EQ(ASN1_INTEGER_set(X509_get_serialNumber(serverCert.get()), 1),
                  1);
        ASSERT_THAT(X509_gmtime_adj(X509_get_notBefore(serverCert.get()), 0),
                    NotNull());
        ASSERT_THAT(X509_gmtime_adj(X509_get_notAfter(serverCert.get()),
                                    static_cast<long>(60 * 60)),
                    NotNull());
        serverCert.signWithKey(serverKey);

        ASSERT_EQ(SSL_CTX_use_certificate(serverCtx, serverCert.get()), 1);
        ...
        ASSERT_EQ(X509_STORE_add_cert(clientStore, serverCert.get()), 1);
