...
TEST(GetUPNFromCert, NonOthernameSubjectAlternativeName)
{
    ...
    ASSERT_EQ(x509.add1_ext_i2d(NID_subject_alt_name, gens), 1);
...
TEST(GetUPNFromCert, NonUPNSubjectAlternativeName)
{
    ...
    ASSERT_EQ(sk_GENERAL_NAME_push(gens.get(), gen), 1);
    ASSERT_EQ(
        X509_add1_ext_i2d(x509.get(), NID_subject_alt_name, gens.get(), 0, 0),
        1);
