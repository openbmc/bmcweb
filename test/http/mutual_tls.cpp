...
class MtlsHandshake
{
  ...
  public:
    ...
    void init(X509* clientCert)
    {
        ...
        const unsigned char* srvCN = reinterpret_cast<const unsigned char*>(
            "server");
        X509_NAME_add_entry_by_txt(sName, "CN", MBSTRING_ASC, srvCN, -1, -1,
                                   0);
...
TEST(GetUPNFromCert, NonUTF8UPNSubjectAlternativeName)
{
    ...
    ASN1_STRING_set(octetString, user.data(), static_cast<int>(user.size()));

    ASN1_TYPE_set1(value, V_ASN1_OCTET_STRING, octetString);
