...
void readHeader(boost::beast::http::serializer<false, bmcweb::FileBody>& sr)
{
    while (!sr.is_header_done())
    {
        ...
        sr.next(ec, [&sr](const boost::system::error_code& ec2, const auto& buffer) {
            ASSERT_FALSE(ec2);
...
std::string collectFromBuffers(const auto& buffer,
...
std::string readBody(boost::beast::http::serializer<false, bmcweb::FileBody>& sr)
{
    std::string ret;
    while (!sr.is_done())
    {
        boost::system::error_code ec;
        sr.next(ec, [&sr, &ret](const boost::system::error_code& ec2, const auto& buffer) {
            ASSERT_FALSE(ec2);
...
{
    auto& fb = boost::variant2::get<crow::Response::file_response>(res.response);
