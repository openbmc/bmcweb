                   ...
                   std::vector<std::pair<std::string, std::string>>& headers)
{
    ...
    while (!dataField.empty())
    {
        ...
        {
            EXPECT_EQ(inflater.end_headers(), 0);
            break;
        }
    }
...
TEST(http_connection, RequestPropogates)
{
    ...
    EXPECT_TRUE(outStr.starts_with(output));
    outStr.remove_prefix(output.size());
    // outStr.remove_suffix(9);
    std::vector<std::pair<std::string, std::string>> headers;
    unpackHeaders(outStr.substr(0, 0x346), headers);
    outStr.remove_prefix(0x346);
    ...
    EXPECT_THAT(
        ...
        UnorderedElementsAre(
            ...
            Pair("date", "TestTime")));
    // Data Frame, Length 15, Stream 1, End Stream flag set
    std::string_view expected2 =
        "\x00\x00\x0c\x00\x01\x00\x00\x00\x01StringOutput"sv;
