...
static std::shared_ptr<persistent_data::UserSession> performXtokenAuth(
...
static std::shared_ptr<persistent_data::UserSession> performCookieAuth(
...
static std::shared_ptr<persistent_data::UserSession> performTLSAuth(
    [[maybe_unused]] const crow::Request& req, Response& res,
    std::unordered_map<std::string, std::string_view>& headerValues,
    const std::weak_ptr<persistent_data::UserSession>& session)
{
    if (auto sp = session.lock())
    {
        ...
        if (headerValues["User-Agent"].empty())
        {
...
static bool isOnWhitelist(std::string_view url,
                          boost::beast::http::verb method)
{
...
static std::shared_ptr<persistent_data::UserSession> authenticate(
    crow::Request& req, Response& res, boost::beast::http::verb method,
    std::unordered_map<std::string, std::string_view>& headerValues,
    [[maybe_unused]] const std::weak_ptr<persistent_data::UserSession>& session)
{
    if (isOnWhitelist(req.url, method))
    {
        return nullptr;
    }
