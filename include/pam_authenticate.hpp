    ...
    for (size_t i = 0; i < msgCount; ++i)
    {
        ...
        switch (msg.msg_style)
        {
            ...
            case PAM_PROMPT_ECHO_OFF:
            {
                ...
                std::string totp_prompt = "Verification code";
                if (message.find(password_prompt) != std::string::npos)
                {
                    response.resp =
                        strdup(appPass->password.c_str()); // Password input
                }
                else if (message.find(totp_prompt) != std::string::npos)
                {
                    if (!appPass->token)
                    {
                        return PAM_CONV_ERR;
                             ...
                             const std::string& password)
{
    ...
    const struct pam_conv localConversation = {pamFunctionConversationPasswordChange,
                                               passStrNoConst};
    pam_handle_t* localAuthHandle = nullptr; // this gets set by pam_start
