                               ...
                               std::optional<std::string> token)
{
    ...
    PasswordData data;
    if (data.addPrompt("Password: ", password) == PAM_CONV_ERR)
    {
        return PAM_CONV_ERR;
    }
    if (token)
    {
        if (data.addPrompt("Verification code: ", *token) == PAM_CONV_ERR)
        {
            return PAM_CONV_ERR;
        }
    }
...
{
    PasswordData data;
    if (data.addPrompt("New password: ", password) == PAM_CONV_ERR)
    {
        return PAM_CONV_ERR;
    }
    if (data.addPrompt("Retype new password: ", password) == PAM_CONV_ERR)
    {
        return PAM_CONV_ERR;
    }
    const struct pam_conv localConversation = {pamFunctionConversation, &data};
