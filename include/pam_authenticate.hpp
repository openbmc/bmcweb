...
struct PamData
{
    ...
    PamData(PromptData data) : promptData(std::move(data)) {}
    ...
    int makeResponse(std::string_view prompt, pam_response& response)
    {
        ...
        if (promptDataIter == promptData.end() ||
            !promptDataIter->second.has_value())
        {
            return PAM_CONV_ERR;
        }
        response.resp = strdup(promptDataIter->second.value().data());
        return PAM_SUCCESS;
    }
    int validatePrompt(std::string_view prompt)
    {
                               ...
                               std::optional<std::string> token)
{
    ...
    PamData data(PamData::PromptData{
        {"Password", password}, {"Verification code", token}, {"TOTP", token}});
                             ...
                             const std::string& password)
{
    ...
    PamData data(PamData::PromptData{{"Password", password.c_str()}});
    const struct pam_conv localConversation = {pamFunctionConversation, &data};
    pam_handle_t* localAuthHandle = nullptr; // this gets set by pam_start
...
struct Totp
{
    static bool verify(std::string_view service, std::string_view username,
                       std::string_view t)
    {
        PamData::PromptData data{{"TOTP", t}, {"Verification code", t}};
