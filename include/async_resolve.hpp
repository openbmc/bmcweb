...
inline bool endpointFromResolveTuple(
    const std::tuple<int32_t, int32_t, std::vector<uint8_t>>& resolveList,
    const std::string_view port, boost::asio::ip::tcp::endpoint& endpoint)
{
    ...
    uint16_t portNum = 0;
    auto it = std::from_chars(port.data(), port.data() + port.size(), portNum);
    if (it.ec != std::errc())
    {
        ...
        crow::connections::systemBus->async_method_call(
            [port, handler{std::move(handler)}](
                         ...
                         resolveList : resp)
                {
                    ...
                    if (!endpointFromResolveTuple(resolveList, port, endpoint))
                    {
