...
inline ContentType getContentType(std::string_view contentTypeHeader)
{
    ...
    const auto typeCharset = +(bp::char_('a', 'z') | bp::char_('A', 'Z') |
                               bp::char_('0', '9') | bp::char_(".+-"));

    const auto parameters = *(
        bp::lit(';') >> *bp::ws >> typeCharset >> bp::lit('=') >> typeCharset);
    const auto parser = bp::no_case[knownMimeType] >> bp::omit[parameters];
    ...
    const std::span<const Encoding> availableEncodings)
{
    ...
    // A plain alternation of literals is used instead of bp::symbols to avoid
    // instantiating the (large) symbol trie.
    const auto knownAcceptEncoding =
    ...
    const auto parameters = *(bp::lit(';') >> bp::lit("q=") >> bp::uint_ >>
                              -(bp::lit('.') >> bp::uint_));
    const auto typeCharset =
        bp::char_('a', 'z') | bp::char_('A', 'Z') | bp::char_(".+-");
    // Unknown encodings produce NoMatch, which is ignored below
    const auto encodeType =
        knownAcceptEncoding |
        (bp::omit[+typeCharset] >> bp::attr(Encoding::NoMatch));
    const auto parser =
