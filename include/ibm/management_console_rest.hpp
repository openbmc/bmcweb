                           ...
                           const std::string& entryId)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, entryId](const boost::system::error_code ec,
            ...
            const std::string* status = std::get_if<std::string>(&certificate);
            if (status->empty())
            {
                BMCWEB_LOG_ERROR << "Invalid Certificate";
                asyncResp->res.result(
                    boost::beast::http::status::internal_server_error);
            ...
            asyncResp->res.jsonValue["Certificate"] = *status;
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, timeout](const boost::system::error_code ec,
            ...
            auto callback = [asyncResp, timeout,
                ...
                {
                    BMCWEB_LOG_ERROR << "Erasing match of entryId " << entryId;
                    ackMatches.erase(entryId);
                }
                getCSREntryAck(asyncResp, entryId);
            };

            std::string path = "/xyz/openbmc_project/certs/entry/" + entryId;
