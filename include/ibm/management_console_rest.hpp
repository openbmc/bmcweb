...
bool createMultipartFiles(crow::Response &res, const MimeEntity &mimeticObj)
{
    ...
    {
        BMCWEB_LOG_ERROR << "File Id empty. Skipping this file.";
    }
...
void handleFilePost(const crow::Request &req, crow::Response &res)
{
    ...
    // create a temporary file with session id as the file name, which will
    // become the input to the mimetic constructor
    std::string outFilePath = "/tmp/" + req.session->uniqueId;
    ...
    if (inputFile.is_open())
    {
        ...
        BMCWEB_LOG_DEBUG
            << "Creating files from a multipart request data of size: "
            << mimeticObj.size();
    ...
    {
        redfish::messages::internalError(res);
        BMCWEB_LOG_ERROR << "handleFilePost: The file used to create config "
                            "files is not found";
    }
}
