                          ...
                          const std::string& fileID)
{
    ...
    {
        BMCWEB_LOG_DEBUG
            << "Invalid content for PUT. Json format only accepted";
    ...
    if (std::filesystem::exists(loc))
    {
        ...
        // because there is no increase in the total directory size. We need to
        // add the diff only if the incoming data is larger than the existing
        // filesize
