                          ...
                          const std::string& fileID)
{
    ...
    BMCWEB_LOG_DEBUG << "data capacity : " << data.capacity();

    if (data.capacity() < minSaveareaFileSize)
    {
    ...
    if (std::filesystem::exists(loc))
    {
        ...
        if (data.capacity() < std::filesystem::file_size(loc))
        {
            newSizeToWrite = std::filesystem::file_size(loc) - data.capacity();
        }
        else
        {
            newSizeToWrite = data.capacity() - std::filesystem::file_size(loc);
        }
    }
    else
    {
        // This is a new file upload
        newSizeToWrite = data.capacity();
    }
            ...
            [](const crow::Request& req, crow::Response& res,
               const std::string& path) {
                ...
                if (std::filesystem::weakly_canonical(uploadPath) != uploadPath)
                {
                    ...
                    res.result(boost::beast::http::status::bad_request);
                    res.jsonValue["Description"] = "Bad file path in the url";
                    res.end();
                    return;
                }
                if (path.length() > 20)
