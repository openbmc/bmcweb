...
inline bool createSaveAreaPath(const std::shared_ptr<AsyncResp>& asyncResp)
{
...
inline void handleFileGet(const std::shared_ptr<AsyncResp>& asyncResp,
                          const std::string& fileID)
{
...
inline void handleFileDelete(const std::shared_ptr<AsyncResp>& asyncResp,
                             const std::string& fileID)
{
...
inline void handleAcquireLockAPI(const crow::Request& req,
                                 const std::shared_ptr<AsyncResp>& asyncResp,
...
inline void handleRelaseAllAPI(const crow::Request& req,
                               const std::shared_ptr<AsyncResp>& asyncResp)
{
...
inline void handleGetLockListAPI(const std::shared_ptr<AsyncResp>& asyncResp,
...
inline bool isValidConfigFileName(const std::string_view fileName)
{
...
inline void requestRoutes(App& app)
{
    ...
    BMCWEB_ROUTE(app, "/ibm/v1/Host/ConfigFiles/<path>")
        ...
        .methods(
            ...
            {
                BMCWEB_LOG_ERROR << "Unsupported character in filename: "
                                 << path;
                asyncResp->res.result(boost::beast::http::status::bad_request);
                asyncResp->res.jsonValue["Description"] =
            ...
            handleFileUrl(req, asyncResp, path);
        });
