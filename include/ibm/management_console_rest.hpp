...
constexpr int pollTimeout = 15; // 5 seconds
...
inline void deleteVMIDbusEntry(crow::Response& res, const std::string& entryID)
{
    ...
    crow::connections::systemBus->async_method_call(
        respHandler, "com.ibm.VMICertificate",
        "/com/ibm/VMICertificate/entry/" + entryID,
                           ...
                           const std::string& entryId)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, entryId](const boost::system::error_code ec,
            ...
            BMCWEB_LOG_DEBUG << *status;
            if (*status ==
                "com.ibm.VMICertificate.Entry.Acknowledgement.InProgress")
            {
                asyncResp->res.result(
                    boost::beast::http::status::internal_server_error);
                ...
                return;
                     ...
                     "com.ibm.VMICertificate.Entry.Acknowledgement.BadCSR")
            {
        ...
        "com.ibm.VMICertificate", "/com/ibm/VMICertificate/entry/" + entryId,
        "org.freedesktop.DBus.Properties", "Get",
        "com.ibm.VMICertificate.Entry", "hostAck");
}
        ...
        [asyncResp, req, csrString](const boost::system::error_code errorCode,
            ...
            crow::connections::systemBus->async_method_call(
                [asyncResp, req, csrString](const boost::system::error_code ec,
                    ...
                    std::string certPath =
                        CLIENT_CERT_PATH + std::to_string(entryId) + "/cert";
                    ...
                    auto wd = -1;
                    wd =
                        inotify_add_watch(fd, certPath.c_str(), IN_CLOSE_WRITE);
                    if (wd < 0)
                    {
                    ...
                    int rc = poll(&fds, 1, pollTimeout * 1000);
                    if (rc < 0)
                    {
                    ...
                    else if (rc == 0)
                    {
                        ...
                        inotify_rm_watch(fd, wd);
                        close(fd);
                        getCSREntryAck(asyncResp, std::to_string(entryId));
                        return;
                    }

                    std::ifstream inFile;
                    inFile.open(certPath);
                    if (inFile.fail())
                    {
                    ...
                    deleteVMIDbusEntry(asyncResp->res, std::to_string(entryId));
