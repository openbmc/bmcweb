...
inline void handleFilePut(const crow::Request &req,
  ...
  std::string_view path = "/var/lib/bmcweb/ibm-management-console/configfiles";
  ...
  if (fileExists) {
    ...
    if (ec) {
      asyncResp->res.result(boost::beast::http::status::internal_server_error);
      asyncResp->res.jsonValue["Description"] = internalServerError;
      BMCWEB_LOG_DEBUG << "handleIbmPut: Failed to find file size. ec : " << ec;
      return;
    }
  ...
  if (file.fail()) {
    ...
    asyncResp->res.result(boost::beast::http::status::internal_server_error);
    asyncResp->res.jsonValue["Description"] = "Error while creating the file";
    return;
  }
  file << data;
...
handleConfigFileList(const std::shared_ptr<bmcweb::AsyncResp> &asyncResp) {
  ...
  asyncResp->res.jsonValue["Actions"]["#IBMConfigFiles.DeleteAll"] = {
      {"target", "/ibm/v1/Host/ConfigFiles/Actions/IBMConfigFiles.DeleteAll"}};
}
...
handleAcquireLockAPI(const crow::Request &req,
  ...
  if (varAcquireLock.first) {
    ...
    auto validityStatus = std::get<std::pair<bool, int>>(varAcquireLock.second);
...
deleteVMIDbusEntry(const std::shared_ptr<bmcweb::AsyncResp> &asyncResp,
  ...
  auto respHandler = [asyncResp](const boost::system::error_code ec) {
    if (ec) {
      BMCWEB_LOG_ERROR << "deleteVMIDbusEntry respHandler got error " << ec;
      asyncResp->res.result(boost::beast::http::status::internal_server_error);
      return;
    }
  };
  ...
  crow::connections::systemBus->async_method_call(
      [asyncResp](const boost::system::error_code ec,
        ...
        if (cert->empty()) {
          ...
          asyncResp->res.result(
              boost::beast::http::status::internal_server_error);
          asyncResp->res.jsonValue["Description"] = "Empty Client Certificate";
          return;
        }
...
void handleCsrRequest(const std::shared_ptr<bmcweb::AsyncResp> &asyncResp,
  ...
  crow::connections::systemBus->async_method_call(
      [asyncResp, timeout](const boost::system::error_code ec,
        ...
        auto timeoutHandler = [asyncResp, timeout,
...
inline void requestRoutes(App &app) {
  ...
  BMCWEB_ROUTE(app, "/ibm/v1/Host/ConfigFiles/Actions/IBMConfigFiles.DeleteAll")
      .privileges({{"ConfigureComponents", "ConfigureManager"}})
  ...
  BMCWEB_ROUTE(app, "/ibm/v1/Host/Actions/SignCSR")
      ...
      .methods(boost::beast::http::verb::post)(
          [](const crow::Request &req,
            ...
            if (!redfish::json_util::readJson(req, asyncResp->res, "CsrString",
                                              csrString)) {
              return;
            }
