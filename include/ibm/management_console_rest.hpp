...
bool createMultipartFiles(const MimeEntity &mimeticObj)
{
    ...
    for (auto BodyItr : parts)
    {
        if (createMultipartFiles(*BodyItr))
        {
            continue;
        }
        else
...
{
    BMCWEB_LOG_DEBUG
        << "handleFilePost: Request for multipart upload of save-area files";
    if (!createSaveAreaPath(res))
    {
        redfish::messages::internalError(res);
        BMCWEB_LOG_ERROR << "handleFilePost: Creation of save-area path failed";
        return;
    }
    ...
    outFile << data;
    outFile.close();
    std::ifstream inputFile("/tmp/tmp.txt");
    ...
    {
        redfish::messages::internalError(res);
        BMCWEB_LOG_ERROR << "handleFilePost: Failed to create save-area files";
    }
    inputFile.close();
                          ...
                          const std::string &fileID)
{
    ...
    if (req.method() == "POST"_method)
    {
        ...
        {
            BMCWEB_LOG_ERROR << "handleFileUrl: Invalid Path";
            redfish::messages::resourceNotFound(res, "FileID in the URL",
                                                fileID);
        }
        res.end();
        return;
