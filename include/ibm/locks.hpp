...
class Lock
{
  ...
  protected:
     ...
     * Returns : False (if not conflicting)
     */
    virtual bool isConflictRecord(const LockRequest& refLockRecord1, const LockRequest& refLockRecord2);
    ...
    static bool checkByte(uint64_t resourceId1, uint64_t resourceId2, uint32_t position);
