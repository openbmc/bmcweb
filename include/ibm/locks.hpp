...
class Lock
{
  ...
  private:
    bool createPersistentLockFilePath()
    {
        ...
        {
            BMCWEB_LOG_DEBUG
                << "Failed to prepare persistent lock file directory. ec : "
                << ec;
            return false;
        }
        return true;
    ...
    RcRelaseLock isItMyLock(const ListOfTransactionIds &refRids,
    ...
    bool validateRids(const ListOfTransactionIds &refRids)
    {
  ...
  public:
    ...
    RcGetLockList getLockList(const ListOfSessionIds &listSessionId)
    {
        ...
        // given session id or else return an empty list
        return lockList;
    }
