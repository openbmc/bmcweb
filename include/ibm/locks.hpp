...
inline bool Lock::checkByte(uint64_t resourceId1, uint64_t resourceId2,
                            uint32_t position)
{
    uint8_t* res1 = new uint8_t[LockResouceIdSizeInBytes];
    uint8_t* res2 = new uint8_t[LockResouceIdSizeInBytes];
    std::memset(res1, 0, LockResouceIdSizeInBytes);
    std::memset(res2, 0, LockResouceIdSizeInBytes);
    std::memcpy(res1, &resourceId1, LockResouceIdSizeInBytes);
    std::memcpy(res2, &resourceId2, LockResouceIdSizeInBytes);

    uint8_t pPosition = 0;
    uint8_t qPosition = 0;
    if (position < LockResouceIdSizeInBytes)
    {
