...
inline bool Lock::checkByte(std::array<uint8_t, 8> resourceId1, std::array<uint8_t, 8> resourceId2,
                            uint32_t position)
{

    BMCWEB_LOG_ERROR << "checkByte";
    std::array<uint8_t,8> res1;
    std::array<uint8_t,8> res2;

    std::memcpy(res1.data(), &resourceId1, sizeof(resourceId1));
    std::memcpy(res2.data(), &resourceId2, sizeof(resourceId2));
    BMCWEB_LOG_ERROR << "comparing resource ids";
