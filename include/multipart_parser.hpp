...
const char* ParserErrCategory::name() const noexcept
{
...
std::error_code make_error_code(ParserError e)
{
    return {static_cast<int>(e), parserErrCategory};
}

namespace std
{
template <>
struct is_error_code_enum<ParserError> : true_type
{};
} // namespace std

std::error_code make_error_code(ParserError);
...
class MultipartParser
{
  private:
    ...
    static constexpr int part_boundary = 1;
    static constexpr int last_boundary = 2;
                         ...
                         State& state, int& flags, std::error_code& ec)
    {
        ...
        if (index > 0)
        {
            ...
            {
                ec = ParserError::ERROR_OVERFLOW;
                return;
            }
            if (index == 0)
            {
                ec = ParserError::ERROR_UNDERFLOW;
                return;
            }
  ...
  public:
    MultipartParser(crow::Request& reqIn, std::error_code& ec) : req(reqIn)
    {
        ...
        {
            ec = ParserError::ERROR_BOUNDARY_FORMAT;
            return;
        }

        std::string_view ctBoundary =
            contentType.substr(sizeof(boundaryFormat) - 1);
            ...
            switch (state)
            {
                ...
                case State::START_BOUNDARY:
                    if (index == boundary.size() - 2)
                    {
                        if (c != cr)
                        {
                            ec = ParserError::ERROR_BOUNDARY_CR;
                            return i;
                        }
                        index++;
                        break;
                    ...
                    else if (index - 1 == boundary.size() - 2)
                    {
                        if (c != lf)
                        {
                            ec = ParserError::ERROR_BOUNDARY_LF;
                            return i;
                        }
                        index = 0;
                    ...
                    {
                        ec = ParserError::ERROR_BOUNDARY_DATA;
                ...
                case State::HEADER_FIELD:
                    ...
                    if (c == colon)
                    {
                        ...
                        {
                            // empty header field
                            ec = ParserError::ERROR_EMPTY_HEADER;
                            return i;
                        }
                ...
                case State::HEADER_VALUE_ALMOST_DONE:
                    if (c != lf)
                    {
                        ec = ParserError::ERROR_HEADER_VALUE;
                        return i;
                    }
                ...
                case State::HEADERS_ALMOST_DONE:
                    if (c != lf)
                    {
                        ec = ParserError::ERROR_HEADER_ENDING;
                        return i;
                    }
