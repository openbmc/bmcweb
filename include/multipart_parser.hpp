...
#include "temporary_releasable_file_handle.hpp"
...
struct FormPart
{
    ...
    std::variant<std::string, TemporaryReleasableFileHandle> content{
        std::string{}};

    std::string_view contentString() const
    {
        const std::string* s = std::get_if<std::string>(&content);
        return s ? std::string_view(*s) : std::string_view{};
    }
};

static bool isBinaryPart(const boost::beast::http::fields& f)
{
    auto it = f.find("Content-Disposition");
    if (it == f.end())
        return false;

    std::string_view cd = it->value();
    return cd.find("filename=") != std::string_view::npos ||
           cd.find("name=\"UpdateFile\"") != std::string_view::npos;
}
...
class MultipartParser
{
  public:
    ...
    ParserError finish()
    {
        ...
        // Convert any string content(<1 MB) to files
        for (auto& field : mime_fields)
        {
            if (!isBinaryPart(field.fields))
            {
                continue;
            }

            if (auto* str = std::get_if<std::string>(&field.content))
            {
                if (str->empty())
                {
                    continue;
                ...
                TemporaryReleasableFileHandle tmp;
                boost::system::error_code ec;

                tmp.file.write(str->data(), str->size(), ec);
                if (ec)
                {
                    BMCWEB_LOG_ERROR(
                        "Failed to flush in-memory multipart data: {}",
                        ec.message());
                    state = State::ERROR;
    ...
    ParserError parsePart(std::string_view buffer)
    {
        ...
        if (state == State::PART_DATA)
        {
            ...
            {
                writeData(buffer.substr(partDataMark, flushEnd - partDataMark));
            }
            partDataMark = 0;
        }
  ...
  private:
    ...
    void writeData(std::string_view buffer)
    {
        constexpr std::size_t inMemoryLimit = 1 * 1024 * 1024; // 1 MB

        auto& content = mime_fields.back().content;

        if (auto* str = std::get_if<std::string>(&content))
        {
            if (str->size() + buffer.size() > inMemoryLimit)
            {
                TemporaryReleasableFileHandle tmp;
                boost::system::error_code ec;
                ...
                if (!ec)
                {
                    tmp.file.write(buffer.data(), buffer.size(), ec);
                }
                if (ec)
                {
        ...
        if (auto* file = std::get_if<TemporaryReleasableFileHandle>(&content))
        {
            boost::system::error_code ec;
            file->file.write(buffer.data(), buffer.size(), ec);
            if (ec)
            {
                BMCWEB_LOG_ERROR("Failed to write multipart data: {}",
                                 ec.message());
    ...
    ParserError processPartData(std::string_view buffer, size_t& i, char c)
    {
        ...
        if (index < boundary.size())
        {
            if (boundary[index] == c)
            {
                if (index == 0)
                {
                    ...
                    writeData(std::string_view(start, size));
                    partDataMark = i;
                }
                index++;
            }
        ...
        else if (prevIndex > 0)
        {
            ...
            writeData(lookbehind.substr(0, prevIndex));
            partDataMark = i;
