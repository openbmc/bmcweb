...
class Middleware
{
    ...
    static constexpr sd_id128_t bmcUuidAppId = SD_ID128_MAKE(
        d0, 39, cd, 88, 79, a8, 48, 98, 8f, 8d, 0a, 52, 6f, 7a, 15, 2c);
    int jsonRevision = 1;
  ...
  public:
    ...
    void readData()
    {
        ...
        if (systemUuid.empty())
        {
            ...
            sd_id128_to_string(bmcUuid, bmcUuidCstr);
            systemUuid = std::string(&bmcUuidCstr[0], 8) + '-' +
                         std::string(&bmcUuidCstr[8], 4) + '-' +
                         std::string(&bmcUuidCstr[12], 4) + '-' +
                         std::string(&bmcUuidCstr[16], 4) + '-' +
                         std::string(&bmcUuidCstr[20], 12);
            needWrite = true;
        }
