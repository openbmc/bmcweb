...
class Handler : public std::enable_shared_from_this<Handler>
{
  public:
    Handler(boost::asio::io_context& ios, const std::string& entryIDIn,
        ...
        unixSocketPath(unixSocketPathIn), peerSocket(ios), dumpSize(0)
    {}
...
inline void requestRoutes(App& app)
{
    BMCWEB_ROUTE(app,
        ...
        .onopen([](crow::streamsocket::Connection& conn) {
            ...
            try
            {
                handlers[&conn]->acceptor =
                    new boost::asio::local::stream_protocol::acceptor(
                        *ioCon, unixSocketPath);
            }
            catch (std::exception& e)
            {
                BMCWEB_LOG_DEBUG << "UNIX socket path is already in use "
                                 << e.what();
                handlers[&conn]->connection->sendStreamErrorStatus(
                    boost::beast::http::status::service_unavailable);
                handlers[&conn]->connection->close();
                return;
            }
