...
inline void requestRoutes(App& app)
{
    BMCWEB_ROUTE(app,
        ...
        .onopen([](crow::streamsocket::Connection& conn) {
            if (handler != nullptr)
            {
                BMCWEB_LOG_DEBUG << "Handler is not null ptr";
                conn.close();
                return;
            }
            ...
            boost::asio::io_context* io_con = conn.getIoContext();

            std::string unixSocketPath =
                unixSocketPathDir + dumpType + "_dump_" + dumpId;

            ::unlink(unixSocketPath.c_str());
            handler = std::make_shared<Handler>(*io_con, dumpId, dumpType,
                                                unixSocketPath);
            handler->connection = &conn;

            handler->getDumpSize(dumpId, dumpType);

            handler->acceptSocketConnection();
        })
        .onclose([](crow::streamsocket::Connection&) {
            if (handler->connection)
            {
                ::unlink(handler->unixSocketPath.c_str());
                std::remove(handler->unixSocketPath.c_str());

                handler->connection = nullptr;
