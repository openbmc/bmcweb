  ...
  public:
    ...
    void waitForMessageOnSocket()
    {
        ...
        boost::asio::async_read(
            *stream, inputBuffer->prepare(bytes),
            ...
            [&](const boost::system::error_code& ec,
                ...
                if (negotiation_done == false)
                {
                    ...
                    char* buf = static_cast<char*>(
                        malloc((sizeof(char) * bytes_transferred)));
                    memset(buf, 0, bytes_transferred);

                    boost::asio::buffer_copy(
                        boost::asio::buffer(buf, bytes_transferred),
                        inputBuffer->data());
                    std::string reply_magic("gDf");
                    std::string reply_string(buf, bytes_transferred);
            ...
            [this, self(shared_from_this())](boost::beast::error_code ec,
                ...
                if (negotiation_done == false)
                {
                    ...
                    boost::asio::buffer_copy(
                        ...
                        inputBuffer->data());
                    std::string reply_magic("gDf");
                    BMCWEB_LOG_DEBUG << "checking reply magic";
                    std::string reply_string(buf, bytesWritten);
                    std::size_t found = reply_string.find(reply_magic);
                    if (found != std::string::npos)
                    {
                        std::cout << "found reply magic " << found << '\n';
                        negotiation_done = true;
                        writeonnbd = true;
                    }
                    free(buf);
...
template <typename... Middlewares> void requestRoutes(Crow<Middlewares...>& app)
{
    ...
    BMCWEB_ROUTE(app, "/dump/0/1")
        .methods("GET"_method)(
            [](const crow::Request& req, crow::Response& res) {
                BMCWEB_LOG_DEBUG << "calling dump offload handler";

                handleDumpOffloadUrl(req, res);
            });
}
