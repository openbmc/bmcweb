...
inline std::shared_ptr<persistent_data::UserSession> performCookieAuth(
    Response& res, boost::beast::http::verb method [[maybe_unused]],
    ...
    const bool& supportBootStrapAuth)
{
    ...
    for (auto it = cookies.first; it != cookies.second; it++)
    {
        ...
        if (!checkBootStrapAuth(sessionOut->username, supportBootStrapAuth))
        {
            res.result(boost::beast::http::status::unauthorized);
            res.jsonValue["Description"] = std::format(
                "The user {} is unauthorized.", sessionOut->username);
            return nullptr;
        }
    ...
    const AuthMode& httpAuthMode)
{
    ...
    if constexpr (BMCWEB_COOKIE_AUTH)
    {
        ...
        {
            sessionOut =
                performCookieAuth(res, method, reqHeader, supportBootStrapCred);
        }
    }
