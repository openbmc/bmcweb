...
// Remove a connection from the sessions if exist and return true
// if session is removed.
inline bool removeConnectionFromMap(crow::websocket::Connection& conn)
{
                                    ...
                                    const std::vector<uint8_t>& socketNameBytes)
{
    if (ec)
    {
        ...
        if (removeConnectionFromMap(conn))
        {
