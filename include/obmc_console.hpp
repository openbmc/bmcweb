  ...
  BMCWEB_ROUTE(app, "/console0")
      ...
      .onopen([](crow::websocket::Connection& conn) {
        ...
        if (host_socket == nullptr) {
          ...
          // This is a hack.  For whatever reason boost local endpoint has a
          // check to see if a string is null terminated, and if it is, it drops
          // the path character count by 1.  For abstract sockets, we need the
          // count to be the full sizeof(s->sun_path) (ie 108), even though our
          // path _looks_ like it's null terminated.  This is likely a bug in
          // asio that needs to be submitted Todo(ed).  so the cheat here is to
          // break the abstraction for a minute, write a 1 to the last byte,
          // this causes the check at the end of resize here:
          // https://www.boost.org/doc/libs/1_68_0/boost/asio/local/detail/impl/endpoint.ipp
          // to not decrement us unesssesarily.
          struct sockaddr_un* s = reinterpret_cast<sockaddr_un*>(ep.data());
