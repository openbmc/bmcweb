...
struct InProgressActionData
{
    ...
    ~InProgressActionData()
    {
        ...
        //   * Otherwise if there was a processing failure - error

        if (!methodFailed && !methodPassed)
        {
            setErrorResponse(res, boost::beast::http::status::not_found,
                             methodNotFoundDesc, notFoundMsg);
        }
        else if (methodFailed && !methodPassed)
        {
            setErrorResponse(res,
                             boost::beast::http::status::internal_server_error,
                             "Method call failed", methodFailedMsg);
        }
        else if (methodPassed)
        {
            if (!methodResponse.empty() || (!outputFailed))
            {
...
int readStructFromMessage(const std::string &typeCode,
                          sdbusplus::message::message &m, nlohmann::json &data)
{
    auto containedTypes = typeCode.substr(1, typeCode.size() - 2);
    auto types = dbusArgSplit(containedTypes);

    auto r = sd_bus_message_enter_container(m.get(), SD_BUS_TYPE_STRUCT,
    ...
    for (const auto &type : types)
    {
        ...
        r = convertDBusToJSON(type, m, value);
        if (r < 0)
        {
            return r;
        }
...
int readVariantFromMessage(const std::string &typeCode,
                           sdbusplus::message::message &m, nlohmann::json &data)
{
    const char *containerType;
    auto r = sd_bus_message_peek_type(m.get(), NULL, &containerType);
    if (r < 0)
    {
                      ...
                      sdbusplus::message::message &m, nlohmann::json &response)
{
    ...
    for (const auto &typeCode : returnTypes)
    {
        ...
        {
            r = readStructFromMessage(typeCode, m, response);
            if (r < 0)
            {
                return r;
            }
        ...
        {
            r = readVariantFromMessage(typeCode, m, response);
            if (r < 0)
            {
                return r;
            }
