convertDBusToJSON // Copyright (c) 2018 Intel Corporation
//
    ...
    namespace crow
{
    ...
    const std::string forbiddenResDesc =
                ...
                else
                {
                    ...
                    while (node != nullptr)
                    {
                        ...
                        if (childPath != nullptr)
                        {
                            ...
                            introspectObjects(processName, newpath,
                                              transaction);
                        }
                                   ...
                                   std::shared_ptr<bmcweb::AsyncResp> asyncResp)
    {
        ...
        crow::connections::systemBus->async_method_call(
            [asyncResp, objectPath, service, interface](
                ...
                if (ec)
                {
                    BMCWEB_LOG_ERROR << "GetAll on path " << objectPath
        ...
        for (const auto &[path, interface_map] : *subtree)
        {
            ...
            if (dataJson.find(path) == dataJson.end())
            {
                for (const auto &[service, interfaces] : interface_map)
                {
                    ...
                    {
                        if (!boost::starts_with(interface,
        ...
        crow::connections::systemBus->async_method_call(
            [transaction, object_name,
             connection_name](const boost::system::error_code ec,
                ...
                if (ec)
                {
                    BMCWEB_LOG_ERROR << "GetManagedObjects on path "
                                     << object_name << " on connection "
                                     << connection_name << " failed with code "
                                     << ec;
                    return;
                }
        ...
        std::shared_ptr<InProgressEnumerateData> transaction)
    {
        ...
        crow::connections::systemBus->async_method_call(
            [transaction](const boost::system::error_code ec,
                ...
                BMCWEB_LOG_DEBUG << "Got " << connections.size()
                                 << " connections";
                ...
                for (const auto &connection : connections)
                {
                    ...
                    else
                    {
                        ...
                        findObjectManagerPathForEnumerate(
                            transaction->objectPath, connection.first,
                            transaction);
                    }
                }
            },
        ...
        ~InProgressActionData()
        {
            ...
            if (res.result() == boost::beast::http::status::ok)
            {
                if (!methodPassed)
                {
                    ...
                    {
                        setErrorResponse(
                            res, boost::beast::http::status::bad_request,
    ...
    {
        BMCWEB_LOG_DEBUG << "handleAction on path: " << objectPath
    ...
    {
        BMCWEB_LOG_DEBUG << "handleGet: " << objectPath
                         << " prop:" << destProperty;
            ...
            [&res, path, propertyName](const boost::system::error_code ec,
                         ...
                         connection : object_names)
                {
                    ...
                    for (const std::string &interface : interfaceNames)
                    {
                        ...
                        m.append(interface);
                        crow::connections::systemBus->async_send(
                            m, [&res, response, propertyName](
                                   const boost::system::error_code ec,
                                   sdbusplus::message::message &msg) {
                                if (ec)
                                {
                                    BMCWEB_LOG_ERROR
                                        << "Bad dbus request error: " << ec;
                                }
                                else
                                {
                                    nlohmann::json properties;
                                    int r = convertDBusToJSON("a{sv}", msg,
                                                              properties);
                                    if (r < 0)
                                    {
                                        BMCWEB_LOG_ERROR
                   ...
                   const std::string &destProperty)
    {
        ...
        nlohmann::json::const_iterator propertyIt =
                              ...
                              std::string &objectPath)
    {
        ...
        {
            destProperty = objectPath.substr(
                attrPosition + strlen(attrSeperator), objectPath.length());
        ...
        if (req.method() == "POST"_method)
        {
            ...
            if (actionPosition != objectPath.npos)
            {
                std::string postProperty = objectPath.substr(
                    (actionPosition + strlen(actionSeperator)),
                    objectPath.length());
        ...
        BMCWEB_ROUTE(app, "/bus/system/<str>/<path>")
            .methods("GET"_method,
                ...
                else
                {
                    ...
                    auto transaction =
                        std::make_shared<InProgressActionData>(res);
