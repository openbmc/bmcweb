...
struct NbdProxyServer : std::enable_shared_from_this<NbdProxyServer>
{
    NbdProxyServer(crow::websocket::Connection& conn,
                   std::pair<std::string, bool>& socketIdIn,
                   std::function<void()> close_handler) :
        socketId(socketIdIn),
        connection(&conn), closeHandler(std::move(close_handler)),
        acceptor(conn.getIoService(), stream_protocol::endpoint(socketId.first))
    {
        // Accquire socket
        socketId.second = false;
        do_accept();
    }
    ...
    void close()
    {
        ...
        if (peerSocket)
        {
            BMCWEB_LOG_DEBUG << "nbd-proxy: peerSocket->close()";
            peerSocket->close();
            peerSocket.reset();
        }
        connection = nullptr; // so nobody would use it anymore
        closeHandler();
        std::experimental::filesystem::remove(socketId.first);
        // return socket to the pool
        socketId.second = true;
    }
