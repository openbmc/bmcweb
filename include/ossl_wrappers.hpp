...
// Explicitly set the memory functions to avoid using the default malloc
// in cases where we're using sanitizers  The library code is unlikely
// to be sanitized.  Doing this explicitly doesn't hurt anything, so
// just do it in all cases.
class OpenSSLInit
{
  public:
    static void* malloc(size_t num, const char* /*file*/, int /*line*/)
    {
        // NOLINTNEXTLINE(cppcoreguidelines-no-malloc)
        return ::malloc(num);
    }
    static void* realloc(void* ptr, size_t num, const char* /*file*/,
                         int /*line*/)
    {
        // NOLINTNEXTLINE(cppcoreguidelines-no-malloc)
    ...
    OpenSSLInit()
    {
        if (CRYPTO_set_mem_functions(OpenSSLInit::malloc, OpenSSLInit::realloc,
                                     OpenSSLInit::free) != 1)
        {
