...
{
  public:
    OpenSSLASN1String(std::string_view data) :
        OpenSSLASN1String(ASN1_STRING_new())
    {
    ...
    {
        const unsigned char* data = ASN1_STRING_get0_data(ptr);
        int length = ASN1_STRING_length(ptr);
        return {std::bit_cast<const char*>(data), static_cast<size_t>(length)};
    }
...
struct OpenSSLGeneralName
{
  public:
    ...
    OpenSSLGeneralName(OpenSSLGeneralName&& other) noexcept : ptr(other.ptr)
    {
        other.ptr = nullptr;
    }
    OpenSSLGeneralName& operator=(const OpenSSLGeneralName&) = delete;
    OpenSSLGeneralName& operator=(OpenSSLGeneralName&& other) noexcept
    {
        if (this == &other)
        {
            return *this;
...
class OpenSSLGeneralNames
{
  ...
  public:
    ...
    {
        OpenSSLGeneralName temp(std::move(name));
        return sk_GENERAL_NAME_push(ptr, temp.release()) == 1;
    }
    ...
    class Iterator
    {
      ...
      private:
        ...
        Iterator(GENERAL_NAMES* namesIn, int indexIn) :
            names(namesIn), index(indexIn)
        {}
    ...
    Iterator end()
    {
        return {ptr, sk_GENERAL_NAME_num(ptr)};
    }
