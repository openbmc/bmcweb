...
class OpenSSLX509Extension
{
  public:
    ...
    OpenSSLX509Extension& operator=(OpenSSLX509Extension&& other) noexcept = delete;
...
class OpenSSLASN1String
{
  public:
    OpenSSLASN1String(std::string_view data) : ptr(ASN1_STRING_new()) {
        ...
        if (ASN1_STRING_set(ptr, data.data(), static_cast<int>(data.size())) != 1) {
    ...
    std::string_view getAsString()
    {
        return std::string_view(std::bit_cast<const char*>(ptr->data), static_cast<size_t>(ptr->length));
    }
...
class OpenSSLX509
{
  public:
    ...
    static std::optional<OpenSSLX509> fromPEMFile(const std::filesystem::path& filepath)
    {
...
struct OpenSSLGeneralName
{
    ...
    void set0Value(int type, OpenSSLASN1String& value)
    {
