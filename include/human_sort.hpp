...
bool simpleIsDigit(const char c)
{
    return c >= '0' && c <= '9';
}
...
int alphanumComp(const std::string_view left, const std::string_view right)
{
    enum class ModeType
    {
        STRING,
    ...
    while (l != left.end() && r != right.end())
    {
        if (mode == ModeType::STRING)
        {
            while (l != left.end() && r != right.end())
            {
                ...
                const bool l_digit = details::simpleIsDigit(*l);
                const bool r_digit = details::simpleIsDigit(*r);
...
template <class Ty>
struct alphanumLess : public std::binary_function<Ty, Ty, bool>
{
