  ...
  public:
    static const std::filesystem::path& getPath()
    {
        ...
        static const std::filesystem::path path =
            std::filesystem::path(stateDir ? stateDir : ".") / "bmcweb_persistent_data.json";
        return path;
    }
    ...
    void readData()
    {
        ...
        persistentFile.open(getPath().c_str(), boost::beast::file_mode::read, ec);
        ...
        if (!ec)
        {
            ...
            if (ec)
            {
                BMCWEB_LOG_CRITICAL("Can't get filesize of {}", getPath().c_str());
                return;
            }
            std::string str;
            ...
            if (ec)
            {
                BMCWEB_LOG_CRITICAL("Failed to read file {}", getPath().c_str());
                return;
            }
    ...
    void writeData()
    {
        ...
        boost::beast::file_posix persistentFile;
        boost::system::error_code ec;
        persistentFile.open(getPath().c_str(), boost::beast::file_mode::write, ec);
        if (ec)
        {
