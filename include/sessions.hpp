    ...
    void fromJson(const nlohmann::json::object_t& j)
    {
        for (const auto& element : j)
        {
            ...
            if (intValue != nullptr)
            {
                ...
                {
                    MTLSCommonNameParseMode tmpMTLSCommonNameParseMode = static_cast<MTLSCommonNameParseMode>(*intValue);
                    if (tmpMTLSCommonNameParseMode <= MTLSCommonNameParseMode::UserPrincipalName ||
                        tmpMTLSCommonNameParseMode == MTLSCommonNameParseMode::Meta)
                    {
