...
// TODO: move IncrementalParser some place so we don't need forward declaration
...
class PackageParser
{
  ...
  private:
    ...
    std::optional<std::vector<DescriptorMatcher>> parseDescriptors(
        IncrementalParser& parser, uint8_t count, uint16_t bitMapLength);
    bool parseComponents(IncrementalParser& p,
                         const std::vector<DescriptorMatcher>& matchers,
                         uint16_t imageCount);
