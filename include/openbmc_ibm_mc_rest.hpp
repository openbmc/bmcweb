                              ...
                              std::pair<stype,stype> ids)
{
    for (uint32_t i = 0; i < refrids->size(); i++)
    {
        ...
        std::string expectedsessionid = std::get<0>(locktable[refrids->at(i)][0]);

        if ((expectedclientid == ids.first) && (expectedsessionid==ids.second))
        {
        ...
        .methods(
            ...
            */
            ...
            else
            {
                ...
                // requesting HMC
                auto status =
                    lockobject.isitmylock(&listtransactionIDs, std::make_pair(clientid,sessionid));
                if (status.first)
                {
    ...
    BMCWEB_ROUTE(app, "/ibm/v1/HMC/LockService/GetLockList")
        ...
        .methods(
            ...
            */
            ...
            {
                res.result(boost::beast::http::status::bad_request);
                res.jsonValue["Description"] = "Session ID is Invalid or not present in locktable ";
