...
using rcgetlocklist = std::pair<bool,std::variant<std::string,std::vector<lockrecord>>>;
...
public:
 ...
 template <typename... Middlewares> friend void requestRoutes(Crow<Middlewares...> &app);
...
rcrelaselock lock::isitmylock(std::vector<uint32_t>* refrids,std::string clientid)
{
    for(uint32_t i=0; i< refrids->size();i++)
    {
...
bool lock::isvalidlockrecord(lockrecord* reflockrecord)
{
    ...
    if(!((boost::iequals(std::get<2>(*reflockrecord),"read") || (boost::iequals(std::get<2>(*reflockrecord),"write")))))
    {
    ...
    for (const auto& p : std::get<4>(*reflockrecord))
        {
            ...
            if(!( (boost::iequals(p.first,"locksame") || (boost::iequals(p.first,"lockall")) ||(boost::iequals(p.first,"dontlock")) )))
                 {
    ...
    else
    {
        ...
        // the entries in the lock table
            ...
            for(uint32_t i=0; i< reflockrequeststructure->size();i++)
            {
                for (auto it = locktable.begin(); it != locktable.end(); it++ )
                {
                    bool status = isconflictrecord(&reflockrequeststructure->at(i),&it->second);
                    if(status)
                    {
            ...
            // Reached here, so no conflict with the locktable, so we are safe to add the request
            // records into the lock table
...
bool lock::isconflictrequest(lockrequest* reflockrequeststructure)
{
    ...
    else
    {
        ...
        for (uint32_t i = 0; i < reflockrequeststructure->size(); i++)
        {
            ...
            {
                bool status = isconflictrecord(&reflockrequeststructure->at(i),&reflockrequeststructure->at(j));
...
bool lock::checkbyte(uint64_t resourceid1, uint64_t resourceid2, uint32_t j)
{
   ...
   // {
        BMCWEB_LOG_DEBUG << "Comparing bytes "<< std::to_string(p[j])<<","<< std::to_string(q[j]);
        if(p[j] != q[j])
        {
            return false;
        }
...
bool lock::isconflictrecord(lockrecord* reflockrecord1, lockrecord* reflockrecord2)
{
    // No conflict if both are read locks

    if(boost::iequals(std::get<2>(*reflockrecord1),"read") && boost::iequals(std::get<2>(*reflockrecord2),"read"))
    {
    ...
    for (const auto& p : std::get<4>(*reflockrecord1))
    {
        ...
        // return conflict when any of them is try to lock all resources under the current resource level.
        if(boost::iequals(p.first,"lockall") || boost::iequals(std::get<4>(*reflockrecord2)[i].first,"lockall"))
        {
        ...
        if((boost::iequals(p.first,"locksame") || boost::iequals(std::get<4>(*reflockrecord2)[i].first,"locksame")) &&
        ...
        // compare segment data
         ...
         {
             // if the segment data is different , then the locks is on a different resource
             // So no conflict between the lock records
             if(!(checkbyte(std::get<3>(*reflockrecord1),std::get<3>(*reflockrecord2),i)))
             {
                 return false;
             }
         }
        ...
        .methods(
            "POST"_method)([](const crow::Request &req, crow::Response &res) {
                ...
                for (auto element : body)
                {
                    ...
                    lockrequeststructure.push_back(make_tuple(req.session->uniqueId,"hmc-id",locktype,resourceid,seginfo));
                }
                ...
                for(uint32_t i=0;i< lockrequeststructure.size();i++)
                {
                    bool status = lockobject.isvalidlockrecord(&lockrequeststructure[i]);
                ...
                else
                {
                    ...
                    auto status = lockobject.isconflictwithtable(&lockrequeststructure);
