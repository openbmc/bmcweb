...
using rcgetlocklist = std::pair<bool, std::variant<std::string, std::vector<std::pair<uint32_t,lockrequest>>>>;
                              ...
                              std::string clientid)
{
    for (uint32_t i = 0; i < refrids->size(); i++)
    {
        ...
        std::string expectedclientid = std::get<1>(locktable[refrids->at(i)][0]);
...
rc lock::isconflictwithtable(lockrequest* reflockrequeststructure)
{
    ...
    if (locktable.empty())
    {
        ...
        //{
            ...
            //rid = getmyrequestid();
            locktable.emplace(std::pair<uint32_t,lockrequest>(transactionID,*reflockrequeststructure));
            //vrid.push_back(rid);
       // }
    ...
    else
    {
        ...
        for (uint32_t i = 0; i < reflockrequeststructure->size(); i++)
        {
            for (auto it = locktable.begin(); it != locktable.end(); it++)
            {
                ...
                {
                    bool status = isconflictrecord(&reflockrequeststructure->at(i),&it->second[j]);
                    if (status)
                    {
...
template <typename... Middlewares> void requestRoutes(Crow<Middlewares...> &app)
{
    ...
    BMCWEB_ROUTE(app, "/ibm/v1/HMC/locks/ReleaseLock")
        ...
        .methods(
            ...
            if (!redfish::json_util::readJson(req, res, "TransactionIDs", listtransactionIDs))
            {
                return;
            }
            ...
            */
            ...
            else
            {
                ...
                // requesting HMC
                auto status = lockobject.isitmylock(&listtransactionIDs, clientid);
                if (status.first)
                {
    ...
    BMCWEB_ROUTE(app, "/ibm/v1/HMC/locks/GetLockList")
        ...
        .methods("POST"_method)(
            [](const crow::Request &req, crow::Response &res) {
                ...
                if (!redfish::json_util::readJson(req, res, "SessionIDs", listSessionIDs))
                {
                    return;
                }
...
*/
                ...
                if (status.first)
                {
                    ...
                    auto var = std::get<std::vector<std::pair<uint32_t,lockrequest>>>(status.second);
                    ...
                    for (uint32_t i = 0; i < var.size(); i++)
                    {
                        for(uint32_t k =0;k< var[i].second.size();k++)
                        {
                            ...
                            returnjson["ResourceID"] = std::get<3>(var[i].second[k]);
