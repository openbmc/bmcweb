...
{
static std::vector<std::unique_ptr<sdbusplus::bus::match::match>> matches;
...
void registerHostStateChangeSignal()
{
    matches.emplace_back( std::make_unique<sdbusplus::bus::match::match>(
        *crow::connections::systemBus,
        ...
        "arg0='xyz.openbmc_project.State.Host'",HostStatePropertyChange));
}
...
void registerBMCStateChangeSignal()
{
    matches.emplace_back( std::make_unique<sdbusplus::bus::match::match>(
        *crow::connections::systemBus,
        ...
        "arg0='xyz.openbmc_project.State.BMC'",BMCStatePropertyChange));
}
...
void registerStateChangeSignal()
{
    if (matches.size())
    {
        ...
        BMCWEB_LOG_DEBUG << "Host/BMC state change signal already registered";
        return;
    }

    BMCWEB_LOG_DEBUG << "Host/BMC state change signal - Register";
