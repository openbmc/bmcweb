                      ...
                      const std::string& filepath)
{
    ...
    // BMCWEB_LOG_INFO << "Test fileEnd " << fileEnd << "\n";
    // lseek(fd, 0, SEEK_SET);
    // std::string buff(static_cast<size_t>(fileEnd), ' ');
    // BMCWEB_LOG_INFO << "Test buff.size() " << buff.size() << "\n";
    ...
    asyncResp->res.body() = std::move(str);
}
                           ...
                           const std::vector<std::string>& controllerList)
{
    ...
    crow::connections::systemBus->async_method_call(
        ...
        // NVMe actions
        asyncResp->res.jsonValue["Actions"]["#Nvme.Log"]["target"] =
...
// http://localhost:8080/google/v1/NVMeSubsystemController/test0/Actions/NVMe.Identify
...
// $ curl  -u root:0penBmc -X POST
// http://localhost:8080/google/v1/NVMeSubsystemController/test0/Actions/NVMe.Log
...
{
    BMCWEB_ROUTE(app,
                 "/google/v1/NVMe/<str>/controllers/<str>/Actions/NVMe.Log")
        .privileges({{"ConfigureManager"}})
        .methods(boost::beast::http::verb::post)(
