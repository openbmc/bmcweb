  ...
  protected:
    void validateRootOfTrustCollectionGet(crow::Response& res)
    {
        ...
        for (size_t i = 0; i < members.size(); ++i)
            EXPECT_EQ(members[i]["@odata.id"],
                      "/google/v1/RootOfTrustCollection/" + rootOfTrustIds_[i]);
    }
    ...
    void loadRoTSubTreeInMapperWithErrorCode(boost::system::errc::errc_t errc)
    {
        ...
        EXPECT_CALL(*mock_obj_mapper_,
                    getSubTree(EqualsAddr(objMapperGetSubtreeAddr_),
                               EqualsSubTreeParams(rotSubTreeSearchParams_),
                               ::testing::_))
            .Times(1)
            .WillOnce(::testing::DoAll(
                testing::InvokeArgument<2>(error_code, std::ref(rotSubTree_)),
                ::testing::Return()));
    }
                                              ...
                                              std::vector<uint8_t>& output)
    {
        ...
        EXPECT_CALL(*mock_hoth_iface_,
                    sendHostCommand(EqualsAddr(hothSendHostCommandAddr_),
        ...
        .WillOnce(::testing::DoAll(::testing::InvokeWithoutArgs([&]() {
            ...
            for (auto& rotId : rootOfTrustIds_)
                members.push_back(
                    {{"@odata.id",
                      "/google/v1/RootOfTrustCollection/" + rotId}});
            async_resp->res.jsonValue["Members@odata.count"] =
                rootOfTrustIds_.size();
        })));
...
TEST_F(GoogleServiceTest, RootOfTrustGetInternalError)
{
    ...
    auto async_resp = std::make_shared<bmcweb::AsyncResp>();
    getRootOfTrust(req, async_resp, "Foo");
    EXPECT_THAT(async_resp->res.jsonValue["error"]["code"],
...
TEST_F(GoogleServiceTest, RootOfTrustSendCommandSuccess)
{
    ...
    EXPECT_EQ(async_resp->res.jsonValue["CommandResponse"],
