...
inline void getRootOfTrustCollection(const crow::Request&,
                const std::shared_ptr<bmcweb::AsyncResp>& asyncResp) {
    asyncResp->res.jsonValue["@odata.id"] = ROT_COLLECTION_PREFIX;
    asyncResp->res.jsonValue["@odata.type"] = "#RootOfTrustCollection.RootOfTrustCollection";
    rf_utils->populateCollectionMembers(asyncResp, ROT_COLLECTION_PREFIX, std::vector<const char*>{HOTH_INTERFACE}, HOTH_SEARCH_PATH);
}

using ResolvedEntityHandler = std::function<void(const crow::Request&, const std::shared_ptr<bmcweb::AsyncResp>&, const ResolvedEntity&)>;

inline void resolveRoT(const crow::Request& req,
            const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
            const std::string& rotId,
            ResolvedEntityHandler&& entityHandler) {
    auto validateFunc = [&req, asyncResp, rotId, entityHandler{std::forward<ResolvedEntityHandler>(entityHandler)}](
...
inline void populateRootOfTrustEntity(const crow::Request&,
        const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    nlohmann::json& protectedComponents = asyncResp->res.jsonValue["Links"]["ComponentsProtected"];
...
inline void invokeRoTCommand(const crow::Request& request,
                    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                    const ResolvedEntity& resolvedEntity) {
    std::string command;
    if (!redfish::json_util::readJsonAction(request, asyncResp->res,
                                            "Command", command))
    {
    ...
    hoth_iface->sendHostCommand(toDbusMethodAddr(resolvedEntity, "SendHostCommand"), bytes, handleFunc);
}
