...
static const DbusMethodAddr
    kObjMapperGetSubTreeAddr("xyz.openbmc_project.ObjectMapper",
    ...
    const std::shared_ptr<GoogleServiceAsyncResp>& serviceResp)
{
    ...
    serviceResp->rfUtils->populateCollectionMembers(
        asyncResp, kRoTCollectionPrefix,
        std::vector<const char*>{kHothInterface}, kHothSearchPath);
}
               ...
               const std::string& rotId, ResolvedEntityHandler&& entityHandler)
{
    ...
    serviceResp->objMapper->getSubTree(kObjMapperGetSubTreeAddr,
                                       kRotSubTreeSearchParams, validateFunc);
}
