...
inline void getIdentity(uint8_t controllerNamespace,
                        uint8_t controllerNamespaceId, uint8_t identityId,
                        const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                        const std::string& controllerId,
                        const boost::system::error_code ec,
                        const std::vector<std::string>& controllerList)
...
{
    uint8_t controllerNamespace;
    uint8_t controllerNamespaceId;
    uint8_t idendityId;
...
inline void getLogPage(uint8_t logSpecificId, uint8_t logSpecificField,
                       uint8_t logId,
                       const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                       const std::string& controllerId,
                       const boost::system::error_code ec,
                       const std::vector<std::string>& controllerList)
{
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, logSpecificId, logSpecificField, logId,
         controllerPath](const boost::system::error_code ec2,
                         const dbus::utility::MapperGetObject& objects) {
        if (ec2)
        {
...
inline void handleGoogleNvmeControllerLogAction(
    ...
    const std::string& nvmeId, const std::string& controllerId)
{
    uint8_t logSpecificId;
    uint8_t logSpecificField;
    uint8_t logId;

    if (!redfish::json_util::readJsonAction(req, asyncResp->res, "LID",
                                            logSpecificId, "LSP", logSpecificField,
                                            "LSI", logId))
    {
    ...
    handleGoogleNvmeController(
        nvmeId, [logSpecificId, logSpecificField, logId, asyncResp,
                 controllerId](const boost::system::error_code ec,
                               const std::vector<std::string>& controllerList) {
            getLogPage(logSpecificId, logSpecificField, logId, asyncResp,
                       controllerId, ec, controllerList);
        });
}
