             ...
             std::vector<dbus_interface>& nvmes)
{
    ...
    for (uint8_t i = 0; i < 4; ++i)
    {
        ...
        nvme->register_method("GetLogPage", [i, &fds](const std::string& arg0,
        ...
        */
        nvme->register_method("Identify", [i, &fds](const std::string& arg0,
                                                    const std::string& arg1) {
            fds.emplace_back(std::array<int, 2>());
            auto& fd = fds.back();
            int res = pipe(fd.data());
    ...
    auto nvme = nvmeServer.add_interface(
        nvmePath, "xyz.openbmc_project.Configuration.NVME1000");
    auto association = nvmeServer.add_interface(
