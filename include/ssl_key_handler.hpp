...
inline std::string generateSslCertificate(const std::string& filepath,
                                          const std::string& cn)
{
    ...
    if (pPrivKey != nullptr)
    {
        ...
        if (x509 != nullptr)
        {
            ...
            int s = PEM_write_bio_PrivateKey(bufio, pPrivKey, nullptr, nullptr,
                                             0, nullptr, nullptr);
            if (!s)
            {
                BMCWEB_LOG_ERROR("Failed to write pkey");
            }
            ...
            s = PEM_write_bio_X509(bufio, x509);
            if (!s)
            {
                BMCWEB_LOG_ERROR("Failed to write X509");
            }
            dataLen = BIO_get_mem_data(bufio, &data);
            buffer += std::string_view(data, static_cast<size_t>(dataLen));

            BIO_free(bufio);
            BMCWEB_LOG_ERROR("Cert size is {}", buffer.size());
            X509_free(x509);
            boost::system::error_code ec;
            boost::beast::file_posix file;
...
inline std::string ensureOpensslKeyPresentAndValid(const std::string& filepath)
{
    ...
    if (cert.empty())
    {
        ...
        cert = generateSslCertificate(filepath, "testhost");
    }
    return cert;
}
