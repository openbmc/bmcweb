...
bmcweb uses `/tmp/bmcweb` for temporary file storage (configurable via the
`temp-dir` meson option) during multipart form uploads. This directory is
automatically cleaned up by systemd on service restart via the
`TemporaryFileSystem` directive in the service file.
