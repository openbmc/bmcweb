...
- `disabled`:
Turns off all bmcweb log traces.

- `debug` (or `enabled`):
  Information that's overly verbose, and only required in debug, but rarely useful.

- `info` :
  Information: Information for the golden path debugging.

- `warning`:
  Something seems odd, but might occur in the normal path (not used often).

- `error`:
  Something went wrong, and we weren't able to give the expected response because of it. Service is still operational.

- `critical`:
  Something went wrong, and we're no longer able to serve traffic.
