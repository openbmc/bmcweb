                              ...
                              const std::shared_ptr<task::TaskData>& taskData)
{
    ...
    {
        BMCWEB_LOG_ERROR("Failed to parse dbus message for iface {}. Continuing", iface);
        return !task::completed;
    }
    ...
    if (state != nullptr)
    {
        ...
        else if (*state == "xyz.openbmc_project.Software.Activation.Staged")
        {
    ...
    task::Payload&& payload, const sdbusplus::message::object_path& objPath)
{
    std::string matchExpr =
        std::format("type='signal',interface='org.freedesktop.DBus.Properties',"
                    "member='PropertiesChanged',path='{}'",
                    objPath.str);
    std::shared_ptr<task::TaskData> task =
        task::TaskData::createTask(handleTaskMessage, matchExpr);
