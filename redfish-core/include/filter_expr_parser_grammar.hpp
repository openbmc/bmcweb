...
const auto logical_or_def = expression >>
                            *(char_('o') >> char_('r') >> expression);

const auto logical_and_def = logical_or >> *((char_('a') >> char_('n') >>
                                              lit('d') >> logical_or));
...
const auto relational_comparison_def =
...
const auto logical_negation_def = char_("n") >> lit("o") >>
                                  lit("t") >> expression;

const auto expression_def = '(' >> relational_comparison >> ')' |
                            logical_negation | basic_types;

BOOST_SPIRIT_DEFINE(logical_and, logical_or, equality_comparison, quoted_string,
                    logical_negation, unquoted_string, relational_comparison,
                    expression);

static auto grammar = relational_comparison;
} // namespace details
