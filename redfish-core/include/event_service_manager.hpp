...
class Subscription
{
  public:
    ...
    void filterAndSendReports(const std::string& objPath,
                              const ReadingsObjType& readings)
    {
        ...
        std::string metricReportDef =
            "/redfish/v1/TelemetryService/MetricReportDefinitions/" + idStr;
...
class EventServiceManager
{
  ...
  public:
    ...
    void getMetricReading(const std::string& objPath, const std::string& intf)
    {

        crow::connections::systemBus->async_method_call(
            [&, objPath{std::move(objPath)}, intf{std::move(intf)}](
                const boost::system::error_code ec,
                const std::vector<
                    std::pair<std::string, std::vector<std::string>>>& resp) {
                if (ec)
                {
                    BMCWEB_LOG_ERROR << "Failed to get service name";
                    return;
                }
                ...
                crow::connections::systemBus->async_method_call(
                    [&, objPath{std::move(objPath)}](
                        ...
                        {
                            BMCWEB_LOG_DEBUG
                                << "No metrics report to be transferred";
                            return;
                        }

                        for (const auto& it : EventServiceManager::getInstance()
                                                  .subscriptionsMap)
                        {
                            std::shared_ptr<Subscription> entry = it.second;
                            if (entry->eventFormatType == "MetricReport")
                            {
                                entry->filterAndSendReports(objPath,
                                                            *readingsPtr);
                            }
                        }
                    },
