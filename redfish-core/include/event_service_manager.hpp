                        ...
                        std::vector<nlohmann::json>& metricReportDefinitions)
{
    ...
    for (std::size_t i = 0; i < result.size(); i += divisor)
    {
        ...
        else if (key == "DeliveryRetryPolicy")
        {
            retryPolicy = value;
        }
        else if (key == "MessageIds")
        {
            messageIds.push_back(value);
        }
        else if (key == "RegistryPrefixes")
        {
            registryPrefixes.push_back(value);
        }
        else if (key == "MetricReportDefinitions")
        {
...
class EventServiceManager
{
  ...
  public:
    ...
    static int startEventLogMonitor(boost::asio::io_context& ioc)
    {
        ...
        {
            BMCWEB_LOG_ERROR
                << "inotify_add_watch failed for Evett log directory.";
            return -1;
        }
