...
class Subscription : std::enable_shared_from_this<Subscription>
{
  public:
    ...
    inline void deleteIfTerminated(std::shared_ptr<Subscription> /*self*/,
                                   const crow::Response& res)
    {
        ...
        BMCWEB_LOG_INFO("Deleting subscription {}", subId);
        deleter();
    }
    ...
    bool sendEventToSubscriber(std::string&& msg)
    {
        ...
        if (client)
        {
            client->sendDataWithCallback(
                ...
                std::bind_front(&Subscription::deleteIfTerminated, this,
                                shared_from_this()));
            return true;
        }
