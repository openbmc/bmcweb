...
class Subscription
{
  public:
    ...
    Subscription(const std::shared_ptr<crow::sse::Connection>& adaptor) :
        sseConn(std::move(adaptor)), eventSeqNum(1)
    {}
    ...
    std::optional<std::string>
        getSubscriptionId(const std::shared_ptr<crow::sse::Connection> connPtr)
    {
  ...
  private:
    std::shared_ptr<crow::sse::Connection> sseConn = nullptr;
...
class EventServiceManager
{
  ...
  public:
    ...
    void deleteSubscription(
        const std::shared_ptr<crow::sse::Connection>& connPtr)
    {
