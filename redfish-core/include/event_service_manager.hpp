...
static constexpr const char* defaultEventFormatType = "Event";
static constexpr const char* defaultSubscriptionType = "RedfishEvent";
static constexpr const char* defaultRetryPolicy = "TerminateAfterRetries";
        ...
        GetSubscriptionfromJson(const nlohmann::json& j)
    {
        ...
        for (const auto& element : j.items())
        {
            if (element.key() == "Id")
            {
                ...
                if (value == nullptr)
                {
                    BMCWEB_LOG_DEBUG << "Invalid subscription Id exist.";
                    return nullptr;
                }
                id = *value;
            }
            ...
            else if (element.key() == "EventFormatType")
            {
                ...
                if (value == nullptr)
                {
                    eventFormatType = defaultEventFormatType;
                }
                else
                {
