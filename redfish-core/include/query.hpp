    ...
    const query_param::QueryCapabilities& queryCapabilities)
{
    ...
    RedfishAggregator::getInstance().attemptAggregation(req, asyncResp);
#endif
    ...
    asyncResp->res.setCompleteRequestHandler(
        [&app, handler(std::move(handler)),
         query{*queryOpt}](crow::Response & asyncResp->res) mutable {
            processAllParams(app, query, handler, asyncResp->res);
        });
    return true;
}
