    ...
    const query_param::QueryCapabilities& queryCapabilities)
{
    ...
    // make a copy of the request using shared_ptr
    // The lambda capture of newReq by value in the lambda is causing problems because the lambda needs to be copy-constructible, but crow::Request isn't copy-constructible.
    // So we use a shared_ptr to the request to avoid copying the request.
