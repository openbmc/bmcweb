...
inline bool handleIfMatch(bmcweb::App& app, const crow::Request& req,
                          const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    getReqAsyncResp->res.setCompleteRequestHandler(std::bind_front(
        ...
        std::make_shared<bmcweb::Request>(req.copy()), std::move(ifMatch)));
    ...
    const query_param::QueryCapabilities& queryCapabilities)
{
    ...
    // because the lambda needs to be copy-constructible, but bmcweb::Request
    ...
    auto newReq = std::make_shared<bmcweb::Request>(req.copy());
    ...
    asyncResp->res.setCompleteRequestHandler(
        [&app, handler(std::move(handler)), query{std::move(*queryOpt)},
         delegated{delegated},
         newReq{std::move(newReq)}](bmcweb::Response& resIn) mutable {
