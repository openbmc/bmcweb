...
class RedfishAggregator
{
  private:
            ...
            rdeSatelliteInfo)
    {
        ...
        BMCWEB_LOG_DEBUG("There were {} RDE Device configs found at startup",
        ...
        std::string_view memberName)
    {
        ...
        for (const auto& rdeSatellite : rdeSatelliteInfo)
        {
            ...
            if (memberName.starts_with(targetPrefix))
            {
                BMCWEB_LOG_DEBUG("\"{}\" is a known prefix",
                                 rdeSatellite.first);
                // Remove the known prefix from the request's URI and
    ...
    {
        nlohmann::json jsonVal = nlohmann::json::parse(respString, nullptr,
                                                       false);
  ...
  public:
            ...
            handler)
    {
        ...
        dbus::utility::getManagedObjects(
            ...
            {
                BMCWEB_LOG_DEBUG(
                    "Redfish Aggregation enabled with {} RDE Device",
                    std::to_string(rdeSatelliteInfo.size()));
            }
            else
            {
