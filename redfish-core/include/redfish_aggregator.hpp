...
class RedfishAggregator : public std::enable_shared_from_this<RedfishAggregator>
{
  ...
  public:
    ...
    static void beginAggregation(const crow::Request& thisReq,
                                 const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
    {
        // Create a copy of thisReq so we we can still locally process the req
        boost::beast::http::request<boost::beast::http::string_body> req = thisReq.req;
        std::error_code ec;
        ...
        auto cb = [&copyReq, &asyncResp](
                      const std::unordered_map<std::string, boost::urls::url>&
                          satelliteInfo) {
            getInstance().aggregateAndHandle(copyReq, asyncResp, satelliteInfo);
        };
