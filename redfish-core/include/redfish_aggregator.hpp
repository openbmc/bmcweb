...
class RedfishAggregator
{
  ...
  public:
    ...
    std::unordered_map<std::string, AggregationSource> currentAggregationSources;
        ...
        const std::string& prefix) const
    {
        ...
        auto it = currentAggregationSources.find(prefix);
        if (it != currentAggregationSources.end())
        {
            ...
            handler) const
    {
        ...
        // Extract just the URLs from currentAggregationSources for the handler
        std::unordered_map<std::string, boost::urls::url> satelliteInfo;
        for (const auto& [prefix, source] : currentAggregationSources)
        {
