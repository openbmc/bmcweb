...
class RedfishAggregator
{
  ...
  public:
    ...
    // Separate map for credentials - only contains entries that have auth
    std::unordered_map<std::string, AggregationCredentials> aggregationCredentials;

    // Helper function to create Basic Auth header
    static std::string createBasicAuthHeader(const std::string& username, const std::string& password)
    {
        std::string credentials = username + ":" + password;
        std::string encoded = crow::utility::base64encode(credentials);
        ...
        const std::string& prefix) const
    {
        ...
        if (aggregationCredentials.contains(prefix))
        {
            const auto& creds = aggregationCredentials.at(prefix);
            std::string authHeader = createBasicAuthHeader(creds.username,
                                                           creds.password);
            fields.set(boost::beast::http::field::authorization, authHeader);
        }
        return fields;
    }
