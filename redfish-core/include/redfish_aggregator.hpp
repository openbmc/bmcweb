...
static constexpr const char* satelliteControllerIface =
...
class RedfishAggregator
{
  private:
        ...
        std::unordered_map<std::string, boost::urls::url>& satelliteInfo)
    {
        ...
        for (const auto& prop : properties)
        {
            ...
            else if (prop.first == "AuthType")
            {
                ...
                {
                    BMCWEB_LOG_ERROR(
                        R"(Unsupported AuthType value: {} , only "None" and "MTLS" are supported)",
                        *propVal);
                    return;
                }
            }
        ...
        std::string_view memberName)
    {
        ...
        {
            std::string targetPrefix = satellite.first + "_";
        ...
        BMCWEB_LOG_WARNING("Unknown satellite prefix in request, returning 404 for {}",
                           nameStr);
  ...
  public:
                            ...
                            const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
    {
        ...
        std::string_view forwardedHeader =
            thisReq.getHeaderValue("X-Forwarded-For");
        if (forwardedHeader == "bmcweb")
        {
            BMCWEB_LOG_DEBUG(
