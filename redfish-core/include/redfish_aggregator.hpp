...
class RedfishAggregator
{
  private:
    ...
    static void constructorCallback(
        const std::unordered_map<std::string_view, boost::urls::url,
                                 std::less<>>& satelliteInfo)
    {
    ...
    static void getSatelliteConfigs(
        const std::function<
            void(const std::unordered_map<std::string_view, boost::urls::url,
                                          std::less<>>&)>& handler)
    {
        ...
        crow::connections::systemBus->async_method_call(
            ...
            std::unordered_map<std::string_view, boost::urls::url, std::less<>>
                satelliteInfo;
                                 ...
                                 std::less<>>& satelliteInfo)
    {
        ...
        if (segments.size() >= 4)
        {
            ...
            // std::string_view
            // targetURI(boost::urls::string_view(segments[3]));
                                 ...
                                 std::less<>>& satelliteInfo)
    {
        ...
        std::string targetURI(thisReq.target());
        size_t pos = targetURI.find(tmpPrefix + "_");
        if (pos == std::string_view::npos)
        {
            // If this fails then something went wrong
            BMCWEB_LOG_ERROR << "Error removing prefix \"" << prefix
                        ...
                        crow::Response& resp)
    {
        ...
        if (resp.getBaseValue("Content-Type") == "application/json")
        {
