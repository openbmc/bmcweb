...
struct SatelliteConfig
{
    ...
    std::string hostname;
    uint16_t port = 0;
    ControllerType type = ControllerType::Http;
    std::string authType;

    void dumpSatelliteConfig() const
    {
        ...
        std::unordered_map<std::string, SatelliteConfig>& satelliteInfo)
    {
        ...
        for (const auto& prop : properties)
        {
            if (prop.first == "Hostname")
            {
                ...
                satelliteConfig.hostname = *propVal;
            }
            ...
            else if (prop.first == "AuthType")
            {
                ...
                satelliteConfig.authType = *propVal;
            }
