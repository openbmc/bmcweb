    ...
    if (object != nullptr)
    {
        for (std::pair<const std::string, nlohmann::json>& item : *object)
        {
            if (item.first == "@odata.id")
            {
                ...
                if (!parsed)
                {
                    BMCWEB_LOG_CRITICAL << "Couldn't parse URI from resource "
                                        << *strValue;
                    continue;
                }
                ...
                if (crow::utility::readUrlSegments(
                        thisUrl, "redfish", "v1", "UpdateService",
                        std::ref(collectionName), std::ref(softwareItem)))
                {
                    softwareItem += prefix;
                    softwareItem += "_";
                    item.second = crow::utility::urlFromPieces(
                                                   ...
                                                   std::ref(collectionName)))
                {
                    ...
                    collectionName += "_";
                    item.second = crow::utility::urlFromPieces("redfish", "v1",
                                                               collectionName)
                                      .string();
                }
...
class RedfishAggregator
{
  private:
        ...
        const std::unordered_map<std::string, boost::urls::url>& satelliteInfo)
    {
        ...
        // UpdateService is 4 segments
        // /redfish/v1/UpdateService/FirmwareInventory
        if (segments.size() >= 4)
        {
            std::string_view targetURI = segments[3];
                        ...
                        crow::Response& resp)
    {
        ...
        if (resp.getHeaderValue("Content-Type") == "application/json")
        {
            ...
            // TODO: For collections we  want to add the satellite responses
            // to our response rather than just straight overwriting them if
            // our local handling was successful (i.e. would return a 200).
