...
static inline void addHeaderPrefixes(crow::Response& resp,
                                     std::string_view prefix)
{
    std::string location(resp.getHeaderValue("Location"));
    if (!location.empty())
    {
                                                ...
                                                std::string_view prefix)
{
    ...
    if (!resp.getHeaderValue("Location").empty())
    {
        ...
        addHeaderPrefixes(asyncResp, prefix);
    }
  ...
  public:
                        ...
                        crow::Response& resp)
    {
        ...
        if (resp.getHeaderValue("Content-Type") == "application/json")
        {
            ...
            addLinksToAggregatedResponse(asyncResp->res, resp, prefix);
