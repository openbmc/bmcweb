  ...
  private:
        ...
        const std::unordered_map<std::string, boost::urls::url>& satelliteInfo)
    {
        ...
        BMCWEB_LOG_DEBUG << "MYDEBUG: size is "
                         << std::to_string(satelliteInfo.size());
        BMCWEB_LOG_DEBUG << "MYDEBUG: So that's that";
        ...
        if (!satelliteInfo.empty())
        {
            // TODOME: The next line is causing a segfault.  thisReq has
            ...
            if (std::regex_match(targetURI, urlRegex))
            {
                ...
                // We won't actually have "main" as a prefix
                /*
                if (ids[0] == "main")
                {
                    BMCWEB_LOG_DEBUG
                    ...
                    if (pos == std::string::npos)
                    {
                        ...
                        BMCWEB_LOG_ERROR
                            << "Error removing prefix \"main_\" from request
                URI"; messages::internalError(asyncResp->res); return;
                    }
            ...
            {
                /*
                  // Make sure the previous check did not fail because a
                  required
                  ...
                  if (std::regex_match(targetURI, illegalRegex))
                  {
                      ...
                      // The local BMC will handle this request so we don't need
        ...
        BMCWEB_LOG_DEBUG << "MYDEBUG: We're at the end of aggregateAndHandle()";
  ...
  public:
                         ...
                         const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
    {
        ...
        boost::beast::http::request<boost::beast::http::string_body> req =
            thisReq.req;
        // auto req =
        // std::make_shared<boost::beast::http::request<boost::beast::http::string_body>>(thisReq.req);
        std::error_code ec;
        // crow::Request copyReq(req, ec);
