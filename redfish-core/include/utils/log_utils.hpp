...
{
    asyncResp->res.jsonValue["logPath"] = logPath;
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, logPath, logURIBase, messageId,
         deviceId](const boost::system::error_code ec,
            ...
            nlohmann::json conditionArray = nlohmann::json::array();
            for (const auto& obj : subtree)
            {
                // Remove the base log path to makesure the log service name
