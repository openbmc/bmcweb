...
class MultiRouteResp : public std::enable_shared_from_this<MultiRouteResp>
{
  public:
    ...
    static void handleFragmentRules(
        ...
        const std::shared_ptr<std::vector<BaseRule*>>& fragments,
    ...
    std::shared_ptr<bmcweb::AsyncResp>& multiAsyncResp)
{
    ...
    auto uriFragments = std::make_shared<std::vector<BaseRule*>>(fragments);
    auto uriParams = std::make_shared<std::vector<std::string>>(params);

    multiResp->res.setCompleteRequestHandler(
        std::bind_front(MultiRouteResp::handleFragmentRules,
