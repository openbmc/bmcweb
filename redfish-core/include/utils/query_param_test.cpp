...
TEST(QueryParams, FindNavigationReferencesNonLink)
{
    ...
    json singleTreeNode = "{\"Foo\" : {\"@odata.id\": \"/foobar\"}}"_json;
    ...
    EXPECT_THAT(findNavigationReferences(ExpandType::NotHyperlinks,
                                         singleTreeNode,
                                         json::json_pointer("")),
                UnorderedElementsAre(redfish::query_param::ExpandNode{
                    json::json_pointer("/Foo"), "/foobar"}));
    ...
    EXPECT_TRUE(findNavigationReferences(ExpandType::Hyperlinks, singleTreeNode,
                                         json::json_pointer(""))
                    .empty());
}
...
TEST(QueryParams, FindNavigationReferencesLink)
{
    ...
    EXPECT_THAT(findNavigationReferences(ExpandType::Hyperlinks, singleLinkNode,
                                         json::json_pointer("")),
                UnorderedElementsAre(redfish::query_param::ExpandNode{
                    json::json_pointer("/Links/Sessions"), "/foobar"}));
    ...
    EXPECT_TRUE(findNavigationReferences(ExpandType::NotHyperlinks,
                                         singleLinkNode, json::json_pointer(""))
                    .empty());
}