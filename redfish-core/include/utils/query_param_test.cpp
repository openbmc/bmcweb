...
TEST(QueryParams, FindNavigationReferences)
{
    using nlohmann::json;
    using redfish::query_param::findNavigationReferences;
    json j = "{\"Foo\" : {\"@odata.id\": \"/foobar\"}}"_json;
    std::vector<redfish::query_param::ExpandNode> expected{
        redfish::query_param::ExpandNode{json::json_pointer("/Foo"),
                                         "/foobar"}};
    ASSERT_THAT(findNavigationReferences(j, json::json_pointer("")),
