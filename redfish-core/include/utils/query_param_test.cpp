...
{
    std::vector<std::filesystem::path> properties = {
        std::filesystem::path("/foo/bar/213")};
    EXPECT_THAT(getIntermediatePaths(properties),
                UnorderedElementsAre(Eq("/foo/bar"), Eq("/foo"), Eq("/")));
}

TEST(RecursiveSelect, ExpectedKeysAreSelectInSimpleObject)
{
    std::vector<std::filesystem::path> shouldSelect = {"/select_me"};
...
{
    std::vector<std::filesystem::path> shouldSelect = {
