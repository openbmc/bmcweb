...
inline void afterGetPortList(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::function<void(const dbus::utility::MapperEndPoints&)>& callback,
    ...
    std::function<void(const dbus::utility::MapperEndPoints&)>&& callback)
{
    ...
    dbus::utility::getAssociatedSubTreePaths(
        ...
        portInterface, std::bind_front(afterGetPortList, asyncResp, callback));
}
