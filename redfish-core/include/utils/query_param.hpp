...
class MultiAsyncResp : public std::enable_shared_from_this<MultiAsyncResp>
{
  public:
    ...
    {
        // OEM fragment merging (startMultiFragmentHandle), not $expand;
        // BudgetAction::Skip so it's never charged against, or checked
        // against, the $expand budget.
    ...
    void startSubquery(size_t nodeIndex)
    {
        ...
        BudgetAction action =
            queryStr.empty() ? BudgetAction::Charge : BudgetAction::VerifyOnly;
        asyncResp->res.setCompleteRequestHandler(
            std::bind_front(placeResultStatic, shared_from_this(),
                            nodes[nodeIndex].location, action));
        if (app != nullptr)
        {
            app->handle(newReq, asyncResp);
        }
                     ...
                     crow::Response& res, BudgetAction action)
    {
        ...
        // OEM fragment merges (BudgetAction::Skip) are not part of any
        // $expand tree, so budgetExceeded and the $expand budget state from
        // a concurrent $expand tree must not suppress or fail this merge.
                    ...
                    const crow::Request& req)
    {
        ...
        if (req.expandPayloadUsed != nullptr)
        {
            ...
            // BudgetAction::VerifyOnly to avoid counting the same bytes
            // again.
