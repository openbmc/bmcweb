                    ...
                    crow::Response& res)
{
    ...
    for (const boost::urls::params_view::value_type& it : urlParams)
    {
        ...
        else if (key == "$skip")
        {
            ...
            else if (key == "$select")
            {
                ...
                boost::split(ret.selectedProperties, value,
                             boost::is_any_of(","));
            }
            else
            {
        ...
        if (array != nullptr)
        {
            ...
            for (auto& element : *array)
            {
                ...
                BMCWEB_LOG_DEBUG << "Traversing response at "
                                 << newPtr.to_string();
                findNavigationReferencesRecursive(eType, element, newPtr,
                                                  inLinks, out);
                index++;
            }
        }
        ...
        std::vector<nlohmann::json::json_pointer>& selected)
    {
        ...
        if (object != nullptr)
        {
            ...
            for (const auto& [k, v] : currRoot.items())
            {
                ...
                recursiveSelect(v, currRootPtr / k, newPrefix,
                                parentSelected ||
                                    shouldSelect.contains(newPrefix),
        ...
        if (std::any_of(
                odataProperties.begin(), odataProperties.end(),
    ...
    inline nlohmann::json performSelect(
        const nlohmann::json& root, std::span<const std::string> shouldSelect)
    {
        std::vector<nlohmann::json::json_pointer> selected;
        // start recursion at the root
    ...
    class MultiAsyncResp : public std::enable_shared_from_this<MultiAsyncResp>
    {
      public:
        ...
        void startQuery(const Query& query)
        {
            ...
            if (!query.selectedProperties.empty())
            {
                finalRes->res.jsonValue = performSelect(
                    finalRes->res.jsonValue, query.selectedProperties);
            }
            // then find all nodes that need to expand
            std::vector<ExpandNode> nodes = findNavigationReferences(
                query.expandType, finalRes->res.jsonValue);
            ...
            {
                BMCWEB_LOG_DEBUG << "URL of subquery:  "
                                 << (node.uri + queryStr);
                std::error_code ec;
                crow::Request newReq(
                    {boost::beast::http::verb::get, node.uri + queryStr, 11},
                    ec);
                if (ec)
                {
    ...
    inline void processAllParams(
        crow::App & app, const Query& query,
        ...
        crow::Response& intermediateResponse)
    {
        ...
        if (query.expandType != ExpandType::None)
        {
            ...
            asyncResp->res.jsonValue =
                std::move(intermediateResponse.jsonValue);
