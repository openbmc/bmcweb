...
class SelectTrieNode
{
  public:
    ...
    SelectTrieNode(SelectTrieNode&& other) = default;
    SelectTrieNode& operator=(SelectTrieNode&& other) = default;

    SelectTrieNode* find(const std::string& jsonKey) const
    {
    ...
    // Creates a new node of the given key and returns the reference
    SelectTrieNode* insert(const std::string& jsonKey)
    {
        auto node = std::make_unique<SelectTrieNode>();
        SelectTrieNode* res = node.get();
        children[jsonKey] = std::move(node);
        return res;
    }
  ...
  private:
    std::unordered_map<std::string, std::unique_ptr<SelectTrieNode>> children;
    bool leaf = false;
};
...
struct SelectTrie
{
    SelectTrie() : root(std::make_unique<SelectTrieNode>())
    {}
    ...
    bool insertNode(std::string_view nestedProperty)
    {
        ...
        boost::split(properties, nestedProperty, boost::is_any_of("/"));
        SelectTrieNode* currNode = root.get();
        ...
        for (const auto& property : properties)
        {
            ...
            SelectTrieNode* nextNode = currNode->find(property);
            if (nextNode == nullptr)
            {
                nextNode = currNode->insert(property);
            }
            currNode = nextNode;
        }
    ...
    std::unique_ptr<SelectTrieNode> root;
};
                            ...
                            const SelectTrieNode& currNode)
{
    ...
    if (object != nullptr)
    {
        ...
        while (it != currRoot.end())
        {
            ...
            SelectTrieNode* nextNode = currNode.find(it.key());
            if (nextNode != nullptr && nextNode->isLeaf())
            {
                it = nextIt;
                continue;
            }
