                    ...
                    crow::Response& res)
{
    ...
    {
        std::string_view key = std::string(it.key.data(), it.key.size());
        std::string_view value = std::string(it.value.data(), it.value.size());
        ...
        else
        {
            ...
            if (it.key.starts_with("$"))
            {
                ...
                messages::queryParameterValueFormatError(res, value, key);
                res.code(boost::beast::http::status::
                             not_implemented) return std::nullopt;
            }
...
// might need resolved
inline void findNavigationReferencesRecursive(nlohmann::json& jsonResponse,
                             ...
                             nlohmann::json::json_pointer root)
{
    ...
    findNavigationReferencesRecursive(jsonResponse, root, ret);
    return ret;
}
...
struct MultiAsyncResp : public std::enable_shared_from_this<MultiAsyncResp>
{
    ...
    MultiAsyncResp(crow::App& app, std::shared_ptr<bmcweb::AsyncResp> final) :
        app(app), finalRes(final)
    {}

    void addAwaitingResponse(Query query,
                             std::shared_ptr<bmcweb::AsyncResp>& res,
                             nlohmann::json::json_pointer finalExpandLocation)
    {
  ...
  private:
    static void onEndStatic(std::shared_ptr<MultiAsyncResp> multi, Query query,
