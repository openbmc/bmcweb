...
inline void findNavigationReferencesRecursive(
    ...
    const nlohmann::json::json_pointer& p, std::vector<ExpandNode>& out,
    bool inLinks)
{
    ...
    if (array != nullptr)
    {
        ...
        for (auto& element : *array)
        {
            ...
            findNavigationReferencesRecursive(eType, element, newPtr, out,
                                              inLinks);
            index++;
        }
    }
    ...
    for (auto& element : *obj)
    {
        ...
        if (inLinks && eType == ExpandType::NotHyperlinks)
        {
            continue;
        }
        if (!inLinks && eType == ExpandType::Hyperlinks)
        {
            continue;
        }
        ...
        findNavigationReferencesRecursive(eType, element.second, newPtr, out,
                                          inLinks);
    }
}
                             ...
                             const nlohmann::json::json_pointer& root)
{
    ...
    findNavigationReferencesRecursive(eType, jsonResponse, root, ret, false);
    return ret;
}
