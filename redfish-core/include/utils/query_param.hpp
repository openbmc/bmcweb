...
class SelectTrieNode
{
  public:
    ...
    SelectTrieNode* insertIfNotExist(std::string_view jsonKey)
    {
        std::string jsonKeyStr(jsonKey);
        auto it = children.find(jsonKeyStr);
        if (it != children.end())
        {
            return &it->second;
        }
        auto [newIt, _] =
            children.insert({std::move(jsonKeyStr), SelectTrieNode{}});
        return &newIt->second;
    }
  ...
  private:
    std::map<std::string, SelectTrieNode> children;
    bool selected = false;
};
...
struct SelectTrie
{
    ...
    bool insertNode(std::string_view nestedProperty)
    {
        ...
        {
            std::string_view property = nestedProperty;
            if (index != std::string::npos)
            {
                property = nestedProperty.substr(0, index);
            }
            if (!isSelectedPropertyAllowed(property))
            {
                return false;
            }
            currNode = currNode->insertIfNotExist(property);
            if (index == std::string::npos)
            {
                break;
            }
