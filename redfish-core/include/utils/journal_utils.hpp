...
inline std::optional<long int> getJournalMetadataInt(JournalReadState& journal, const char* field)
{
    ...
    JournalReadState& journal, nlohmann::json::object_t& bmcJournalLogEntryJson)
{
    ...
    std::string message;
    std::string_view syslogID = getJournalMetadata(journal, "SYSLOG_IDENTIFIER");
    if (!syslogID.empty())
    {
    ...
    std::optional<long int> severity = getJournalMetadataInt(journal, "PRIORITY");
    if (!severity)
    {
