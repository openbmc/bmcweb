...
template <typename Handler, typename Type, typename Ret>
concept OptionalReturnHandler = requires(Handler h, const Type& in) {
                                    {
                                        h(in)
                                    } -> std::same_as<std::optional<Ret>>;
                                };
...
struct GenericNodeMapper : BaseNodeMapper
{
              ...
              MetaData& /*unused*/) override
    {
        return std::visit(
            ...
            {
                auto ret = makeJson(key, std::forward<decltype(val)>(val));
                if (!ret.empty())
                {
                    return ret;
                }
              ...
              MetaData& /*unused*/) override
    {
        return std::visit(
            [&](auto&& val) -> std::optional<nlohmann::json> {
                ...
                {
                    auto ret = handler(path, ifaceName, name, val);
                    if (!ret.empty())
                    {
                        return ret;
                    }
            ...
            [&](auto&& val) -> std::optional<nlohmann::json> {
                ...
                if constexpr (std::is_same_v<T, Type>)
                {
                    ...
                    auto result = handler(path, ifaceName, name, val, mr,
                                          metaData);
                    if (mr == MapperResult::Ok && !result.empty())
                    {
                        return result;
                    }
                }
...
{
    return std::make_unique<NodeMapper<Type, decltype(handler)>>(
        std::move(handler));
}
template <typename Type>
inline auto makeNodeMapper(MapperHandlerWithError<Type> handler)
{
    return std::make_unique<NodeMapperWithError<Type, decltype(handler)>>(
        std::move(handler));
}
...
template <typename Type>
inline auto mapToKeyOrError(const std::string& key)
{
    return mapToKey<Type>(key, [](auto s, MapperResult& mr, MetaData& md) {
...
template <typename Type, typename Ret = Type,
          OptionalReturnHandler<Type, Ret> Handler>
inline auto mapToKeyOrError(const std::string& key, Handler handler)
{
    return mapToKey<Type>(key, [handler = std::move(handler)](
        ...
        if (auto ret = handler(s); ret)
        {
            return *ret;
        }
        mr = MapperResult::WrongValue;
        redfish::messages::setInternalError(md.res);
        return Ret{};
    });
}
...
inline auto mapToEnumKey(const std::string& key, Handler handler)
{
    ...
    return mapToKey<std::string>("/", std::move(enumMapper));
}
using handler_pair =
