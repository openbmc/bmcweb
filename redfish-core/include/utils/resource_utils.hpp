    ...
    const std::optional<bool>& functional)
{
    ...
    // Default to true if interface is missing
 ...
 *
 * Sets the Redfish Status.State only after all async calls are completed and no
 * errors occur from aggregating status properties.
 */
...
/*
 * @brief Queries the DBus and sets the corresponding ResourceStatus field
 *
 * If no error occurs, corresponding field is set to the value returned
 * If the interface is missing, the corresponding field is defaulted to healthy
 * i.e. true
 * If an error occurs, the corresponding field stays std::nullopt
 */
