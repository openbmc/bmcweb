...
inline void getStatusAvailableState(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const nlohmann::json::json_pointer& jsonPtr, std::optional<bool> present,
    const boost::system::error_code& ec, std::optional<bool> available)
{
    if (ec)
    {
        if (ec.value() != EBADR)
...
inline void getStatusPresentState(
    ...
    const nlohmann::json::json_pointer& jsonPtr,
    const boost::system::error_code& ec, std::optional<bool> present)
{
    if (ec)
    {
        if (ec.value() != EBADR)
    ...
    const nlohmann::json::json_pointer& jsonPtr)
{
    ...
    dbus::utility::getProperty<bool>(
        *crow::connections::systemBus, service, path,
        ...
        std::bind_front(getStatusPresentState, asyncResp, service, path, jsonPtr));
}
