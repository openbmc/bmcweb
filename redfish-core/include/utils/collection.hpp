...
inline std::string_view getMemberOdataId(const nlohmann::json& member)
{
    if (!member.is_object())
    {
        return {};
    ...
    auto it = member.find("@odata.id");
    if (it == member.end())
    {
        return {};
    }

    const auto* odataId = it->get_ptr<const std::string*>();
    if (odataId == nullptr)
    {
        return {};
    }
...
{
    std::ranges::sort(members, [](const nlohmann::json& lhs,
                                  const nlohmann::json& rhs) {
        return AlphanumLess<std::string_view>{}(details::getMemberOdataId(lhs),
                                                details::getMemberOdataId(rhs));
    });
}
    ...
    const dbus::utility::MapperGetSubTreePathsResponse& objects)
{
    ...
    if (ec == boost::system::errc::io_error)
    {
        ...
        asyncResp->res.jsonValue[jsonCountKeyName] =
            members.is_array() ? members.size() : 0;
        return;
    }
