    ...
    const dbus::utility::GetPathsByAssociationResult& result)
{
    ...
    for (const auto& item : result)
    {
        ...
        const sdbusplus::object_path fanPath = std::get<2>(item);
        const std::string& assoc = std::get<1>(item);
...
using FanPathCallback =
    std::function<void(const std::vector<sdbusplus::object_path>& fanPaths)>;
...
// @returns fan object paths of fan resources subordinate to that chassis
inline void getOwnedFanPathsByChassisId(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& chassisId, const FanPathCallback& callback)
{
    constexpr size_t index = (BMCWEB_REDFISH_FAN_LINKS) ? 0 : 1;

    fan_utils::getFanPathsByChassisId(
        asyncResp, chassisId, [chassisId, callback](const FanMap& fanMap) {
            ...
            callback(std::get<index>(fanMap.at(chassisId)));
        });
}
