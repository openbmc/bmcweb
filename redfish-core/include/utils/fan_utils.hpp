...
inline std::vector<std::string> fanPathOracle(
    ...
    const dbus::utility::GetPathsByAssociationResult& result)
{
    ...
    {
        const sdbusplus::message::object_path chassisPath(std::get<0>(item));
    ...
    dbus::utility::getPathsByAssociation(
        ...
        [asyncResp, callback,
         chassisId](const boost::system::error_code& ec,
            ...
            callback(fanPathOracle(chassisId, res));
        });
}
...
inline void getFanPaths(
    ...
    const std::function<void(const dbus::utility::MapperGetSubTreePathsResponse&
                                 fanPaths)>& callback)
{
    sdbusplus::message::object_path path(validChassisPath);
