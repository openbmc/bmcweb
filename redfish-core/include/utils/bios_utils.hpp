...
inline void afterGetBIOSManagerObject(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    CallbackFunc&& callback, const boost::system::error_code& ec,
    const dbus::utility::MapperGetObject& object)
{
    if (ec || object.empty() || object.size() > 1)
    {
    ...
    CallbackFunc&& callback)
{
    dbus::utility::getDbusObject(
        ...
        [asyncResp, callback = std::forward<CallbackFunc>(callback)](
            ...
            afterGetBIOSManagerObject(asyncResp, std::move(callback), ec,
                                      object);
        });
}
