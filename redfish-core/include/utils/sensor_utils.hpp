    ...
    nlohmann::json& sensorJson, InventoryItem* inventoryItem)
{
    ...
    if (!isExcerpt)
    {
        ...
        std::optional <
            std::unordered_map<std::string, std::pair<double, uint64_t>>
                statistic;
        const bool success = sdbusplus::unpackPropertiesNoThrow(
        ...
        if (chassisSubNode == ChassisSubNode::sensorsNode)
        {
            ...
            {
                sensorJson["ReadingBasis"] = *readingBasis;
            }

            if (implementation.has_value())
            {
                sensorJson["Implementation"] = *implementation;
            }
            ...
            if (statistic.has_value())
            {
                ...
                {
                    sensorJson["PeakReading"] = peak->second.first;
                    sensorJson["PeakReadingTime"] = peak->second.second;
                }
            }
        }
