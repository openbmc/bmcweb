    ...
    nlohmann::json& sensorJson, InventoryItem* inventoryItem)
{
    ...
    if ((chassisSubNode == ChassisSubNode::sensorsNode) ||
        isExcerptNode(chassisSubNode))
    {
        // ThermalMetrics excerpts don't include these properties
...
inline void afterGetAllSensorObjects(
    ...
    const std::function<void(SensorServicePathList&)>& callback,
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    callback(sensorsServiceAndPath);
}
    ...
    const std::function<void(SensorServicePathList&)>& callback)
{
    ...
    dbus::utility::getAssociatedSubTree(
        ...
        std::bind_front(afterGetAllSensorObjects, asyncResp, callback));
}
