...
struct DbusTreeParser
{
    ...
    template <typename Handler>
        requires(
            std::invocable<Handler, DbusParserStatus, const nlohmann::json&>)
    void parse(Handler&& handler)
    {
        try
        {
            ...
            handler(DbusParserStatus::Ok, root);
        }
        catch (...)
        {
            handler(DbusParserStatus::Failed, nlohmann::json{});
        }
    }
};
template <typename Handler>
DbusTreeParser(const DU::ManagedObjectType&, Handler&, bool)
    -> DbusTreeParser<Handler>;
} // namespace redfish
