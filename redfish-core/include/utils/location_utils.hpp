...
inline void getAssociatedChassisSubtreeCallback(
    std::set<std::string> upstreamChassisPaths,
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    if (!upstreamChassisPaths.insert(chassisPath).second)
    {
    ...
    sdbusplus::asio::getProperty<std::string>(
        *crow::connections::systemBus, service, chassisPath,
        ...
        "LocationCode", [
            ...
            upstreamChassisPaths{std::move(upstreamChassisPaths)}](
        ...
        getAssociatedChassisSubtree(
            asyncResp, chassisPath + "/containedby", std::bind_front(
                getAssociatedChassisSubtreeCallback,
    ...
    const std::string& associationPath)
{
    ...
    std::set<std::string> upstreamChassisPaths;
    getAssociatedChassisSubtree(asyncResp, associationPath, std::bind_front(
            getAssociatedChassisSubtreeCallback,
            std::move(upstreamChassisPaths), jsonPtr));
}
