...
inline void getLocation(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                        ...
                        const std::string& locationName = "Location")
{
    BMCWEB_LOG_DEBUG << "Get Location for: " << resourceId;

    crow::connections::systemBus->async_method_call(
        [asyncResp, locationName,
         resourceId](const boost::system::error_code ec,
                     ...
                     object : subtree)
            {
                ...
                for (const auto& [typeInterface, type] : supportedLocationTypes)
                {
                    ...
                    {
                        asyncResp->res.jsonValue[locationName]["PartLocation"]
                                                ["LocationType"] = type;
                        break;
                    }
                }
                ...
                BMCWEB_LOG_DEBUG << "BMCweb location " << connection << ": "
                                 << path.str << ": " << locationInterface;
                crow::connections::systemBus->async_method_call(
                    [asyncResp,
...
busctl call xyz.openbmc_project.ObjectMapper / xyz / openbmc_project /
    object_mapper xyz.openbmc_project.ObjectMapper GetSubTree sias / xyz /
    openbmc_project / inventory 0 1 xyz.openbmc_project.Association.Definitions
