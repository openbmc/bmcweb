    ...
    const std::string& key)
{
    ...
    if (!pcieType)
    {
        BMCWEB_LOG_WARNING("Unknown PCIe Generation for {}: {}", key,
                           *generation);
        return true;
    }
    if (*pcieType == pcie_device::PCIeTypes::Invalid)
    {
        BMCWEB_LOG_ERROR("Invalid PCIe Generation for {}: {}", key,
                         *generation);
    ...
    const dbus::utility::DBusPropertiesMap& properties)
{
    if (ec)
    {
        BMCWEB_LOG_ERROR("DBus error getting port properties: {}", ec);
        messages::internalError(asyncResp->res);
        return;
    }
