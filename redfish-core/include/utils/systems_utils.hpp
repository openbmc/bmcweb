    ...
    const uint64_t computerSystemIndex)
{
    ...
    const sdbusplus::message::object_path hostStatePath(path);
...
inline std::string getHostStateServiceName(const uint64_t computerSystemIndex)
{
    ...
    if constexpr (BMCWEB_EXPERIMENTAL_REDFISH_MULTI_COMPUTER_SYSTEM)
    {
        hostStateService = std::format("xyz.openbmc_project.State.Host{}",
                                       computerSystemIndex);
    }
    ...
    const uint64_t computerSystemIndex)
{
    ...
    const sdbusplus::message::object_path chassisStatePath(path);
