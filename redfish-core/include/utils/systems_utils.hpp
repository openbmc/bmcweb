...
{
    sdbusplus::message::object_path path("/xyz/openbmc_project/state");
    path /= std::format("host{}", computerSystemIndex);
    return path;
}
...
{
    sdbusplus::message::object_path path("/xyz/openbmc_project/state");
    path /= std::format("chassis{}", computerSystemIndex);
    return path;
}
...
inline sdbusplus::message::object_path getSettingsObjectPath(
    const uint64_t computerSystemIndex)
{
