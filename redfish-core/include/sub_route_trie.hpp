...
class SubRouteTrie
{
  ...
  struct SubRouteNode : public Trie<ContainedType>::Node
  {
      ...
      {
          return Trie<ContainedType>::Node::isSimpleNode() && fragmentChildren.empty();
      }
  };
  ...
  private:
    void optimizeNode(SubRouteNode& node)
    {
        ...
        while (true)
        {
            ...
            for (const typename SubRouteNode::ChildMap::value_type& kv : node.children)
            {
                SubRouteNode& child = nodes[kv.second];
                if (child.isSimpleNode())
                {
                    for (const typename SubRouteNode::ChildMap::value_type& childKv :
                         child.children)
                    {
  ...
  private:
    FindResult findHelper(const std::string_view reqUrl, const SubRouteNode& node,
  ...
  private:
    void debugNodePrint(SubRouteNode& n, size_t level)
    {
        ...
        for (const typename SubRouteNode::ChildMap::value_type& kv : n.fragmentChildren)
        {
