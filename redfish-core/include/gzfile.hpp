  ...
  public:
    ...
    bool gzGetLines(std::vector<std::string>& logEntries)
    {
        ...
        do
        {
            ...
            // "bytesRead < fileLimitSize" means the last round of gzread
            if (bytesRead < fileLimitSize && !gzeof(logStream))
            {
                int zErrnum = 0;
            ...
            // If bytesRead equal to fileLimitSize and not reach eof, means
            // need to continue reading. Store the message which is not
            // completion.
            if (bytesRead == fileLimitSize && !gzeof(logStream))
            {
                lastMessage =
                    logBuffer.substr(initialPos, bytesRead - initialPos + 1);
