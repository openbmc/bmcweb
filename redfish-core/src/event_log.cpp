    ...
    const std::string& customText, nlohmann::json::object_t& logEntryJson)
{
    ...
    if (message == nullptr)
    {
        BMCWEB_LOG_DEBUG("formatEventLogEntry: could not find messageID '{}' for log entry {} in registry",
            messageID, logEntryID);
        return -1;
    }
    ...
    if (msg.empty())
    {
        BMCWEB_LOG_DEBUG("formatEventLogEntry: message is empty after filling fillMessageArgs");
        return -1;
    }
