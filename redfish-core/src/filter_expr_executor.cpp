...
{
    // TODO, handle paths with / in them
...
struct ApplyFilter
{
    ...
    bool operator()(const filter_ast::RelationalComparison& x);
    bool operator()(const filter_ast::BooleanOperation& operation);
...
bool doDoubleComparison(double left, filter_ast::ComparisonOperator comparator,
                        double right)
{
    ...
    switch (comparator)
    {
        case filter_ast::ComparisonOperator::Equals:
            // Note, floating point comparisons are hard.  Compare equality
            // based on epsilon
            return std::fabs(left - right) <=
                   std::numeric_limits<double>::epsilon();
        case filter_ast::ComparisonOperator::NotEquals:
            return std::fabs(left - right) >
                   std::numeric_limits<double>::epsilon();
        case filter_ast::ComparisonOperator::GreaterThan:
            return left > right;
        case filter_ast::ComparisonOperator::GreaterThanOrEqual:
            return left >= right;
        case filter_ast::ComparisonOperator::LessThan:
            return left < right;
        case filter_ast::ComparisonOperator::LessThanOrEqual:
            return left <= right;
        default:
...
bool doIntComparison(int64_t left, filter_ast::ComparisonOperator comparator,
                     int64_t right)
{
    switch (comparator)
    {
...
bool doStringComparison(std::string_view left,
                        filter_ast::ComparisonOperator comparator,
...
bool ApplyFilter::operator()(const filter_ast::RelationalComparison& x)
{
...
bool ApplyFilter::operator()(const filter_ast::BooleanOperation& operation)
{
...
{
    nlohmann::json::object_t* obj = body.get_ptr<nlohmann::json::object_t*>();
    if (obj == nullptr)
    {
    ...
    nlohmann::json::array_t* memberArr =
    ...
    nlohmann::json::array_t::iterator it = memberArr->begin();
    size_t index = 0;
