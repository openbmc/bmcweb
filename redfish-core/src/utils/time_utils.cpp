...
inline std::string formatOffsetString(std::chrono::seconds offset)
{
    char sign = '+';
    ...
    std::chrono::minutes offsetMinutes =
        std::chrono::duration_cast<std::chrono::minutes>(offset - offsetHours);
                                 ...
                                 const std::chrono::time_zone* tz)
{
    ...
    if (tz != nullptr)
    {
        ...
        // Cast to signed std::chrono::seconds before subtraction to handle
        // negative offsets without overflow when IntType is unsigned
        offset = std::chrono::duration_cast<std::chrono::seconds>(
