...
std::string toISO8061ExtendedStr(
    const std::chrono::duration<IntType, Period> t,
    const std::optional<const std::chrono::time_zone*> tz)
{
    ...
    if (tz.has_value())
    {
        ...
        auto info = tz.value()->get_info(sysTime);
        offset = std::chrono::duration_cast<std::chrono::minutes>(info.offset);
            ...
            sysSeconds.count() > SubType::max().count() - offsetAsDur.count())
        {
            duration = SubType::max();
                 ...
                 sysSeconds.count() <
                     SubType::min().count() - offsetAsDur.count())
        {
...
std::string getDateTimeUint(uint64_t secondsSinceEpoch)
{
    ...
    return details::toISO8061ExtendedStr(sinceEpoch, std::nullopt);
}
                                 ...
                                 const std::chrono::time_zone* tz)
{
    using DurationType = std::chrono::duration<std::time_t>;
    DurationType sinceEpoch(secondsSinceEpoch);
    return details::toISO8061ExtendedStr(sinceEpoch, std::optional{tz});
}
...
std::pair<std::string, std::string> getDateTimeOffsetNow(DateFormat dateFormat)
{
    ...
    std::string dateTime =
        (dateFormat == DateFormat::LocalTimezone)
            ? getDateTimeStdtimeTz(time, std::chrono::current_zone())
            : getDateTimeStdtime(time);
