...
std::optional<MappedInfo> getMappedInfo(const DbusEventLogEntry& entry)
{
    ...
    std::string messageId =
        std::format("{}.0.0.{}",
            std::string_view(dbusToRfInfo.registryName),
    ...
    if (dbusToRfInfo.numberOfArgs > 0) {
        ...
        {
            auto additionalDataElement =
                entry.AdditionalData.find(std::string(dbusToRfInfo.argsInfo[index].name));
