 ...
 * @endinternal
 */
nlohmann::json arraySizeTooLong(std::string_view property, uint64_t value)
{
...
{
    res.result(boost::beast::http::status::method_not_allowed);
    addMessageToErrorJson(res.jsonValue, arraySizeTooLong(property, value));
}
 ...
 * @endinternal
 */
nlohmann::json stringValueTooLong(std::string_view property, uint64_t value)
{
    std::string valStr = std::to_string(value);
    return getLog(redfish::registries::base::Index::stringValueTooLong,
                  std::to_array<std::string_view>({property, valStr}));
