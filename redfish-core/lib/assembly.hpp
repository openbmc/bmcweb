    ...
    const std::map<std::string, bool> locationIndicatorActiveMap)
{
    ...
    for (const auto& assembly : assemblies)
    {
        ...
        {
            setLocationIndicatorActive(aResp, assembly, iter->second);
        }
        assemblyIndex++;
    }
    ...
    const std::map<std::string, bool> locationIndicatorActiveMap)
{
    // std::vector<std::string> updatedAssemblyList;
    crow::connections::systemBus->async_method_call(
        [aResp, chassis, assemblies, locationIndicatorActiveMap](
            const boost::system::error_code ec,
            const std::vector<std::pair<
                std::string,
                std::vector<std::pair<std::string, std::vector<std::string>>>>>&
    ...
    const std::map<std::string, bool> locationIndicatorActiveMap)
{
    ...
    // check if this chassis hosts any assmebly
    crow::connections::systemBus->async_method_call(
        [aResp, chassis, assemblyPath, chassisPath, locationIndicatorActiveMap](
            const boost::system::error_code ec,
            const std::vector<std::pair<std::string, std::vector<std::string>>>&
                object) {
            if (ec)
 ...
 */
inline void
    getChassis(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
               const std::string& chassisID,
               const std::map<std::string, bool> locationIndicatorActiveMap)
{
    BMCWEB_LOG_DEBUG << "Get chassis path";
    ...
    // get the chassis path
    crow::connections::systemBus->async_method_call(
        [aResp, chassisID, locationIndicatorActiveMap](
            const boost::system::error_code ec,
            const std::vector<std::string>& chassisPaths) {
            if (ec)
            {
...
class Assembly : public Node
{
  ...
  private:
                 ...
                 const std::vector<std::string>& params) override
    {
        ...
        if (!json_util::readJson(req, asyncResp->res, "Assemblies",
                                 assemblyData))
        {
            return;
        }
        ...
        std::vector<nlohmann::json> slots = std::move(*assemblyData);
        ...
        for (auto& slot : slots)
        {
            ...
            if (!json_util::readJson(
                    slot, asyncResp->res, "LocationIndicatorActive",
        ...
        const std::string& chassisID = params[0];
        BMCWEB_LOG_DEBUG << "Chassis ID that we got called for " << chassisID;
        assembly::getChassis(asyncResp, chassisID, locationIndicatorActiveMap);
    }
};
