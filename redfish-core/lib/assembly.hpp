 ...
 * @param[in] assemblies - list of all the assemblies associated with the chassis.
                                       ...
                                       const std::string* chassisPath)
{
   ...
   // sdbusplus::message::object_path path("/abc/def");
   // std::string chassis = path.filename();

    std::string chassis = sdbusplus::message::object_path("/abc/def").filename();
    if(chassis.empty())
    {
    ...
    //std::size_t lastPos = (*chassisPath).rfind('/');
    //if (lastPos == std::string::npos)
    //{
        
    //}
    //std::string chassisId = (*chassisPath).substr(lastPos + 1);
