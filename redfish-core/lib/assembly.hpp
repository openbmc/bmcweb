                                  ...
                                  const std::string& chassisID)
{
    ...
    std::string dataID =
        "/redfish/v1/Chassis/" + chassisID + "/Assembly#/Assemblies/";

    for (const auto& assembly : assemblies)
    {
        crow::connections::systemBus->async_method_call(
            [aResp, dataID(std::string(dataID)),
                    ...
                    crow::connections::systemBus->async_method_call(
                        [aResp, dataID(std::string(dataID)),
                         assembly(std::string(assembly))](
                            ...
                            {
                                BMCWEB_LOG_ERROR
                                    << "Failed to find / in Assembly path";
                                messages::internalError(aResp->res);
                                return;
                            }
 ...
 * @return None.
 */
inline void getAssembliesLinkedToChassis(const std::shared_ptr<AsyncResp> aResp,
                                         const std::string& chassisPath)
{
    ...
    crow::connections::systemBus->async_method_call(
        [aResp, chassis, assemblyPath(std::string(assemblyPath))](
            ...
            aResp->res.jsonValue["Assemblies@odata.count"] =
                (*assemblyList).size();
    ...
    crow::connections::systemBus->async_method_call(
        [aResp, chassisID(std::string(chassisID))](
            ...
            for (const auto& path : chassisPaths)
            {
                ...
                aResp->res.jsonValue["Id"] = "Assembly info";
