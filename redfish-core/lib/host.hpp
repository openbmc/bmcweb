...
/**
 *  * VMI Systems derived class for delivering Computer Systems Schema.
 *   */
class VmiSystems : public Node
{
  public:
    /*
     *      * Default Constructor
     *           */
    VmiSystems(CrowApp &app) : Node(app, "/redfish/v1/Systems/vmi/")
    {
        entityPrivileges = {
  ...
  private:
               ...
               const std::vector<std::string> &params) override
    {
        ...
        res.jsonValue["@odata.id"] = "/redfish/v1/Systems/vmi";
        res.jsonValue["Description"] = "Virtual Management System";
        res.jsonValue["Name"] = "Computer System";
        res.jsonValue["Id"] = "vmi";
        res.jsonValue["Links"]["ManagedBy"] = {
            {{"@odata.id", "/redfish/v1/Managers/bmc"}}};
        res.jsonValue["EthernetInterfaces"] = {
            {"@odata.id", "/redfish/v1/Systems/vmi/EthernetInterfaces"}};
        res.end();
    }
};

/**
 *  * VmiInterfaceCollection class to handle the GET and PATCH on VMI Interface
 *   */
class VmiInterfaceCollection : public Node
{
  public:
    template <typename CrowApp>
    VmiInterfaceCollection(CrowApp &app) :
        Node(app, "/redfish/v1/Systems/vmi/EthernetInterfaces/")
    {
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"Login"}}},
            {boost::beast::http::verb::head, {{"Login"}}},
  ...
  private:
               ...
               const std::vector<std::string> &params) override
    {
        ...
        res.jsonValue["@odata.id"] =
            "/redfish/v1/Systems/vmi/EthernetInterfaces";
        res.jsonValue["Name"] =
            "Virtual Management Ethernet Network Interface Collection";
        res.jsonValue["Description"] =
            "Collection of Virtual Management Interfaces for this system";
        nlohmann::json &iface_array = res.jsonValue["Members"];
        iface_array.push_back(
            {{"@odata.id",
              "/redfish/v1/Systems/vmi/EthernetInterfaces/intf0"}});
        iface_array.push_back(
            {{"@odata.id",
              "/redfish/v1/Systems/vmi/EthernetInterfaces/intf1"}});
...
/**
 * VmiInterface derived class for delivering Ethernet Schema
 */
class VmiInterface : public Node
{
  public:
    /*
     * Default Constructor
     */
    template <typename CrowApp>
    VmiInterface(CrowApp &app) :
        Node(app, "/redfish/v1/Systems/vmi/EthernetInterfaces/<str>/",
             std::string())
    {
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"Login"}}},
  ...
  private:
        ...
        const boost::container::flat_set<IPv4AddressData> &ipv4Data)
    {
        ...
        json_response["@odata.id"] =
            "/redfish/v1/Systems/vmi/EthernetInterfaces/" + iface_id;
