...
/**
 * @brief Retrieves valid chassis-fan ID
 * @param asyncResp   Pointer to object holding response data
 * @param callback  Callback for next step to get valid chassis-fan ID
 */
template <typename Callback>
inline void getValidChassisFanID(const std::shared_ptr<AsyncResp>& asyncResp,
    ...
    auto respHandler =
        [callback{std::move(callback)}, asyncResp, chassisId,
         fanId](const boost::system::error_code ec,
                const std::vector<std::string>& chassisfanPaths) {
            BMCWEB_LOG_DEBUG << "getValidChassisFanID respHandler enter";
            if (ec)
            {
                BMCWEB_LOG_ERROR
                    << "getValidChassisFanID respHandler DBUS error: " << ec;
                messages::internalError(asyncResp->res);
                return;
            }

            std::optional<std::string> validChassisFanId;
            std::string chassisfanName;
            for (const std::string& chassisfan : chassisfanPaths)
            {
                const std::string path(chassisfan);
                std::string::size_type nPos1 = std::string::npos;
                std::string::size_type nPos2 = std::string::npos;
                std::string::size_type nPos3 = std::string::npos;
                nPos1 = path.find_last_of("/");
                nPos2 = path.find_last_of("/", nPos1 - 1);
                nPos3 = path.find_last_of("/", nPos2 - 1);
                chassisfanName = path.substr(nPos3 + 1, nPos2 - nPos3 - 1);
                if (chassisfanName.empty())
                {
                    BMCWEB_LOG_ERROR << "Failed to find '/' in " << chassisfan;
                    continue;
                }
                if (chassisfanName == chassisId)
                {
                    validChassisFanId = chassisId;
                    break;
                }
            }
            callback(validChassisFanId);
        };
                       ...
                       const std::string& chassisId, const std::string& fanId)
{
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, fanId](
            ...
            if (subtree.size() == 0)
            {
                BMCWEB_LOG_DEBUG << "Can't find Fan D-Bus object!";
...
class Fan : public Node
{
  ...
  private:
               ...
               const std::vector<std::string>& params) override
    {
        ...
        auto getFanPath = [asyncResp, chassisId,
                           fanId](const std::optional<std::string>& fanPath) {
            if (!fanPath)
            {
            ...
            auto getChassisFanPath =
                [asyncResp, chassisId,
                 fanId](const std::optional<std::string>& chassisfanPath) {
                    if (!chassisfanPath)
                    {
            ...
            getValidChassisFanID(asyncResp, chassisId, fanId,
                                 std::move(getChassisFanPath));
        };
