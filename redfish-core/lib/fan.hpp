...
inline bool checkFanId(const std::string& fanPath, const std::string& fanId)
{
    const std::string& fanName =
        sdbusplus::message::object_path(fanPath).filename();
    if (fanName.empty() || fanName != fanId)
    {
        return false;
    }
                ...
                if (service == "xyz.openbmc_project.PLDM")
                {
                    sdbusplus::asio::getProperty<std::vector<std::string>>(
                        *crow::connections::systemBus,
                        ...
                        [callback{std::move(callback)}, asyncResp, service,
                         objectPath, fanId](const boost::system::error_code ec1,
                ...
                else
                {
                    sdbusplus::asio::getProperty<std::vector<std::string>>(
                        *crow::connections::systemBus,
                        ...
                        "xyz.openbmc_project.Association", "endpoints",
                        [callback{std::move(callback)}, asyncResp, service,
                         objectPath, fanId](const boost::system::error_code ec2,
                                            const std::vector<std::string>&
                                                fanSensorsEndpoints) {
                        if (ec2)
                        {
