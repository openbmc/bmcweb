            ...
            for (const auto& tempObject : subtree)
            {
                ...
                crow::connections::systemBus->async_method_call(
                    [asyncResp, fanId, tempPath, connectionName](
                        ...
                        auto* values = std::get_if<std::vector<
                            std::tuple<std::string, std::string, std::string>>>(
                            &property);
                        if (values == nullptr)
                        {
                        ...
                        {
                            const auto& [forward, reverse, endpoint] = value;
                            const std::string& objPath = endpoint;
                            std::vector<std::string> split;
                            // Reserve space for
                            // /xyz/openbmc_project/sensors/<name>/<subname>
                            split.reserve(6);
                            ...
                            {
                                BMCWEB_LOG_ERROR
                                    << "The associated fan was not obtained !"
                                    << fanName;
                                continue;
                            }
                        }
                    },
                    connectionName, tempPath, "org.freedesktop.DBus.Properties",
                    "Get", "xyz.openbmc_project.Association.Definitions",
                    "Associations");
            }
        },
                    ...
                    for (const auto& tempObject : subtree)
                    {
                        ...
                        crow::connections::systemBus->async_method_call(
                            [asyncResp, chassisId, &fanList, newPath](
                                ...
                                for (const auto& value : *values)
                                {
                                    const auto& [forward, reverse, endpoint] =
                                        value;
                                    const std::string& objPath = endpoint;
                                    std::vector<std::string> split;
                                    // Reserve space for
