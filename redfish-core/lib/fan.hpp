                       ...
                       const std::string& chassisId, const std::string& fanId)
{
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, chassisId, fanId,
         interfaces](const boost::system::error_code ec,
            ...
            {
                const std::string& tempPath = tempsubtreepath + "/sensors";

                crow::connections::systemBus->async_method_call(
                    [asyncResp, fanId, tempPath, tempsubtreepath](
                        ...
                        for (const auto& tempObject : object)
                        {
                            ...
                            crow::connections::systemBus->async_method_call(
                                [asyncResp, fanId, tempsubtreepath,
                                 connectionName](
                                    const boost::system::error_code ec,
                                    const std::variant<
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, chassisId, fanId,
         interfaces](const boost::system::error_code ec,
            ...
            for (const auto& tempPath : subtreepaths)
            {
                    ...
                    for (const auto& tempObject : subtreepaths)
                    {
                        ...
                        crow::connections::systemBus->async_method_call(
                            [asyncResp, chassisId, &fanList, newPath](
                                ...
                                asyncResp->res.jsonValue["Members@odata."
                                                         "count"] =
                                    fanList.size();
                            },
                            "xyz.openbmc_project.ObjectMapper", tempPath,
