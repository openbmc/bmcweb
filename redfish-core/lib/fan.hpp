...
inline void
    getFanSpeed(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                       const std::string& service,
                       const std::string& sensorPath,
                       const std::string& name)
{
    sdbusplus::asio::getProperty<uint64_t>(
        *crow::connections::systemBus, service, sensorPath,
        ...
        [asyncResp, name]
         (const boost::system::error_code ec, const uint64_t speed) {
        if (ec)
        {
            if (ec.value() != EBADR)
            {
    ...
    const dbus::utility::MapperEndPoints& sensorPaths)
{
    dbus::utility::getDbusObject(
        ...
        if (ec || object.empty())
        {
            BMCWEB_LOG_ERROR << "DBUS response error for getDbusObject " << ec;
            messages::internalError(asyncResp->res);
            return;
        }
