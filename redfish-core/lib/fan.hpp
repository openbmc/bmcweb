...
inline void
    afterGetFanSensorPriority(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                              const std::string& service, const std::string& sensorPath,
                              const std::string& chassisId,
                              const boost::system::error_code& ec, size_t priority)
{
    if (ec)
    {
        if (ec.value() != EBADR)
                         ...
                         const std::string& chassisId)
{
    ...
    sdbusplus::asio::getProperty<size_t>(
        *crow::connections::systemBus, service, sensorPath,
        ...
        std::bind_front(afterGetFanSensorPriority, asyncResp, service, sensorPath, chassisId));
}
