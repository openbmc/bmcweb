...
inline void afterChassisDriveCollectionSubtree(
    ...
    const dbus::utility::MapperEndPoints& driveList)
{
    ...
    if (ec)
    {
        // no drives associated should not fail storage resource fetch
        BMCWEB_LOG_DEBUG("Drive association call error {}", ec);
        return;
    }
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    getDrives(asyncResp, storageId, storage->first);
