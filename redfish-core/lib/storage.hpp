        ...
        [asyncResp, health, parent,
         storageId](const boost::system::error_code ec,
            ...
            for (const std::string& objpath : storageList)
            {
                ...
                if (!boost::algorithm::starts_with(objpath, parent))
                {
                    continue;
                }
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, health, parent,
         storageId](const boost::system::error_code ec,
            ...
            {
                // Skip path if the object is not under the parent object path.
                if (!boost::algorithm::starts_with(path, parent))
                {
                    continue;
                }
                ...
                crow::connections::systemBus->async_method_call(
                    [asyncResp, health,
                     storageId](const boost::system::error_code ec,
                        ...
                        {
                            if (!boost::algorithm::ends_with(object,
                                                             "/" + storageId))
                            {
                                continue;
                            }

                            getDrives(asyncResp, health, object, storageId);
                            getStorageControllers(asyncResp, health, object,
                                                  storageId);
                        }
                    },
                    "xyz.openbmc_project.ObjectMapper",
                ...
                [asyncResp, storageId,
                 driveId](const boost::system::error_code ec,
                    ...
                    asyncResp->res.jsonValue["Links"]["Chassis"] = {
                        {"@odata.id", "/redfish/v1/Chassis/" + storageId}};
                    ...
                    asyncResp->res
                        .jsonValue["Actions"]["#Drive.Reset"]["target"] =
                        "/redfish/v1/Systems/system/Storage/" + storageId +
...
/**
 * Function reset the Drive.
 *
 * @param[in] asyncResp - Shared pointer for completing asynchronous calls
 */
inline void
    doDriveForceRestart(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                        const std::string& driveId)
{
         ...
         interfaceName](const boost::system::error_code ec,
            ...
            auto driveState = std::find_if(
                subtree.begin(), subtree.end(), [&driveId](auto& object) {
                    const sdbusplus::message::object_path path(object.fist);
            ...
            const std::string& propertyValue =
                "xyz.openbmc_project.State.Drive.Transition.Reboot";
...
/**
 * Function powercycle the Drive.
 *
 * @param[in] asyncResp - Shared pointer for completing asynchronous calls
 */
inline void
    doDrivePowerCycle(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                      const std::string& driveId)
{
    const char* interfaceName = "xyz.openbmc_project.State.Drive";
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, driveId,
         interfaceName](const boost::system::error_code ec,
            ...
            const std::string& connectionName = connectionNames[0].first;
            const std::string& propertyValue =
                "xyz.openbmc_project.State.Drive.Transition.Powercycle";
            const char* destProperty = "RequestedDriveTransition";
            std::variant<std::string> dbusPropertyValue(propertyValue);
            ...
            [](const crow::Request& req,
               ...
               const std::string&, const std::string& driveId) {
                ...
                if (resetType == "ForceRestart")
                {
                    ...
                    doDriveForceRestart(asyncResp, driveId);
                    return;
                }
                if (resetType == "PowerCycle")
                {
                    BMCWEB_LOG_DEBUG << "Proceeding with " << resetType;
                    doDrivePowerCycle(asyncResp, driveId);
                    return;
                }
