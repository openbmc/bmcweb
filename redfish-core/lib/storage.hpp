        ...
        [asyncResp, health, parent,
         storageId](const boost::system::error_code ec,
            ...
            for (const std::string& objpath : storageList)
            {
                ...
                while (!path.str.empty() && path.str.size() > parent.size())
                {
                    if (path.str == parent)
                    {
                        found = true;
        ...
        [asyncResp, health, parent,
         storageId](const boost::system::error_code ec,
            ...
            for (const auto& [path, interfaceDict] : subtree)
            {
                ...
                while (!objPath.str.empty() &&
                       objPath.str.size() > parent.size())
                {
                    if (objPath.str == parent)
                    {
                        found = true;
                        break;
                    }

                    objPath = objPath.parent_path();
                }
                 ...
                 driveId](const boost::system::error_code ec,
                    ...
                    getChassisId(
                        ...
                        [&path](const std::string& chassisPath) {
                            ...
                            sdbusplus::message::object_path objPath(path);
                            while (!objPath.str.empty() &&
                                   objPath.str.size() > chassisPath.size())
                            {
                                if (objPath.str == chassisPath)
                                {
                                    return true;
                                }
