...
{
    nlohmann::json::object_t driveReset;
    driveReset["target"] = crow::utility::urlFromPieces(
        "redfish", "v1", "Systems", "system", "Storage", "1", "Drives", driveId,
        "Actions", "Drive.Reset");
    driveReset["@Redfish.ActionInfo"] = crow::utility::urlFromPieces(
        "redfish", "v1", "Systems", "system", "Storage", "1", "Drives", driveId,
        "ResetActionInfo");
    asyncResp->res.jsonValue["Actions"]["#Drive.Reset"] = std::move(driveReset);
}
                           ...
                           const std::string& path, bool hasDriveState)
{
    sdbusplus::asio::getAllProperties(
        *crow::connections::systemBus, connectionName, path,
        ...
        [asyncResp, driveId, hasDriveState](
            const boost::system::error_code ec,
                 ...
                 property : propertiesList)
        {
            ...
            // Only add reset links if Item.Drive has Resettable == true and
            // State.Drive also exist within the service.
...
inline void requestDriveResetAction(App& app)
{
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/system/Storage/1/Drives/<str>/"
        ...
        std::array<const char*, 2> interfaces = {
            "xyz.openbmc_project.Inventory.Item.Drive",
