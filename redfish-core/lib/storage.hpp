        ...
        [asyncResp](
                     ...
                     property : propertiesList)
            {
                    ...
                    (propertyName == "Model"))
                {
                    ...
                    {
                        // illegal property
                        BMCWEB_LOG_DEBUG << "Illegal property for "
                                         << propertyName;
                        messages::internalError(asyncResp->res);
                        return;
                    }
                            ...
                            const std::string& path)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, path](const boost::system::error_code ec,
            ...
            if (!std::holds_alternative<bool>(present))
            {
                          ...
                          const std::string& path)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp](const boost::system::error_code ec,
            ...
            if (!std::holds_alternative<bool>(rebuilding))
            {
                BMCWEB_LOG_DEBUG << "Illegal property: Rebuilding";
                messages::internalError(asyncResp->res);
                return;
            }
                         ...
                         const std::string& path)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, path](const boost::system::error_code ec,
            ...
            const std::string& type = std::get<std::string>(type);
            std::optional<std::string> mediaType = convertDriveType(type);
            if (!mediaType)
            {
                BMCWEB_LOG_DEBUG << type << " is not a supported Media Type";
                messages::internalError(asyncResp->res);
                return;
            }
