...
inline void removeActiveSoftwareForDrive(std::string objPath)
{
    sdbusplus::asio::getProperty<
        ...
        *crow::connections::systemBus, "com.google.gbmc.ssd", objPath,
        ...
        for (auto sw = swAssociations.begin(); sw != swAssociations.end(); sw++)
        {
            if (get<2>(*sw) != "/xyz/openbmc_project/software/functional")
            {
                newSwAssociations.push_back(*sw);
            }
        }
    ...
    sdbusplus::asio::setProperty(
        *crow::connections::systemBus, "com.google.gbmc.ssd",
        ...
        BMCWEB_LOG_DEBUG
            ...
            << " Property: xyz.openbmc_project.Association.Definitions to: \"functional\" \"gssd_software\" \"/xyz/openbmc_project/software/functional\"";
