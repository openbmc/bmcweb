...
inline std::optional<std::string> convertDriveType(const std::string& driveType)
{
    if (driveType == "xyz.openbmc_project.Inventory.Item.Drive.DriveType.HDD")
    {
        return "HDD";
    }
    if (driveType == "xyz.openbmc_project.Inventory.Item.Drive.DriveType.SSD")
    {
        return "SSD";
    }
                         ...
                         const std::string& path)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, path](const boost::system::error_code ec2,
            ...
            const std::string* foundType = std::get_if<std::string>(&type);
            ...
            std::optional<std::string> driveType = convertDriveType(*foundType);
            if (!driveType)
            {
                BMCWEB_LOG_DEBUG << *foundType
