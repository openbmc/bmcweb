    ...
    const dbus::utility::MapperGetSubTreeResponse& object)
{
    ...
    if (object.empty())
    {
        messages::actionNotSupported(asyncResp->res, "NetworkAdapter.Reset");
        return;
    }
    if (object.size() > 1)
    {
    ...
    // Request the reset by writing the PendingReset property on the
    // Control.Reset object. The daemon performs the reset asynchronously and
    // writes PendingReset back to "None" when it completes.
    setDbusPropertyAction(
        asyncResp, service, sdbusplus::object_path(resetPath),
        "xyz.openbmc_project.Control.Reset", "PendingReset", "ResetType",
        "NetworkAdapter.Reset",
        std::string(
