...
inline void afterGetNDFMacAddressObject(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const boost::system::error_code& ec,
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    const auto& [macPath, services] = subtree[0];
    if (services.empty())
    {
        return;
    }

    const std::string& serviceName = services[0].first;
    dbus::utility::getAllProperties(
        serviceName, macPath, "xyz.openbmc_project.Network.MACAddress",
        std::bind_front(afterGetNDFMacAddress, asyncResp));
}

inline void getNDFPermanentMacAddress(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& portPath)
{
    ...
    dbus::utility::getAssociatedSubTree(
        ...
        std::bind_front(afterGetNDFMacAddressObject, asyncResp));
}
    ...
    const std::string& serviceName)
{
    ...
    getNDFPermanentMacAddress(asyncResp, portPath);
}
