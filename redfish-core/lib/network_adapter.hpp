...
inline void handleNetworkAdapterPortMetricsPathPortMetricsGet(
    ...
    const dbus::utility::MapperGetObject& object)
{
    if (ec)
    {
        BMCWEB_LOG_ERROR("DBus response error on GetDbusObject {}", ec);
        return;
    }

    if (object.empty())
    {
        BMCWEB_LOG_DEBUG("No Metrics Object Path found");
        return;
    }

    if (object.size() != 1)
    {
        BMCWEB_LOG_DEBUG("Multiple Metrics Object Paths found");
        return;
    }
    ...
    [[maybe_unused]] const std::string& serviceName)
{
    ...
    dbus::utility::getDbusObject(
        portPath / "Metrics", {},
        std::bind_front(handleNetworkAdapterPortMetricsPathPortMetricsGet,
                        asyncResp, portPath));
}
