...
// Maps the D-Bus nic metric name to its Redfish PortMetrics location. A
// table keeps the mapping readable as counters are added, instead of a
// long if/else chain.
    ...
    const dbus::utility::MapperGetSubTreeResponse& object)
{
    ...
    for (const auto& [path, service] : object)
    {
        ...
        for (const auto& [name, jsonPtr] : nicPortMetricMap)
        {
            if (metricName == name)
            {
                getMetricProperty(
                    asyncResp, serviceName, path,
                    nlohmann::json::json_pointer(std::string(jsonPtr)));
                break;
            }
        }
    }
