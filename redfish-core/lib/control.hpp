...
inline void afterClockLimitReset(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const boost::system::error_code& ec, const sdbusplus::message_t& msg)
{
    ...
    BMCWEB_LOG_ERROR("Clock limit Reset method failed: {}", ec);
    messages::internalError(asyncResp->res);
}

inline void dispatchClockLimitReset(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& service, const std::string& objectPath)
{
    // The Control.OperatingClockSpeed Reset method is a deferred D-Bus
    // method: dbus-sensors holds the call open until the NSM
    // SET_CLOCK_LIMIT round-trip to the device completes, then replies
    // with the transport result. bmcweb maps that single reply straight
    // to the Redfish response and lets sd-bus enforce the method
    // timeout, so no ResetComplete signal subscription or local timer is
    // needed.
    crow::connections::systemBus->async_method_call(
        [asyncResp](const boost::system::error_code& ec,
                    const sdbusplus::message_t& msg) {
            afterClockLimitReset(asyncResp, ec, msg);
        },
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    dispatchClockLimitReset(asyncResp, service, controlPath);
}
