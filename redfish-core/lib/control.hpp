        ...
        [asyncResp, chassisId, controlId, setPoint, controlMode,
         controlModeStr](const std::optional<std::string>& validChassisPath) {
            ...
            getControlObject(
                ...
                [asyncResp, setPoint, controlMode,
                 controlModeStr](const std::string& controlPath,
                                 const dbus::utility::MapperGetObject& object) {
                    if (object.empty())
                    {
                        messages::internalError(asyncResp->res);
                    ...
                    {
                        BMCWEB_LOG_ERROR(
                            "No supported control interface found for path: {}",
                            controlPath);
                        messages::internalError(asyncResp->res);
                        return;
                    }
                    ...
                    {
                        std::string_view dbusProperty =
                            findDbusPropertyName(*mapping, "SetPoint");
                        if (dbusProperty.empty())
                        {
                            BMCWEB_LOG_ERROR(
                                "SetPoint property not found in interface mapping");
                            messages::internalError(asyncResp->res);
                            return;
                        }
                        ...
                        dbus::utility::getAllProperties(
                            ...
                            [asyncResp, serviceName, controlPath,
                             matchedInterface, dbusProperty,
                             setPoint](const boost::system::error_code& ec,
                                ...
                                {
                                    BMCWEB_LOG_ERROR(
                                        "SetPoint property {} not found",
                                        dbusProperty);
                                    messages::internalError(asyncResp->res);
                                    return;
                                }

                                // Convert JSON to the same type and set
                                // property
                                std::visit(
                                    [&](auto&& val) {
                                        ...
                                        sdbusplus::asio::setProperty(
                                            *crow::connections::systemBus,
                                            serviceName, controlPath,
                                            matchedInterface,
                                            std::string(dbusProperty), newValue,
                                            [asyncResp](
                    ...
                    if (controlMode)
                    {
                        ...
                        {
                            BMCWEB_LOG_ERROR(
                                "ControlMode property not found in interface mapping");
                            messages::internalError(asyncResp->res);
                            return;
                        }
                        ...
                        {
                            BMCWEB_LOG_ERROR(
                                "ControlMode converter not defined for interface: {}",
                                matchedInterface);
                        ...
                        if (!dbusValue)
                        {
                            messages::propertyValueNotInList(
                                asyncResp->res, "ControlMode", *controlModeStr);
                            return;
                        }

                        std::visit(
                            [&asyncResp, &serviceName, &controlPath,
                             &matchedInterface, &dbusProperty](auto&& val) {
                                sdbusplus::asio::setProperty(
                                    *crow::connections::systemBus, serviceName,
                                    controlPath, matchedInterface,
                                    std::string(dbusProperty), val,
                                    [asyncResp](
                                        const boost::system::error_code& ec) {
                                        if (ec)
                                        {
                                            BMCWEB_LOG_ERROR(
                                                "Error setting ControlMode: {}",
                                                ec);
                                            messages::internalError(
                                                asyncResp->res);
                                        }
