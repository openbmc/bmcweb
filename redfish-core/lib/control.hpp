            ...
            for (const std::string& iface : interfaces)
            {
                ...
                if (prefix)
                {
                    ...
                    std::string controlName =
                        control_utils::makeControlName(*prefix, endpointName);
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    if (ec)
    {
        if (ec.value() != EBADR)
        {
            BMCWEB_LOG_ERROR("GetAssociatedSubTree error for containing: {}",
                             ec);
            messages::internalError(asyncResp->res);
        }
        return;
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    for (const auto& [objectPath, serviceMap] : subtree)
    {
        ...
        if (objPath.filename() == endpointName && !serviceMap.empty())
        {
            ...
            inventoryService = serviceMap.front().first;
            break;
        }
    }
    ...
    dbus::utility::getAssociatedSubTree(
        ...
        std::bind_front(afterVerifyControlledByForGet, asyncResp, chassisId,
                        controlId, prefix, endpointName, inventoryPath,
                        inventoryService));
}
                         ...
                         const std::optional<std::string>& validChassisPath)
{
    ...
    if (chassisObjPath.filename() == endpointName)
    {
        dbus::utility::getAssociatedSubTree(
            ...
            control_utils::controlRequiredInterfaces,
            std::bind_front(afterVerifyControlledByForGet, asyncResp, chassisId,
                            controlId, prefix, endpointName, *validChassisPath,
                            std::string()));
        return;
    }
