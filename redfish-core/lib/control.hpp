...
inline void dispatchControlReset(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& service, const std::string& objectPath,
    std::string_view iface)
{
    // The control Reset method is a deferred D-Bus method: the owning
    // daemon (for ClockLimit, dbus-sensors) holds the call open until the
    // device round-trip (NSM SET_CLOCK_LIMIT) completes, then replies with
    // the transport result. bmcweb maps that single reply straight to the
    // Redfish response and lets sd-bus enforce the method timeout, so no
    // ResetComplete signal subscription or local timer is needed. The
    // interface is derived from the ControlId prefix rather than hardcoded
    // so a new control type cannot be routed to the wrong interface.
    crow::connections::systemBus->async_method_call(
        [asyncResp](const boost::system::error_code& ec,
            ...
            if (dbusError != nullptr && dbusError->name != nullptr)
            {
                ...
                {
                    // A reset is already in flight on this control; ask
                    // the client to retry.
            ...
            // Encode failure (WriteFailure), a transport error reported
            // by the deferred reply, or any other bus error.
            BMCWEB_LOG_ERROR("Control Reset method failed: {}", ec);
            messages::internalError(asyncResp->res);
        },
        service, objectPath, std::string(iface), "Reset");
}

inline void afterFindControlForReset(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& controlId, std::string_view iface,
    const boost::system::error_code& ec,
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    if (ec)
    ...
    dispatchControlReset(asyncResp, service, controlPath, iface);
}
