...
inline void handlerTemperatureReadingsCelsius(
        ...
        for (const auto& [service, interfaces] : object)
        {
            sdbusplus::asio::getProperty<double>(
                *crow::connections::systemBus, service, path,
                ...
                [asyncResp, chassisId,
                 path](const boost::system::error_code& ec, double val) {
                if (ec)
                {
                    if (ec.value() != EBADR)
                    {
                        BMCWEB_LOG_ERROR(
                            "DBUS response error for Functional {}",
                            ec.value());
    ...
    const std::string& validChassisPath, const std::string& chassisId)
{
    ...
    dbus::utility::getAssociatedSubTree(
        ...
        handlerTemperatureReadingsCelsius(asyncResp, objects, chassisId);
        });
}
