    ...
    const std::optional<std::string>& validChassisPath)
{
    ...
    getLeakDetectorSubTree(
        ...
        std::bind_front(afterGetLeakDetectorCollection, asyncResp, chassisId));
}
...
inline void afterGetLeakDetectorSubTreeForDetector(
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    auto found = std::ranges::find_if(subtree, [&detectorId](const auto& e) {
        return sdbusplus::object_path(e.first).filename() == detectorId;
    });
