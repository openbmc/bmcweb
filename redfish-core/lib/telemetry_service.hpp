                                 ...
                                 const char* postfix)
{
    ...
    if (t.count() != 0)
    {
        ...
        {
            fmt = std::to_string(static_cast<double>(t.count()) /
                                 static_cast<double>(
                                    std::chrono::milliseconds::period::den));
            boost::algorithm::trim_right_if(fmt,
                                            [] (char x) { return x == '0' ||
                                                                 x == '.'; });
        }
...
static bool assignIfPresent(const boost::container::flat_map<
                                std::string, std::variant<V...>>& ret,
                            const char* propertyName,
                            nlohmann::json& member,
                            const S& convert)
{
    if constexpr (T t; std::is_same<
                        std::function<decltype(convert(t))(const T&)>,
                        S>::value)
    {
        if (!convert)
        {
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp] (const boost::system::error_code ec,
                     const boost::container::flat_map<
                         std::string, std::variant<uint32_t>>& ret) {
            if (ec)
            {
            ...
            assignIfPresent<uint32_t>(ret, "PollRateResolution",
...
{
  public:
    TelemetryService(CrowApp& app) :
        Node(app, "/redfish/v1/TelemetryService/")
    {
        entityPrivileges = {
...
static std::vector<std::string> toReportActions(const std::vector<std::string>&
                                                            actions)
{
...
using Metrics = std::vector<std::map<std::string,
...
static void getReportDefinitonProperties(const std::shared_ptr<AsyncResp>
                                            asyncResp,
                                         ...
                                         const std::string& id)
{
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp] (const boost::system::error_code ec,
            ...
            assignIfPresent<std::vector<std::string>>(ret, "ReportAction",
                                                      asyncResp->res.jsonValue[
                                                        "ReportActions"],
                                                      toReportActions);
            auto assigned = assignIfPresent<
                                std::string>(ret, "ReportingType",
                                             asyncResp->res.jsonValue[
            ...
            if (assigned &&
                ...
                "Periodic")
            {
                assignIfPresent<uint32_t>(ret, "ScanPeriod",
                                          asyncResp->res.jsonValue[
                                            "Schedule"]["RecurrenceInterval"],
                                          toDurationFormat);
            }
            assignIfPresent<ReadingParameters>(ret, "ReadingParameters",
                                               asyncResp->res.jsonValue[
                                                "Metrics"], toMetrics);
        },
        "xyz.openbmc_project.MonitoringService",
...
using Readings = std::vector<std::tuple<std::string, std::string,
                                        double, std::string>>;
                                ...
                                const std::string& id)
{
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp] (const boost::system::error_code ec,
                     const boost::container::flat_map<std::string,
                        std::variant<Readings, std::string>>& ret) {
            if (ec)
            {
                messages::internalError(asyncResp->res);
                BMCWEB_LOG_ERROR << "respHandler DBus error " << ec;
        ...
        crow::connections::systemBus->async_method_call(
            [asyncResp, id] (const boost::system::error_code ec,
                ...
                const std::string target = "/xyz/openbmc_project/"
                                           "MonitoringService/Reports/"
                                           "TelemetryService/" + id;
                auto objpath = std::find(reports.begin(), reports.end(),
                                         target);
