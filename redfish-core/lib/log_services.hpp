...
[[maybe_unused]] static LogParseError fillEventLogMessage(
    std::string& eventMessage, const std::vector<std::string>& messageArgs)
{
    int i = 0;
        ...
        crow::connections::systemBus->async_method_call(
            ...
            for (const auto& objectPath : resp)
            {
                ...
                if ((messageId != nullptr) && (!(*messageId).empty()))
                {
                    ...
                    {
                        BMCWEB_LOG_WARNING
                            << "messageId not found in registry: " << messageId;
                        continue;
                    }
