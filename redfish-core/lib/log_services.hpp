...
inline void hostLogEntryParser(const std::string& logEntry,
               ...
               const std::shared_ptr<bmcweb::AsyncResp>& asyncResp) {
                ...
                for (const std::filesystem::path& it : hostLoggerFiles)
                {
                    ...
                    while (logFile.gzGetLine(logEntry))
                    {
                        ...
                        // So we use gzGetLine to get one line and use '\r' to
                        // spilt log entry. Also, '\t' will effect the display
                        // of log entry, so we remove it.
                        if (logEntry.find('\t') != std::string::npos)
                        {
                            boost::replace_all(logEntry, "\t", "");
                        }
                        hostLogEntryParser(logEntry, msgBuf);
