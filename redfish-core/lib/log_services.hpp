        ...
        [asyncResp, entryID, dumpPath, dumpType, dumpId](
            ...
            for (auto& objectPath : resp)
            {
                ...
                else if (dumpType == "System" || dumpType == "Resource")
                {
                    std::string dumpEntryId(dumpType + "_");
                    dumpEntryId.append(entryID);
                    asyncResp->res.jsonValue["@odata.id"] = dumpPath + dumpEntryId;
                    asyncResp->res.jsonValue["Id"] = dumpEntryId;
                    asyncResp->res.jsonValue["Name"] = "System Dump Entry";
                    asyncResp->res.jsonValue["DiagnosticDataType"] = "OEM";
                    asyncResp->res.jsonValue["OEMDiagnosticDataType"] = dumpType;
                    asyncResp->res.jsonValue["AdditionalDataURI"] =
                         "/redfish/v1/Systems/system/LogServices/Dump/"
                         "attachment/" +
                         dumpEntryId;
        ...
        [dumpEntryPath](boost::system::error_code err,
            ...
            std::find_if(
                ...
                [&status](
                    ...
                    {
                        status = std::get<std::string>(statusProp.second);
                        return true;
                    }
                    return false;
                });
            ...
            {
                BMCWEB_LOG_ERROR << "Status: Not complete";
                return !task::completed;
            }
                       ...
                       const std::string& dumpType)
{
    ...
    std::vector<std::pair<std::string, std::string>> createDumpParamVec(createDumpParams);
