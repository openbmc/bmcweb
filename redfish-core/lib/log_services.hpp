    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, entryID, dumpPath, dumpType](
            ...
            for (auto& objectPath : resp)
            {
                ...
                asyncResp->res.jsonValue["Created"] = timestamp;
                       ...
                       const std::string& dumpType)
{
    ...
    {
        BMCWEB_LOG_ERROR << "readJson failed";
        messages::internalError(asyncResp->res);
        return;
    }
    ...
    if (dumpType == "System")
    {
        if (!oemDiagnosticDataType || diagnosticDataType.empty())
        {
                 ...
                 (diagnosticDataType != "OEM"))
        {
            ...
            messages::invalidObject(asyncResp->res, "System Dump creation parameters");
            return;
        }
    }
    else if (dumpType == "BMC")
    {
        ...
        else if (diagnosticDataType != "Manager")
        {
            ...
            messages::invalidObject(asyncResp->res, "BMC Dump creation parameters");
            return;
        }
    }
