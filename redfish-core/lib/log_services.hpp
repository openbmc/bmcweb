    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/LogServices/DeviceLog/")
        ...
        .methods(boost::beast::http::verb::get)(
            [](const crow::Request&,
               ...
               const std::string& chassisId) {
                asyncResp->res.jsonValue["@odata.id"] =
                    "/redfish/v1/Systems/system/LogServices/" + chassisId;
                asyncResp->res.jsonValue["@odata.type"] =
                    "#LogService.v1_1_0.LogService";
...
{
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/LogServices/DeviceLog/Entries")
        .privileges({{"Login"}})
        .methods(
            ...
            asyncResp->res.jsonValue["@odata.type"] =
                "#LogEntryCollection.LogEntryCollection";
            asyncResp->res.jsonValue["@odata.id"] =
                "/redfish/v1/Systems/test/LogServices/" + chassisId +
                "/Entries";
                 ...
                 chassisId](const boost::system::error_code ec,
                    ...
                    for (const std::string& objpath : subtreePath)
                    {
                        sdbusplus::message::object_path path(
                            objpath) if (path.filename().empty())
                        {
            ...
            if (chassisId == "warthog_2")
            {
                ...
                for (const std::string& objpath : objpaths)
                {
                    ...
                    logEntryArray.push_back({{"@odata.id", entry + objpath}});
                }
                    ...
                    crow::connections::systemBus->async_method_call(
                        [asyncResp, chassisId,
                         entryId](const boost::system::error_code ec,
                            ...
                            std::string* severity = nullptr;
                            std::string* message = nullptr;
                            std::string* entryType = nullptr;
                            std::string* oemRecordFormat = nullptr;
                            ...
                            for (auto& propertyMap : resp)
                            {
                                ...
                                else if (propertyMap.first == "EntryType")
                                {
                                    entryType = std::get_if<std::string>(
                                        &propertyMap.second);
                                }
                                else if (propertyMap.first == "OemRecordFormat")
                                {
                                    oemRecordFormat = std::get_if<std::string>(
                                        &propertyMap.second);
                                }
                            }
                            ...
                            auto type = (entryType)
                                            ? entryType->substr(
                                                  entryType->rfind('.') + 1)
                                            : "Event";
                            asyncResp->res.jsonValue["EntryType"] = type;
                        ...
                        objects[0].first,
                        "/xyz/openbmc_project/logging/devices/" + chassisId +
                            "/" + entryId,
                        "org.freedesktop.DBus.Properties", "GetAll",
                        "xyz.openbmc_project.Logging.Entry");
                },
