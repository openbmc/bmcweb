...
{
    uint64_t len = std::to_string(usecSinceEpoch).length() - 10;
    time_t t = static_cast<time_t>(usecSinceEpoch /
                                   static_cast<uint64_t>(std::pow(10, len)));
...
class DumpService : public Node
{
  public:
    template <typename CrowApp>
    DumpService(CrowApp& app) : Node(app, PATH)
    {
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"Login"}}},
            {boost::beast::http::verb::head, {{"Login"}}},
  ...
  private:
               ...
               const std::vector<std::string>& params) override
    {
        ...
        std::string dumpType;
        if (std::strcmp(INTERFACE, "xyz.openbmc_project.Dump.Entry.BMC") == 0)
        {
            dumpType = "BMC";
        }
        else
        ...
        asyncResp->res.jsonValue["@odata.id"] = std::string(PATH);
        asyncResp->res.jsonValue["@odata.type"] =
        ...
        asyncResp->res.jsonValue["Id"] = dumpType;
        asyncResp->res.jsonValue["OverWritePolicy"] = "WrapsWhenFull";
        asyncResp->res.jsonValue["Entries"] = {
            {"@odata.id", std::string(DUMPENTRYPATH)}};
        asyncResp->res.jsonValue["Actions"] = {
            {"#LogService.ClearLog",
             {{"target", std::string(PATH) + "Actions/LogService.ClearLog"}}},
            {"Oem",
...
class DumpEntryCollection : public Node
{
  public:
    template <typename CrowApp>
    DumpEntryCollection(CrowApp& app) : Node(app, PATH)
    {
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"Login"}}},
            {boost::beast::http::verb::head, {{"Login"}}},
  ...
  private:
               ...
               const std::vector<std::string>& params) override
    {
        ...
        if (std::string(INTERFACE) == "xyz.openbmc_project.Dump.Entry.BMC")
        {
            asyncResp->res.jsonValue["Name"] = "BMC Dump Entries";
            asyncResp->res.jsonValue["Description"] =
                "Collection of BMCDump Entries";
        }
        else
        {
            ...
            [asyncResp](const boost::system::error_code ec,
                ...
                for (auto& object : resp)
                {
                    ...
                    thisEntry = {{"@odata.type", "#LogEntry.v1_5_1.LogEntry"},
                                 {"@odata.id", std::string(PATH) + entryID},
                                 {"Id", entryID},
                                 {"EntryType", "Event"},
                                 {"Created", timestamp}};
                    ...
                    thisEntry["Oem"]["OpenBmc"]["AdditionalDataSizeBytes"] =
                        size;

                    if (std::string(INTERFACE) ==
                        "xyz.openbmc_project.Dump.Entry.BMC")
                    {
                        thisEntry["Name"] = "BMC Dump Entry";
...
template <const char* PATH, const char* INTERFACE>
class DumpEntry : public Node
{
  public:
    DumpEntry(CrowApp& app) : Node(app, PATH, std::string())
    {
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"Login"}}},
            {boost::beast::http::verb::head, {{"Login"}}},
        ...
        crow::connections::systemBus->async_method_call(
            [asyncResp, entryID](const boost::system::error_code ec,
                ...
                for (const std::string& path : subTreePaths)
                {
                    ...
                    {
                        deleteDumpEntry(asyncResp->res, entryID);
                        return;
                    }
                }
                BMCWEB_LOG_ERROR << "Invalid Dump ID";
                messages::resourceNotFound(asyncResp->res, "EntryID", entryID);
            },
            "xyz.openbmc_project.ObjectMapper",
            "/xyz/openbmc_project/object_mapper",
...
class DumpCreate : public Node
{
  ...
  private:
                ...
                const std::vector<std::string>& params) override
    {
        ...
        std::string diagnosticDataType, oemDiagnosticDataType;

        nlohmann::json createParams =
            nlohmann::json::parse(data, nullptr, false);

        for (const auto& parameter : createParams.items())
        {
            if (parameter.key() == "DiagnosticDataType")
            {
                diagnosticDataType = parameter.value();
            }
            else if (parameter.key() == "OEMDiagnosticDataType")
            {
                oemDiagnosticDataType = parameter.value();
            }
            else
            {
                BMCWEB_LOG_ERROR << "Invalid parameter: " << parameter.key();
                messages::actionParameterUnknown(res, "CollectDiagnosticData",
                                                 parameter.key());
                return;
            }
        }

        if (std::string(INTERFACE) == "xyz.openbmc_project.Dump.Entry.System")
        {
            if (oemDiagnosticDataType.empty() || diagnosticDataType.empty())
            {
            ...
            else if ((oemDiagnosticDataType.compare("System") != 0) ||
                     (diagnosticDataType.compare("OEM") != 0))
            {
                BMCWEB_LOG_ERROR << "Wrong parameter values passed";
                messages::invalidObject(res, "System Dump creation parameters");
                return;
            }
        }
        else
        {
            if (diagnosticDataType.empty())
            {
                BMCWEB_LOG_ERROR << "CreateDump action parameter "
                                    "'DiagnosticDataType' not found!";
                messages::actionParameterMissing(res, "CollectDiagnosticData",
                                                 "DiagnosticDataType");
                return;
            }
            else if (diagnosticDataType.compare("Manager") != 0)
            {
                BMCWEB_LOG_ERROR
                    << "Wrong parameter value passed for 'DiagnosticDataType'";
                messages::invalidObject(res, "BMC Dump creation parameters");
                return;
            }
        }
