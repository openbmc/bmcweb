        ...
        .methods(boost::beast::http::verb::get)(
            [](const crow::Request&,
               ...
               const std::string& chassisId) {
                getChassisId(
                    asyncResp,
                    [&chassisId](const std::string& chassisPath) {
                        return chassisId ==
                               sdbusplus::message::object_path(chassisPath)
                                   .filename();
                    },
                    [](const std::string& chassisId,
                       const std::shared_ptr<bmcweb::AsyncResp>& asyncResp) {
               ...
               const std::string& chassisId, const std::string& entryId) {
                ...
                crow::connections::systemBus->async_method_call(
                    [asyncResp, chassisId, entryId](
                        ...
                        {
                            messages::resourceNotFound(
                                asyncResp->res, "#LogEntry.v1_6_0.LogEntry",
                                entryId);
                            return;
                        }
                        crow::connections::systemBus->async_method_call(
                            [asyncResp, chassisId,
                             entryId](const boost::system::error_code ec,
                                ...
                                asyncResp->res.jsonValue = {
                                    ...
                                    {"Created",
                                     crow::utility::getDateTime(timestamp)},
                                    {"Modified", crow::utility::getDateTime(
                                                     updateTimestamp)}};
                            },
