                       ...
                       const std::string& dumpType)
{
    ...
    auto respHandler = [asyncResp, dumpPath](const boost::system::error_code ec,
                                             const std::string& introspectXml) {
        if (ec)
        {
            BMCWEB_LOG_ERROR << "getDumpServiceInfo respHandler got error "
                             << ec;
            // Assume that getting an error simply means the specified dump
            // type isn't enabled (its D-Bus object doesn't exist). Return
            // without adding any error response.
            return;
        }

        if (dbus::utility::introspectXmlContains(deleteAllInterface,
                                                 introspectXml))
        {
            asyncResp->res
