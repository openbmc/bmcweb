...
inline void requestRoutesBMCDumpCreate(App& app)
{
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Managers/bmc/LogServices/Dump/Actions/"
                      "LogService.CollectDiagnosticData/")
...
{
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/system/LogServices/Dump/Actions/"
                      "LogService.CollectDiagnosticData/")
        .privileges(redfish::privileges::postLogService)
        .methods(boost::beast::http::verb::post)(
            [](const crow::Request& req,
               ...
               const std::shared_ptr<bmcweb::AsyncResp>& asyncResp) {
                ...
                asyncResp->res.jsonValue["Actions"]["#LogService.ClearLog"] = {
                    {"target", "/redfish/v1/Systems/system/LogServices/"
                               "PostCodes/Actions/LogService.ClearLog"}};
            });
}
        ...
        .methods(boost::beast::http::verb::get)(
            [](const crow::Request&,
               ...
               const std::string& chassisId) {
                checkValidChassisId(
                    asyncResp, chassisId, [asyncResp, chassisId](bool valid) {
                        if (!valid)
                        {
                            messages::resourceNotFound(
                                asyncResp->res,
...
inline void requestRoutesDeviceLogService(App& app)
{
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/LogServices/DeviceLog/")
        .privileges({{"Login"}})
        .methods(boost::beast::http::verb::get)(
            [](const crow::Request&,
               const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
               const std::string& chassisId) {
                checkValidChassisId(
                    asyncResp, chassisId, [asyncResp, chassisId](bool valid) {
                        ...
                        asyncResp->res.jsonValue["@odata.type"] =
                            "#LogService.v1_1_0.LogService";
                        asyncResp->res.jsonValue["Name"] = "Device Log Service";
                        asyncResp->res.jsonValue["Description"] =
...
inline void requestRoutesDeviceLogEntryCollection(App& app)
{
    BMCWEB_ROUTE(app,
                 "/redfish/v1/Chassis/<str>/LogServices/DeviceLog/Entries/")
        .privileges({{"Login"}})
        .methods(boost::beast::http::verb::get)(
            [](const crow::Request&,
               const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
               const std::string& chassisId) {
                ...
                asyncResp->res.jsonValue["Description"] =
                    "Collection of Device Event Log Entries for Chassis " +
                    chassisId;
                asyncResp->res.jsonValue["Members"] = nlohmann::json::array();
                asyncResp->res.jsonValue["Members@odata.count"] = 0;
        ...
        [asyncResp, objPath](const boost::system::error_code ec,
            ...
            sdbusplus::asio::getProperty<std::vector<std::string>>(
                *crow::connections::systemBus,
                ...
                [asyncResp, storage, objPath](
                    const boost::system::error_code ec,
                    const std::vector<std::string>& storageControllerList) {
                    if (ec)
                    {
                        BMCWEB_LOG_DEBUG
                            << "Failed to call getProperty for Storage -> "
                           ...
                           const std::string& entryId)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, path, chassisId, entryId](
            ...
            crow::connections::systemBus->async_method_call(
                [asyncResp, path, chassisId,
                 entryId](const boost::system::error_code ec,
                    ...
                    asyncResp->res.jsonValue["@odata.type"] =
                        "#LogEntry.v1_6_0.LogEntry";
                    asyncResp->res.jsonValue["@odata.id"] = entry + entryId;
                    asyncResp->res.jsonValue["Name"] =
                        "Device Log Entry for " + entryId;
                    asyncResp->res.jsonValue["Id"] = entryId;
            ...
            sdbusplus::asio::getProperty<std::vector<std::string>>(
                *crow::connections::systemBus,
                "xyz.openbmc_project.ObjectMapper", path + "/LogOrigin",
                "xyz.openbmc_project.Association", "endpoints",
                [asyncResp,
                 chassisId](const boost::system::error_code ec,
                    ...
                    {
                        BMCWEB_LOG_DEBUG
                            << "Failed to call LogOrigin Association: " << ec;
                        return;
                    }
                    ...
                    {
                        BMCWEB_LOG_ERROR
                            << "Resource can only have one Log Origin";
                        return;
                    }
                    ...
                    crow::connections::systemBus->async_method_call(
                        [asyncResp,
                         origin](const boost::system::error_code ec,
                                 const std::vector<std::pair<
                            ...
                            sdbusplus::message::object_path path(origin);
                            for (const auto& object : objects)
                            {
                                for (const auto& interface : object.second)
                                {
                                    ...
                                    {
                                        getLogOriginStorageController(asyncResp,
                                                                      path);
                                        return;
                                    }
                                }
...
inline void requestRoutesDeviceLogEntry(App& app)
{
    BMCWEB_ROUTE(
        app, "/redfish/v1/Chassis/<str>/LogServices/DeviceLog/Entries/<str>/")
        .privileges({{"Login"}})
        .methods(boost::beast::http::verb::get)(
            [](const crow::Request&,
               const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
               const std::string& chassisId, const std::string& entryId) {
                checkValidChassisId(
                    asyncResp, chassisId, [asyncResp, entryId](bool valid) {
                        if (valid)
                        {
                            return;
                        }
                    ...
                    [asyncResp, chassisId,
                     entryId](const boost::system::error_code ec,
                        ...
                        for (const std::string& objpath : subtreePath)
                        {
                            ...
                            deviceLogEntry(asyncResp, path.str, chassisId,
                                           entryId);
