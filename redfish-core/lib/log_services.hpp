    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, dumpPath, dumpType](
            ...
            for (auto& object : resp)
            {
                ...
                {
                    if (interfaceMap.first == ("xyz.openbmc_project.Dump.Entry." + dumpType))
                    {
                        foundDumpEntry = true;
                        break;
                    }
                                ...
                                const std::string& dumpType)
{
    crow::connections::systemBus->async_method_call(
        ...
        std::array<const char*, 1>{("xyz.openbmc_project.Dump.Entry." + dumpType).c_str()});
}
