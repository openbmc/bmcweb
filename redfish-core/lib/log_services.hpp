                                   ...
                                   const std::string& dumpType)
{
    ...
    const char* dumpInterface = nullptr;
    if (dumpType == "BMC")
    {
        dumpPath = "/redfish/v1/Managers/bmc/LogServices/Dump/Entries/";
        dumpInterface = "xyz.openbmc_project.Dump.Entry.BMC";
    }
    else if (dumpType == "System")
    {
        dumpPath = "/redfish/v1/Systems/system/LogServices/Dump/Entries/";
        dumpInterface = "xyz.openbmc_project.Dump.Entry.System";
    }

    crow::connections::systemBus->async_method_call(
        [asyncResp, dumpPath, dumpInterface, dumpType](
            ...
            for (auto& object : resp)
            {
                ...
                {
                    if (interfaceMap.first == std::string(dumpInterface))
                    {
                        foundDumpEntry = true;
                        break;
                    }
                             ...
                             const std::string& dumpType)
{
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, entryID, dumpPath, dumpInterface, dumpType](
            const boost::system::error_code ec, GetManagedObjectsType& resp) {
            if (ec)
            {
                BMCWEB_LOG_ERROR << "DumpEntry resp_handler got error " << ec;
