        ...
        .methods(boost::beast::http::verb::get)(
            [](const crow::Request& req,
               const std::shared_ptr<bmcweb::AsyncResp>& asyncResp) {
                ...
                // If total entries less than top(max entries per page)
                if (logEntries.size() < top)
                {
                    top = logEntries.size();
                }
                ...
                {
                    for (entryShow = skip; entryShow < top; entryShow++)
                    {
                        std::string msg(
                            logEntries.at(static_cast<char>(entryShow)));
                        logEntryArray.push_back({});
        ...
        .methods(boost::beast::http::verb::get)(
            [](const crow::Request&,
               ...
               const std::string& param) {
                ...
                int vertexPos = std::atoi(targetID.c_str()) - 1;
                std::string msg(logEntries.at(static_cast<char>(vertexPos)));
