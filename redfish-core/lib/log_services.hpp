...
class LogServiceBmc : public Node
{
  public:
        ...
        Node(app, "/redfish/v1/Managers/openbmc/LogServices/BmcLog/")
    {
        ...
        Node::json["@odata.id"] =
            "/redfish/v1/Managers/openbmc/LogServices/BmcLog";
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"ConfigureComponents"}}},
            {boost::beast::http::verb::head, {{"ConfigureComponents"}}},
            {boost::beast::http::verb::patch, {{"ConfigureComponents"}}},
  ...
  private:
               ...
               const std::vector<std::string> &params) override
    {
        ...
        SD_JOURNAL_FOREACH(j)
        {
            ...
            if (r < 0)
            {
                BMCWEB_LOG_ERROR << "Failed to read PRIORITY field: "
                                 << strerror(-r);
                res.result(boost::beast::http::status::internal_server_error);
                res.end();
                return;
            ...
            // Only use the content after the "=" character.
            priority.remove_prefix(
                std::min(priority.find("=") + 1, priority.size()));
            severity = strtol(priority.data(), nullptr, 10);

            // Get the Created time from the timestamp
            time_t t = static_cast<time_t>(curTs / 1000 /
                                           1000); // Convert from us to s
            ...
            const std::string entryTimeStr(t1.to_string() + ":" +
                                           t2.to_string());
