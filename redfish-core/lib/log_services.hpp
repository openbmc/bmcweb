...
static void fillPostCodeEntry(
    ...
    const boost::container::flat_map<uint64_t, std::tuple<uint64_t,std::vector<uint8_t>>>& postcode,
    ...
    const uint64_t skip = 0, const uint64_t top = 0)
{
    ...
    for (const std::pair<uint64_t, std::tuple<uint64_t,std::vector<uint8_t>>>& code : postcode)
    {
        currentCodeIndex++;
