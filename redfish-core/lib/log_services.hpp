...
/** @brief Generate logEntry in MessageRegistry format.
 *  @param[out] logEntry    - reference for the log entry to be generated.
 *  @param[in] odataId      - the log entry collection URI, e.g.
 * /redfish/v1/Systems/system/LogServices/EventLog/Entries/
 *  @param[in] odataTypeVer - odata type version, e.g. v1_9_0
 *  @param[in] id           - log entry id.
 *  @param[in] name         - log entry name.
 *  @param[in] timestamp    - log created timestamp.
 *  @param[in] messageId    - MessageId.
 *  @param[in] messageArgs  - MessageArgs.
 *  @param[in] resolution   - log entry resolution.
 *  @param[in] severity     - log entry severity, optional, using the severity
 * based on 'messageId' by default.
 */
static void generateMessageRegistry(
    nlohmann::json& logEntry, const std::string& odataId,
    const std::string& odataTypeVer, const std::string& id,
