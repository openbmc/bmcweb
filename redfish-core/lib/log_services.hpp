...
inline DumpCreationProgress getDumpCompletionStatus(
    boost::container::flat_map<std::string,
                               std::variant<std::string, uint64_t>>& values)
{
    auto find = values.find("Status");
    if (find != values.end())
    {
        std::string* value = std::get_if<std::string>(&(find->second));
        if (value == nullptr)
        {
        ...
        if (boost::ends_with(*value, "Failed") ||
            boost::ends_with(*value, "Aborted"))
        {
            return DUMP_CREATE_FAILED;
        }

        if (boost::ends_with(*value, "Completed"))
        {
    ...
    find = values.find("Size");
    if (find != values.end())
    {
        uint64_t* size = std::get_if<uint64_t>(&(find->second));
        if (size == nullptr || *size == 0)
        {
            BMCWEB_LOG_ERROR << "Size property value is null or size 0";
            return DUMP_CREATE_INPROGRESS;
        }
        return DUMP_CREATE_SUCCESS;
    }

    // return DUMP_CREATE_INPROGRESS if the Status/Size prop is not found.
    return DUMP_CREATE_INPROGRESS;
}
    ...
    const sdbusplus::message::object_path& createdObjPath)
{
    ...
    std::shared_ptr<task::TaskData> task = task::TaskData::createTask(
        [createdObjPath, dumpEntryPath{std::move(dumpEntryPath)},
         dumpId{dumpId}](boost::system::error_code err,
            ...
            {
                int dumpStatus = getDumpCompletionStatus(values);
