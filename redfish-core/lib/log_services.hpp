                       ...
                       std::vector<std::filesystem::path>& hostLoggerFiles)
{
    ...
    std::sort(hostLoggerFiles.begin(), hostLoggerFiles.end(),
              [](const std::filesystem::path& logFileName1,
                 const std::filesystem::path& logFileName2) -> bool {
                  return std::forward_as_tuple(logFileName1.string().size(),
                                               logFileName1) >
                         std::forward_as_tuple(logFileName2.string().size(),
                                               logFileName2);
              });
                         ...
                         std::vector<std::string>& logEntries)
{
    ...
    // 16kb)
    constexpr int maxTotalFilesSize = 262144;
    int totalFilesSize;
    ...
    for (const std::filesystem::path& it : hostLoggerFiles)
    {
        ...
        totalFilesSize = accumulate(logEntries.begin(), logEntries.end(), 0,
    ...
    {
        totalFilesSize += static_cast<int>(lastMessage.length());
        if (totalFilesSize > maxTotalFilesSize)
        {
            BMCWEB_LOG_ERROR << "File size exceeds maximum allowed size of "
                             << maxTotalFilesSize;
            ...
            [](const crow::Request& req,
               const std::shared_ptr<bmcweb::AsyncResp>& asyncResp) {
                ...
                for (uint64_t id = skip;
                     ...
                     id++)
                {
                    std::string msg(logEntries.at(static_cast<size_t>(id)));
                    logEntryArray.push_back({});
                    nlohmann::json& hostLogEntry = logEntryArray.back();
                    fillHostLoggerEntryJson(std::to_string(id), msg,
                                            hostLogEntry);
                }
        ...
        .methods(boost::beast::http::verb::get)(
            [](const crow::Request&,
               ...
               const std::string& param) {
                ...
                {
                    std::string msg(logEntries.at(static_cast<size_t>(idInt)));
