...
class DBusEventLogEntryDownload : public Node
{
  ...
  private:
               ...
               const std::vector<std::string>& params) override
    {
        ...
        if (accept.empty())
        {
            messages::propertyMissing(asyncResp->res, "Accept");
            return;
        }
        if (!boost::iequals(accept, "text/plain"))
        {
            messages::propertyValueNotInList(asyncResp->res,
                                             std::string(accept), "Accept");
            return;
        }
        std::string_view acceptEncoding = req.getHeaderValue("Accept-Encoding");
        if (acceptEncoding.empty())
        {
            messages::propertyMissing(asyncResp->res, "Accept-Encoding");
            return;
        }
        if (!boost::iequals(acceptEncoding, "Base64"))
        {
            messages::propertyValueNotInList(
                asyncResp->res, std::string(acceptEncoding), "Accept-Encoding");
            return;
        }
