                         ...
                         const std::vector<std::string>& additionalDatas)
{
    ...
    {
        if(additionalData.find("REDFISH_MESSAGE_ID") !=
            std::string::npos)
        {
            messageId =
                additionalData.substr(additionalData.find("="));
            messageId.erase(messageId.begin());
        }
        if(additionalData.find("REDFISH_MESSAGE_ARGS") !=
            std::string::npos)
        {
            boost::split(messageArgs, additionalData, boost::is_any_of(","),
                boost::token_compress_on);
            messageArgs[0] =
                messageArgs[0].substr(messageArgs[0].find("="));
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/LogServices/EventLog/Entries/")
        ...
        crow::connections::systemBus->async_method_call(
            ...
            for (const auto& objectPath : resp)
            {
                ...
                const std::vector<std::string>* additionalDatas = nullptr;
                ...
                if (additionalDatas != nullptr)
                {
                    std::string eventMessage = "";
                    std::string messageId = "";
                    std::vector<std::string> messageArgs = {};
                    LogParseError status =
                        parseEventLogMessage(eventMessage, messageId,
                                             messageArgs, *additionalDatas);
                    if (status != LogParseError::success)
                    {
                        messages::internalError(asyncResp->res);
                        return;
                    ...
                    if (!messageId.empty())
                    {
                        ...
                        thisEntry["MessageId"] = std::move(messageId);
                        if (!messageArgs.empty())
                        {
                            thisEntry["MessageArgs"] =
        ...
        sdbusplus::asio::getAllProperties(
            *crow::connections::systemBus, "xyz.openbmc_project.Logging",
            ...
            const bool success = sdbusplus::unpackPropertiesNoThrow(
                ...
                "ServiceProviderNotify", notify, "AdditionalData",
                additionalDatas);
            ...
            if (additionalDatas != nullptr)
            {
                ...
                std::vector<std::string> messageArgs = {};
                LogParseError status =
                    parseEventLogMessage(eventMessage, messageId, messageArgs,
                                         *additionalDatas);
                if (status != LogParseError::success)
                {
                    messages::internalError(asyncResp->res);
