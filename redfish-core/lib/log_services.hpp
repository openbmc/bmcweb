...
class JournalEventLogClear : public Node
{
  ...
  private:
                ...
                const std::vector<std::string> &params) override
    {
        ...
        if (getRedfishLogFiles(redfishLogFiles, 0))
        {
...
class JournalEventLogEntry : public Node
{
  ...
  private:
               ...
               const std::vector<std::string> &params) override
    {
        ...
        std::vector<std::filesystem::path> redfishLogFiles;
        getRedfishLogFiles(redfishLogFiles, 0);
        std::string logEntry;
...
class SlvNodeLogServiceCollection : public Node
{
  public:
    template <typename CrowApp>
    SlvNodeLogServiceCollection(CrowApp &app) :
        Node(app, "/redfish/v1/Systems/Node<str>/LogServices/", std::string())
    {
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"Login"}}},
            {boost::beast::http::verb::head, {{"Login"}}},
  ...
  private:
               ...
               const std::vector<std::string> &params) override
    {
        ...
        asyncResp->res.jsonValue["@odata.type"] =
            "#LogServiceCollection.LogServiceCollection";
        asyncResp->res.jsonValue["@odata.id"] =
            "/redfish/v1/Systems/Node" + nodeId + "/LogServices";
        asyncResp->res.jsonValue["Name"] = "System Log Services Collection";
        asyncResp->res.jsonValue["Description"] =
            "Collection of LogServices for this Computer System";
        nlohmann::json &logServiceArray = asyncResp->res.jsonValue["Members"];
