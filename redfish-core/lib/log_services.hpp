...
inline void requestRoutesSystemLogServiceCollection(App& app)
{
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/LogServices/")
        ...
        std::string computerSystemIndex =
            collection_util::getComputerSystemIndex(systemName);
        if (systemName != "system")
        {
            if (!(collection_util::isValidSystem(computerSystemIndex)))
            {
        ...
        asyncResp->res.jsonValue["@odata.id"] = ("/redfish/v1/Systems/" + systemName + "/LogServices").c_str();
        ...
        crow::connections::systemBus->async_method_call(
            ...
            for (const auto& pathStr : subtreePath)
            {
                if (pathStr.find("PostCode") != std::string::npos)
                {
                    ...
                    member["@odata.id"] = ("/redfish/v1/Systems/" + systemName + "/LogServices/PostCodes").c_str();
...
inline void requestRoutesDBusEventLogEntryCollection(App& app)
{
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/LogServices/EventLog/Entries/")
        ...
        asyncResp->res.jsonValue["@odata.type"] =
            "#LogEntryCollection.LogEntryCollection";
        asyncResp->res.jsonValue["@odata.id"] = ("/redfish/v1/Systems/" + systemName + "/LogServices/EventLog/Entries").c_str();
        asyncResp->res.jsonValue["Name"] = "System Event Log Entries";
        asyncResp->res.jsonValue["Description"] =
            "Collection of System Event Log Entries";
...
inline void requestRoutesPostCodesLogService(App& app)
{
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/LogServices/PostCodes/")
        ...
        asyncResp->res.jsonValue["@odata.id"] = ("/redfish/v1/Systems/" + systemName + "/LogServices/PostCodes").c_str();
        asyncResp->res.jsonValue["@odata.type"] =
            "#LogService.v1_1_0.LogService";
        ...
        asyncResp->res.jsonValue["OverWritePolicy"] = "WrapsWhenFull";
        asyncResp->res.jsonValue["Entries"]["@odata.id"] = ("/redfish/v1/Systems/" + systemName + "/LogServices/PostCodes/Entries").c_str();
...
static void fillPostCodeEntry(
    ...
    const std::string& systemIdx, const uint16_t bootIndex, const uint64_t codeIndex = 0,
    const uint64_t skip = 0, const uint64_t top = 0)
{
             ...
             code : postcode)
    {
        ...
        bmcLogEntry["@odata.id"] = ("/redfish/v1/Systems/system" + systemIdx + "/LogServices/PostCodes/Entries/" + postcodeEntryID).c_str();
        ...
        {
            bmcLogEntry["AdditionalDataURI"] =("/redfish/v1/Systems/system" + systemIdx + "/LogServices/PostCodes/Entries/" + postcodeEntryID + "/attachment").c_str();
        }
    }
}
...
static void getPostCodeForEntry(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                                ...
                                const uint64_t codeIndex, const std::string& systemIdx)
{
    std::string dbusService = "xyz.openbmc_project.State.Boot.PostCode" + systemIdx;

    crow::connections::systemBus->async_method_call(
        [aResp, systemIdx, bootIndex,
        ...
        fillPostCodeEntry(aResp, postcode, systemIdx, bootIndex, codeIndex);
...
static void getPostCodeForBoot(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                               ...
                               size_t top, std::string systemIdx)
{
std::string dbusService = "xyz.openbmc_project.State.Boot.PostCode" + systemIdx;

    crow::connections::systemBus->async_method_call(
        [aResp, bootIndex, bootCount, entryCount, skip,
         top, systemIdx](const boost::system::error_code ec,
              const boost::container::flat_map<
                  uint64_t, std::tuple<uint64_t, std::vector<uint8_t>>>&
                  postcode) {
        if (ec)
        ...
        if (!postcode.empty())
        {
            ...
            if (skip < endCount && (top + skip) > entryCount)
            {
                ...
                uint64_t thisBootTop =
                    ...
                    entryCount;
                fillPostCodeEntry(aResp, postcode, systemIdx, bootIndex, 0, thisBootSkip,
                                  thisBootTop);
            }
        ...
        if (bootIndex < bootCount)
        {
            getPostCodeForBoot(aResp, static_cast<uint16_t>(bootIndex + 1),
                               bootCount, endCount, skip, top, systemIdx);
        }
...
static void
    getCurrentBootNumber(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                         size_t skip, size_t top, std::string systemIdx)
{
std::string dbusService = "xyz.openbmc_project.State.Boot.PostCode" + systemIdx;
    ...
    sdbusplus::asio::getProperty<uint16_t>(
        *crow::connections::systemBus,
        ...
        [aResp, entryCount, skip, top, systemIdx](const boost::system::error_code ec,
                                       const uint16_t bootCount) {
        if (ec)
        {
            BMCWEB_LOG_DEBUG << "DBUS response error " << ec;
        ...
        getPostCodeForBoot(aResp, 1, bootCount, entryCount, skip, top, systemIdx);
        });
}
...
inline void requestRoutesPostCodesEntryCollection(App& app)
{
    BMCWEB_ROUTE(app,
        ...
        asyncResp->res.jsonValue["@odata.type"] =
            "#LogEntryCollection.LogEntryCollection";
        asyncResp->res.jsonValue["@odata.id"] = ("/redfish/v1/Systems/" + systemName + "/LogServices/PostCodes/Entries").c_str();
        ...
        size_t top = delegatedQuery.top.value_or(query_param::Query::maxTop);
               ...
               std::string systemIdx;
                ...
                {
                   systemIdx = redfish::collection_util::getComputerSystemIndex(systemName);
                }


        getCurrentBootNumber(asyncResp, skip, top, systemIdx);
        });
}
...
inline void requestRoutesPostCodesEntry(App& app)
{
    BMCWEB_ROUTE(
        ...
        getPostCodeForEntry(asyncResp, bootIndex, codeIndex, systemIdx);
        });
}
