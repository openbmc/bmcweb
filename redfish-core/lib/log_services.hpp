...
inline void appendRsyslogRedfishResolvedEntry(const std::string& targetID)
{
    ...
    resolvedEntries << targetID << std::endl;
}
...
inline void removeRsyslogRedfishResolvedEntry(const std::string& targetID)
{
    ...
    for (const auto& entryID : resolvedEntries)
    {
        ...
        resolvedEntriesNew << entryID << std::endl;
    }
}
...
inline void appendRsyslogRedfishDeletedEntry(const std::string& targetID)
{
    ...
    deletedEntries << targetID << std::endl;
}

// returns true if the entry was found and deleted
inline bool deleteRsyslogRedfishEventLogEntry(const std::string& targetID)
{
    // for keeping track of which event ids have been deleted
    // 1. look for the event, if it actually exists
    // 2. append the target ID to the file
