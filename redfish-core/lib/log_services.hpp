...
inline void populateBMCDumpLogServiceMembers(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const boost::system::error_code ec,
    const dbus::utility::MapperGetSubTreePathsResponse& subTreePaths)
{
    if (ec)
    {
        BMCWEB_LOG_ERROR << "populateBMCDumpLogServiceMembers got error " << ec;
        // Assume that getting an error simply means there are no dump
        // LogServices. Return without adding any error response.
        return;
    }

    nlohmann::json& logServiceArrayLocal = asyncResp->res.jsonValue["Members"];
...
#ifdef BMCWEB_ENABLE_REDFISH_DUMP_LOG
    crow::connections::systemBus->async_method_call(
        std::bind_front(populateBMCDumpLogServiceMembers, asyncResp),
        "xyz.openbmc_project.ObjectMapper",
        "/xyz/openbmc_project/object_mapper",
        "xyz.openbmc_project.ObjectMapper", "GetSubTreePaths",
