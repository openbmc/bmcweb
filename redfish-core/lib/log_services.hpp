                       ...
                       std::vector<std::filesystem::path>& hostLoggerFiles)
{
    ...
    const std::filesystem::directory_iterator& logPath =
        std::filesystem::directory_iterator(hostLoggerFilePath, ec);
    if (ec.value() != 0)
    {
    ...
    for (const std::filesystem::directory_entry& it : logPath)
    {
        ...
        // Prefix of each files created by phosphor-hostlogger is "host_"
        // If we find a HostLogger file, save the path
        if (boost::starts_with(filename, "host_"))
        {
...
inline bool getTimestampAndMsg(const std::string& logEntry, std::string& msg,
                               std::string& timestamp)
{
    // Get the Timestamp. Here is an example for one entry:
    // "[ 1970-01-01T00:00:31+00:00 ] >>> Log collection started at
    // 1970-01-01 00:00:31".
    const char* bufCp = logEntry.c_str();
    boost::iterator_range<const char*> beginOfTimestamp =
        boost::algorithm::find_first(bufCp, "[ ");
    boost::iterator_range<const char*> endOfTimestamp =
        boost::algorithm::find_first(bufCp, " ] ");
    if (beginOfTimestamp.empty() || endOfTimestamp.empty())
    {
        BMCWEB_LOG_ERROR << "Get timestamp error, log entry: " << logEntry;
        return false;
    }
    timestamp.assign(logEntry.substr(beginOfTimestamp.size(),
                                     strlen(beginOfTimestamp.end()) -
                                         strlen(endOfTimestamp.begin())));

    // Check timestamp format
    std::tm timeStruct = {};
    std::istringstream timestampChecker(timestamp.data());
    timestampChecker >> std::get_time(&timeStruct, "%Y-%m-%dT%H:%M:%S");
    if (timestampChecker.fail())
    {
        BMCWEB_LOG_ERROR << "Parsing timestamp failed. ";
        return false;
    }
    msg.assign(endOfTimestamp.end());
                                    ...
                                    nlohmann::json& logEntryJson)
{
    // Get the Message from the MessageRegistry
    const message_registries::Message* message =
        message_registries::getMessage("OpenBMC.0.1.HostLogger");

    std::vector<std::string> messageArgs = {msg};

    std::string registryMsg;
    if (message != nullptr)
    {
        registryMsg = message->message;

        // Fill in message of host log
        int i = 0;
        for (const std::string& messageArg : messageArgs)
        {
            std::string argStr = "%" + std::to_string(++i);
            size_t argPos = registryMsg.find(argStr);
            if (argPos != std::string::npos)
            {
                registryMsg.replace(argPos, argStr.length(), messageArg);
            }
        }
    }
    ...
    logEntryJson = {
        ...
        {"MessageArgs", messageArgs},
        {"EntryType", "Event"},
        {"Severity", severity}, // This is always information, so always print OK.
        {"Created", timestamp}};
}
               ...
               const std::vector<std::string>&) override
    {
        ...
        for (const std::filesystem::path& it : hostLoggerFiles)
        {
            ...
            while (logFile.gzGetLine(logEntry))
            {
                ...
                if (!getTimestampAndMsg(logEntry, msg, timestamp))
                {
                    continue;
                }
                ...
                fillHostLoggerEntryJson(idStr, msg, timestamp, hostLogEntry);
            }
        }
        asyncResp->res.jsonValue["Members@odata.count"] = entryCount;
        if (skip + top < entryCount)
               ...
               const std::vector<std::string>& params) override
    {
        ...
        for (const std::filesystem::path& it : hostLoggerFiles)
        {
            ...
            while (logFile.gzGetLine(logEntry))
            {
                ...
                if (idStr == targetID)
                {
                    fillHostLoggerEntryJson(idStr, msg, timestamp,
                                            asyncResp->res.jsonValue);
                    return;
                }
            }
