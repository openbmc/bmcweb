...
inline void deleteBMCDumpEntry(crow::Response &res, const std::string &entryID)
{
    ...
    auto respHandler = [asyncResp](const boost::system::error_code ec) {
        BMCWEB_LOG_DEBUG << "BMC Dump Entry doDelete callback: Done";
        if (ec)
        {
            BMCWEB_LOG_ERROR
                << "BMC Dump (DBus) doDelete respHandler got error " << ec;
...
class BMCJournalLogService : public Node
{
  ...
  private:
               ...
               const std::vector<std::string> &params) override
    {
        ...
        nlohmann::json &journalLogServiceArray = asyncResp->res.jsonValue["Members"];
        journalLogServiceArray = nlohmann::json::array();
#ifdef BMCWEB_ENABLE_REDFISH_BMCDUMP_LOG
        journalLogServiceArray.push_back(
            {{"@odata.id", "/redfish/v1/Managers/bmc/LogServices/Journal/Dump"}});
#endif
        asyncResp->res.jsonValue["Members@odata.count"] =
            journalLogServiceArray.size();
    }
};
...
class BMCDumpService : public Node
{
  public:
    ...
    BMCDumpService(CrowApp &app) :
        Node(app, "/redfish/v1/Managers/bmc/LogServices/Journal/Dump/")
    {
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"Login"}}},
            {boost::beast::http::verb::head, {{"Login"}}},
  ...
  private:
               ...
               const std::vector<std::string> &params) override
    {
        ...
        asyncResp->res.jsonValue["@odata.id"] =
            "/redfish/v1/Managers/bmc/LogServices/Journal/Dump";
        asyncResp->res.jsonValue["@odata.type"] =
            "#LogService.v1_1_3.LogService";
        asyncResp->res.jsonValue["Name"] = "OpenBMC Oem BMC dump Service";
        asyncResp->res.jsonValue["Description"] = "Oem BMC Dump Service";
        asyncResp->res.jsonValue["Id"] = "BMCDump";
        asyncResp->res.jsonValue["OverWritePolicy"] = "WrapsWhenFull";
        asyncResp->res.jsonValue["Oem"]["OpenBmc"]["@odata.type"] =
            "#OemLogService.v1_0_0.OpenBmc";
        asyncResp->res.jsonValue["Oem"]["OpenBmc"]["Type"] = "Dump";
        asyncResp->res.jsonValue["Oem"]["OpenBmc"]["DumpType"] = "BMCDump";
        asyncResp->res.jsonValue["Entries"] = {
            {"@odata.id",
             "/redfish/v1/Managers/bmc/LogServices/Journal/Dump/Entries"}};
        asyncResp->res.jsonValue["Actions"] = {
            {"#LogService.ClearLog",
             {{"target", "/redfish/v1/Managers/bmc/LogServices/Journal/Dump/"
                         "Actions/LogService.ClearLog"}}}};
...
class BMCDumpEntryCollection : public Node
{
  public:
    ...
    BMCDumpEntryCollection(CrowApp &app) :
        Node(app, "/redfish/v1/Managers/bmc/LogServices/Journal/Dump/Entries/")
    {
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"Login"}}},
            {boost::beast::http::verb::head, {{"Login"}}},
  ...
  private:
               ...
               const std::vector<std::string> &params) override
    {
        ...
        asyncResp->res.jsonValue["@odata.id"] =
            "/redfish/v1/Managers/bmc/LogServices/Journal/Dump/Entries";
        asyncResp->res.jsonValue["Name"] = "OpenBMC BMCDump Entries";
        asyncResp->res.jsonValue["Description"] =
        ...
        crow::connections::systemBus->async_method_call(
            [asyncResp](const boost::system::error_code ec,
                ...
                for (auto &object : resp)
                {
                    ...
                    bool foundBMCDumpEntry = false;
                    for (auto &interfaceMap : object.second)
                    {
                        if (interfaceMap.first ==
                            "xyz.openbmc_project.Dump.Entry.BMC")
                        {
                            foundBMCDumpEntry = true;
                            break;
                        }
                    }

                    if (foundBMCDumpEntry == false)
                    {
                        continue;
                    }
                        ...
                        {"Oem",
                         {"OpenBmc",
                          {{"#LogEntry.DownloadLog",
                            {{"target", "/redfish/v1/Managers/bmc/"
                                        "LogServices/Journal/Dump/Entries/" +
                                            entryID +
                                            "/Actions/Oem/OpenBmc/"
                                            "LogEntry.DownloadLog"}}}}}}};
                }
...
class BMCDumpEntry : public Node
{
  public:
    BMCDumpEntry(CrowApp &app) :
        Node(app,
             "/redfish/v1/Managers/bmc/LogServices/Journal/Dump/Entries/<str>/",
             std::string())
    {
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"Login"}}},
        ...
        crow::connections::systemBus->async_method_call(
            [asyncResp, entryID](const boost::system::error_code ec,
                ...
                for (auto &objectPath : resp)
                {
                    ...
                    if (foundBMCDumpEntry == false)
                    {
                        BMCWEB_LOG_ERROR << "Can't find BMC Dump Entry";
                        messages::internalError(asyncResp->res);
                        return;
                    }
        ...
        crow::connections::systemBus->async_method_call(
            [asyncResp, entryID](const boost::system::error_code ec,
                ...
                for (const std::string &path : subTreePaths)
                {
                    ...
                    {
                        deleteBMCDumpEntry(asyncResp->res, entryID);
                        return;
                    }
                }
                BMCWEB_LOG_ERROR << "Invalid BMC Dump ID";
