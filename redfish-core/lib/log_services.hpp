                           ...
                           const std::string& dumpType)
{
    std::shared_ptr<task::TaskData> task = task::TaskData::createTask(
        [dumpId, dumpPath, dumpType](
            boost::system::error_code err, sdbusplus::message::message& message,
        ...
        .methods(boost::beast::http::verb::post)(
            [](const crow::Request&,
               const std::shared_ptr<bmcweb::AsyncResp>& asyncResp) {
                ...
                crow::connections::systemBus->async_method_call(
                    [asyncResp](const boost::system::error_code err) {
                        if (err)
                        {
...
{
    std::error_code err;
    std::filesystem::directory_iterator logPath(hostLoggerFilePath, err);
    if (err)
    {
        BMCWEB_LOG_ERROR << err.message();
        return false;
    }
                         ...
                         std::vector<std::string>& logEntries, size_t& logCount)
{
    ...
    {
        if (!logFile.gzGetLines(hostLoggerFile.string(), skip, top, logEntries, logCount))
        {
