...
namespace redfish
{
    ...
    void getMainChassisId(std::shared_ptr<AsyncResp> asyncResp, CallbackFunc &&callback)
    {
        ...
        crow::connections::systemBus->async_method_call(
            [&](const boost::system::error_code ec,
                ...
                if (subtree.size() == 0) 
                {
                    //asyncResp->res.jsonValue["Links"]["ManagerForChassis@odata.count"] = 0;
                    //asyncResp->res.jsonValue["Links"]["ManagerForChassis"] = {};
                    return;
                }
                
                std::size_t idPos;
                if ((idPos = subtree[0].first.rfind("/")) == std::string::npos)
                {
                    messages::internalError(asyncResp->res);
                ...
                BMCWEB_LOG_DEBUG << "/redfish/v1/Chassis/" + chassisId;
