...
{
    auto handleUserInfo = [asyncResp,
                           &req](const std::map<std::string,
                                                dbus::utility::DbusVariantType>&
                                     userInfo) {
        bool userLocked = false;
        const auto& lockedIter = userInfo.find("UserLockedForFailedAttempt");
        ...
        dbus::utility::getProperty<uint32_t>(
            *crow::connections::systemBus, "xyz.openbmc_project.User.Manager",
            ...
            [asyncResp, &req](const boost::system::error_code& ec,
                ...
                if (ec)
                {
                    BMCWEB_LOG_ERROR("Failed to get AccountUnlockTimeout: {}",
                                     ec.message());
                    messages::internalError(asyncResp->res);
                    return;
                }
