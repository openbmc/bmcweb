                                       ...
                                       float millisNow)
{
    ...
    struct stat tStat
    {};
    stat(("/proc/" + std::to_string(pid)).c_str(), &tStat);
    struct timespec stAtim = tStat.st_atim;
    float millisCtime = static_cast<float>(stAtim.tv_sec) * 1000.0F +
                        static_cast<float>(stAtim.tv_nsec) / 1000000.0F;
