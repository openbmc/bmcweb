                             ...
                             const MapperServiceMap& serviceMap)
{
    for (const auto& [serviceName, interfaceList] : serviceMap)
    {
        ...
        {
            // Find `endpoints` array by this interface and the
            // <cpu object path>/identify_led_group object path,
            // and then get the Asserted property value by this
            // path.
...
inline void patchAppliedOperatingConfig(
    ...
    const std::string& cpuObjectPath, const std::string& serviceName)
{
    ...
    crow::connections::systemBus->async_method_call(
        ...
        serviceName, cpuObjectPath, "org.freedesktop.DBus.Properties", "Set",
        "xyz.openbmc_project.Control.Processor.CurrentOperatingConfig",
        "AppliedConfig",
    ...
    for (const auto& [serviceName, interfaceList] : serviceMap)
    {
        for (const auto& interface : interfaceList)
        {
                ...
                value)
            {
                ...
                // path>/identify_led_group object path, and then set the
                // Asserted property value by this path and the current value.
                setLocationIndicatorActive(aResp, objectPath, *value);
                return;
            }

            if (interface == "xyz.openbmc_project.Control."
                             "Processor.CurrentOperatingConfig" &&
                appliedConfigJson)
            {
                nlohmann::json json = *appliedConfigJson;
                std::string appliedConfigUri;
                if (!json_util::readJson(json, aResp->res, "@odata.id",
                                         appliedConfigUri))
                {
                    return;
                }

                patchAppliedOperatingConfig(aResp, processorId,
 ...
 */
inline void
    setProcessorObject(const std::shared_ptr<bmcweb::AsyncResp>& resp,
                       const std::string& processorId,
                       const std::optional<nlohmann::json>& appliedConfigJson,
                       std::optional<bool> value)
{
    // GetSubTree on all interfaces which provide info about a Processor
    crow::connections::systemBus->async_method_call(
        [resp, processorId, appliedConfigJson,
         value](boost::system::error_code ec,
                const MapperGetSubTreeResponse& subtree) mutable {
            if (ec)
            {
                BMCWEB_LOG_DEBUG << "DBUS response error: " << ec;
            ...
            for (const auto& [objectPath, serviceMap] : subtree)
            {
                ...
                // matching objects. Assume all interfaces we want to process
                // must be on the same object path.
                setProcessorData(resp, processorId, serviceMap, objectPath,
                                 appliedConfigJson, value);
                return;
            }
...
class Processor : public Node
{
  ...
  private:
                 ...
                 const std::vector<std::string>& params) override
    {
        ...
        const std::string& processorId = params[0];
        setProcessorObject(asyncResp, processorId, appliedConfigJson,
                           locationIndicatorActive);
    }
};
