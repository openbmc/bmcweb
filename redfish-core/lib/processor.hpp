 ...
 *
 * @return Returns as a string, the throttle cause in Redfish terms. If translation
 * cannot be done, returns "Unknown"
 */
inline std::string dbusToRfThrottleCause(const std::string& dbusSource)
{
    if (dbusSource == "xyz.openbmc_project.Control.Power.Throttle.ThrottleReasons.ClockLimit")
    {
        return "ClockLimit";
    }
    if (dbusSource == "xyz.openbmc_project.Control.Power.Throttle.ThrottleReasons.ManagementDetectedFault")
    {
        return "ManagementDetectedFault";
    }
    if (dbusSource == "xyz.openbmc_project.Control.Power.Throttle.ThrottleReasons.PowerLimit")
    {
        return "PowerLimit";
    }
    if (dbusSource == "xyz.openbmc_project.Control.Power.Throttle.ThrottleReasons.ThermalLimit")
    {
        return "ThermalLimit";
    }
    return "Unknown";
                          ...
                          const std::string& objectPath)
{
    ...
    sdbusplus::asio::getAllProperties(
        *crow::connections::systemBus, service, objectPath,
        ...
        if (ec)
        {
            // power manager service not available, no error, return no data
            BMCWEB_LOG_DEBUG << "Processor power manager service not available"
                             << ec;
            return;
        }
        ...
        const bool success = sdbusplus::unpackPropertiesNoThrow(
            dbus_utils::UnpackErrorPrinter(), properties, "Throttled", status,
        ...
        if (status != nullptr)
        {
            if (*status)
            {
                ...
                {
                    std::vector<std::string> rCauses;
                    for (auto cause : *causes)
                    {
                        auto rfCause = dbusToRfThrottleCause(cause);
                        if (!rfCause.empty())
                        {
                            rCauses.push_back(rfCause);
                        }
                    }
                    aResp->res.jsonValue["ThrottleCause"] = rCauses;
                }
            }
