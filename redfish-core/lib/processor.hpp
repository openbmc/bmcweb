...
inline void getCpuLocationType(std::shared_ptr<bmcweb::AsyncResp> asyncResp,
                               const std::string& service,
                               const std::string& objPath)
{
    ...
    dbus::utility::getProperty<std::string>(
        ...
        [objPath, asyncResp](const boost::system::error_code& ec,
            ...
            std::string locationType =
                redfish::location_utils::toLocationType(property);
            if (locationType.empty())
            {
                BMCWEB_LOG_ERROR("Invalid LocationType: {}", locationType);
                messages::internalError(asyncResp->res);
            }
            if (locationType == "Unknown")
            {
                BMCWEB_LOG_WARNING("Unknown locationType");
            }
            asyncResp->res
