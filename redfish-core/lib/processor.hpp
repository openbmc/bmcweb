...
template <typename Handler>
inline void
    getProcessorPaths(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                      const std::string& processorId, Handler&& handler)
{
    crow::connections::systemBus->async_method_call(
        [processorId, aResp, handler{std::move(handler)}](
            ...
            boost::system::error_code ecHandler =
...
{
    auto callback = [aResp, handler{std::move(handler)}, processorId,
                     coreId](const boost::system::error_code ecHandler,
                             const std::string& cpuPath) {
        if (ecHandler)
        {
            BMCWEB_LOG_DEBUG << "DBUS response error, ecHandler: "
                             << ecHandler.value();
            // No processor objects found by mapper
            if (ecHandler.value() == boost::system::errc::io_error)
            {
        ...
        crow::connections::systemBus->async_method_call(
            [processorId, coreId, aResp, handler{std::move(handler)}](
                const boost::system::error_code ec,
                const std::vector<std::string>& subTreePaths) {
                if (ec)
                {
                ...
                // Set an error code since valid processor core path is not found
                boost::system::error_code ecHandler =
                    boost::system::errc::make_error_code(boost::system::errc::no_such_file_or_directory);
                handler(ecHandler, {});
            },
            ...
            "xyz.openbmc_project.ObjectMapper", "GetSubTreePaths",
            cpuPath, 0,
            std::array<const char*, 1>{"xyz.openbmc_project.Inventory.Item.CpuCore"});
    };
                             ...
                             "xyz.openbmc_project.Inventory.Item.CpuCore")
                    {
                        for (const auto& [proName, proValue] : properties)
                        {
                            if (proName == "Microcode")
                            {
                                ...
                                aResp->res
                                    .jsonValue["ProcessorId"]["MicrocodeInfo"] =
                                    "0x" + intToHexString(*value, 8);
                            }
                        }
                    }
...
inline void
    getSubProcessorCoreData(const crow::Request&,
                            const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                            const std::string& processorId,
                            const std::string& coreId)
{
    BMCWEB_LOG_DEBUG << "Get available system sub processor core resources.";
    ...
    auto callback = [aResp, processorId,
        ...
        crow::connections::systemBus->async_method_call(
            [aResp, processorId, coreId](
                ...
                for (const std::pair<std::string, boost::container::flat_map<
                        std::string, std::vector<std::string>>>& object : subtree)
                {
...
inline void
    getSubProcessorCoreMembers(const crow::Request&,
                               const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                               const std::string& processorId)
{
    auto callback = [aResp,
                     processorId](const boost::system::error_code ecHandler,
                                  const std::string& cpuPath) {
        if (ecHandler)
        {
            BMCWEB_LOG_DEBUG << "DBUS response error, ecHandler: "
        ...
        collection_util::getCollectionMembers(aResp,
            subProcessorsPath, interfaces, cpuPath.c_str());
    };
...
inline void
    getCoreThreadDataByService(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                               const std::string& service,
                               const std::string& objPath)
{
    BMCWEB_LOG_DEBUG << "Get available system cpu thread resources by service.";
...
inline void
    getSubProcessorThreadData(const crow::Request&,
                              const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                              const std::string& processorId,
                              const std::string& coreId,
                              const std::string& threadId)
{
    BMCWEB_LOG_DEBUG << "Get available system sub processor thread resources.";

    auto callback = [aResp, processorId, coreId,
                     threadId](const boost::system::error_code ecHandler,
        ...
        crow::connections::systemBus->async_method_call(
            [aResp, processorId, coreId, threadId](
                const boost::system::error_code ec,
                const boost::container::flat_map<
                    std::string, boost::container::flat_map<
                                     std::string, std::vector<std::string>>>&
                        ...
                        std::string, std::vector<std::string>>>& object : subtree)
                {
                    ...
                    for (const std::pair<std::string, std::vector<std::string>>&
                            service : object.second)
                    {
                        getCoreThreadDataByService(aResp, service.first,
                                                   object.first);
                        break;
                    }
                    return;
    ...
    getSubProcessorCorePaths(aResp, processorId, coreId, std::move(callback));
}

inline void
    getSubProcessorThreadMembers(const crow::Request&,
                                 const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                                 const std::string& processorId,
                                 const std::string& coreId)
{
    auto callback = [aResp, processorId,
        ...
        collection_util::getCollectionMembers(aResp,
            subProcessorsPath, interfaces, corePath.c_str());
    };
...
inline void requestRoutesSubProcessorCoreCollection(App& app)
{
    BMCWEB_ROUTE(app,
                ...
                .privileges(redfish::privileges::getProcessorCollection)
                .methods(boost::beast::http::verb::get)(getSubProcessorCoreMembers);
}
...
inline void requestRoutesSubProcessorCore(App& app)
{
    BMCWEB_ROUTE(app,
                ...
                .privileges(redfish::privileges::getProcessor)
                .methods(boost::beast::http::verb::get)(getSubProcessorCoreData);
}
...
inline void requestRoutesSubProcessorThreadCollection(App& app)
{
    BMCWEB_ROUTE(app,
                ...
                .privileges(redfish::privileges::getProcessorCollection)
                .methods(boost::beast::http::verb::get)(getSubProcessorThreadMembers);
}
...
inline void requestRoutesSubProcessorThread(App& app)
{
    BMCWEB_ROUTE(app,
                ...
                .privileges(redfish::privileges::getProcessor)
                .methods(boost::beast::http::verb::get)(getSubProcessorThreadData);
}
