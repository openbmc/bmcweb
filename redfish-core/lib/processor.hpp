...
constexpr std::array<std::string_view, 2> processorChassisInterfaces = {
    ...
    const dbus::utility::MapperGetSubTreePathsResponse& pcieFunctionPaths)
{
    if (ec.value() == EBADR)
    {
        asyncResp->res.jsonValue["Links"]["PCIeFunctions"] =
            nlohmann::json::array();
        asyncResp->res.jsonValue["Links"]["PCIeFunctions@odata.count"] = 0;
        return;
    }
    ...
    const std::vector<std::string>& interfaceList)
{
    ...
    dbus::utility::getAssociatedSubTreePaths(
        ...
        std::bind_front(afterGetPCIeFunctionLinks, asyncResp,
