...
{
    auto callback = [aResp, handler{std::forward<Handler>(handler)},
                     processorId,
...
inline void
    getSubProcessorCoreData(const crow::Request& /* req */,
                            const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                            const std::string& processorId,
                            const std::string& coreId)
{
    ...
    auto callback =
        [aResp, processorId, coreId](const boost::system::error_code ecHandler,
                                     const std::string& cpuPath) {
        if (ecHandler)
        {
            BMCWEB_LOG_DEBUG << "DBUS response error, ecHandler: "
        ...
        crow::connections::systemBus->async_method_call(
                     ...
                     object : subtree)
            {
                ...
                const std::pair<std::string, std::vector<std::string>> service =
                    *object.second.begin();
...
inline void
    getSubProcessorCoreMembers(const crow::Request& /* req */,
                               const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                               const std::string& processorId)
{
    auto callback =
        [aResp, processorId](const boost::system::error_code ecHandler,
                             const std::string& cpuPath) {
        if (ecHandler)
        {
            BMCWEB_LOG_DEBUG << "DBUS response error, ecHandler: "
        ...
        collection_util::getCollectionMembers(aResp, subProcessorsPath,
                                              interfaces, cpuPath.c_str());
    };
...
inline void
    getSubProcessorThreadData(const crow::Request& /*unused*/,
                              const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                              const std::string& processorId,
                              const std::string& coreId,
    ...
    auto callback = [aResp, processorId, coreId,
        ...
        crow::connections::systemBus->async_method_call(
                     ...
                     object : subtree)
            {
                ...
                const std::pair<std::string, std::vector<std::string>> service =
                    *object.second.begin();
                getCoreThreadDataByService(aResp, service.first, object.first);
