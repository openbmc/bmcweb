...
inline void getProcessorPCIeDataByService(
    ...
    const std::string& objectPath)
{
    BMCWEB_LOG_DEBUG("Get Processor PCIe data by service");
    dbus::utility::getAllProperties(
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    {
        sdbusplus::message::object_path portPath(path);
    ...
    const dbus::utility::MapperServiceMap& /*serviceMap*/)
{
    ...
    dbus::utility::getAssociatedSubTree(
        sdbusplus::message::object_path(objectPath + "/connecting"),
...
inline void afterGetPortCollectionSubtree(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& processorId, const boost::system::error_code& ec,
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    if (ec)
    {
        BMCWEB_LOG_DEBUG("No connecting ports for processor {}: {}",
    ...
    {
        sdbusplus::message::object_path portPath(path);
        const std::string portId = portPath.filename();
        if (portId.empty())
        {
            continue;
        }
    ...
    const dbus::utility::MapperServiceMap& /*serviceMap*/)
{
    ...
    dbus::utility::getAssociatedSubTree(
        ...
        sdbusplus::message::object_path("/xyz/openbmc_project/inventory"), 0,
        portInterface,
        std::bind_front(afterGetPortCollectionSubtree, asyncResp, processorId));
}

inline void afterGetPortSubtreeForMetrics(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    const dbus::utility::MapperServiceMap& /*serviceMap*/)
{
    ...
    dbus::utility::getAssociatedSubTree(
        ...
        sdbusplus::message::object_path("/xyz/openbmc_project/inventory"), 0,
        portInterface,
        std::bind_front(afterGetPortSubtreeForMetrics, asyncResp, portId));
}
    ...
    const std::string& portId)
{
    ...
    asyncResp->res.jsonValue["Name"] = std::format("{} Port Metrics", portId);
