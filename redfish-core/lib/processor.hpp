                             ...
                             const std::string& coreId, Handler&& handler)
{
    auto callback = [aResp, handler{std::forward<Handler>(handler)},
                     processorId, coreId](const boost::system::error_code ec,
...
inline void
    getSubProcessorCoreData(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                            const std::string& processorId,
                            const std::string& coreId)
{
    ...
    auto callback =
        [aResp, processorId, coreId](const boost::system::error_code ec,
                                     const std::string& cpuPath) {
        if (ec)
        {
            BMCWEB_LOG_DEBUG << "DBUS response error, ec: " << ec.value();
        ...
        crow::connections::systemBus->async_method_call(
            ...
            for (const auto& [objectPath, serviceMap] : subtree)
            {
                ...
                getCpuCoreDataByService(aResp, serviceMap.begin()->first,
                                        objectPath);
...
inline void
    getSubProcessorCoreMembers(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                               const std::string& processorId)
{
    auto callback = [aResp, processorId](const boost::system::error_code ec,
                                         const std::string& cpuPath) {
        if (ec)
        {
            BMCWEB_LOG_DEBUG << "DBUS response error, ec: " << ec.value();
...
inline void
    getSubProcessorThreadData(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                              const std::string& processorId,
                              const std::string& coreId,
                              const std::string& threadId)
{
    ...
    auto callback = [aResp, processorId, coreId,
                     threadId](const boost::system::error_code ec,
        ...
        crow::connections::systemBus->async_method_call(
            ...
            for (const auto& [objectPath, serviceMap] : subtree)
            {
                ...
                getCoreThreadDataByService(aResp, serviceMap.begin()->first,
                                           objectPath);
...
inline void requestRoutesSubProcessorCoreCollection(App& app)
{
    BMCWEB_ROUTE(app,
        ...
        getSubProcessorCoreMembers(asyncResp, processorId);
        });
}
...
inline void requestRoutesSubProcessorCore(App& app)
{
    BMCWEB_ROUTE(
        ...
        getSubProcessorCoreData(asyncResp, processorId, coreId);
        });
}
...
inline void requestRoutesSubProcessorThreadCollection(App& app)
{
    BMCWEB_ROUTE(
        ...
        getSubProcessorThreadMembers(asyncResp, processorId, coreId);
        });
}
...
inline void requestRoutesSubProcessorThread(App& app)
{
    BMCWEB_ROUTE(
        ...
        getSubProcessorThreadData(asyncResp, processorId, coreId, threadId);
        });
}
