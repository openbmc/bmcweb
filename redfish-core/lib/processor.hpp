                                ...
                                const std::string& coreId)
{
    ...
    auto callback = [aResp, processorId,
                     coreId](const boost::system::error_code ecHander,
                             const std::string& cpuPath) {
        if (ecHander)
        {
            BMCWEB_LOG_DEBUG << "DBUS response error, ecHander: "
                             << ecHander.value();
            // No processor objects found by mapper
            if (ecHander.value() == boost::system::errc::io_error)
            {
...
{
    auto callback = [aResp,
                     processorId](const boost::system::error_code ecHander,
                                  const std::string& cpuPath) {
        if (ecHander)
        {
            BMCWEB_LOG_DEBUG << "DBUS response error, ecHander: "
