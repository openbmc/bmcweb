...
static constexpr std::array<std::string_view, 1> processorPortInterfaces{
...
inline std::optional<std::pair<std::string, std::string>> resolveProcessorPort(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    if (ec)
    {
        ...
        BMCWEB_LOG_ERROR("DBus error getting port subtree: {}", ec);
        messages::internalError(asyncResp->res);
        return std::nullopt;
    }
    ...
    {
        sdbusplus::object_path portPath(path);
        if (portPath.filename() != portId)
        {
            continue;
        }
        if (serviceMap.empty())
        ...
        {
            BMCWEB_LOG_ERROR(
                "More than one service found for Port {} on path {}", portId,
                path);
            messages::internalError(asyncResp->res);
            return std::nullopt;
        }
        return std::make_pair(serviceMap.begin()->first, path);
    }
...
inline void afterGetPortSubtreeForGet(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& processorId, const std::string& portId,
    const boost::system::error_code& ec,
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    std::optional<std::pair<std::string, std::string>> port =
        resolveProcessorPort(asyncResp, portId, ec, subtree);
    if (!port)
    {
        return;
    }
...
inline void afterGetPortSubtreeForHead(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& portId, const boost::system::error_code& ec,
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    if (!resolveProcessorPort(asyncResp, portId, ec, subtree))
    {
        return;
    }
    ...
    const dbus::utility::MapperServiceMap& /*serviceMap*/)
{
    dbus::utility::getAssociatedSubTree(
        ...
        std::bind_front(afterGetPortSubtreeForGet, asyncResp, processorId,
                        portId));
}

inline void afterGetProcessorObjectForPortHead(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    const dbus::utility::MapperServiceMap& /*serviceMap*/)
{
    dbus::utility::getAssociatedSubTree(
        ...
        sdbusplus::object_path("/xyz/openbmc_project/inventory"), 0,
        processorPortInterfaces,
        std::bind_front(afterGetPortSubtreeForHead, asyncResp, portId));
}

inline void handleProcessorPortHead(
    App& app, const crow::Request& req,
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& systemName, const std::string& processorId,
    const std::string& portId)
{
    ...
    getProcessorObject(
        asyncResp, processorId,
        std::bind_front(afterGetProcessorObjectForPortHead, asyncResp, portId));
}
    ...
    const std::string& portId)
{
    ...
    getProcessorObject(asyncResp, processorId,
                       std::bind_front(afterGetProcessorObjectForPort,
    ...
    const dbus::utility::MapperGetSubTreePathsResponse& paths)
{
    ...
    collection_util::handleCollectionMembers(
        asyncResp, collectionUrl, nlohmann::json::json_pointer("/Members"), ec,
        paths);
}
...
inline void afterGetProcessorObjectForPortCollectionHead(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& /*objectPath*/,
    const dbus::utility::MapperServiceMap& /*serviceMap*/)
{
    ...
    const std::string& systemName, const std::string& processorId)
{
    ...
    getProcessorObject(
        asyncResp, processorId,
        std::bind_front(afterGetProcessorObjectForPortCollectionHead,
                        asyncResp));
}
...
inline void requestRoutesProcessor(App& app)
{
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/Processors/<str>/Ports/")
        ...
        .methods(boost::beast::http::verb::head)(
            std::bind_front(handleProcessorPortCollectionHead, std::ref(app)));
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/Processors/<str>/Ports/<str>/")
        ...
        .methods(boost::beast::http::verb::head)(
            std::bind_front(handleProcessorPortHead, std::ref(app)));
