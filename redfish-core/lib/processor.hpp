    ...
    const dbus::utility::ManagedObjectType& dbusData)
{
    ...
    asyncResp->res.jsonValue["ProcessorType"] = processor::ProcessorType::CPU;
    ...
    const std::string& objPath)
{
    ...
    dbus::utility::getManagedObjects(
        service, path,
        std::bind_front(afterGetCpuDataByService, asyncResp, cpuId, objPath));
}
...
inline void afterGetCpuAssetData(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const boost::system::error_code& ec,
    const dbus::utility::DBusPropertiesMap& properties)
{
    ...
    const dbus::utility::MapperGetSubTreePathsResponse& chassisPaths)
{
    if (ec)
    {
        if (ec.value() == EBADR)
        {
            BMCWEB_LOG_DEBUG("No contained_by association for processor");
            return;
        }
        BMCWEB_LOG_ERROR("GetAssociatedSubTreePaths error for contained_by: {}",
                         ec);
        return;
    }
    ...
    const dbus::utility::DBusPropertiesMap& properties)
{
    ...
    {
        BMCWEB_LOG_ERROR(
            "DBUS response error for Control.OperatingClockSpeed: {}", ec);
        messages::internalError(asyncResp->res);
        return;
    }
    ...
    {
        asyncResp->res.jsonValue["SpeedLimitMHz"] =
            *presentSpeedLimitMaxHz / hzPerMhz;
    }
    if (presentSpeedLimitMaxHz != nullptr && presentSpeedLimitMinHz != nullptr)
    {
        asyncResp->res.jsonValue["SpeedLocked"] =
            *presentSpeedLimitMaxHz == *presentSpeedLimitMinHz;
    }
    if (requestedSpeedLimitMaxHz != nullptr)
    {
        asyncResp->res.jsonValue["OperatingSpeedRangeMHz"]["SettingMax"] =
            *requestedSpeedLimitMaxHz / hzPerMhz;
    }
    if (requestedSpeedLimitMinHz != nullptr)
    {
        asyncResp->res.jsonValue["OperatingSpeedRangeMHz"]["SettingMin"] =
            *requestedSpeedLimitMinHz / hzPerMhz;
    }
}

// Resolves a processor's Control.OperatingClockSpeed object by walking the
// controlled_by association. Shared by the GET and PATCH paths, so the
// outcome is delivered to the callback as a boost::system::error_code that
// each caller maps to its own Redfish error:
//   - (no error)          : found; service and controlPath are valid
//   - EBADR               : controlled_by association/object not present
//   - errc::io_error      : association resolved but no Control object found
//   - errc::protocol_error: more than one Control object found (ambiguous)
inline void afterGetProcessorControlSpeedObject(
    const std::function<void(const boost::system::error_code& ec,
                             ...
                             const std::optional<std::string>& controlPath)>&
        callback,
    const boost::system::error_code& ec,
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    if (ec)
    {
        callback(ec, std::nullopt, std::nullopt);
        return;
    }
...
inline void getProcessorControlSpeedData(
    ...
    const std::optional<std::string>& controlService,
    const std::optional<std::string>& controlPath)
{
    if (ec)
    {
        ...
        {
            BMCWEB_LOG_DEBUG(
                "No Control.OperatingClockSpeed object for processor");
            return;
        }
    ...
    // Past the error check the lookup guarantees a valid service + path;
    // assert it so the optional dereferences below are checked.
    if (!controlService || !controlPath)
    {
        messages::internalError(asyncResp->res);
        return;
    }
...
inline void setOperatingClockSpeed(
    ...
    const std::optional<std::string>& minRedfishPropertyName,
    const std::optional<std::string>& maxRedfishPropertyName,
    const boost::system::error_code& ec,
    const std::optional<std::string>& controlService,
    const std::optional<std::string>& controlPath)
{
    if (ec)
    {
        if (ec.value() == EBADR || ec == boost::system::errc::io_error)
        {
            // The processor exists but has no associated
            // Control.OperatingClockSpeed object, so the clock-speed
            // property the client tried to set is not supported here.
            BMCWEB_LOG_WARNING(
                "Processor has no associated Control.OperatingClockSpeed");
            messages::propertyUnknown(asyncResp->res,
                                      minRedfishPropertyName.value_or(
                                          maxRedfishPropertyName.value_or("")));
            return;
        }
        if (ec == boost::system::errc::protocol_error)
        {
    ...
    if (requestedMinHz && minRedfishPropertyName)
    {
    ...
    if (requestedMaxHz && maxRedfishPropertyName)
    {
    ...
    const dbus::utility::DBusPropertiesMap& properties)
{
    ...
    {
        BMCWEB_LOG_DEBUG("Non-finite Metric.Value for OperatingSpeedMHz");
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    if (ec)
    {
        if (ec.value() == EBADR)
        {
            BMCWEB_LOG_DEBUG("No measured_by association for processor");
            return;
        }
        BMCWEB_LOG_ERROR("GetAssociatedSubTree error for measured_by: {}", ec);
        return;
    }

    for (const auto& [metricPath, serviceMap] : subtree)
    {
        if (sdbusplus::object_path(metricPath).filename() ==
                "OperatingFrequency" &&
            !serviceMap.empty())
        {
            ...
            {
                BMCWEB_LOG_ERROR(
                    "More than one service found for OperatingFrequency metric in measured_by association");
                messages::internalError(asyncResp->res);
                return;
            }
...
inline void getCpuAssetData(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                            ...
                            const std::string& objPath)
{
    ...
    dbus::utility::getAllProperties(
        ...
        std::bind_front(afterGetCpuAssetData, asyncResp));
}

inline void afterGetCpuRevisionData(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const boost::system::error_code& ec,
    const dbus::utility::DBusPropertiesMap& properties)
{
...
inline void afterGetAcceleratorSpeeds(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const boost::system::error_code& ec,
    const dbus::utility::DBusPropertiesMap& properties)
{
    if (ec)
    {
        BMCWEB_LOG_ERROR("DBUS response error for Accelerator: {}", ec);
        messages::internalError(asyncResp->res);
        return;
    }
    ...
    {
        asyncResp->res.jsonValue["BaseSpeedMHz"] = *baseSpeedInHz / hzPerMhz;
    }
    if (maxSpeedInHz != nullptr)
    {
        uint64_t mhz = *maxSpeedInHz / hzPerMhz;
        asyncResp->res.jsonValue["MaxSpeedMHz"] = mhz;
        asyncResp->res.jsonValue["OperatingSpeedRangeMHz"]["AllowableMax"] =
            mhz;
    }
    if (minSpeedInHz != nullptr)
    {
        uint64_t mhz = *minSpeedInHz / hzPerMhz;
        asyncResp->res.jsonValue["MinSpeedMHz"] = mhz;
        asyncResp->res.jsonValue["OperatingSpeedRangeMHz"]["AllowableMin"] =
            mhz;
    }
}
    ...
    const std::string& objPath)
{
    ...
    dbus::utility::getAllProperties(
        service, objPath, "xyz.openbmc_project.Inventory.Item.Accelerator",
        std::bind_front(afterGetAcceleratorSpeeds, asyncResp));
}
    ...
    const std::string& objPath)
{
    ...
    dbus::utility::getAllProperties(
        ...
        std::bind_front(afterGetCpuConfigData, asyncResp, cpuId, service));
}
...
inline void doPatchProcessor(
    ...
    std::optional<std::string> minRedfishPropertyName,
    std::optional<std::string> maxRedfishPropertyName,
    const std::string& objectPath,
    const dbus::utility::MapperServiceMap& serviceMap)
{
    if (appliedConfigUri)
