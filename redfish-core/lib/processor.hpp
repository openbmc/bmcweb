 ...
 * @param[in,out]   resp            Async HTTP response.
 * @param[in]       processorId     Redfish Processor Id.
 * @param[in]       active          The LocationIndicatorActive property value
 * @param[in]       handler         Callback to continue processing request upon
 *                                  successfully finding object.
 */
template <typename Handler>
inline void setProcessorObject(const std::shared_ptr<bmcweb::AsyncResp>& resp,
                               const std::string& processorId, bool active,
                               Handler&& handler)
{
    // GetSubTree on all interfaces which provide info about a Processor
    crow::connections::systemBus->async_method_call(
        [resp, processorId, active, handler = std::forward<Handler>(handler)](
            boost::system::error_code ec,
            const MapperGetSubTreeResponse& subtree) mutable {
            if (ec)
            {
            ...
            for (const auto& [objectPath, serviceMap] : subtree)
            {
                ...
                for (const auto& [serviceName, interfaceList] : serviceMap)
                {
                    for (auto& process : processorInterfaces)
                    {
                        auto it = std::find_if(
                            interfaceList.begin(), interfaceList.end(),
                            [process](auto const& interface) {
                ...
                // matching objects. Assume all interfaces we want to process
                // must be on the same object path.
                handler(resp, serviceMap, objectPath, active);
                return;
            }
...
inline void setProcessorData(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                             const MapperServiceMap& serviceMap,
                             const std::string& objectPath, bool active)
{
    for (const auto& [serviceName, interfaceList] : serviceMap)
    {
        for (const auto& interface : interfaceList)
        {
                             ...
                             "Association.Definitions")
            {
                ...
                setLocationIndicatorActive(aResp, objectPath, active);
                return;
            }
        }
    }
...
class Processor : public Node
{
  ...
  private:
                 ...
                 const std::vector<std::string>& params) override
    {
        if (params.size() != 1)
        {
            BMCWEB_LOG_DEBUG << "Processor doPatch param size < 1";
            messages::internalError(asyncResp->res);
            return;
        }
