 ...
 */
inline void handleAppliedConfigResponse(const std::shared_ptr<AsyncResp>& resp,
                                        ...
                                        const MapperServiceMap& serviceMap)
{
    ...
    // to AppliedConfig is a valid path. Instead of blindly escaping the last
    // path component (and any slashes therein), we could use
    // sd_bus_object_path_is_valid (or wrapper) to check it, and return an error
    // response here if it's not valid.
    ...
    std::string configPath(cpuObjectPath);
    configPath += '/';
...
class Processor : public Node
{
  ...
  private:
                 ...
                 const std::vector<std::string>& params) override
    {
        ...
        getProcessorObject(
            asyncResp, params[0],
            [appliedConfigJson = std::move(appliedConfigJson)](
