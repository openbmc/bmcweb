    ...
    std::optional<std::vector<nlohmann::json>>& discreteTriggers, Context& ctx)
{
    ...
    {
        DiscreteThresholdParams discreteTrigger;

        if (!parseSingleDiscreteTrigger(res, thresholdInfo, discreteTriggers))
        {
            return false;
        }

        parsedParams.emplace_back(discreteTrigger);
    }
    ...
    std::vector<NumericThresholdParams>& parsedParams)
{
    ...
    for (const std::string& thresholdName : supportedNumericThresholdNames)
    {
        nlohmann::json mergedThresholdInfo;
        json_util::getValueFromJsonObject(oldNumericThresholds, thresholdName,
                                          mergedThresholdInfo);
        ...
        if (!json_util::readJson(mergedThresholdInfo, asyncResp->res, "Reading",
                                 reading, "Activation", activation, "DwellTime",
                                 dwellTimeStr))
        {
            return false;
        }
