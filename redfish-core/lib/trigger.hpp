    ...
    getTriggerActions(const TriggerGetParamsMap& params)
{
    ...
    if (const auto log = get_value<bool>("LogToJournal", params); true == *log)
    {
        triggerActions.emplace_back("LogToLogService");
    }
    if (const auto log = get_value<bool>("LogToRedfish", params); true == *log)
    {
        triggerActions.emplace_back("RedfishEvent");
    }
    if (const auto log = get_value<bool>("UpdateReport", params); true == *log)
    {
...
nlohmann::json getMetricReportDefinitions(const TriggerGetParamsMap& params)
{
    ...
    if (const auto reportNames =
            get_value<std::vector<std::string>>("ReportNames", params))
    {
        for (auto& name : *reportNames)
        {
            reports.push_back({
    ...
    getMetricProperties(const TriggerGetParamsMap& params)
{
    ...
    if (const auto sensors = get_value<TriggerSensorsParams>("Sensors", params))
    {
    ...
    const std::vector<std::pair<std::string, TriggerGetParamsVariant>>& ret)
{
    ...
    if (const auto discrete = get_value<bool>("Discrete", params))
    {
        if (const auto thresholdParams =
                get_value<TriggerThresholdParamsExt>("Thresholds", params))
        {
            if (*discrete)
            {
                auto& triggers = asyncResp->res.jsonValue["DiscreteTriggers"];
