...
inline bool parseSingleDiscreteTrigger(crow::Response& res,
                                       nlohmann::json& discreteTrigger,
                                       DiscreteThresholdParams& parsed)
{
    ...
    if (!json_util::readJson(discreteTrigger, res, "Name", name, "Value", value,
                             "DwellTime", dwellTimeStr, "Severity", severity))
    {
        return false;
    }
    ...
    parsed = std::make_tuple(name.value_or(""), severity,
                             static_cast<uint64_t>(dwellTime->count()), value);
    return true;
}

inline bool parseSingleNumericThreshold(crow::Response& res,
                                        const std::string& thresholdName,
                                        nlohmann::json& numericThreshold,
                                        NumericThresholdParams& parsed)
{
    ...
    if (!json_util::readJson(numericThreshold, res, "Reading", reading,
    ...
    parsed = std::make_tuple(thresholdName,
                                   ...
                                   Context& ctx)
{
    ...
    for (auto& [thresholdName, thresholdData] : numericThresholds.items())
    {
        ...
        if (!parseSingleNumericThreshold(res, thresholdName, thresholdData,
                                         parsed))
        {
            return false;
        }

        parsedParams.emplace_back(parsed);
    }
    ...
    std::optional<std::vector<nlohmann::json>>& discreteTriggers, Context& ctx)
{
    ...
    for (nlohmann::json& thresholdInfo : *discreteTriggers)
    {
        ...
        if (!parseSingleDiscreteTrigger(res, thresholdInfo, parsed))
        {
            return false;
        }
...
inline bool parseNumericThresholdsPatch(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    nlohmann::json& oldNumericThresholds, nlohmann::json numericThresholdsPatch,
    std::vector<NumericThresholdParams>& parsedParams)
{
    parsedParams.reserve(supportedNumericThresholdNames.size());

    for (const std::string_view& thresholdName : supportedNumericThresholdNames)
    {
        std::string thresholdNameStr(thresholdName);

        nlohmann::json mergedThresholdInfo;
        json_util::getValueFromJsonObject(
            oldNumericThresholds, thresholdNameStr, mergedThresholdInfo);

        nlohmann::json newTresholdInfo;
        json_util::getValueFromJsonObject(numericThresholdsPatch,
                                          thresholdNameStr, newTresholdInfo);

        mergedThresholdInfo.merge_patch(newTresholdInfo);
        if (mergedThresholdInfo.empty())
        {
            continue;
        }
        ...
        if (!parseSingleNumericThreshold(asyncResp->res, thresholdNameStr,
                                         mergedThresholdInfo, parsed))
        {
            return false;
        }
...
inline void
    handleSensorsPatch(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                       const std::string& dbusPath,
                       const std::vector<std::string>& newVal)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, newVal](const boost::system::error_code ec) {
            if (ec)
            {
                BMCWEB_LOG_DEBUG << "Error updating Sensors property";
                messages::internalError(asyncResp->res);
                return;
            }
        },
        service, dbusPath, "org.freedesktop.DBus.Properties", "Set",
        triggerInterface, "Sensors",
        std::variant<std::vector<std::string>>(newVal));
}

inline void
    handleReportNamesPatch(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                           const std::string& dbusPath,
                           const std::vector<std::string>& newVal)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, newVal](const boost::system::error_code ec) {
            if (ec)
            {
                BMCWEB_LOG_DEBUG << "Error updating ReportNames property";
                messages::internalError(asyncResp->res);
                return;
            }
        },
        service, dbusPath, "org.freedesktop.DBus.Properties", "Set",
        triggerInterface, "ReportNames",
        std::variant<std::vector<std::string>>(newVal));
}
                         ...
                         const std::string& id)
{
    ...
    std::optional<std::vector<nlohmann::json>> discreteTriggers;
    std::optional<nlohmann::json> numericThresholds;
    std::optional<std::vector<nlohmann::json>> metricProperties;
    std::optional<nlohmann::json> links;
    ...
    telemetry::get_trigger::getPropertiesJson(
        id, [asyncResp, id, name, discreteTriggers, numericThresholds,
             metricProperties,
             metricReportDefinitions](telemetry::get_trigger::GetResult result,
                                      nlohmann::json& propertiesJson) {
            ...
            std::vector<std::string> parsedMetricProperties;
            std::vector<std::string> parsedMetricReportDefinitions;
            ...
            if (numericThresholds)
            {
                if (propertiesJson.find("DiscreteTriggers") !=
                    propertiesJson.end())
                {
                    messages::propertyValueConflict(asyncResp->res,
                                                    "NumericThresholds",
                ...
                nlohmann::json oldNumericThresholds;
                if (!json_util::getValueFromJsonObject(propertiesJson,
                                                       "NumericThresholds",
                                                       oldNumericThresholds))
                {
                    messages::internalError(asyncResp->res);
                    return;
                }
                tresholds = std::vector<NumericThresholdParams>();
                std::vector<NumericThresholdParams>& resultThresholds =
                    std::get<std::vector<NumericThresholdParams>>(tresholds);
                if (!parseNumericThresholdsPatch(
                        asyncResp, oldNumericThresholds, *numericThresholds,
                        resultThresholds))
                {
                    return;
                }
            ...
            if (metricReportDefinitions)
            {
                nlohmann::json oldLinks;
                if (!json_util::getValueFromJsonObject(propertiesJson, "Links",
                                                       oldLinks))
                {
                    messages::internalError(asyncResp->res);
                    return;
                }
                std::vector<std::string> oldMetricReportDefinitions;
                if (!json_util::getValueFromJsonObject(
                        oldLinks, "MetricReportDefinitions",
                        oldMetricReportDefinitions))
                {
                    messages::internalError(asyncResp->res);
                    return;
                }
                if (!updateStringCollection(
                        asyncResp->res, "MetricReportDefinitions",
                        oldMetricReportDefinitions, *metricReportDefinitions,
                        parsedMetricReportDefinitions))
                {
                    return;
                }
            }
            ...
            if (metricProperties)
            {
                std::vector<std::string> oldMetricProperties;
                if (!json_util::getValueFromJsonObject(propertiesJson,
                                                       "MetricProperties",
                                                       oldMetricProperties))
                {
                ...
                if (!updateStringCollection(
                        asyncResp->res, "MetricProperties", oldMetricProperties,
                        *metricProperties, parsedMetricProperties))
                {
                    return;
                }
            }
            ...
            if (metricReportDefinitions)
            {
                handleReportNamesPatch(asyncResp, triggerDbusPath,
                                       parsedMetricReportDefinitions);
            }

            if (metricProperties)
            {
                handleSensorsPatch(asyncResp, triggerDbusPath,
                                   parsedMetricProperties);
            }
        });
}
