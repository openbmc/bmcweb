...
inline std::optional<std::string> getDurationStringFromMs(const uint64_t timeMs)
{
    ...
    {
        duration =
            time_utils::toDurationString(std::chrono::milliseconds(timeMs));
    }
    return duration.empty() ? std::nullopt : std::make_optional(duration);
}
    ...
    getDiscreteTriggers(const TriggerThresholdParamsExt& thresholdParams)
{
    ...
    for (const auto& [name, severity, dwellTime, value] : *discreteParams)
    {
        std::optional<std::string> duration =
            getDurationStringFromMs(dwellTime);
        ...
        properties)
{
    std::string name;
    bool discrete = false;
    TriggerSensorsParams sensors;
    std::vector<std::string> reports;
    std::vector<std::string> actions;
    TriggerThresholdParamsExt thresholds;

    if (std::optional<std::string> missing = sdbusplus::unpackPropertiesNoThrow(
            properties, "Thresholds", thresholds, "Sensors", sensors,
            "ReportNames", reports, "Discrete", discrete, "Name", name,
            "TriggerActions", actions))
    {
        BMCWEB_LOG_ERROR << "Property type mismatch or property is missing: "
                         << *missing << " in Trigger: " << id;
        return false;
    }
