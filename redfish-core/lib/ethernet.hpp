            ...
            if (objpath.first == "/xyz/openbmc_project/network/" + ethifaceId)
            {
                         ...
                         "xyz.openbmc_project.Network.EthernetInterface")
                {
                    for (const auto& propertyPair : ifacePair.second)
                    {
                        ...
                        {
                            const std::vector<std::string_view>* nameservers =
                                std::get_if<std::vector<std::string_view>>(
                                    &propertyPair.second);
                            if (nameservers != nullptr)
                            {
                                ethData.nameServers = std::vector<std::string>(
                            ...
                            if (staticNameServers != nullptr)
                            {
                                ethData.staticNameServers =
                                    std::vector<std::string>(
                                        staticNameServers->begin(),
                                        staticNameServers->end());
                            }
                        }
                        ...
                        else if (propertyPair.first == "DomainName")
                        {
                            ...
                            {
                                ethData.domainnames = std::vector<std::string>(
                                    domainNames->begin(), domainNames->end());
                            }
                        }
