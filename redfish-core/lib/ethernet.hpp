    ...
    EthernetInterfaceData& ethData)
{
    ...
    for (const auto& objpath : dbusData)
    {
        for (const auto& ifacePair : objpath.second)
        {
            ...
            const std::string dhcp4PathStart = "/xyz/openbmc_project/network/" +
                                               ethifaceId + "/dhcp4";
            ...
            const std::string dhcp6PathStart = "/xyz/openbmc_project/network/" +
                                               ethifaceId + "/dhcp6";
...
inline void setDHCPConfig(const std::string& propertyName, const bool& value,
                          const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                          const std::string& ethifaceId, const NwType& type)
{
    BMCWEB_LOG_DEBUG("{} = {}", propertyName, value);
    sdbusplus::message::object_path path = "/xyz/openbmc_project/network/" +
                                           ethifaceId;
    path /= type == NwType::dhcp4 ? "dhcp4" : "dhcp6";
