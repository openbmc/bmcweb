...
/**
 * @brief Handles DBus error for blocked interface IP changes
 *
 * @param[io] asyncResp    Response object that will be returned to client
 * @param[in] msg          DBus message containing the error
 * @param[in] propertyName Redfish property name to report as not writable
 *
 * @return None
 */
inline void handlePropertyWriteError(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const sdbusplus::message_t& msg, std::string_view propertyName)
{
    ...
    if (dbusError != nullptr)
    {
        ...
        if (std::string_view("xyz.openbmc_project.Common.Error.NotAllowed") ==
            dbusError->name)
        {
            messages::propertyNotWritable(asyncResp->res, propertyName);
            return;
        }
    }
                            ...
                            std::string_view propertyName)
{
    dbus::utility::async_method_call(
        ...
        [asyncResp, propertyName](const boost::system::error_code& ec,
            ...
            if (ec)
            {
                handlePropertyWriteError(asyncResp, msg, propertyName);
                return;
            }
        },
                       ...
                       const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    auto createIpHandler =
        [asyncResp, ifaceId, gateway](const boost::system::error_code& ec,
            ...
            if (ec)
            {
                handlePropertyWriteError(asyncResp, msg, "IPv4StaticAddresses");
                return;
            }
        };
...
inline void handleErrorAndCreateIpAddress(
    IpVersion version, const std::string& ifaceId, uint8_t prefixLength,
    ...
    const boost::system::error_code& ec, const sdbusplus::message_t& msg)
{
    ...
    dbus::utility::async_method_call(
        asyncResp,
        [asyncResp, propertyName](const boost::system::error_code& ec2,
                                  const sdbusplus::message_t& msg2) {
            if (ec2)
            {
                handlePropertyWriteError(asyncResp, msg2, propertyName);
            }
        },
        "xyz.openbmc_project.Network",
        "/xyz/openbmc_project/network/" + ifaceId,
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    dbus::utility::async_method_call(
        asyncResp,
        [version, ifaceId, prefixLength, address, gateway, propertyName,
         asyncResp](const boost::system::error_code& ec,
                    const sdbusplus::message_t& msg) {
            handleErrorAndCreateIpAddress(version, ifaceId, prefixLength,
                                          address, gateway, propertyName,
                                          asyncResp, ec, msg);
        },
        "xyz.openbmc_project.Network",
...
inline void afterCreateIPv6(const std::string& address,
                            const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                            ...
                            const sdbusplus::message_t& msg)
{
    if (ec)
    {
        ...
        handlePropertyWriteError(asyncResp, msg, "IPv6StaticAddresses");
    }
}
                       ...
                       const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    dbus::utility::async_method_call(
        asyncResp,
        [address, asyncResp](const boost::system::error_code& ec,
                             const sdbusplus::message_t& msg) {
            afterCreateIPv6(address, asyncResp, ec, msg);
        },
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    dbus::utility::async_method_call(
        ...
        [asyncResp](const boost::system::error_code& ec,
            ...
            if (ec)
            {
                handlePropertyWriteError(asyncResp, msg,
...
)
{
    if (ec)
    {
        ...
        if (std::string_view("xyz.openbmc_project.Common.Error.NotAllowed") ==
            dbusError->name)
        {
            messages::propertyNotWritable(asyncResp->res, "VLANs");
            return;
        }
        messages::internalError(asyncResp->res);
        return;
