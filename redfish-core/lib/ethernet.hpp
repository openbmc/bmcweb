...
inline bool extractEthernetInterfaceData(
  ...
  for (const auto& objpath : dbusData) {
    for (const auto& ifacePair : objpath.second) {
      ...
      sdbusplus::object_path dhcp4Path = path / ethifaceId / "dhcp4";

      if (sdbusplus::object_path(objpath.first) == dhcp4Path) {
        if (ifacePair.first ==
      ...
      sdbusplus::object_path dhcp6Path = path / ethifaceId / "dhcp6";

      if (sdbusplus::object_path(objpath.first) == dhcp6Path) {
        if (ifacePair.first ==
            "xyz.openbmc_project.Network.DHCPConfiguration") {
          for (const auto& propertyPair : ifacePair.second) {
            if (propertyPair.first == "DNSEnabled") {
        ...
        if (interface.first == "xyz.openbmc_project.Network.IP") {
          ...
          for (const auto& property : interface.second) {
            if (property.first == "Address") {
              ...
              BMCWEB_LOG_ERROR("Got extra property: {} on the {} object",
  ...
  auto createIpHandler = [asyncResp,
    ...
    if (ec) {
      if (ec == boost::system::errc::io_error) {
        messages::propertyValueFormatError(asyncResp->res, address, "Address");
      } else {
        messages::internalError(asyncResp->res);
      }
    }
...
inline void createIPv6DefaultGateway(
  ...
  dbus::utility::async_method_call(
      asyncResp, std::move(createIpHandler), "xyz.openbmc_project.Network",
      path, "xyz.openbmc_project.Network.StaticGateway.Create", "StaticGateway",
...
inline void handleIPv6DefaultGateway(
  ...
  for (std::variant<nlohmann::json::object_t, std::nullptr_t>& thisJson :
    ...
    if (staticGatewayEntry != staticGatewayData.end()) {
      deleteAndCreateIPv6DefaultGateway(ifaceId, staticGatewayEntry->id, *addr,
                                        asyncResp);
      staticGatewayEntry++;
    } else {
...
inline void handleHostnamePatch(
  ...
  setDbusProperty(asyncResp, "HostName", "xyz.openbmc_project.Network",
...
inline void handleMTUSizePatch(
  ...
  objPath /= ifaceId;
  setDbusProperty(asyncResp, "MTUSize", "xyz.openbmc_project.Network", objPath,
...
enum class NetworkType { dhcp4, dhcp6 };
...
inline void setDHCPConfig(const std::string& propertyName, const bool& value,
  ...
  setDbusProperty(asyncResp, redfishPropertyName, "xyz.openbmc_project.Network",
                  path, "xyz.openbmc_project.Network.DHCPConfiguration",
                  propertyName, value);
}
...
inline void handleSLAACAutoConfigPatch(
  ...
  sdbusplus::object_path path("/xyz/openbmc_project/network");
  path /= ifaceId;
  setDbusProperty(asyncResp, "StatelessAddressAutoConfig/IPv6AutoConfigEnabled",
                  "xyz.openbmc_project.Network", path,
                  "xyz.openbmc_project.Network.EthernetInterface",
...
inline bool parseAddresses(
  ...
  for (std::variant<nlohmann::json::object_t, std::nullptr_t>& thisJson :
       input) {
    std::string pathString = std::format("IPv4StaticAddresses/{}", entryIdx);
    ...
    if (obj == nullptr) {
      ...
      if (subnetMask) {
        ...
        if (!ip_util::ipv4VerifyIpAndGetBitcount(*subnetMask, &prefixLength)) {
          messages::propertyValueFormatError(res, *subnetMask,
        ...
        // nothing we can do
        ip_util::ipv4VerifyIpAndGetBitcount(nicIpEntry->netmask, &prefixLength);
      ...
      if (!thisAddress.gateway.empty() && thisAddress.gateway != "0.0.0.0") {
      ...
      case AddrChange::Update: {
        ...
        if (!address.existingDbusId.empty()) {
          ...
          deleteAndCreateIPAddress(IpVersion::IpV4, ifaceId,
                                   address.existingDbusId, address.prefixLength,
                                   address.address, address.gateway, asyncResp);
        } else {
          // Otherwise, just create a new one
          BMCWEB_LOG_ERROR(
              "creating ip {} prefix {} gateway {} on interface {}",
              address.address, address.prefixLength, address.gateway, ifaceId);
    ...
    if (obj != nullptr && !obj->empty()) {
      ...
      if (nicIpEntry != ipv6Data.end()) {
        deleteAndCreateIPAddress(IpVersion::IpV6, ifaceId, nicIpEntry->id,
                                 *prefixLength, *address, "", asyncResp);
...
inline void parseInterfaceData(
  ...
  if (ethData.nicEnabled) {
    jsonResponse["LinkStatus"] = ethData.linkUp
                                     ? ethernet_interface::LinkStatus::LinkUp
  ...
  if (ethData.vlanId) {
    ...
    parentInterface["@odata.id"] = boost::urls::format(
        ...
        BMCWEB_REDFISH_MANAGER_URI_NAME, extractParentInterfaceName(ifaceId));
    jsonResponse["Links"]["RelatedInterfaces"] = std::move(relatedInterfaces);
  } else {
    jsonResponse["EthernetInterfaceType"] =
...
inline void afterDelete(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
  ...
  if (std::string_view("org.freedesktop.DBus.Error.UnknownObject") ==
      dbusError->name) {
    messages::resourceNotFound(asyncResp->res, "EthernetInterface", ifaceId);
    return;
  }
...
inline void afterVlanCreate(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
  ...
  if (ec) {
    ...
    if (std::string_view("xyz.openbmc_project.Common.Error.ResourceNotFound") ==
        dbusError->name) {
    ...
    if (std::string_view("xyz.openbmc_project.Common.Error.InvalidArgument") ==
        dbusError->name) {
      messages::resourceAlreadyExists(asyncResp->res, "EthernetInterface", "Id",
                                      vlanInterface);
      return;
    }
    messages::internalError(asyncResp->res);
             ...
             v6dhcpParms = std::move(v6dhcpParms), interfaceEnabled](
              ...
              if (interfaceEnabled) {
                setDbusProperty(
                    ...
                    "xyz.openbmc_project.Network",
                    sdbusplus::object_path("/xyz/openbmc_project/network") /
                        ifaceId,
                    "xyz.openbmc_project.Network.EthernetInterface",
