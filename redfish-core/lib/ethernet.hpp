    ...
    EthernetInterfaceData& ethData)
{
    ...
    for (const auto& objpath : dbusData)
    {
        for (const auto& ifacePair : objpath.second)
        {
            ...
            const std::string dhcp4PathStart = "/xyz/openbmc_project/network/" + ethifaceId + "/dhcp4";

            if (objpath.first.str.starts_with(dhcp4PathStart))
            {
                if (ifacePair.first ==
           ...
           const std::string dhcp6PathStart = "/xyz/openbmc_project/network/" + ethifaceId + "/dhcp6";

            if (objpath.first.str.starts_with(dhcp6PathStart))
            {
                if (ifacePair.first ==
                    "xyz.openbmc_project.Network.DHCPConfiguration")
                {
                          ...
                          const NwType& type)
{
    ...
    sdbusplus::message::object_path path = "/xyz/openbmc_project/network" + ethifaceId + type;
    sdbusplus::asio::setProperty(
                            ...
                            const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    setDHCPConfig("DNSEnabled", nextDNSv4, asyncResp, ifaceId, NwType::dhcp4);
    BMCWEB_LOG_DEBUG("set NTPEnabled...");
    setDHCPConfig("NTPEnabled", nextNTPv4, asyncResp, ifaceId, NwType::dhcp4);
    ...
    setDHCPConfig("DNSEnabled", nextDNSv6, asyncResp, ifaceId, NwType::dhcp6);
    BMCWEB_LOG_DEBUG("set NTPEnabled for dhcp6...");
    setDHCPConfig("NTPEnabled", nextNTPv6, asyncResp, ifaceId, NwType::dhcp6);
