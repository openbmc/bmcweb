...
{
   auto& DHCPConfigTypeJson = json_response["DHCPv4Configuration"];

   DHCPConfigTypeJson["UseDNSServers"] = configData.dnsenabled;
   DHCPConfigTypeJson["HostNameEnabled"] = configData.hostnameenabled;
...
inline void getDHCPConfigData(const std::shared_ptr<AsyncResp> asyncResp)
{
    auto getConfig = [asyncResp](const boost::system::error_code error_code,
        ...
        {
            auto value = sdbusplus::message::variant_ns::get_if<bool>(
                &property.second);
...
class EthernetInterface : public Node
{
  ...
  private:
        ...
        const boost::container::flat_set<IPv4AddressData> &ipv4Data)
    {
        ...
        auto& DHCPv4Json = json_response["DHCPv4Configuration"];
        DHCPv4Json["DHCPEnabled"] = ethData.DHCPEnabled;
