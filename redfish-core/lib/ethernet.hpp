    ...
    EthernetInterfaceData& ethData)
{
    ...
    for (const auto& objpath : dbusData)
    {
        for (const auto& ifacePair : objpath.second)
        {
            ...
            if (objpath.first.str.starts_with(dhcp4Path))
            {
                if (ifacePair.first ==
            ...
            if (objpath.first.str.starts_with(dhcp6Path))
            {
                if (ifacePair.first ==
                    "xyz.openbmc_project.Network.DHCPConfiguration")
                {
                            ...
                            const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    setDHCPConfig("DNSEnabled", nextDNSv4, asyncResp, ifaceId, NetworkType::dhcp4);
    BMCWEB_LOG_DEBUG("set NTPEnabled...");
    setDHCPConfig("NTPEnabled", nextNTPv4, asyncResp, ifaceId, NetworkType::dhcp4);
    ...
    setDHCPConfig("DNSEnabled", nextDNSv6, asyncResp, ifaceId, NetworkType::dhcp6);
    BMCWEB_LOG_DEBUG("set NTPEnabled for dhcp6...");
    setDHCPConfig("NTPEnabled", nextNTPv6, asyncResp, ifaceId, NetworkType::dhcp6);
