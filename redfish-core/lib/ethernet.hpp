...
template <typename CallbackFunc>
void afterCheckActiveBondSlaveDisableConflict(
    const std::string& ifaceId, CallbackFunc& callback,
    const boost::system::error_code& ec,
    const dbus::utility::ManagedObjectType& resp)
{
    if (ec)
    {
        BMCWEB_LOG_ERROR("Failed to get managed objects for {}: {}", ifaceId,
                         ec.message());
        ...
        for (const auto& [ifaceName, properties] : ifaceList)
        {
            ...
            auto activeSlaveIt =
                std::ranges::find_if(properties, [](const auto& property) {
                    return property.first == "ActiveSlave";
                });
    ...
    dbus::utility::getManagedObjects(
        ...
        [ifaceId{std::string{ifaceId}},
         callback = std::forward<CallbackFunc>(callback)](
            const boost::system::error_code& ec,
            const dbus::utility::ManagedObjectType& resp) mutable {
            afterCheckActiveBondSlaveDisableConflict(ifaceId, callback, ec,
                                                     resp);
        });
}
