...
enum class LinkType { Local, Global };
        ...
        if (ifacePair.first == "xyz.openbmc_project.Network.MACAddress") {
          ...
          for (const auto &propertyPair : ifacePair.second) {
            if (propertyPair.first == "AutoNeg") {
              const bool *auto_neg = std::get_if<bool>(&propertyPair.second);
              if (auto_neg != nullptr) {
              ...
              const std::vector<std::string> *nameservers =
                  sdbusplus::message::variant_ns::get_if<
                      std::vector<std::string>>(&propertyPair.second);
          ...
          for (const auto &propertyPair : ifacePair.second) {
            if (propertyPair.first == "DNSEnabled") {
              ...
              if (SendHostNameEnabled != nullptr) {
                ethData.SendHostNameEnabled = *SendHostNameEnabled;
              }
            }
          }
        }
    ...
    if (boost::starts_with(objpath.first.str, ipv6PathStart)) {
      for (auto &interface : objpath.second) {
        if (interface.first == "xyz.openbmc_project.Network.IP") {
          ...
          ipv6_address.id = objpath.first.str.substr(ipv6PathStart.size());
    ...
    if (boost::starts_with(objpath.first.str, ipv4PathStart)) {
      for (auto &interface : objpath.second) {
        if (interface.first == "xyz.openbmc_project.Network.IP") {
          ...
          ipv4_address.id = objpath.first.str.substr(ipv4PathStart.size());
          for (auto &property : interface.second) {
            if (property.first == "Address") {
              const std::string *address =
                  std::get_if<std::string>(&property.second);
...
void getEthernetIfaceData(const std::string &ethiface_id,
  ...
  crow::connections::systemBus->async_method_call(
      [ethiface_id{std::string{ethiface_id}}, callback{std::move(callback)}](
        ...
        bool found = extractEthernetInterfaceData(ethiface_id, resp, ethData);
        if (!found) {
          callback(false, ethData, ipv4Data, ipv6Data);
          return;
        }
...
void getEthernetIfaceList(CallbackFunc &&callback) {
  crow::connections::systemBus->async_method_call(
      [callback{std::move(callback)}](
          const boost::system::error_code error_code, GetManagedObjects &resp) {
...
private:
  ...
  void doGet(crow::Response &res, const crow::Request &req,
    ...
    res.jsonValue["@odata.id"] = "/redfish/v1/Managers/bmc/EthernetInterfaces";
    ...
    getEthernetIfaceList(
        [asyncResp](const bool &success,
          ...
          asyncResp->res.jsonValue["Members@odata.count"] = iface_array.size();
...
private:
    ...
    for (nlohmann::json &thisJson : input) {
      ...
      if (!thisJson.is_null() && !thisJson.empty()) {
        ...
        if (!json_util::readJson(thisJson, asyncResp->res, "Address", address,
                                 ...
                                 gateway)) {
          messages::propertyValueFormatError(asyncResp->res, thisJson.dump(),
                                             pathString);
          return;
        }
        ...
        if (subnetMask) {
          ...
          if (!ipv4VerifyIpAndGetBitcount(NICIPentry->netmask, &prefixLength)) {
        ...
        if (NICIPentry != ipv4Data.cend()) {
          ...
          deleteAndCreateIPv4(ifaceId, NICIPentry->id, prefixLength, *gw, *addr,
                              asyncResp);
  ...
  bool isDomainnameValid(const std::string &domainname) {
    // define a regular expression
    const std::regex pattern("[a-zA-Z]{2,}$");
  ...
  void parseInterfaceData(
    ...
    nlohmann::json &ipv4_static_array = json_response["IPv4StaticAddresses"];
    ...
    for (auto &ipv4_config : ipv4Data) {
      ...
      if (ipv4_config.origin == "Static") {
        ipv4_static_array.push_back({{"AddressOrigin", ipv4_config.origin},
                                     {"SubnetMask", ipv4_config.netmask},
                                     {"Address", ipv4_config.address},
                                     {"Gateway", gatewayStr}});
      }
    ...
    nlohmann::json &ipv6_static_array = json_response["IPv6StaticAddresses"];
    ...
    for (auto &ipv6_config : ipv6Data) {
      ...
      if (ipv6_config.origin == "Static") {
        ipv6_static_array.push_back({{"Address", ipv6_config.address},
                                     {"PrefixLength", ipv6_config.prefixLength},
                                     {"AddressOrigin", ipv6_config.origin}});
      }
    }
  ...
  void doGet(crow::Response &res, const crow::Request &req,
    ...
    getEthernetIfaceData(
        ...
        [this, asyncResp, iface_id{std::string(params[0])}](
          ...
          parseInterfaceData(asyncResp, iface_id, ethData, ipv4Data, ipv6Data);
        });
  }
  ...
  void doPatch(crow::Response &res, const crow::Request &req,
    ...
    if (!json_util::readJson(
            req, res, "HostName", hostname, "FQDN", fqdn, "IPv4StaticAddresses",
            ipv4StaticAddresses, "MACAddress", macAddress, "StaticNameServers",
            staticNameServers, "IPv6DefaultGateway", ipv6DefaultGateway,
            "IPv6StaticAddresses", ipv6StaticAddresses, "DHCPv4", dhcpv4,
            "DHCPv6", dhcpv6, "InterfaceEnabled", interfaceEnabled)) {
      return;
    }
    if (dhcpv4) {
        ...
        [this, asyncResp, iface_id, hostname = std::move(hostname),
         ...
         interfaceEnabled = std::move(interfaceEnabled)](
          ...
          if (fqdn) {
            ...
            boost::split(Names, *fqdn, boost::is_any_of("."));

            if (!isHostnameValid(Names[0]) || !isDomainnameValid(Names[1])) {
              messages::queryParameterValueFormatError(asyncResp->res, "FQDN",
                                                       *fqdn);
              return;
            }

            handleHostnamePatch(Names[0], asyncResp);
            std::vector<std::string> domainname;
            domainname.push_back(Names[1]);
            handleDomainnamePatch(iface_id, domainname, asyncResp);
          }
          ...
          if (ipv4StaticAddresses) {
            ...
            handleIPv4StaticPatch(iface_id, ipv4Static, ipv4Data, asyncResp);
          }
          ...
          if (ipv6StaticAddresses) {
            ...
            handleIPv6StaticAddressesPatch(iface_id, ipv6Static, ipv6Data,
                                           asyncResp);
          }

          if (interfaceEnabled) {
            setEthernetInterfaceBoolProperty(iface_id, "NICEnabled",
        ...
        [this, asyncResp, parent_iface_id{std::string(params[0])},
         iface_id{std::string(params[1])}](
          ...
          if (success && ethData.vlan_id.size() != 0) {
            parseInterfaceData(asyncResp->res.jsonValue, parent_iface_id,
                               iface_id, ethData, ipv4Data, ipv6Data);
          } else {
            // ... otherwise return error
            // TODO(Pawel)consider distinguish between non existing
            // object, and other errors
...
private:
  ...
  void doPost(crow::Response &res, const crow::Request &req,
    ...
    crow::connections::systemBus->async_method_call(
        std::move(callback), "xyz.openbmc_project.Network",
        "/xyz/openbmc_project/network",
        "xyz.openbmc_project.Network.VLAN.Create", "VLAN", rootInterfaceName,
        vlanId);
  }
};
} // namespace redfish
