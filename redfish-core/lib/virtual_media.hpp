                      ...
                      const std::string &resName)
{
    ...
    crow::connections::systemBus->async_method_call(
		...
		aResp->res.jsonValue["Actions"]["#VirtualMedia.EjectMedia"]["target"] =
		    "/redfish/v1/Managers/" + name + "/VirtualMedia/" + resName +
...
class VirtualMediaActionInsertMedia : public Node
{
  ...
  private:
                ...
                const std::vector<std::string> &params) override
    {
        ...
        crow::connections::systemBus->async_method_call(
		    ...
		    lastIndex += 1;
			    ...
			    doVmAction(std::move(aResp), resName, true,
                                       imageUrl);
                        }
                    }
                }
     ...
     */
    void doVmAction(std::shared_ptr<AsyncResp> asyncResp, const std::string &name, bool legacy,
        ...
        crow::connections::systemBus->async_method_call(
		    ...
		    lastIndex += 1;
                    ...
                    if (path.substr(lastIndex) == resName)
                    {
                        ...
                        {
			    // Proxy mode
                            doVmAction(std::move(aResp), resName, false);
                        }
