...
class VirtualMediaActionInsertMedia : public Node
{
  ...
  private:
        ...
        const std::optional<std::string> &transferProtocolType)
    {
        ...
        static const boost::container::flat_map<std::string, TransferProtocol>
            protocolMap{{"CIFS", TransferProtocol::smb},
                        {"HTTPS", TransferProtocol::https}};

        auto protocol = protocolMap.find(*transferProtocolType);

        if (protocol == std::end(protocolMap))
        {
            return TransferProtocol::invalid;
        }
    ...
    {
        static const boost::container::flat_map<TransferProtocol, std::string>
            prefixMap{{TransferProtocol::smb, "smb://"},
                      {TransferProtocol::https, "https://"}};

        auto prefix = prefixMap.find(transferProtocol);

        return prefix->second + imageUri;
    }
