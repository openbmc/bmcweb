...
constexpr std::array<std::string_view, 2> processorMetricsInterfaces = {
 ...
 */
inline void
    getProcessorMetricsECCData(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    const std::string& processorId, const std::string& systemName)
{
    ...
    dbus::utility::getSubTree(
        ...
        {
            std::string objectName = sdbusplus::message::object_path(objectPath)
                                         .filename();
                ...
                for (const auto& [serviceName, interfaceList] : serviceMap)
                {
                    for (const auto& interface : interfaceList)
                    {
                        if (interface == "xyz.openbmc_project.Memory.MemoryECC")
                        {
                            ...
                            getProcessorMetricsECCData(asyncResp, serviceName,
                                                       objectPath);
                            return;
                        }
                    }
 ...
 */
inline void
    doProcessorMetrics(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
...
{
    BMCWEB_ROUTE(
        app, "/redfish/v1/Systems/<str>/Processors/<str>/ProcessorMetrics/")
