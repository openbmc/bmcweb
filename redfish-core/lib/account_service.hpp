    ...
    const std::shared_ptr<persistent_data::UserSession>& session)
{
    ...
    dbus::utility::checkDbusPathExists(dbusObjectPath, [dbusObjectPath,
        ...
        if (mfaBypass)
        {
            ...
            if (mfaBypassVal == "GoogleAuthenticator" || mfaBypassVal == "None")
            {
                mfaBypasDbusVal = mfaBypassDbusPrefix + mfaBypassVal;
            }
            else
            {
            ...
            sdbusplus::asio::setProperty(
                *crow::connections::systemBus,
                ...
                "BypassedProtocol", mfaBypasDbusVal,
                [asyncResp,
                 mfaBypasDbusVal](const boost::system::error_code& ec,
    ...
    sdbusplus::asio::getProperty<std::string>(
        *crow::connections::systemBus, "xyz.openbmc_project.User.Manager",
        ...
        [asyncResp](const boost::system::error_code& ec,
            ...
            {
                BMCWEB_LOG_ERROR(
                    "DBUS response error while fetching MultiFactorAuth property");
                messages::internalError(asyncResp->res);
                return;
            }
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    if (!json_util::readJsonPatch(
            ...
            "MultiFactorAuth/GoogleAuthenticator/Enabled", googleAuthenticatorEnabled,
    ...
    std::string mfaGoogleAuth;
    std::string multiFactorAuthDbusPrefix =
        "xyz.openbmc_project.User.MultiFactorAuthConfiguration.Type.";
    if (googleAuthenticatorEnabled && *googleAuthenticatorEnabled)
    {
        mfaGoogleAuth = multiFactorAuthDbusPrefix + "GoogleAuthenticator";
    }
                    ...
                    "xyz.openbmc_project.User.TOTPAuthenticator")
                {
                    for (const auto& property : interface.second)
                    {
                        if (property.first == "BypassedProtocol")
                        {
                            ...
                            constexpr std::string_view mfaGoogleAuthDbusVal =
                                "xyz.openbmc_project.User.MultiFactorAuthConfiguration.Type.GoogleAuthenticator";
                            mfaBypassArray.push_back(
                                (*bypassedProtocol == mfaGoogleAuthDbusVal)
                                    ? "GoogleAuthenticator"
                                    : "None");
                        }
                       ...
                       const std::string& username)
{
    ...
    if (userHasConfigureUsers)
    {
        ...
        if (!json_util::readJsonPatch(
                ...
                locked, "AccountTypes", accountTypes, "MFABypass/BypassTypes",
                mfaBypass))
        {
            return;
        }
...
inline void
    handleCreateSecretKey(App& app, const crow::Request& req,
                          const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                          const std::string& username)
{
    if (!redfish::setUpRedfishRoute(app, req, asyncResp))
    ...
    // Check if there is already a secret key generated for user and return
    sdbusplus::asio::getProperty<bool>(
        *crow::connections::systemBus, "xyz.openbmc_project.User.Manager",
        ...
        [asyncResp, username, userPath](const boost::system::error_code& ec,
            ...
            if (isSecretKeySet)
            {
                // Secret Key already setup
                BMCWEB_LOG_ERROR("Secret key is already setup for the user");
                messages::actionNotSupported(asyncResp->res,
                                             "GenerateSecretKey");
                return;
            }
            ...
            crow::connections::systemBus->async_method_call(
                [asyncResp,
                 username](const boost::system::error_code& ec1,
                    ...
                    FILE* secretKeyFile = fdopen(unixfd, "r");
                    if (secretKeyFile == nullptr)
                    {
                        BMCWEB_LOG_ERROR("Failed to open file from fd");
                        messages::internalError(asyncResp->res);
                        close(unixfd);
                        return;
                    }
                    ...
                    std::span<char> bufferSpan(buffer.data(), buffer.size());

                    while (fgets(bufferSpan.data(), static_cast<int>(bufferSpan.size()), secretKeyFile) != nullptr)
                    {
                        data += bufferSpan.data();
                    }
...
inline void requestAccountServiceRoutes(App& app)
{
    ...
    BMCWEB_ROUTE(
        ...
        .privileges({{"ConfigureUsers"}, {"ConfigureSelf"}})
        .methods(boost::beast::http::verb::post)(
            std::bind_front(handleCreateSecretKey, std::ref(app)));
}
