...
inline void afterGetAccountsForMethodNotAllowed(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& accountName, const boost::system::error_code& ec,
    const dbus::utility::ManagedObjectType& users)
{
    if (ec)
    {
    ...
    const std::string& accountName)
{
    ...
    dbus::utility::getManagedObjects(
        "xyz.openbmc_project.User.Manager", path,
        std::bind_front(afterGetAccountsForMethodNotAllowed, asyncResp,
                        accountName));
}
