    ...
    const UserUpdateParams& params, int rc)
{
    ...
    std::shared_ptr<persistent_data::UserSession> userSession =
        sessionStore.loginSessionByToken(params.session->sessionToken);

    if (params.mfaBypass && userSession && userSession->isConfigureSelfOnly)
    {
        BMCWEB_LOG_ERROR("Bypassing MFA for self is not allowed");
        boost::urls::url accountUri = boost::urls::format("/redfish/v1/AccountService/Accounts/{}", params.username);
...
inline void getMultiFactorAuthConfigUtil(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string_view& multiFactorAuthEnabled)
{
    constexpr std::string_view mfaGoogleAuthDbusVal =
        "xyz.openbmc_project.User.MultiFactorAuthConfiguration.Type.GoogleAuthenticator";
    bool googleAuthEnabled =
        multiFactorAuthEnabled == mfaGoogleAuthDbusVal;
    ...
    sdbusplus::asio::getProperty<std::string>(
        *crow::connections::systemBus, "xyz.openbmc_project.User.Manager",
        ...
        [asyncResp](const boost::system::error_code& ec,
            ...
            if (ec)
            {
                if (ec.message() == "org.freedesktop.DBus.Error.UnknownInterface")
                {
                    BMCWEB_LOG_WARNING("MFA is not supported.");
                    return;
                }
            ...
            getMultiFactorAuthConfigUtil(asyncResp, multiFactorAuthEnabledVal);
        });
}
