    ...
    std::vector<std::variant<nlohmann::json::object_t, std::nullptr_t>>& input)
{
    ...
    std::vector<ResolvedEntry> creates;
    std::vector<ResolvedEntry> privOnlyWrites;

    for (auto&& [i, thisJson] : std::views::enumerate(input))
    {
        ...
        if (obj == nullptr)
        {
            if (index >= roleMapObjData.size())
            {
                ...
                messages::propertyValueTypeError(
                    asyncResp->res, "null",
                    std::format("RemoteRoleMapping/{}", index));
                return;
            }
            // deleted by user intention
            deleteIndices.push_back(index);
            continue;
        }

        // {} placeholder - leave whatever already exists at this slot.
        if (obj->empty())
        {
            continue;
        }
        ...
        std::string pathString = std::format("RemoteRoleMapping/{}", index);

        if (index < roleMapObjData.size())
        {
            // Carry forward fields the caller omitted from the existing
            // entry, so the (possibly recreated) entry preserves them.
            const auto& [existingPath, existing] = roleMapObjData[index];
            if (!remoteGroup)
            {
                remoteGroup = existing.groupName;
            }
            if (!localRole)
            {
                localRole = getRoleIdFromPrivilege(existing.privilege);
            }
        }
        else
        {
            // Pure create - both fields are mandatory.
            if (!localRole)
            {
        ...
        {
            const auto& [existingPath, existing] = roleMapObjData[index];
            if (existing.groupName == *remoteGroup)
            {
                if (existing.privilege == priv)
                {
                    continue;
                }
                privOnlyWrites.push_back({
                    .index = index,
                    .remoteGroup = std::move(*remoteGroup),
                    .localRole = std::move(*localRole),
                    .privilege = std::move(priv),
                });
                continue;
            }
            // Delete and recreate it to prevent group name collisions
            deleteIndices.push_back(index);
        }
        creates.push_back({
            .index = index,
    ...
    for (const ResolvedEntry& op : privOnlyWrites)
    {
        BMCWEB_LOG_DEBUG("In-place privilege update at index {} (role={})",
                         op.index, op.localRole);
        setDbusProperty(asyncResp,
                        std::format("RemoteRoleMapping/{}/LocalRole", op.index),
                        ldapDbusService, roleMapObjData[op.index].first,
    ...
    auto applyCreates =
        ...
        [asyncResp, serverType, creates = std::move(creates)]() {
            ...
            for (const ResolvedEntry& op : creates)
            {
                dbus::utility::async_method_call(
                    ...
                    "Create", op.remoteGroup, op.privilege);
            }
        };
    ...
    auto remaining = std::make_shared<size_t>(deleteIndices.size());
    auto deleteFailed = std::make_shared<bool>(false);
    auto deferred =
        std::make_shared<std::function<void()>>(std::move(applyCreates));
    ...
    for (size_t index : deleteIndices)
    {
        dbus::utility::async_method_call(
            asyncResp,
            // ast-grep-ignore: long-lambda
            [asyncResp, serverType, remaining, deleteFailed,
             deferred](const boost::system::error_code& ec) {
                if (ec)
                {
                    BMCWEB_LOG_ERROR("DBUS response error: {}", ec);
                    messages::internalError(asyncResp->res);
         ...
         ldapType](const boost::system::error_code& ec,
            ...
            dbus::utility::getManagedObjects(
                ...
                [callback, ldapType](const boost::system::error_code& ec2,
                    ...
                    std::ranges::sort(
                        confData.groupRoleList, AlphanumLess<std::string>{},
                        &std::pair<std::string, LDAPRoleMapData>::first);
    ...
    const std::variant<std::string, std::nullptr_t>& value)
{
    ...
    const std::optional<std::chrono::duration<int64_t, std::micro>> usTime =
        time_utils::dateStringToEpoch(*str);
    if (!usTime)
    {
        BMCWEB_LOG_WARNING("Failed to parse date time string {}", *str);
        return std::nullopt;
    }
    ...
    {
        BMCWEB_LOG_WARNING(
            "Setting incorrect epoch datetime 1970-01-01T00:00:00");
        return std::nullopt;
    }
    ...
    const UserUpdateParams& params, int rc)
{
    ...
    if (params.passwordExpiration)
    {
        ...
        setDbusProperty(
            asyncResp, "PasswordExpiration", "xyz.openbmc_project.User.Manager",
            params.dbusObjectPath, "xyz.openbmc_project.User.Attributes",
    ...
    const std::vector<std::string>& allGroupsList)
{
    ...
    dbus::utility::DBusPropertiesMap userProps;

    // If user specified account types then convert them to unix user groups
    if (userCreateProps.accountTypes)
    {
        if (!getUserGroupFromAccountType(asyncResp->res,
    ...
    for (const auto& grp : allGroupsList)
    {
        ...
        if ((grp == "hostconsole") && (userCreateProps.roleId != "priv-admin"))
        {
            if (!accountTypeUserGroups.empty())
            {
                BMCWEB_LOG_ERROR(
    ...
    {
        // value for CreateUser2 indicating that password expiration is not
        // specified
        userProps.emplace_back(
            "xyz.openbmc_project.User.Manager.UserProperty.PasswordExpiration",
    ...
    userProps.emplace_back(
        "xyz.openbmc_project.User.Manager.UserProperty.Privilege",
        userCreateProps.roleId);
    userProps.emplace_back(
        "xyz.openbmc_project.User.Manager.UserProperty.GroupNames",
        std::move(userGroups));
    userProps.emplace_back(
        "xyz.openbmc_project.User.Manager.UserProperty.Enabled",
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    UserCreateProperties userCreateProps;

    if (!json_util::readJsonPatch(                                    //
            req, asyncResp->res,                                      //
            "AccountTypes", userCreateProps.accountTypes,             //
            "Enabled", enabledJson,                                   //
            "Password", password,                                     //
    ...
    userCreateProps.roleId = priv;

    userCreateProps.enabled = enabledJson.value_or(true);
    ...
    dbus::utility::getProperty<std::vector<std::string>>(
        ...
        [asyncResp, username, password{std::move(password)},
         userCreateProps](const boost::system::error_code& ec,
                          const std::vector<std::string>& allGroupsList) {
            if (ec)
            {
         ...
         accountName](const boost::system::error_code& ec,
            ...
            for (const auto& interface : userIt->second)
            {
                if (interface.first == "xyz.openbmc_project.User.Attributes")
                {
                    ...
                    // default value indicating that password expiration is not
                    // specified
                    if (passwordExpiration &&
                        (*passwordExpiration !=
                         std::numeric_limits<uint64_t>::max()))
