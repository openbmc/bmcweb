...
inline void
    handleAccountServiceGet(const crow::Request& req,
                            const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, thisUser,
         effectiveUserPrivileges](const boost::system::error_code ec,
...
inline void requestAccountServiceRoutes(App& app)
{
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/AccountService/")
        .privileges({{"Login"}})
        .methods(boost::beast::http::verb::get)(handleAccountServiceGet);
