    ...
    const std::shared_ptr<persistent_data::UserSession>& session)
{
    ...
    dbus::utility::checkDbusPathExists(
        dbusObjectPath, [dbusObjectPath, username, password, roleId, enabled,
                         ...
                         session, mfaBypass, asyncResp](auto&& rc) {
            processAccountUpdate(std::forward<decltype(rc)>(rc), dbusObjectPath,
                                 username, password, roleId, enabled, locked,
                                 capture0, userSelf, session, mfaBypass,
                                 asyncResp);
        });
}
