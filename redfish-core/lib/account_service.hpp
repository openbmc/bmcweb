...
constexpr const std::string_view passwordExpirationCreate =
    "xyz.openbmc_project.User.Manager.UserProperty.PasswordExpiration";
constexpr const std::string_view userPrivilegeCreate =
    "xyz.openbmc_project.User.Manager.UserProperty.Privilege";
constexpr const std::string_view groupNamesCreate =
    "xyz.openbmc_project.User.Manager.UserProperty.GroupNames";
constexpr const std::string_view enabledCreate =
    ...
    const std::variant<std::string, std::nullptr_t>& value)
{
    ...
    return std::chrono::duration_cast<std::chrono::seconds>(*usTime).count();
}
...
inline void updateUserProperties(
    ...
    const std::optional<std::variant<std::string, std::nullptr_t>>&
        passwordExpiration,
    bool userSelf, const std::shared_ptr<persistent_data::UserSession>& session)
{
    ...
    UserUpdateParams params{
        ...
        accountTypes, passwordExpiration, userSelf, session, dbusObjectPath};
...
using UserCreateMap = std::map<std::string_view, UserProperty>;
    ...
    const std::vector<std::string>& allGroupsList)
{
    ...
    if (passwordExpiration)
    {
        ...
        userProps[passwordExpirationCreate] = *passwordExpirationTime;
    }
    else
    {
        // value for CreateUser2 indicating that password expiration is not
        // specified
        userProps[passwordExpirationCreate] =
            std::numeric_limits<uint64_t>::max();
        ;
    }

    userProps[userPrivilegeCreate] = roleId;
    userProps[groupNamesCreate] = std::move(userGroups);
    userProps[enabledCreate] = enabled;
         ...
         accountName](const boost::system::error_code& ec,
            ...
            for (const auto& interface : userIt->second)
            {
                if (interface.first == "xyz.openbmc_project.User.Attributes")
                {
                    ...
                    {
                        BMCWEB_LOG_ERROR("PasswordExpiration wasn't a number");
                        messages::internalError(asyncResp->res);
                        return;
                    }
