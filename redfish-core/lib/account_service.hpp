...
inline void processAccountUpdate(
    int rc, const std::string& dbusObjectPath, const std::string& username,
    const std::optional<std::string>& password,
    const std::optional<std::string>& roleId,
    const std::optional<bool>& enabled, const std::optional<bool>& locked,
    std::optional<std::vector<std::string>>
        accountTypes, // std::vector<std::string> accountTypes,
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    if (accountTypes)
    {
        patchAccountTypes(*accountTypes, asyncResp, dbusObjectPath, userSelf);
    }
}
    ...
    const std::shared_ptr<persistent_data::UserSession>& session)
{
    ...
    dbus::utility::checkDbusPathExists(
        dbusObjectPath, [dbusObjectPath, username, password, roleId, enabled,
                         locked, capture0 = std::move(accountTypes), userSelf,
                         session, mfaBypass, asyncResp](auto&& PH1) {
            processAccountUpdate(std::forward<decltype(PH1)>(PH1),
                                 dbusObjectPath, username, password, roleId,
                                 enabled, locked, capture0, userSelf, session,
                                 mfaBypass, asyncResp);
        });
}
