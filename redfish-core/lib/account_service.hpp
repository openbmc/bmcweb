                ...
                if (remoteGroup)
                {
                    crow::connections::systemBus->async_method_call(
                        ...
                        if (ec)
                        {
                            ...
                            {
                                messages::propertyValueFormatError(asyncResp->res, "RemoteGroup",*remoteGroup);
                                return;
                            }
                            messages::internalError(asyncResp->res);
                            return;
                    ...
                    crow::connections::systemBus->async_method_call(
                        ...
                        if (ec)
                        {
                            ...
                            if ((strcmp(dbusError->name,
                               "xyz.openbmc_project.Common.Error.InvalidArgument") ==0))
                            {
                                 messages::propertyValueFormatError(asyncResp->res, "LocalRole",*localRole);
                                 return;
                            }
                            messages::internalError(asyncResp->res);
                            return;
    ...
    crow::connections::systemBus->async_method_call(
        ...
        if (ec)
        {
            ...
            if ((strcmp(dbusError->name,
                 "xyz.openbmc_project.Common.Error.InvalidArgument") ==0))
            {
                 messages::propertyValueFormatError(asyncResp->res, "ServiceAddresses",serviceAddressList.front());
                 return;
            }
            messages::internalError(asyncResp->res);
            return;
    ...
    crow::connections::systemBus->async_method_call(
        ...
        if (ec)
        {
            ...
            if ((strcmp(dbusError->name,
                "xyz.openbmc_project.Common.Error.InvalidArgument") ==0))
            {
                messages::propertyValueFormatError(asyncResp->res, "BaseDistinguishedNames",baseDNList.front());
                return;
            }
            messages::internalError(asyncResp->res);
            return;
