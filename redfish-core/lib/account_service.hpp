    ...
    const std::shared_ptr<persistent_data::UserSession>& session)
{
    ...
    dbus::utility::checkDbusPathExists(
        dbusObjectPath, std::bind(processAccountUpdate, std::placeholders::_1,
                                  dbusObjectPath, username, password, roleId,
                                  enabled, locked, std::move(accountTypes),
                                  userSelf, session, mfaBypass, asyncResp));
}
