                           ...
                           const AuthMethods& auth)
{
    ...
    if (auth.basicAuth)
    {
        if constexpr (!bmcweb::BASIC_AUTH)
        {
    ...
    if (auth.cookie)
    {
        if constexpr (!bmcweb::COOKIE_AUTH)
        {
            messages::actionNotSupported(
                asyncResp->res,
    ...
    {
        if constexpr (!bmcweb::SESSION_AUTH)
        {
            messages::actionNotSupported(
                asyncResp->res,
    ...
    if (auth.xToken)
    {
        if constexpr (!bmcweb::XTOKEN_AUTH)
        {
            messages::actionNotSupported(
                asyncResp->res,
                     ...
                     const std::string& accountName)
{
    ...
    if constexpr (bmcweb::INSECURE_DISABLE_AUTH)
    {
