...
void errorMessage(const boost::system::error_code ec, const char* errorMessage,
                  ...
                  std::optional<std::string> username)
{
    ...
    {
        messages::actionParameterUnknown(asyncResp->res, "UserName", newUser);
    }
    else if (strcmp(errorMessage,
                    "xyz.openbmc_project.User.Common.Error.NoResource") == 0)
    {
        messages::createLimitReachedForResource(asyncResp->res);
                                  ...
                                  "UserNameGroupFail") == 0)
    {
        messages::stringValueTooLong(asyncResp->res, "UserName", 16);
    }
    else
    {
...
class ManagerAccount : public Node
{
  ...
  private:
                 ...
                 const std::vector<std::string>& params) override
    {
        ...
        else if (newUserName.value() == username)
        {
            updateUserProperties(asyncResp, username, password, enabled, roleId,
                                 locked);
            return;
                ...
                [this, asyncResp, username, password(std::move(password)),
                 ...
                 locked(std::move(locked))](const boost::system::error_code ec,
                    ...
                    if (ec)
                    {
                        ...
                        errorMessage(ec, e->name, asyncResp, newUser, username);
                ...
                crow::connections::systemBus->async_method_call(
                    [asyncResp, username](const boost::system::error_code ec,
                        ...
                        if (ec)
                        {
                            const sd_bus_error* e = m.get_error();
                            if (strcmp(e->name,
                                       "xyz.openbmc_project.User.Common.Error."
                                       "UserNameDoesNotExist") == 0)
                            {
