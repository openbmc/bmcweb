...
class AccountService : public Node
{
  ...
  private:
                 ...
                 const std::vector<std::string>& params) override
    {
        ...
        std::optional<uint8_t> minPasswdLen;
        std::optional<uint32_t> unlockTimeout;
        std::optional<uint16_t> lockoutThreshould;
        if (!json_util::readJson(req, res, "MinPasswordLength", minPasswdLen,
                                 "AccountLockoutDuration", unlockTimeout,
                                 "AccountLockoutThreshold", lockoutThreshould))
        {
            return;
        }
        if (minPasswdLen)
        {
            crow::connections::systemBus->async_method_call(
                ...
                sdbusplus::message::variant<uint8_t>(*minPasswdLen));
        }
        if (unlockTimeout)
        {
            crow::connections::systemBus->async_method_call(
        ...
        if (lockoutThreshould)
        {
            crow::connections::systemBus->async_method_call(
                ...
                sdbusplus::message::variant<uint16_t>(*lockoutThreshould));
        }
    }
};
