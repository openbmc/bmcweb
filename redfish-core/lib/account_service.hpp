...
inline void fillLocalAccountAuth(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& loginPriority)
{
    if (loginPriority ==
    ...
    {
        BMCWEB_LOG_ERROR("Unknown LoginPriority value: {}", loginPriority);
        messages::internalError(asyncResp->res);
    }
}
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    dbus::utility::getAllProperties(
        ...
        [asyncResp](const boost::system::error_code& ec,
            ...
            {
                fillLocalAccountAuth(asyncResp, *loginPriority);
            }
        });
...
inline void handleLocalAccountAuthPatch(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& localAccountAuth)
{
    std::string propVal;
    ...
    setDbusProperty(
        asyncResp, "LocalAccountAuth", "xyz.openbmc_project.User.Manager",
        sdbusplus::object_path("/xyz/openbmc_project/user"),
        "xyz.openbmc_project.User.AccountPolicy", "LoginPriority", propVal);
}
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    if (localAccountAuth)
    {
        handleLocalAccountAuthPatch(asyncResp, *localAccountAuth);
    }
}
