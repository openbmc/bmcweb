    ...
    const std::string_view& multiFactorAuthEnabled)
{
    constexpr std::string_view mfaGoogleAuthDbusVal =
        "xyz.openbmc_project.User.MultiFactorAuthConfiguration.Type.GoogleAuthenticator";
    bool googleAuthEnabled = multiFactorAuthEnabled == mfaGoogleAuthDbusVal;
    asyncResp->res
...
void checkAndAddGenerateSecretKeyAction(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& accountName)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, accountName](const boost::system::error_code& ec,
                                 const MapperGetObject&) {
            if (ec)
            {
                BMCWEB_LOG_DEBUG(
...
void checkAndAddGenerateSecretKeyActionCallback(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& accountName)
{
