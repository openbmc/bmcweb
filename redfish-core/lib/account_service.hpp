        ...
        if (password)
        {
            ...
            else if (retval == PAM_AUTHTOK_ERR)
            {
                ...
                messages::propertyValueFormatError(asyncResp->res, nullptr,
                                                   "Password");
                BMCWEB_LOG_WARNING("pamUpdatePassword Failed");
            }
        ...
        [asyncResp, username, userPath](const boost::system::error_code& ec,
            ...
            crow::connections::systemBus->async_method_call(
                [asyncResp, username](const boost::system::error_code& ec1) {
                    ...
                    std::string userAuthHomePath =
                        std::format("/home/{}/.google_authenticator", username);
                    if (!std::filesystem::exists(userAuthHomePath))
                    {
                        messages::internalError(asyncResp->res);
                        return;
                    }
                    std::string secretKey;
                    std::ifstream file(userAuthHomePath);
                    if (file.is_open())
                    {
                    ...
                    asyncResp->res.jsonValue["Name"] = "User Account";
                    asyncResp->res.jsonValue["Description"] = "User Account";
                    // asyncResp->res.jsonValue["MultiFactorAuth"]["GoogleAuthenticator"]["SecretKey"]
                    // = secretKey;
