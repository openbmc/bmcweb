    ...
    const std::string& aggregationSourceId)
{
    ...
    //    cb = [aggregationSourceId, asyncResp](
    //             const std::unordered_map<std::string, boost::urls::url>&
    //                 handler) {
    //        populateAggregationSource(aggregationSourceId, asyncResp, handler);
    //    };
    //RedfishAggregator::getSatelliteConfigs(cb);

    auto cb = std::bind_front(populateAggregationSource, aggregationSourceId,
                              asyncResp);
    RedfishAggregator::getSatelliteConfigs(std::move(cb));
    ...
    // std::function<void(
    //     const std::unordered_map<std::string, boost::urls::url>&)> cb;

    //    [cb = std::move(cb)]() { RedfishAggregaor::getSatelliteConfigs(cb); };
    // RedfishAggregator::getSatelliteConfigs(cb);
}
