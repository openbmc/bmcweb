                                    ...
                                    crow::Response& res)
{
    ...
    if (field->length() > 40)
    {
        messages::propertyValueFormatError(res, *field, fieldName);
        return false;
    }
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    for (const auto& [existingPrefix, existingSource] :
         aggregator.currentAggregationSources)
    {
        if (existingSource.url == *url)
        {
            messages::resourceAlreadyExists(
                asyncResp->res, "AggregationSource", "HostName",
                url->buffer());
            return;
        }
    }
    ...
    aggregator.currentAggregationSources.emplace(
        prefix,
    ...
    const std::string& aggregationSourceId)
{
    ...
    // Check if the aggregation source exists
    auto& aggregator = RedfishAggregator::getInstance();
    auto it = aggregator.currentAggregationSources.find(aggregationSourceId);
    if (it == aggregator.currentAggregationSources.end())
    {
