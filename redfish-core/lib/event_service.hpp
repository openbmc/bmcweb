...
inline void
    checkSnmpTrapClient(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                        ...
                        const std::string& destUrl, Handler&& handler)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, host, port, destUrl,
                         ...
                         const std::shared_ptr<Subscription>& subValue)
{
    ...
    auto callback = [asyncResp, host, snmpTrapPort, subValue]() {
        crow::connections::systemBus->async_method_call(
    ...
    checkSnmpTrapClient(asyncResp, host, snmpTrapPort, destUrl,
                        std::move(callback));
}
