...
inline void addSubscriptionDBusSNMPClient(std::shared_ptr<bmcweb::AsyncResp> asyncResp, std::string host, std::uint16_t port)
{


    auto respHandler =
        [asyncResp](const boost::system::error_code ec) mutable {
            BMCWEB_LOG_DEBUG << "addSubscriptionDBusSNMPClient respHandler enter";
            if (ec)
            {
                BMCWEB_LOG_ERROR << "addSubscriptionDBusSNMPClient respHandler DBUS error: " << ec;
    ...
    crow::connections::systemBus->async_method_call(
        ...
        "xyz.openbmc_project.Network.Client.Create", "Client",
        host, port
    );
}

using SNMPTrapVariant =
    std::variant<std::string, uint16_t>;
using SNMPTrapVariantMap = boost::container::flat_map<std::string, SNMPTrapVariant>;
using ManagedObjectsSNMPVectorType = std::vector<std::pair<
    sdbusplus::message::object_path,
    boost::container::flat_map<std::string, SNMPTrapVariantMap>>>;

inline void getSNMPTrapData(std::shared_ptr<bmcweb::AsyncResp> asyncResp, std::string id)
{

    BMCWEB_LOG_DEBUG << "getSNMPTrapData enter";

    crow::connections::systemBus->async_method_call(
        [asyncResp, id](const boost::system::error_code ec,
            ManagedObjectsSNMPVectorType& subtree) {
            BMCWEB_LOG_DEBUG << "getSNMPTrapData RespHandler enter";
            if (ec)
            {
            ...
            {
                BMCWEB_LOG_DEBUG << "getSNMPTrapData RespHandler subtree enter";
                nlohmann::json item;
                ...
                if (dbusNum == idNum)
                {
                    ...
                    auto& clientInterface = object.second.at("xyz.openbmc_project.Network.Client");

                    asyncResp->res.jsonValue["SubscriptionType"] = "SNMPTrap";

                    asyncResp->res.jsonValue["Context"] = "";

                    asyncResp->res.jsonValue["Destination"] = "snmp://" +
                                std::get<std::string>(clientInterface.at("Address")) + ":" +
                                std::to_string(std::get<uint16_t>(clientInterface.at("Port")));
                }
            }
            BMCWEB_LOG_DEBUG << "getSNMPTrapData RespHandler exit";
        },
        "xyz.openbmc_project.Network.SNMP", "/xyz/openbmc_project/network/snmp/manager",
        "org.freedesktop.DBus.ObjectManager", "GetManagedObjects");

    BMCWEB_LOG_DEBUG << "getSNMPTrapData exit";
}

inline void getSNMPTraps(std::shared_ptr<bmcweb::AsyncResp> asyncResp)
{

    BMCWEB_LOG_DEBUG << "getSNMPTraps enter";

    crow::connections::systemBus->async_method_call(
        [asyncResp](const boost::system::error_code ec,
            ManagedObjectsSNMPVectorType& subtree) {
            BMCWEB_LOG_DEBUG << "getSNMPTraps RespHandler enter";
            if (ec)
            {
                BMCWEB_LOG_DEBUG << "DBUS response error" << ec;
                messages::internalError(asyncResp->res);
            ...
            for (const auto& object : subtree)
            {
                BMCWEB_LOG_DEBUG << "getSNMPTraps RespHandler subtree enter";
                nlohmann::json item;
                const std::string& path =
                    static_cast<const std::string&>(object.first);
                ...
                {{"@odata.id",
                  "/redfish/v1/EventService/Subscriptions/SNMP-" + dbusNum}});
            }
            asyncResp->res.jsonValue["Members@odata.count"] = asyncResp->res.jsonValue["Members"].size();
            BMCWEB_LOG_DEBUG << "getSNMPTraps RespHandler exit";
        },
        "xyz.openbmc_project.Network.SNMP", "/xyz/openbmc_project/network/snmp/manager",
        "org.freedesktop.DBus.ObjectManager", "GetManagedObjects");
...
inline void deleteSNMPTrap(std::shared_ptr<bmcweb::AsyncResp> asyncResp, std::string id)
{

    auto respHandler =
        [asyncResp](const boost::system::error_code ec) mutable {
            BMCWEB_LOG_DEBUG << "deleteSNMPTrap respHandler enter";
            if (ec)
            {
    ...
    crow::connections::systemBus->async_method_call(
        respHandler,
        "xyz.openbmc_project.Network.SNMP", "/xyz/openbmc_project/network/snmp/manager/" + dbusId,
...
inline void requestRoutesEventDestinationCollection(App& app)
{
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/EventService/Subscriptions/")
        ...
        .methods(
            ...
            if (((protocol == "Redfish") && !((urlProto == "http") || (urlProto == "https"))) ||
