  ...
  private:
               ...
               const std::vector<std::string>& params) override
    {
        ...
        asyncResp->res.jsonValue["ServiceEnabled"] =
            EventSrvManager::getInstance().enabled;
        asyncResp->res.jsonValue["DeliveryRetryAttempts"] =
            EventSrvManager::getInstance().retryAttempts;
        asyncResp->res.jsonValue["DeliveryRetryIntervalSeconds"] =
            EventSrvManager::getInstance().retryTimeoutInterval;
                 ...
                 const std::vector<std::string>& params) override
    {
        ...
        if (serviceEnabled)
        {
            EventSrvManager::getInstance().enabled = *serviceEnabled;
        }
        ...
        if (retryAttemps)
        {
            ...
            {
                EventSrvManager::getInstance().retryAttempts = *retryAttemps;
            }
        }
  ...
  private:
               ...
               const std::vector<std::string>& params) override
    {
        ...
        std::vector<std::string> subscripIds =
            EventSrvManager::getInstance().getAllIDs();
                ...
                const std::vector<std::string>& params) override
    {
        ...
        if (EventSrvManager::getInstance().getNumberOfSubscriptions() >=
        ...
        // Regex expression for validating http URL's.
        // Accepted formats: http://10.0.0.1/events
        // https://10.0.0.1/events/test?params=xyz
        ...
        std::string id =
            EventSrvManager::getInstance().addSubscription(subValue);
...
class EventDestination : public Node
{
  ...
  private:
               ...
               const std::vector<std::string>& params) override
    {
        ...
        std::shared_ptr<Subscription> subValue =
            EventSrvManager::getInstance().getSubscription(params[0]);
        if (subValue == nullptr)
        {
                  ...
                  const std::vector<std::string>& params) override
    {
        ...
        if (!EventSrvManager::getInstance().isSubscriptionExist(params[0]))
        {
            res.result(boost::beast::http::status::not_found);
            res.end();
            return;
        }
        EventSrvManager::getInstance().deleteSubscription(params[0]);
    }
};
