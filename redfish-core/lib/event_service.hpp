...
static constexpr const uint16_t maxPortNumber = 65535;
...
class EventDestinationCollection : public Node
{
  ...
  private:
                ...
                const std::vector<std::string>&) override
    {
        ...
        if (protocol == "Redfish" && uriProto == "http")
        {
        ...
        if (protocol == "SNMPv2c")
        {
            ...
            // Check the port
            long int tmpPort = strtol(port.c_str(), nullptr, 10);
            if (tmpPort <= maxPortNumber)
            {
                snmpTrapPort = static_cast<uint16_t>(tmpPort);
            }
            else
            {
                messages::propertyValueNotInList(asyncResp->res, port,
                                                 "Destination");
                return;
            }

            crow::connections::systemBus->async_method_call(
                [asyncResp, host, snmpTrapPort, subValue{std::move(subValue)}](
                    ...
                    // Storage DBus SNMP client ID, example:
                    // xyz/openbmc_project/network/SNMP/manager/1. When deleting
                    // the SNMP client, we need to find the DBus SNMP client ID
                    // according to the event service subscription ID
                    subValue->entityID = dbusSNMPid;
                  ...
                  const std::vector<std::string>& params) override
    {
        ...
        if (subValue->protocol == "SNMPv2c")
        {
            crow::connections::systemBus->async_method_call(
                ...
                "xyz.openbmc_project.Network.SNMP", subValue->entityID,
