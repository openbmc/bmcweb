        ...
        for (const auto& [objectPath, interfaces] : objectsForPartition)
        {
            for (const auto& [interface, properties] : interfaces)
            {
                    ...
                    "xyz.openbmc_project.Inventory.Item.PersistentMemory.Partition")
                {
                    ...
                    std::string const& dimm =
                        objectPath.parent_path().filename();
                    if (dimmToPtr.find(dimm) == dimmToPtr.end())
                    {
                        continue;
                    }
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    std::unordered_set<std::string> dimmServicesSet(dimmServices.begin(),
                                                    dimmServices.end());
    BMCWEB_LOG_DEBUG << "There are " << dimmServicesSet.size()
                     << " services which implement the DIMM interface";

    std::unordered_set<std::string> paritionServicesSet(
