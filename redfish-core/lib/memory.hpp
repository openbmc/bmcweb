 ...
 */
inline void handleCapacityUtilization(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const nlohmann::json::json_pointer& jPtr,
    const boost::system::error_code& ec, double utilization)
{
    ...
    asyncResp->res.jsonValue[jPtr] = utilization;
}
    ...
    const nlohmann::json::json_pointer& jPtr)
{
    ...
    dbus::utility::getProperty<double>(
        ...
        std::bind_front(handleCapacityUtilization, asyncResp, jPtr));
}
    ...
    const dbus::utility::MapperEndPoints& endpoints)
{
    for (const std::string& path : endpoints)
    {
        ...
        {
            handleMetricService(asyncResp, path, handleTotalMemory);
        }
        */
    }
}
