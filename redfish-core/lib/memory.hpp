    ...
    crow::connections::systemBus->async_method_call(
        [resp, dimmId, handler = std::forward<Handler>(handler)](
            ...
            for (const auto& [objectPath, serviceMap] : subtree)
            {
                ...
                {
                    for (auto& dimm : dimmInterfaces)
                    {
                        auto it = std::find_if(interfaceList.begin(),
                                               interfaceList.end(),
                                               [dimm](auto const& interface) {
                                                   return interface == dimm;
                                               });
...
inline void setDimmObject(const std::shared_ptr<bmcweb::AsyncResp>& resp,
                          const std::string& dimmId, bool active,
                          Handler&& handler)
{
    // GetSubTree on all interfaces which provide info about a Memory
    crow::connections::systemBus->async_method_call(
        [resp, dimmId, active, handler = std::forward<Handler>(handler)](
            boost::system::error_code ec,
            const MapperGetSubTreeResponse& subtree) mutable {
            if (ec)
            {
...
class Memory : public Node
{
  ...
  private:
                 ...
                 const std::vector<std::string>& params) override
    {
        ...
        const std::string& dimmId = params[0];
        setDimmObject(asyncResp, dimmId, *locationIndicatorActive, setDimmData);
    }
};
