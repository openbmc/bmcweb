...
inline void assembleDimmPartitionData(
    const std::shared_ptr<bmcweb::AsyncResp>& aResp,
    const dbus::utility::DBusPropertiesMap& properties,
    const nlohmann::json::json_pointer& partitionPtr)
{
    for (const auto& [key, val] : properties)
    {
        if (key == "MemoryClassification")
        {
            ...
            aResp->res.jsonValue[partitionPtr][key] = *value;
        }
        else if (key == "OffsetInKiB")
        {
            ...
            aResp->res.jsonValue[partitionPtr]["OffsetMiB"] = (*value >> 10);
        }
        else if (key == "PartitionId")
        {
            ...
            aResp->res.jsonValue[partitionPtr]["RegionId"] = *value;
        }
        ...
        else if (key == "PassphraseState")
        {
            ...
            aResp->res.jsonValue[partitionPtr]["PassphraseEnabled"] = *value;
        }
        else if (key == "SizeInKiB")
        {
            ...
            aResp->res.jsonValue[partitionPtr]["SizeMiB"] = (*value >> 10);
        }
    }
}
                                 ...
                                 const std::string& path)
{
    crow::connections::systemBus->async_method_call(
        ...
        assembleDimmPartitionData(
            aResp, properties,
            "/Regions"_json_pointer /
                (aResp->res.jsonValue["Regions"].size() - 1));
        },
