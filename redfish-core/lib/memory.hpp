                ...
                if (path.find(dimmId) != std::string::npos)
                {
                    for (const auto& [service, interfaces] : object)
                    {
                        for (const auto& interface : interfaces)
                        {
                            ...
                            {
                                // partitions are separate as there can be
                                // multiple per
                                // device, i.e.
...
class Memory : public Node
{
  ...
  private:
               ...
               const std::vector<std::string>& params) override
    {
        ...
        res.jsonValue["@odata.type"] = "#Memory.v1_10_0.Memory";
            ...
            crow::connections::systemBus->async_method_call(
                [asyncResp, dimmId, locationIndicatorActive](
                        ...
                        if (boost::ends_with(object.first, dimmId))
                        {
                            for (const auto& service : object.second)
                            {
                                for (const auto& inventory : service.second)
                                {
                                    ...
                                    {
                                        // Find all the paths whose `fType` is
                                        // equal to `identify_led_group` in the
                                        // `endpoints` array by this interface,
                                        // and then set the Asserted property
                                        // value by this path and the current
                                        // value.
                                        setLocationIndicatorActive(
                                            asyncResp, service.first,
                                            object.first,
                                            locationIndicatorActive);
                                        return;
                                    }
                    ...
                    messages::resourceNotFound(
                        asyncResp->res, "#Memory.v1_10_0.Memory", dimmId);
                },
                "xyz.openbmc_project.ObjectMapper",
                "/xyz/openbmc_project/object_mapper",
                "xyz.openbmc_project.ObjectMapper", "GetSubTree",
