...
// Fixes potential errors resulting from imprecise matching.
// dimmId are in the format of "dimm0", "dimm1", etc.
// Using a string::find would result in "dimm1" to be matched with "dimm10",
// "dimm11", etc. This function prevents this problem from happening.
inline bool containsDimmId(const std::string& dimmId, const std::string& path)
{
    ...
    if (idxOnePast < path.size())
    {
        ...
        if (c >= '0' && c <= '9')
            return false; // the dimmId is imprecisely matched
    }
    return true;
}
