                    ...
                    for (const auto& [service, interfaces] : object)
                    {
                        for (const auto& interface : interfaces)
                        {
                                     ...
                                     "Definitions")
                            {
                                ...
                                // object path, and then get the Asserted
                                // property value by this path.
                                getLocationIndicatorActive(aResp, path);
                            }
                        }
                    }
                }
                        ...
                        if (boost::ends_with(object.first, dimmId))
                        {
                            for (const auto& service : object.second)
                            {
                                for (const auto& inventory : service.second)
                                {
                                    ...
                                    {
                                        // Find `endpoints` array by this
                                        // interface and the <cpu object
                                        // path>/identify_led_group object path,
                                        // and then set the Asserted property
                                        // value by this path and the current
                                        // value.
                                        setLocationIndicatorActive(
                                            asyncResp, object.first,
                                            *locationIndicatorActive);
                                        return;
                                    }
                                }
