...
// Check if the error is EBADR, if so, log a debug message and return true
// Otherwise, log an error message and return false
inline bool checkDbusErrors(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                            const boost::system::error_code& ec)
{
    if (ec)
    {
        if (ec.value() == EBADR)
        {
            BMCWEB_LOG_DEBUG("No interface found, skipping");
            return true;
        }
