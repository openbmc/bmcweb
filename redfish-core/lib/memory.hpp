    ...
    const std::pair<std::string, dbus::utility::DbusVariantType>& property)
{
    ...
    else if (property.first == "AllowedMemoryModes")
    {
        ...
        for (const char* checkValue : values)
        {
            if (boost::ends_with(*value, checkValue))
            {
                aResp->res.jsonValue["OperatingMemoryModes "] = checkValue;
                break;
            }
        }
    }
    else if (property.first == "MemoryMedia")
    {
        ...
        for (const char* checkValue : values)
        {
            if (boost::ends_with(*value, checkValue))
            {
                aResp->res.jsonValue["MemoryMedia"] = checkValue;
                break;
            }
        }
    }
