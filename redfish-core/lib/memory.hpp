...
inline void getDimmPartitionData(std::shared_ptr<bmcweb::AsyncResp> aResp,
                                 const std::string& service,
                                 const std::string& path, nlohmann::json& json)
{
    crow::connections::systemBus->async_method_call(
        [aResp{std::move(aResp)},
         &json](const boost::system::error_code ec,
                const dbus::utility::DBusPropertiesMap& properties) {
            if (ec)
            {
                BMCWEB_LOG_DEBUG << "DBUS response error";
            ...
            nlohmann::json& partition =
                json["Regions"].emplace_back(nlohmann::json::object());
...
struct MemoryAsyncResponse
{
    MemoryAsyncResponse(
        const std::shared_ptr<bmcweb::AsyncResp>& asyncResponse) :
    ...
    {
        BMCWEB_LOG_DEBUG
            << "MemoryAsyncResponse distructs; populate health & partitions data";
...
inline void getAllDimmsCallback(
    std::shared_ptr<memory::MemoryAsyncResponse> memoryAsyncResp,
    const boost::system::error_code ec,
...
inline void
    getAllDimms(std::shared_ptr<memory::MemoryAsyncResponse> memoryAsyncResp,
    ...
    for (const auto& [service, objectManagerPath] : serviceToObjectManagerPath)
    {
        crow::connections::systemBus->async_method_call(
            [memoryAsyncResp](const boost::system::error_code ec,
                              dbus::utility::ManagedObjectType& objects) {
                getAllDimmsCallback(std::move(memoryAsyncResp), ec, objects);
            },
...
inline void getObjectManagerPathsGivenServicesCallBack(
    ...
    boost::container::flat_set<std::string> services,
    const boost::system::error_code ec,
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    boost::container::flat_set<std::string> servicesSet(services.begin(),
                                                        services.end());
    BMCWEB_LOG_DEBUG << "There are " << servicesSet.size()
    ...
    for (const auto& [objectManagerPath, mapperServiceMap] : subtree)
    {
        for (const auto& [service, _] : mapperServiceMap)
        {
            ...
            {
                serviceToObjectManagerPath[service] = objectManagerPath;
            }
        }
    }

    auto memoryAsyncResp = std::make_shared<memory::MemoryAsyncResponse>(aResp);
    getAllDimms(std::move(memoryAsyncResp),
                serviceToObjectManagerPath);
}
...
{
    crow::connections::systemBus->async_method_call(
        [asyncResp{std::move(asyncResp)}, services{std::move(services)}](
            const boost::system::error_code ec,
            const dbus::utility::MapperGetSubTreeResponse& subtree) {
            getObjectManagerPathsGivenServicesCallBack(
                std::move(asyncResp), std::move(services), ec, subtree);
        },
