        ...
        [resp, dimmId, handler = std::forward<Handler>(handler)](
            ...
            for (const auto& [objectPath, serviceMap] : subtree)
            {
                ...
                const bool endsWithDesiredDimmName =
                    !name.empty() && name == dimmId;
                if (!endsWithDesiredDimmName)
                {
                    continue;
                }

                bool isAnyDimmSpecificInterfaceFound = false;
                for (const auto& [serviceName, interfaceList] : serviceMap)
                {
                    ...
                    {
                        isAnyDimmSpecificInterfaceFound = true;
                        break;
                    }
                }

                if (!isAnyDimmSpecificInterfaceFound)
                {
                    continue;
                }
...
/**
 * Find the D-Bus object representing the requested DIMM, and call the
 * setDimmData with the results. If matching object is not found, add 404
 * error to response and don't call the setDimmData.
 *
 * @param[in,out]   resp            Async HTTP response.
 * @param[in]       dimmId          Redfish Dimm Id.
...
inline void setDimmObject(const std::shared_ptr<bmcweb::AsyncResp>& resp,
                          const std::string& dimmId, std::optional<bool> value)
{
    // GetSubTree on all interfaces which provide info about a Memory
    crow::connections::systemBus->async_method_call(
        [resp, dimmId, value](boost::system::error_code ec,
                              const MapperGetSubTreeResponse& subtree) mutable {
            if (ec)
            {
                BMCWEB_LOG_DEBUG << "DBUS response error: " << ec;
            ...
            for (const auto& [objectPath, serviceMap] : subtree)
            {
                ...
                setDimmData(resp, serviceMap, objectPath, value);
                return;
            }
            messages::resourceNotFound(resp->res, "Memory", dimmId);
        },
