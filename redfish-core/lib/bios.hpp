                            ...
                            const nlohmann::json& attrJson)
{
    ...
    constexpr std::array<std::string_view, 2> stringAddition{
        "MinLength", "MaxLength"};
    ...
    for (std::string_view key :
         {std::string_view("CurrentValue"), std::string_view("DefaultValue")})
    {
        ...
        bool valid = ((type == "Enumeration" || type == "String") &&
                      value.is_string()) ||
    ...
    const std::string& service)
{
    ...
    for (const nlohmann::json::object_t& attrMap : baseBiosTableJson)
    {
        ...
        if (type == "Enumeration" || type == "String")
        {
            ...
            defaultValue =
                defaultIsNull
                    ? std::variant<int64_t, std::string, bool>(int64_t{0})
                    : std::variant<int64_t, std::string, bool>(
 ...
 * BaseBIOSTable. Restricted to bootstrap accounts.
 */
    ...
    const std::string& systemName)
{
    ...
    // The attribute registry is owned by the host firmware, so only the
    // Redfish Host Interface bootstrap identity may publish it.
