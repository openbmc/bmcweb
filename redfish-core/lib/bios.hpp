    ...
    void getBiosAttributes(std::shared_ptr<AsyncResp> asyncResp)
    {
        crow::connections::systemBus->async_method_call(
            [asyncResp](const boost::system::error_code ec,
                ...
                BMCWEB_LOG_ERROR << "File Name: " << fileName;
                nlohmann::json &attributeArray =
                    asyncResp->res.jsonValue["Attributes"];
                if (!std::filesystem::exists(fileName))
                {
                    BMCWEB_LOG_ERROR << "File is not exist: " << fileName;
                    messages::internalError(asyncResp->res);
                    return;
                }
                std::ifstream file(fileName);
                attributeArray = nlohmann::json::parse(file, nullptr, false);
            },
...
class BiosSettings : public Node
{
  public:
    static constexpr const char *biosSettingsFilePath =
        "/tmp/BIOSConfig/tmpBiosSettings.json";
        ...
        crow::connections::systemBus->async_method_call(
            [asyncResp](const boost::system::error_code ec, const bool ret) {
                ...
                messages::propertyValueNotInList(
                    asyncResp->res, "Invalid value", "Or invalid key");
            },
            "xyz.openbmc_project.biosconfig_manager",
            "/xyz/openbmc_project/BIOSConfig/BIOSConfigMgr",
            "xyz.openbmc_project.BIOSConfig.BIOSConfigMgr", "PatchBiosSettings",
            biosSettingsFilePath);
    }
    /**
...
class BiosAttributeRegistry : public Node
{
  public:
    BiosAttributeRegistry(CrowApp &app) :
        Node(app, "/redfish/v1/RegistryStore/AttributeRegistries/"
  ...
  private:
               ...
               const std::vector<std::string> &params) override
    {
        auto asyncResp = std::make_shared<AsyncResp>(res);
        asyncResp->res.jsonValue["@odata.id"] =
            "/redfish/v1/RegistryStore/AttributeRegistries/"
            "BiosAttributeRegistry.v1_0_0.json";
    ...
    void getBiosAttributeRegistry(std::shared_ptr<AsyncResp> asyncResp)
    {
        crow::connections::systemBus->async_method_call(
            [asyncResp](const boost::system::error_code ec,
                ...
                std::ifstream file(fileName);
                nlohmann::json tempAttrRegJson =
                    nlohmann::json::parse(file, nullptr, false);
                attributeArray = tempAttrRegJson["RegistryEntries"];
            },
            "xyz.openbmc_project.biosconfig_manager",
            "/xyz/openbmc_project/BIOSConfig/BIOSConfigMgr",
            "xyz.openbmc_project.BIOSConfig.BIOSConfigMgr", "GetAllAttributes",
