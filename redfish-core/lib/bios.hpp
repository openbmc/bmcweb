...
static std::string mapRedfishToAttrType(const std::string_view type)
{
    ...
    {
        return "xyz.openbmc_project.BIOSConfig.Manager.AttributeType."
               "Enumeration";
    }
    else
    {
...
class BiosSettings : public Node
{
  ...
  private:
                 ...
                 const std::vector<std::string>&) override
    {
        ...
        for (std::size_t i = 0; i < inpJson.size(); i++)
        {
            std::optional<std::string> errParam;
            if (!json_util::getValueFromJsonObject(inpJson[i], "AttributeName",
                                                   attrName))
            {
                errParam = "AttributeName";
            }
            else if (!json_util::getValueFromJsonObject(
                         inpJson[i], "AttributeType", attrType))
            {
                errParam = "AttributeType";
            }
            else if (!json_util::getValueFromJsonObject(
                         inpJson[i], "AttributeValue", attrValue))
            {
                ...
                crow::connections::systemBus->async_method_call(
                    [asyncResp](
                        ...
                        for (const BiosBaseTableItemType& item : *baseBiosTable)
                        {
                            ...
                            attributeItem["Options"] = optionsArray;
