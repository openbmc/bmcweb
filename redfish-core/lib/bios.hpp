...
inline void afterBiosChangePassword(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    if (ec)
    {
        ...
        if (dbusError != nullptr &&
            std::string_view(
                ...
                dbusError->name)
        {
            messages::actionParameterValueError(asyncResp->res, "OldPassword",
                                                "ChangePassword");
            return;
        }
...
inline void afterGetBiosPasswordObject(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& passwordName, const std::string& oldPassword,
    const std::string& newPassword, const boost::system::error_code& ec,
    const dbus::utility::MapperGetObject& objType)
{
    ...
    const std::string& service = objType.begin()->first;
    dbus::utility::async_method_call(
        asyncResp,
        [asyncResp](const boost::system::error_code& ec2,
                    const sdbusplus::message_t& msg) {
            afterBiosChangePassword(asyncResp, ec2, msg);
        },
    ...
    const std::string& systemName)
{
    ...
    dbus::utility::getDbusObject(
        ...
        std::bind_front(afterGetBiosPasswordObject, asyncResp, passwordName,
