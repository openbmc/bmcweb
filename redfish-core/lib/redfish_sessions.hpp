...
inline void
    getSessionInfo(const std::shared_ptr<AsyncResp> sessionAsyncResp,
                   std::shared_ptr<std::vector<std::string>> ipmiSessions)
{
    ...
    getIpmiHandleSession(const std::shared_ptr<AsyncResp> sessionAsyncResp)
{
    ...
    crow::connections::systemBus->async_method_call(
        [sessionAsyncResp, sessionSharedPtr{std::move(sessionSharedPtr)}](
            ...
            if (subtree.size() == 0)
            {
                ...
                getSessionInfo(sessionAsyncResp, std::move(sessionSharedPtr));
            }
                         ...
                         objData : object.second)
                {
                    ...
                    crow::connections::systemBus->async_method_call(
                        [sessionAsyncResp,
                         ...
                         objectServiceName, objPath, subtreeEndPtr,
