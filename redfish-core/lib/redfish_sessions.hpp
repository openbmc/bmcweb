    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    auto createSession = [&session, &username,&req, &clientId](bool configureSelfOnly, bool required) {
        session =
    ...
    {
        auto handler = [username, asyncResp, createSession](
                           const boost::system::error_code& ec, bool required) {
            if (ec || !createSession(required, required))
            {
                messages::internalError(asyncResp->res);
                return;
            }
