    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    auto createSession = [&](bool configureSelfOnly, bool required) {
        session = persistent_data::SessionStore::getInstance().generateUserSession(
            username, req.ipAddress, clientId,
            persistent_data::SessionType::Session, configureSelfOnly, required);
    ...
    {
        auto handler = [username, asyncResp, &req, &clientId, &session, createSession]
    ...
    {
        auto accountUri = boost::urls::format("/redfish/v1/AccountService/Accounts/{}",
                                session->username);
