...
inline void
    getManagerInventoryItem(const std::shared_ptr<bmcweb::AsyncResp>& resp,
                            Handler&& handler)
{
    ...
    crow::connections::systemBus->async_method_call(
        [resp, handler = std::forward<Handler>(handler)](
            ...
            {
                bool isAnyBMCSpecificInterfaceFound = false;
                for (const auto& [serviceName, interfaceList] : serviceMap)
                {
                    ...
                    {
                        isAnyBMCSpecificInterfaceFound = true;
                        break;
                    }
                }

                if (!isAnyBMCSpecificInterfaceFound)
                {
                    continue;
                }
            ...
            messages::resourceNotFound(resp->res, "Manager",
...
class Manager : public Node
{
  ...
  private:
               ...
               const crow::Request&, const std::vector<std::string>&) override
    {
        ...
        auto getManagerData = [asyncResp](const std::string& objectPath,
            ...
            for (const auto& [serviceName, interfaceList] : serviceMap)
            {
                ...
                {
                    getLocation(asyncResp, serviceName, objectPath);
                }
        ...
        getManagerInventoryItem(asyncResp, std::move(getManagerData));
    }
                 ...
                 const std::vector<std::string>&) override
    {
        ...
        auto setManagerData = [asyncResp, locationIndicatorActive](
        ...
        getManagerInventoryItem(asyncResp, std::move(setManagerData));
    }
