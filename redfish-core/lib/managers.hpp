...
struct SetPIDValues : std::enable_shared_from_this<SetPIDValues>
{
    ...
    ~SetPIDValues()
    {
        ...
        if (!redfish::json_util::readJson(
                jsonData, response->res, "PidControllers", pidControllers,
                 ...
                 it != container->end(); it++)
            {
                ...
                else
                {
                    ...
                    if (currentProfile.size())
                    {
                        ...
                        auto findProfile = std::find_if(
                            managedObj.begin(), managedObj.end(),
                            [&profile](const auto& interfacePair) {
                                if (!boost::ends_with(interfacePair.first.str,
                                                      profile))
                                {
                                    return false;
                        ...
                        const std::string& path = findProfile->first.str;
                        crow::connections::systemBus->async_method_call(
                            [response, name,
                 ...
                 const std::vector<std::string>& params) override
    {
        ...
        if (oem)
        {
            ...
            if (openbmc)
            {
                ...
                if (fan)
                {
                    auto pid = std::make_shared<SetPIDValues>(response,
                                                              std::move(*fan));
                    pid->run();
                }
            }
