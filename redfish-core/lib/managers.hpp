...
inline void afterGetHostInterfacesObject(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    const dbus::utility::MapperGetObject& objInfo)
{
    if (ec || objInfo.empty())
    {
        BMCWEB_LOG_DEBUG("Host interface not supported: {}", ec.message());
        return;
    }
    ...
    const std::string& managerId)
{
    ...
    dbus::utility::getDbusObject(
        ...
        std::bind_front(afterGetHostInterfacesObject, asyncResp, managerId));
}
