...
class Manager : public Node
{
  ...
  private:
               ...
               const std::vector<std::string>& params) override
    {
        ...
        res.jsonValue["@Redfish.Settings"]["@odata.type"] =
            "#Settings.v1_3_0.Settings";
        res.jsonValue["@Redfish.Settings"]["SettingsObject"] = {
                                ...
                                const std::string&& runningFirmwareTarget)
    {
        ...
        // Since bindly setting below let's do further checking to make sure the
        // id is in the correct format at least, 8 hex characters
        if (!std::all_of(firmwareId.begin(), firmwareId.end(), ::isxdigit))
        {
            messages::propertyValueNotInList(aResp->res, runningFirmwareTarget,
                                             "@odata.id");
            BMCWEB_LOG_DEBUG << "Invalid firmware ID; not hex.";
            return;
        }

        if (firmwareId.length() != 8)
        {
            messages::propertyValueNotInList(aResp->res, runningFirmwareTarget,
                                             "@odata.id");
            BMCWEB_LOG_DEBUG << "Invalid firmware ID; not correct length.";
            return;
        }
        ...
        // Only support ApplyTime = Immediate
        // TODO (Gunnar): Support PATCHing /redfish/v1/Managers/bmc/Settings and
        // allow ActiveSoftwareImageApplyTime = OnReset
        crow::connections::systemBus->async_method_call(
            [aResp](const boost::system::error_code ec) {
                if (ec)
                {
                    // Could still be user with bad Id.
                    // An improvement here might be check if BMC Updater has an
                    // image with that Id first and see if it even needs set.
                    messages::internalError(aResp->res);
                    return;
                }
...
class ManagerSettings : public Node
{
  public:
    ManagerSettings(CrowApp& app) :
        Node(app, "/redfish/v1/Managers/bmc/Settings")
    {
  ...
  private:
    void doGet(crow::Response& res, const crow::Request& req,
               const std::vector<std::string>& params) override
    {
        res.jsonValue["@odata.id"] = "/redfish/v1/Managers/bmc/Settings";
        res.jsonValue["@odata.type"] = "#Settings.v1_3_0.Settings";
        res.jsonValue["Id"] = "Settings";
        res.jsonValue["Name"] = "Manager Settings";
        res.jsonValue["@Redfish.ActiveSoftwareImageApplyTime"] = {
            {"@odata.type", "#Settings.v1_3_0.ActiveSoftwareImageApplyTime"},
            {"ApplyTime", "Immediate"}};
        res.end();
    }
};
