...
inline void getManagerObject(const std::shared_ptr<bmcweb::AsyncResp>& resp,
                             Handler&& handler)
{
    ...
    crow::connections::systemBus->async_method_call(
        [resp, handler = std::forward<Handler>(handler)](
            ...
            for (const auto& [objectPath, serviceMap] : subtree)
            {
                ...
                handler(resp, objectPath, serviceMap);
                return;
            }
                             ...
                             std::optional<bool> value)
{
    ...
    // GetSubTree on all interfaces which provide info about a Manager
    crow::connections::systemBus->async_method_call(
        [aResp, value](boost::system::error_code ec,
                       const MapperGetSubTreeResponse& subtree) mutable {
            if (ec)
            {
                BMCWEB_LOG_DEBUG << "DBUS response error: " << ec;
            ...
            for (const auto& [objectPath, serviceMap] : subtree)
            {
                ...
                setManagerData(aResp, serviceMap, objectPath, value);
                return;
            }
