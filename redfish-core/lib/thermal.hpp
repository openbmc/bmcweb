  ...
  private:
                 ...
                 const std::vector<std::string>& params) override
    {
        ...
        auto asyncResp = std::make_shared<SensorsAsyncResp>(
            ...
            sensors::node::thermal);
        std::string fanMode;
        if (!json_util::readJson(req, res, "FanMode", fanMode))
        {
            messages::actionParameterUnknown(res, "FanMode", "");
            return;
        }
        ...
        {
            messages::actionParameterValueFormatError(res, "Manual", "Auto", "");
            return;
        }
        ...
        [asyncResp{std::move(asyncResp)}, munualMode](
            ...
            for (const auto& object : subtree)
            {
                ...
                if ((iter != std::string::npos) && (iter < object.first.size()))
                {
                    ...
                    crow::connections::systemBus->async_method_call(
                    [asyncResp, objName, munualMode](const boost::system::error_code ec) {
                        if (ec)
                        {
