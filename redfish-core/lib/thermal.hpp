                             ...
                             interfaces.end()))
                        {
                            ...
                            crow::connections::systemBus->async_method_call(
                                [dimmId, aResp{std::move(aResp)}](const boost::system::error_code ec,
                                    ...
                                    {
                                        if (property.first == "TemperatureSummary")
                                        {
                                            auto* value = std::get_if<std::map<std::string, int>>(&property.second);
                                            if (value == nullptr)
                                            {
                                                continue;
                                            }
                                            aResp->res.jsonValue["TemperatureSummary"] = *value;
                                        }
                                    }
                                },
...
inline void requestRoutesMemoryThermal(App& app)
{
     ...
     */
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/system/Memory/<str>/Chassis/ThermalMetrics")
            ...
            [](const crow::Request&,
               ...
               const std::string& dimmId) {
                ...
                asyncResp->res.jsonValue["@odata.id"] =
                    "/redfish/v1/Systems/system/Memory/" + dimmId + "/Chassis/ThermalMetrics";
