 ...
 *
 * SecureBootCurrentBoot and SecureBootMode are read-only per the Redfish
 * SecureBoot schema: they describe the state of the current boot, which
 * only the host firmware can know. The one writer is the host firmware
 * itself, reporting over the Redfish Host Interface (DSP0270) with its
 * bootstrap account; for every other caller they return
 * PropertyNotWritable.
 */
    ...
    const std::string& systemName)
{
    ...
    // A bootstrap account only ever exists for, and is only usable by, the
    // host firmware authenticating over the Redfish Host Interface. Its
    // presence on the session is what identifies the caller as the host.
    ...
    if (!isHostInterfaceCaller)
    {
        ...
        {
            messages::propertyNotWritable(aResp->res,
    ...
    // Validate and translate everything before touching D-Bus, so a bad
    // value in one property cannot leave another half-applied.
    ...
    // Resolve the backend through the mapper rather than a hardcoded
    // service name, the same way the GET does.
    constexpr std::array<std::string_view, 1> interfaces = {
        secureBootInterface};
    dbus::utility::getSubTree(
        "/xyz/openbmc_project", 0, interfaces,
        std::bind_front(afterGetSecureBootSubTreeForPatch, aResp,
                        std::move(values)));
}
