    ...
    const dbus::utility::DBusPropertiesMap& properties)
{
    ...
    // Until the host publishes its state, the D-Bus properties hold
    // Unknown, which maps to Invalid; skip those properties rather than
    // reporting values outside the Redfish SecureBoot enums.
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    if (ec || subtree.empty())
    {
        // No service implements SecureBoot on this system
