...
inline std::pair<std::string, std::string>
    getPowerLimitsPath(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                       const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    if (subtree.size() > 1)
    {
        BMCWEB_LOG_ERROR("Unexpected number paths found {}", subtree.size());
        messages::internalError(asyncResp->res);
        return pathAndService;
    }
    if (subtree.empty())
    {
        BMCWEB_LOG_DEBUG("No paths found");
        return pathAndService;
    }
    ...
    {
        BMCWEB_LOG_ERROR("Unexpected number serviceMaps found {}",
                         subtree.size());
        messages::internalError(asyncResp->res);
        return pathAndService;
    }
    if (serviceMap.empty())
    {
        BMCWEB_LOG_DEBUG("No serviceMaps found");
        return pathAndService;
    }
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    auto pathAndService = getPowerLimitsPath(asyncResp, subtree);
    if (pathAndService.first.empty() || pathAndService.second.empty())
    {
        if (asyncResp->res.result() == boost::beast::http::status::ok)
        {
            ...
            {
                BMCWEB_LOG_ERROR(
                    "Failed to get path and service for Power SetPoint");
                messages::internalError(asyncResp->res);
            }
        }
        return;
    ...
    setDbusProperty(
        asyncResp, "SetPoint", pathAndService.second, pathAndService.first,
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    setDbusProperty(asyncResp, "ControlMode", pathAndService.second,
                    pathAndService.first,
