                        ...
                        const dbus::utility::DBusPropertiesMap& propertiesList)
{
    ...
    redfish::fan_utils::getFanSensorIdAndName(
        fanSensorPath, std::ref(fanSensorId), std::ref(fanSensorName));
    if (fanSensorId.empty() || fanSensorName.empty())
    {
    ...
    item["DeviceName"] = std::move(fanSensorName);
