    ...
    crow::connections::systemBus->async_method_call(
        ...
        for (const auto& [objectPath, serviceName] : sensorsubtree)
        {
            ...
            if (!dbus::utility::getNthStringFromPath(validPath, 3,
                                                     sensorType) ||
                !dbus::utility::getNthStringFromPath(validPath, 4, sensorName))
            {
                BMCWEB_LOG_ERROR << "Got invalid path " << validPath;
                messages::invalidObject(asyncResp->res,
                                        boost::string_view(validPath));
                return;
            }
...
inline void getPowerWatts(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                          const std::string& chassisID)
{
    const std::string sensorPath = "/xyz/openbmc_project/sensors/power";
    const std::array<std::string, 1> sensorInterfaces = {
        "xyz.openbmc_project.Sensor.Value"};
    crow::connections::systemBus->async_method_call(
...
inline void requestRoutesEnvironmentMetrics(App& app)
{
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/EnvironmentMetrics/")
        ...
        auto getChassisID =
            ...
            if(ec)
            {
                BMCWEB_LOG_DEBUG << "DBUS response error in getting Chassis objects";
                messages::internalError(asyncResp->res);
                return;
            }
            ...
            if (!validChassisID)
            {
                BMCWEB_LOG_DEBUG << "Not a valid chassis ID:" << chassisID;
                messages::resourceNotFound(asyncResp->res, "Chassis",
                                           chassisID);
                return;
            }
        ...
        redfish::chassis_utils::getValidChassisPath(chassisID,
