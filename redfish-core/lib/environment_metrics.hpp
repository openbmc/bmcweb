                             ...
                             const std::string& controlPath)>& callback)
{
    ...
    const sdbusplus::message::object_path controlSubtree{
        "/xyz/openbmc_project/control"};

    dbus::utility::getAssociatedSubTree(
        sdbusplus::message::object_path(associationPath), controlSubtree, 0,
        powerCapInterface,
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    for (const auto& [objectPath, serviceMap] : subtree)
    {
        ...
        afterGetProcessorForEnvMetrics(asyncResp, processorId, objectPath,
                                       callback);
        return;
    }
