...
/* TODO: Added prototypes temporarily. Once parent commits merge I will move
 * the functions around to no longer need these. (This makes it easier to
 * resolve merge conflicts of any changes of the parent commits.)
 */
void getPowerLimitWatts(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                        const boost::system::error_code& ec,
                        const std::string& service,
                        const std::string& controlPath);
void getPowerCapObject(
    ...
    const dbus::utility::DBusPropertiesMap& valuesDict)
{
    ...
    if (sensor_utils::objectExcerptToJson(
            path, chassisId,
            sensor_utils::ChassisSubNode::environmentMetricsNode,
            sensor::ReadingType::Power, valuesDict, item))
    {
    ...
    const dbus::utility::DBusPropertiesMap& propertiesList)
{
    ...
    {
        nlohmann::json::array_t* fanSensorList =
            asyncResp->res.jsonValue["FanSpeedsPercent"]
                .get_ptr<nlohmann::json::array_t*>();
        if (fanSensorList == nullptr)
        {
    ...
    const dbus::utility::MapperGetSubTreePathsResponse& fanPaths)
{
    ...
    // Initialize the array then fill in the fans
    ...
    const std::string& service, const std::string& controlPath)
{
    ...
    if (ec)
    {
        if (ec.value() == EBADR)
        {
            messages::propertyUnknown(asyncResp->res, "SetPoint");
            return;
        }
    ...
    const std::string& service, const std::string& controlPath)
{
    ...
    if (ec)
    {
        if (ec.value() == EBADR)
        {
            messages::propertyUnknown(asyncResp->res, "ControlMode");
            return;
        }
        BMCWEB_LOG_ERROR("D-Bus response error {}", ec);
        messages::internalError(asyncResp->res);
    ...
    const std::optional<std::string>& validChassisPath)
{
    ...
    getPowerCapObject(*validChassisPath,
    ...
    const dbus::utility::MapperEndPoints& endpoints)
{
    ...
    if (endpoints.empty())
    {
        BMCWEB_LOG_DEBUG("No chassis association for sensor {}", sensorPath);
        return;
    }
    ...
    sdbusplus::message::object_path chassisPath(endpoints[0]);
...
inline void afterGetProcessorSensorList(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& redfishProperty, sensor::ReadingType readingType,
    const boost::system::error_code& ec,
    const dbus::utility::MapperGetSubTreeResponse& subtree)
    ...
    const std::string& redfishProperty, sensor::ReadingType readingType)
{
    ...
    sdbusplus::message::object_path endpointPath{processorPath};
    ...
    dbus::utility::getAssociatedSubTree(
        endpointPath, sdbusplus::message::object_path(sensorTypePath), 0,
        sensorValueIface,
        std::bind_front(afterGetProcessorSensorList, asyncResp, redfishProperty,
                        readingType));
}
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    for (const auto& [objectPath, serviceMap] : subtree)
    {
        ...
        for (const auto& [serviceName, interfaceList] : serviceMap)
        {
            ...
            {
                afterGetProcessorForEnvMetrics(
                    asyncResp, processorId, objectPath, serviceMap, callback);
                return;
            }
        }
    }
    ...
    const std::optional<std::string>& validChassisPath)
{
    ...
    if (setPoint)
    {
        getPowerCapObject(
            *validChassisPath,
            std::bind_front(setPowerCapSetPoint, asyncResp, *setPoint));
    }
    ...
    if (controlMode)
    {
        getPowerCapObject(
            *validChassisPath,
            std::bind_front(setPowerCapControlMode, asyncResp, *controlMode));
    }
}
