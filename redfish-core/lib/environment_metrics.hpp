...
{
    auto respHandler =
        [asyncResp, chassisId](const std::string& fanSensorPath) {
        ...
        crow::connections::systemBus->async_method_call(
            ...
            for (const auto& obj : object)
            {
                ...
                sdbusplus::asio::getProperty<double>(
                    *crow::connections::systemBus, service, fanSensorPath,
                    "xyz.openbmc_project.Sensor.Value", "Value",
                    [asyncResp, chassisId,
                     fanSensorPath](const boost::system::error_code ec1,
                                    const double value) {
                    if (ec1)
                    {
                        return;
                    ...
                    updateFanSensorList(asyncResp, chassisId, fanSensorPath,
                                        value);
                    });
            }
            },
