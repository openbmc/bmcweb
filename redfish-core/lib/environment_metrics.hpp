...
inline void
    getPowerLimitsPath(const dbus::utility::MapperGetSubTreeResponse& subtree,
                       std::string& path, std::string& service)
{
    ...
    {
        BMCWEB_LOG_DEBUG("Incorrect path or serviceMap");
        path.clear();
        service.clear();
        return;
    }
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    if (ec)
    {
        if (ec.value() != EBADR)
        {
            BMCWEB_LOG_DEBUG("DBUS response error for PowerLimitWatts {}",
                             ec.value());
    ...
    std::string service;
    getPowerLimitsPath(subtree, std::ref(objectPath), std::ref(service));
    if (objectPath.empty() || service.empty())
    {
        BMCWEB_LOG_DEBUG("Failed to get path and service for Power SetPoint");
        return;
    }
        ...
        *crow::connections::systemBus, service, objectPath,
        ...
        [asyncResp](const boost::system::error_code& ec1) {
            if (ec1)
            {
                if (ec1.value() != EBADR)
                {
                    BMCWEB_LOG_ERROR("Failed to set PowerCap {}",
                                     ec1.message());
                    messages::internalError(asyncResp->res);
                }
                return;
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    sdbusplus::asio::setProperty(
        *crow::connections::systemBus, service, objectPath,
        ...
        powerCapEnable, [asyncResp](const boost::system::error_code& ec1) {
            if (ec1)
            {
                if (ec1.value() != EBADR)
                {
