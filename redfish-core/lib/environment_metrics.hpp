...
inline void
    setPowerLimitWatts(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                       uint32_t powerCap)
{
    BMCWEB_LOG_DEBUG << "Set Power Limit Watts";

    crow::connections::systemBus->async_method_call(
        [asyncResp, powerCap](const boost::system::error_code ec,
                              const std::variant<bool>& powerCapEnable) {
            if (ec)
            {
                messages::internalError(asyncResp->res);
            ...
            [](const crow::Request& req,
               ...
               const std::string& chassisID) {
                ...
                if (powerLimitWatts)
                {
                    ...
                    if (!redfish::json_util::readJson(*powerLimitWatts,
                                                      asyncResp->res,
                                                      "SetPoint", setPoint))
                    {
                        return;
                        ...
                        auto getChassisID =
                            [asyncResp, chassisID,
                             setPoint](const std::optional<std::string>&
                                ...
                                setPowerLimitWatts(asyncResp, *setPoint);
                            };
                        redfish::chassis_utils::getValidChassisID(
                            asyncResp, chassisID, std::move(getChassisID));
                    }
