    ...
    const dbus::utility::MapperGetSubTreePathsResponse& trustedComponentPaths)
{
    ...
    for (const std::string& trustedComponentPath : trustedComponentPaths)
    {
        ...
        item["@odata.id"] = boost::urls::format(
            "/redfish/v1/Chassis/{}/TrustedComponents/{}", chassisId, trustedComponentName);

        trustedComponentList.emplace_back(std::move(item));
    }
    asyncResp->res.jsonValue["Members@odata.count"] = trustedComponentList.size();
}
    ...
    const std::optional<std::string>& validChassisPath)
{
    ...
    getTrustedComponentPaths(asyncResp, *validChassisPath,
...
inline bool checkTrustedComponentId(const std::string& trustedComponentPath,
                       const std::string& componentId)
{
    std::string trustedComponentName = sdbusplus::message::object_path(trustedComponentPath).filename();

    return !(trustedComponentName.empty() || trustedComponentName != componentId);
}
                             ...
                             const std::string& service)>& callback)
{
    getTrustedComponentPaths(
        ...
        [componentId, asyncResp, callback](
            const dbus::utility::MapperGetSubTreePathsResponse& trustedComponentPaths) {
            handleTrustedComponentPath(componentId, asyncResp, trustedComponentPaths, callback);
        });
}
    ...
    const std::optional<std::string>& validChassisPath)
{
    ...
    getValidTrustedComponentPath(asyncResp, *validChassisPath, componentId,
                    std::bind_front(afterGetValidTrustedComponentPath,
