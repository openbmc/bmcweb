...
{
    // Default, RackMount, or StandAlone will have Reset Action.
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, chassisId,
         callback](const boost::system::error_code ec,
            ...
            if (ec)
            {
                BMCWEB_LOG_DEBUG << "Chassis: DBUS response error for Chassis";
                messages::internalError(asyncResp->res);
                return;
            }

            callback(getChassisTypeProperty(property));
        },
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, chassisId,
         callback](const boost::system::error_code ec,
            ...
            if (ec)
            {
                BMCWEB_LOG_DEBUG << "Chassis Type not found: default to "
                                    "RackMount with Chassis Action support";
            ...
            [](const crow::Request& req,
               ...
               const std::string& chassisId) {
                ...
                validateChassisResetAction(
                    asyncResp, chassisId,
                    [req, asyncResp,
                     chassisId](const std::optional<std::string>& chassisType) {
                        if (chassisType && *chassisType != "RackMount" &&
                            *chassisType != "StandAlone")
                        {
