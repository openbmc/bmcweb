...
{
    crow::connections::systemBus->async_method_call(
        [aResp{std::move(aResp)}, chassisPath,
         resource](const boost::system::error_code ec,
            ...
            for (const auto& object : objects)
            {
                ...
                resources.push_back(
                    {{"@odata.id",
                      "/redfish/v1/Systems/system/Storage/" + leaf}});
            }

            if (!resources.empty())
            {
                aResp->res.jsonValue["Links"][resource] = std::move(resources);
            }
        },
