 ...
 */
inline chassis::IntrusionSensorReArm dbusChassisIntrusionRearmModeToRf(std::string_view rearmMode)
{
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    if (serviceMap.empty())
    {
        BMCWEB_LOG_ERROR("Error getting service map of Intrusion Sensor object");
        messages::internalError(asyncResp->res);
        return;
    }
...
inline void
    handleIntrusionSensorPatch(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
        const std::string& intrusionSensor)
{
    ...
    dbus::utility::getSubTree(
        ...
        std::bind_front(setIntrusionSensorByService, asyncResp, normalStatusDbusStr));
}
                       ...
                       const std::string& param)
{
    ...
    if (intrusionSensor)
    {
        getMainChassisId(asyncResp, [chassisId, intrusionSensor](const std::string& mainChassisId,
        const std::shared_ptr<bmcweb::AsyncResp>& asyncResp2)
        {
            if (chassisId != mainChassisId)
        {
            BMCWEB_LOG_ERROR("Patching to IntrusionSensor not from a primary chassis");
            messages::actionNotSupported(asyncResp2->res, "Setting IntrusionSensor from a non-primary chassis");
            return;
        }
