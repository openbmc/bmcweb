...
/**
 * @brief Convert DBus string to variant of PhysicalSecurity's properties
 *
 * @param[in] inputStr - chassis intrusion property string on DBus.
 *
 * @return A variant of property values under PhysicalSecurity.
 */
inline std::optional<PhysicalSecurityVar>
    dbusToPhysicalSecurityVar(std::string_view inputStr)
{
    if (inputStr == "xyz.openbmc_project.Chassis.Intrusion.Status.Normal")
    {
    ...
    if (inputStr == "xyz.openbmc_project.Chassis.Intrusion.RearmMode.Manual")
    {
        return chassis::IntrusionSensorReArm::Manual;
    }
    if (inputStr == "xyz.openbmc_project.Chassis.Intrusion.RearmMode.Automatic")
    {
    ...
    crow::Response& resp, const dbus::utility::DBusPropertiesMap& propertiesMap)
{
    ...
    if (!success)
    {
        BMCWEB_LOG_WARNING("DBUS error: failed to unpack properties");
        return;
    }
    ...
    if (status != nullptr)
    {
        ...
        std::optional<PhysicalSecurityVar> physicalSecVar =
        ...
        if (physicalSecVar)
        {
            ...
            if (std::get_if<chassis::IntrusionSensor>(&(*physicalSecVar)) !=
                nullptr)
            {
                intrusionSensor =
                    std::get<chassis::IntrusionSensor>(*physicalSecVar);
            }
        }
        resp.jsonValue["PhysicalSecurity"]["IntrusionSensor"] = intrusionSensor;
    }
    ...
    if (rearmMode != nullptr)
    {
        ...
        if (physicalSecVar)
        {
            if (std::get_if<chassis::IntrusionSensorReArm>(
                    &(*physicalSecVar)) != nullptr)
            {
                intrusionSensorReArm =
                    std::get<chassis::IntrusionSensorReArm>(*physicalSecVar);
            }
        }
