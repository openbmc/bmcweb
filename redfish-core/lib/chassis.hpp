    ...
    const dbus::utility::DBusPropertiesMap& propertiesList)
{
    ...
    auto chassisType = chassis::ChassisType::Invalid;
    if (type != nullptr)
    {
        chassisType = translateChassisTypeToRedfish(*type);
    }
    if (chassisType == chassis::ChassisType::Invalid)
    {
        // Default to RackMount if the chassis type is not found for backward compatibility
        chassisType = chassis::ChassisType::RackMount;
    }
