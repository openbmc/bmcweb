...
{
    crow::connections::systemBus->async_method_call(
        callback, connectionName, path, "org.freedesktop.DBus.Properties",
        "Get", "xyz.openbmc_project.Inventory.Item.Chassis", "Type");
}
        ...
        .methods(boost::beast::http::verb::post)(
            [](const crow::Request& req,
               ...
               const std::string& chassisId) {
                ...
                json_util::readJsonAction(req, asyncResp->res, "ResetType",
                                          resetType);

                validateChassisResetAction(
                    asyncResp, chassisId,
                    std::bind_front(validateChassisResetActionHandler,
                                    asyncResp, resetType));
            });
}
    ...
    const std::variant<std::string>& property)
{
    asyncResp->res.jsonValue = {
        ...
        {"@odata.id", "/redfish/v1/Chassis/" + chassisId + "/ResetActionInfo"},
    ...
    asyncResp->res.jsonValue["Parameters"]["AllowableValues"] = {"PowerCycle"};
}
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/ResetActionInfo/")
        ...
        .methods(boost::beast::http::verb::get)(
...

            {
                validateChassisResetAction(
                    asyncResp, chassisId,
                    std::bind_front(validateChassisResetActionInfoHandler,
                                    asyncResp, chassisId));
            });
}
} // namespace redfish
