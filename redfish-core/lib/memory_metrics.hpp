...
/**
 * @brief Process MemoryECC properties and populate response.
 *
    ...
    const dbus::utility::DBusPropertiesMap& properties)
{
    if (ec)
    {
        BMCWEB_LOG_DEBUG("No MemoryECC interface found: {}", ec.message());
        return;
    }
...
/**
 * @brief Process GetSubTree response and get Memory ECC data.
 *
 * @param[in,out] asyncResp  Async HTTP response.
 ...
 */
inline void afterGetMemorySubTree(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    for (const auto& [path, serviceMap] : subtree)
    {
        ...
        for (const auto& [service, ifaces] : serviceMap)
        {
            ...
            dbus::utility::getProperty<uint16_t>(
                service, path, "xyz.openbmc_project.Inventory.Item.Memory",
                            ...
                            const std::string& memoryId)
{
    ...
    dbus::utility::getSubTree(
        ...
        std::bind_front(afterGetMemorySubTree, asyncResp, memoryId));
}
