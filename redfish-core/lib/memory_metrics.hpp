...
inline void afterGetMemoryMetricsService(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& dimmPath, const boost::system::error_code& ec,
    ...
    const std::string& dimmPath)
{
    ...
    dbus::utility::getDbusObject(
        dimmPath, interfaces,
        std::bind_front(afterGetMemoryMetricsService, asyncResp, dimmPath));
}
    ...
    const std::string& systemName, const std::string& memoryId)
{
    ...
    getValidDimmPath(
        asyncResp, memoryId,
        std::bind_front(afterGetValidDimmPathForMetrics, asyncResp));
}
