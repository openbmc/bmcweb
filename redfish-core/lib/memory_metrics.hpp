    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    if (ec)
    {
        ...
        {
            BMCWEB_LOG_DEBUG(
                "No CapacityUtilization metric association found: {}", ec);
        }
        return;
    }
    ...
    const std::string& inventoryPath)
{
    const sdbusplus::object_path assocPath =
        sdbusplus::object_path(inventoryPath) / "measured_by";
    ...
    const std::string& systemName, const std::string& memoryId)
{
    ...
    getMemoryCapacityUtilization(
        asyncResp,
        sdbusplus::object_path("/xyz/openbmc_project/inventory") / memoryId);
}
