...
inline void afterGetMemoryOperatingSpeed(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const boost::system::error_code& ec, const uint16_t speed)
{
    if (ec)
    {
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    {
        if (sdbusplus::object_path(metricPath).filename() != "memory_bandwidth")
        {
            continue;
        }
        ...
        metrics_util::getMetricPropertyDouble(asyncResp, service, metricPath,
                                              "/BandwidthPercent"_json_pointer);
        return;
    }
}
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    {
        if (sdbusplus::object_path(objectPath).filename() != memoryId)
        {
            continue;
        }
        ...
        // The Memory resource exists (its path matched memoryId), so set the
        // MemoryMetrics identity here rather than in the property callbacks.
        // This keeps it out of the error paths above and guarantees a valid
        // resource even when only a subset of the metric sources responds.
        asyncResp->res.jsonValue["@odata.type"] =
            "#MemoryMetrics.v1_7_0.MemoryMetrics";
        asyncResp->res.jsonValue["@odata.id"] = boost::urls::format(
            "/redfish/v1/Systems/{}/Memory/{}/MemoryMetrics",
            BMCWEB_REDFISH_SYSTEM_URI_NAME, memoryId);
        ...
        dbus::utility::getProperty<uint16_t>(
            ...
            std::bind_front(afterGetMemoryOperatingSpeed, asyncResp));
