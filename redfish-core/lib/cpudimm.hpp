...
void getCpuAssetData(std::shared_ptr<AsyncResp> aResp, const std::string& cpuId,
                     const std::string& service, const std::string& objPath)
{
    BMCWEB_LOG_DEBUG << "Get Cpu Asset Data";
    crow::connections::systemBus->async_method_call(
        [cpuId, objPath, aResp{std::move(aResp)}](
            const boost::system::error_code ec,
            const dbus::utility::ManagedObjectType& dbusData) {
            if (ec)
            {
            ...
            if (boost::ends_with(object.first, cpuId))
            {
                for (const auto& service : object.second)
                {
                    for (const auto& inventory : service.second)
                        if (service.first == "xyz.openbmc_project.CPUInfo")
                        {
                            if (inventory == "xyz.openbmc_project."
                                             "Inventory.Decorator.Asset")
                            {
                                getCpuAssetData(aResp, cpuId, service.first,
                                                object.first);
                            }
                        }
                        else if (inventory ==
...
{
    void getDimmDataByService(
        std::shared_ptr<AsyncResp> aResp, const std::string& dimmId,
        const std::string& service, const std::string& objPath)
    {
        ...
        crow::connections::systemBus->async_method_call(
            [dimmId, aResp{std::move(aResp)}](
                ...
                const auto memorySizeProperty =
                    properties.find("MemorySizeInKB");
    ...
    void getDimmData(std::shared_ptr<AsyncResp> aResp,
    ...
    class Processor : public Node
    {
      public:
        ...
        Processor(CrowApp& app) :
            Node(app, "/redfish/v1/Systems/system/Processors/<str>/",
                 std::string())
        {
            entityPrivileges = {
                {boost::beast::http::verb::get, {{"Login"}}},
