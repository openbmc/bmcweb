    ...
    for (const auto& interface : cpuInterfacesProperties)
    {
        for (const auto& property : interface.second)
        {
            ...
            else if (property.first == "Type")
            {
                ...
                {
                    messages::internalError(aResp->res);
                    BMCWEB_LOG_DEBUG << "Invalid property type for Cpu Type";
                    continue;
                }
                constexpr const std::array<const char*, 5> values{
                    "Other", "Unknown", "Central", "Math", "DSP"};
                ...
                for (const char* v : values)
                {
                    if (boost::ends_with(*value, v))
                    {
                        if (!strcmp(v, "Central"))
                        {
                            aResp->res.jsonValue["Type"] = "Central Processor";
                        }
                        else
                        {
                            aResp->res.jsonValue["Type"] = v;
                        }
                        break;
                    }
                }
            }
            else if (property.first == "Characteristics")
            {
                ...
                if (value == nullptr)
                {
                    messages::internalError(aResp->res);
                    BMCWEB_LOG_DEBUG
                        << "Invalid property type for Characteristics";
                    continue;
                }
                std::string characteristics;
                for (auto valStr : *value)
                {

                    characteristics +=
                        boost::replace_all_copy(valStr,
                                                "xyz.openbmc_project.Inventory."
                                                "Item.Cpu.Capability.",
                                                "") +
                        ", ";
                }
                aResp->res.jsonValue["Characteristics"] = characteristics;
            }
    ...
    crow::connections::systemBus->async_method_call(
        [objPath, aResp{std::move(aResp)}](
            ...
            for (const auto& property : properties)
            {
                ...
                if (valStr != nullptr)
                {
                    aResp->res.jsonValue[property.first] = *valStr;
                }
            }
        },
...
void getCpuRevisionData(std::shared_ptr<AsyncResp> aResp,
                        const std::string& service, const std::string& objPath)
{
