                          ...
                          const std::string &objPath)
{
    ...
    crow::connections::systemBus->async_method_call(
        [dimmId, aResp{std::move(aResp)}](
            ...
            aResp->res.jsonValue["Metrics"].push_back(
                {{"@odata.id",
                  "/redfish/v1/Systems/system/Memory/" + dimmId + "/Metrics"}});
    ...
    crow::connections::systemBus->async_method_call(
        [dimmId, aResp{std::move(aResp)}](
            ...
            for (const auto &property : properties)
            {
                ...
                {
                    aResp->res.jsonValue["CurrentPeriod"]["BlocksRead"] =
                        property.second;
                }
                else if (property.first == "BlocksWrittenCurrent")
                {
                    aResp->res.jsonValue["CurrentPeriod"]["BlocksWritten"] =
                        property.second;
                }
                else if (property.first == "BlocksReadLifetime")
                {
                    aResp->res.jsonValue["LifeTime"]["BlocksRead"] =
                        property.second;
                }
                else if (property.first == "BlocksWrittenLifetime")
                {
                    aResp->res.jsonValue["LifeTime"]["BlocksWritten"] =
                        property.second;
                }
                ...
                {
                    healthdata["AlarmTrips"]["AddressParityError"] =
                        property.second;
                }
                else if (property.first == "CorrectableECCError")
                {
                    healthdata["AlarmTrips"]["CorrectableECCError"] =
                        property.second;
                }
...
class MemoryMetrics : public Node
{
  public:
    ...
    MemoryMetrics(CrowApp &app) :
        Node(app, "/redfish/v1/Systems/system/Memory/<str>/Metrics/",
             std::string())
    {
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"Login"}}},
