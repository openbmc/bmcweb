...
inline std::string getMetadataPieceForFile(std::filesystem::path filename)
{
    std::string xml;
    ...
    std::string edmx = "edmx:Edmx";
    for (tinyxml2::XMLElement* edmxNode = doc.FirstChildElement(edmx.c_str());
         edmxNode != nullptr;
         edmxNode = edmxNode->NextSiblingElement(edmx.c_str()))
    {
        std::string dataServices = "edmx:DataServices";
        for (tinyxml2::XMLElement* node =
                 edmxNode->FirstChildElement(dataServices.c_str());
             node != nullptr;
             node = node->NextSiblingElement(dataServices.c_str()))
        {
            BMCWEB_LOG_DEBUG("Got data service for {}", pathStr);
            std::string schemaTag = "Schema";
            for (tinyxml2::XMLElement* schemaNode =
                     node->FirstChildElement(schemaTag.c_str());
                 schemaNode != nullptr;
                 schemaNode = schemaNode->NextSiblingElement(schemaTag.c_str()))
            {
