    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    for (const auto& [dramPath, serviceMap] : subtree)
    {
        if (serviceMap.empty())
        {
            BMCWEB_LOG_ERROR("No service for DRAM path {}", dramPath);
            acc->hasError = true;
            return;
        }
    ...
    const std::string& systemName, const std::string& processorId)
{
    ...
    getProcessorObject(
        ...
        [asyncResp](const std::string& processorPath,
            ...
            // The service that owns Processor inventory on this path is
            // expected to also expose MemoryECC. If it does not, the call
            // below returns EBADR and the SRAM-absent path falls through to
            // the DRAM association probe inside afterGetSramEccProperties.
            const std::string& service = serviceMap.begin()->first;
            dbus::utility::getAllProperties(
