...
inline void requestRoutesSystemsCollection(App& app)
{
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/")
        ...
        crow::connections::systemBus->async_method_call(
            ...
            std::string firstNodeStr = firstSystemPath.filename();
            for (const auto& object : objects)
            {
                ...
                if ((nodeStr.find("host") + 1) == std::string::npos)
                {
                    bool defaultSystemNameExists = false;
                    /* by default systemName "system" is added in member array.
                    This check is to ensure to keep only one "system" entry
                    after getting system entries got from object mapper */
                    for (auto& member : members)
                    {
                        if (member["@odata.id"] == "/redfish/v1/Systems/system")
                        {
                            defaultSystemNameExists = true;
                            break;
                        }
                    }

                    if (!defaultSystemNameExists)
                    {
                        systemName = "system";
                    }
                }
...
inline void requestRoutesSystemActionsReset(App& app)
{
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/Actions/ComputerSystem.Reset/")
        ...
        if (hostCommand)
        {
            ...
            transtion = "RequestedHostTransition";
        }
        else
        {
            objectPath =
            ...
            transtion = "RequestedPowerTransition";
        }
        ...
        crow::connections::systemBus->async_method_call(
            ...
            collection_util::getHostServiceName(computerSystemIndex),
            objectPath, "org.freedesktop.DBus.Properties", "Set", interface,
            transtion, dbus::utility::DbusVariantType{command});
        });
}
...
inline void requestRoutesSystems(App& app)
{
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/")
        ...
        getPCIeDeviceList(asyncResp, "PCIeDevices", systemName);
