    ...
    const std::string& systemName, const uint64_t computerSystemIndex)
{
    ...
    if constexpr (!BMCWEB_EXPERIMENTAL_REDFISH_MULTI_COMPUTER_SYSTEM)
    {
        ...
        // TODO: Chassis matching is currently incorrect, needs a direction to
        // match a system on single host to its containing chassis as the
        // default "system" system name is not represented by any inventory
        // object on D-Bus
    ...
    else
    {
        systems_utils::getValidSystemsPath(
            asyncResp, systemName,
            [asyncResp,
             systemName](const std::optional<std::string>& systemPath) {
                if (systemPath)
                {
                    // A system is linked to an array of chassis containing it
                    dbus::utility::getAssociatedSubTreePaths(
                        *systemPath + "/contained_by",
                        sdbusplus::message::object_path(
                            "/xyz/openbmc_project/inventory"),
                        0, chassisInterfaces,
                        std::bind_front(getLinkedChassis, asyncResp,
                                        systemName));
                }
            });
    }
