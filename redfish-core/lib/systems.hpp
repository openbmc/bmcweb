...
inline void getStopBootOnFault(const std::shared_ptr<bmcweb::AsyncResp>& aResp)
{
    ...
    crow::connections::systemBus->async_method_call(
        [aResp](
            ...
            {
                BMCWEB_LOG_DEBUG
                    << "DBUS response error on Stop Boot On Fault  GetSubTree "
                    << ec;
                // This is an optional D-Bus object so just return if error
                // occurs
                return;
            }
            if (subtree.size() == 0)
                         ...
                         const std::string& stopBootOnFaultString)
{
    ...
    {
        validstopBootEnabled = true;
    }
    else if (stopBootOnFaultString == "Never")
    {
        validstopBootEnabled = false;
    }
    else
    {
        BMCWEB_LOG_DEBUG << "Invalid property value for StopBootOnFault: "
                         << stopBootOnFaultString;
        messages::propertyValueNotInList(aResp->res, stopBootOnFaultString,
                                         "StopBootOnFault");
    }
    return validstopBootEnabled;
}
