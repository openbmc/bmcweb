    ...
    crow::connections::systemBus->async_method_call(
        [aResp,
         retryAttempts](const boost::system::error_code ec,
            ...
            if (subtree.size() < 3)
            {
                BMCWEB_LOG_DEBUG << "DBUS response did not report 3 paths"
            ...
            if (policyServ.empty() || attemptServ.empty())
            {
            ...
            sdbusplus::asio::getProperty<bool>(
                *crow::connections::systemBus, policyServ, policyPath,
                "xyz.openbmc_project.Control.Boot.RebootPolicy", "AutoReboot",
                [aResp, retryAttempts, attemptPath,
                 attemptServ](const boost::system::error_code ec2,
                    ...
                    {
                        BMCWEB_LOG_DEBUG
                            << "Failed to set AutomaticRebootAttempts. AutomaticReboot"
                            << "is not enabled";
                    }
                });
        },
