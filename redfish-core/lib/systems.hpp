                             ...
                             const std::string& service)>& callback)
{
    ...
    if (subtree.empty())
    {
        BMCWEB_LOG_ERROR("Can't find system D-Bus object!");
        messages::internalError(asyncResp->res);
        return;
    }
    ...
    if (subtree.size() > 1)
    {
        BMCWEB_LOG_ERROR("Found more than 1 system D-Bus object!");
        messages::internalError(asyncResp->res);
        return;
    }
    ...
    if (subtree[0].first.empty() || subtree[0].second.size() != 1)
    {
        BMCWEB_LOG_ERROR("Error getting system path!");
        messages::internalError(asyncResp->res);
        return;
    }
    const std::string& path = subtree[0].first;
    ...
    if (service.empty())
    {
        BMCWEB_LOG_ERROR("Error getting system service!");
        messages::internalError(asyncResp->res);
        return;
    }
