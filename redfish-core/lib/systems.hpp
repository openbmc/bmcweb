...
static void getBootSource(std::shared_ptr<AsyncResp> aResp, bool oneTimeEnabled)
{
    ...
    crow::connections::systemBus->async_method_call(
        [aResp, bootDbusObj](const boost::system::error_code ec,
            ...
            aResp->res.jsonValue["Boot"]["BootSourceOverrideTarget"] =
                dbusToRfBootSource(*bootSourceStr);
        },
        "xyz.openbmc_project.Settings", bootDbusObj,
        "org.freedesktop.DBus.Properties", "Get",
                                ...
                                std::optional<std::string> bootEnable)
{
    ...
    bool oneTimeSetting =
        (bootEnable) ? (*bootEnable == "Once") : oneTimeEnabled;
    ...
    struct DbusParams
    {
        ...
        DbusParams(const char *o, const std::string &i, const std::string &p,
                   const std::string &v) :
            obj(o),
    ...
    std::vector<DbusParams> busParams;

    BMCWEB_LOG_DEBUG << "Got source: " << *bootSource
                     << ", enable: " << *bootEnable;

    // Figure out what properties to set
    if (!bootSource && bootEnable)
    {
        BMCWEB_LOG_DEBUG << "Boot source enabled: " << *bootEnable;
    ...
    else if (bootSource)
    {
        ...
        auto bootSourceStr = rfToDbusBootSource(*bootSource);
        auto bootModeStr = rfToDbusBootMode(*bootSource);
        ...
        {
            busParams.emplace_back(
                DbusParams{bootObj, "xyz.openbmc_project.Control.Boot.Source",
        ...
        {
            busParams.emplace_back(
                DbusParams{bootObj, "xyz.openbmc_project.Control.Boot.Mode",
    ...
    // Do bus operations
    for (const auto &busParams : busParams)
    {
        BMCWEB_LOG_DEBUG << "DBUS args:: " << busParams.obj << ", "
                         << busParams.iface << ", " << busParams.prop << ", "
                         << busParams.val;
        ...
        crow::connections::systemBus->async_method_call(
            ...
            "xyz.openbmc_project.Settings", busParams.obj,
            "org.freedesktop.DBus.Properties", "Set", busParams.iface,
            busParams.prop,
            sdbusplus::message::variant<std::string>{busParams.val});
    }
    crow::connections::systemBus->async_method_call(
        [aResp{std::move(aResp)}](const boost::system::error_code ec) {
            if (ec)
