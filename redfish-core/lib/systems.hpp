...
/**
 * @brief checks to see if autoreboot policy is enabled.
 *
 * @return true if autoreboot is enabled otherwise false.
*/
inline bool autoRebootPolicyEnabled(const std::shared_ptr<bmcweb::AsyncResp>& aResp)
{
    ...
    sdbusplus::asio::getProperty<bool>(
        *crow::connections::systemBus, "xyz.openbmc_project.Settings",
        ...
        [aResp, &autoReboot](const boost::system::error_code ec, bool autoRebootEnabled) {
            if (ec)
            {
                BMCWEB_LOG_DEBUG << "D-BUS response error " << ec;
                return false;
 ...
 * @return None.
 */
inline void getAutomaticRetryAttempts(const std::shared_ptr<bmcweb::AsyncResp>& aResp)
{
    ...
    crow::connections::systemBus->async_method_call(
        [aResp](const boost::system::error_code ec,
            ...
            sdbusplus::asio::getProperty<uint32_t>(
                            *crow::connections::systemBus, serv,
                            path,
 ...
 * @return None.
 */
inline void getAutomaticRetryPolicy(const std::shared_ptr<bmcweb::AsyncResp>& aResp)
{
    BMCWEB_LOG_DEBUG << "Get Automatic Retry policy";

    auto autoRebootEnabled = autoRebootPolicyEnabled(aResp);
                              ...
                              const uint32_t& retryAttempts)
{
    ...
    auto autoReboot = autoRebootPolicyEnabled();
    if (!autoReboot)
    {
        BMCWEB_LOG_DEBUG << "Cannot set RetryAttempts, autoReboot disabled";
        //Set not allowed, return gracefully.
        return;
    }
    crow::connections::systemBus->async_method_call(
        [aResp,
