                ...
                {
                    {
                        ...
                        /* by default systemName "system" is added in member
                        array. This check is to ensure to keep only one "system"
                        entry after getting system entries got from object
                        mapper */
                    ...
                    if (nodeStr.find("host") != std::string::npos)
                    {
                        ...
                        nodeStr.erase(
                            remove(nodeStr.begin(), nodeStr.end(), '\"'),
                            nodeStr.end());
        ...
        BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/")
            ...
            crow::connections::systemBus->async_method_call(
                ...
                for (const auto& object : objects)
                {
                    std::string service = object.first;
                    auto location =
                        service.rfind("xyz.openbmc_project.State.Host");
                    if (location != std::string::npos)
                    {
                ...
                BMCWEB_LOG_DEBUG << "host dbus service name "
                                 << hostDbusService;
                asyncResp->res.addHeader(
                    boost::beast::http::field::link,
                    "</redfish/v1/JsonSchemas/ComputerSystem/ComputerSystem.json>; rel=describedby");
                boost::urls::url computerSystemURL =
                ...
                asyncResp->res
                    .jsonValue["ProcessorSummary"]["Status"]["State"] =
                    "Disabled";
                asyncResp->res
                    .jsonValue["MemorySummary"]["TotalSystemMemoryGiB"] =
                    uint64_t(0);
                asyncResp->res.jsonValue["MemorySummary"]["Status"]["State"] =
                    "Disabled";
                asyncResp->res.jsonValue["@odata.id"] =
                    computerSystemURL.string();
                ...
                asyncResp->res
                    .jsonValue["SerialConsole"]["IPMI"]["ServiceEnabled"] =
                    true;

                // TODO (Gunnar): Should look for obmc-console-ssh@2200.service
                asyncResp->res
                    .jsonValue["SerialConsole"]["SSH"]["ServiceEnabled"] = true;
