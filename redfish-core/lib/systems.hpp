...
inline void getIdlePowerSaver(const std::shared_ptr<bmcweb::AsyncResp>& aResp)
{
    ...
    crow::connections::systemBus->async_method_call(
        [aResp](
            ...
            // Valid Power IdlePowerSaver object found, now read the current
            // value
            //
                ...
                [aResp](const boost::system::error_code ec,
                    ...
                    for (const auto& property : properties)
                    {
                        ...
                        else if (property.first == "EnterDwellTime")
                        {
                            ...
                            const std::chrono::milliseconds ms(*time_ms);
        ...
        [aResp, ipsEnable, ipsEnterUtil, ipsEnterTime, ipsExitUtil,
         ipsExitTime](
            ...
            if (ipsEnterTime)
            {
                ...
                const uint64_t time_ms = *ipsEnterTime * 1000;
                crow::connections::systemBus->async_method_call(
                    [aResp](const boost::system::error_code ec) {
                        if (ec)
                        {
                    ...
                    service, path, "org.freedesktop.DBus.Properties", "Set",
                    "xyz.openbmc_project.Control.Power.IdlePowerSaver",
                    "EnterDwellTime", std::variant<uint64_t>(time_ms));
            }
            if (ipsExitUtil)
            {
                crow::connections::systemBus->async_method_call(
            ...
            {
                // Convert from seconds into milliseconds for DBus
                const uint64_t time_ms = *ipsExitTime * 1000;
                crow::connections::systemBus->async_method_call(
                    [aResp](const boost::system::error_code ec) {
                        if (ec)
                        {
                    ...
                    service, path, "org.freedesktop.DBus.Properties", "Set",
                    "xyz.openbmc_project.Control.Power.IdlePowerSaver",
                    "ExitDwellTime", std::variant<uint64_t>(time_ms));
            }
        },
        "xyz.openbmc_project.ObjectMapper",
        "/xyz/openbmc_project/object_mapper",
