 ...
 * @return None.
 */
inline std::string getPowerModeObject(const std::shared_ptr<bmcweb::AsyncResp>& aResp)
{
    std::string pmObj;
    ...
    crow::connections::systemBus->async_method_call(
        [aResp,pmObj](
            ...
            if (resp.size() == 0)
            {
                ...
                BMCWEB_LOG_DEBUG << "DEBUG: No PowerMode Object (ok - optional)";
                return pmObj;
            }
            ...
            if (resp.size() > 1)
            {
                ...
                // error
                BMCWEB_LOG_DEBUG << "DEBUG: Multiple PowerMode Objects! size: " << resp.size();
                messages::internalError(aResp->res);
                return pmObj;
            }
