...
inline void populateComputerSystemInfo(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp, std::string systemName,
    ...
    std::string hostDbusService)
{
    ...
    asyncResp->res.jsonValue["@odata.id"] = computerSystemURL.buffer();

    asyncResp->res.jsonValue["Processors"] = {
        {"@odata.id", processorURL.buffer()}};
    asyncResp->res.jsonValue["Memory"] = {{"@odata.id", memoryURL.buffer()}};
    asyncResp->res.jsonValue["Storage"] = {{"@odata.id", storageURL.buffer()}};
    ...
    asyncResp->res.jsonValue["LogServices"] = {
        {"@odata.id", crow::utility::urlFromPieces("redfish", "v1", "Systems",
