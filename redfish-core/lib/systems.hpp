...
void modifyCpuPresenceState(std::shared_ptr<AsyncResp> aResp,
  ...
  if (*isCpuPresent == true) {
    ...
    auto procCountPtr = procCount.get_ptr<nlohmann::json::number_integer_t *>();
          ...
          for (const auto &connection : connectionNames) {
            for (const auto &interfaceName : connection.second) {
              if (interfaceName == "xyz.openbmc_project.Inventory.Item.Dimm") {
                ...
                crow::connections::systemBus->async_method_call(
                    [aResp, service{connection.first}, path(std::move(path))](
                        const boost::system::error_code ec,
                      ...
                      if (properties.size() > 0) {
                        for (const std::pair<std::string, VariantType>
                          ...
                          if (value == nullptr) {
                            BMCWEB_LOG_DEBUG << "Find incorrect type of "
                                                "MemorySize";
                            continue;
                          }
                          nlohmann::json &totalMemory =
                              aResp->res.jsonValue["MemorySummar"
                                                   "y"]["TotalSystemMe"
                                                        "moryGiB"];
                          uint64_t *preValue =
                          ...
                          aResp->res
                              .jsonValue["MemorySummary"]["TotalSystemMemoryGi"
                                                          "B"] =
                              *value / (1024 * 1024) + *preValue;
                          aResp->res
                              .jsonValue["MemorySummary"]["Status"]["State"] =
                              "Enabled";
                        }
                      } else {
                        auto getDimmProperties =
                            [aResp](const boost::system::error_code ec,
                                    const std::variant<bool> &dimmState) {
                              if (ec) {
                                BMCWEB_LOG_ERROR << "DBUS response "
                                                    "error "
                                                 << ec;
                                return;
                              }
                        ...
                        crow::connections::systemBus->async_method_call(
                            std::move(getDimmProperties), service, path,
                      ...
                      if (properties.size() > 0) {
                            ...
                            if (value != nullptr) {
                              nlohmann::json &procSummary =
                                  aResp->res.jsonValue["ProcessorSumm"
                                                       "ary"];
                              nlohmann::json &procCount = procSummary["Count"];
                              ...
                              procSummary["Status"]["State"] = "Enabled";
                        ...
                        auto getCpuFunctionalState =
                            [aResp](
                              ...
                              modifyCpuFunctionalState(aResp,
                                                       cpuFunctionalCheck);
                            };
                ...
                crow::connections::systemBus->async_method_call(
                    [aResp](
                      ...
                      fw_util::getActiveFwVersion(aResp, fw_util::biosPurpose,
                                                  "BiosVersion");
                    },
                    connection.first, path, "org.freedesktop.DBus.Properties",
                    "GetAll",
...
static void getBootMode(std::shared_ptr<AsyncResp> aResp,
  ...
  crow::connections::systemBus->async_method_call(
      [aResp](const boost::system::error_code ec,
        ...
        const std::string *bootModeStr = std::get_if<std::string>(&bootMode);
        ...
        if (*bootModeStr !=
          ...
          if (!rfMode.empty()) {
            aResp->res.jsonValue["Boot"]["BootSourceOverrideTarget"] = rfMode;
          }
        }
 ...
 * @return None.
 */
static void getBootSource(std::shared_ptr<AsyncResp> aResp,
                          bool oneTimeEnabled) {
  std::string bootDbusObj =
...
static void setBootModeOrSource(std::shared_ptr<AsyncResp> aResp,
  ...
  const char *bootObj = oneTimeSetting
                            ? "/xyz/openbmc_project/control/host0/boot/one_time"
                            : "/xyz/openbmc_project/control/host0/boot";
 ...
 */
static void
setWDTProperties(std::shared_ptr<AsyncResp> aResp,
...
private:
  ...
  void doGet(crow::Response &res, const crow::Request &req,
    ...
    res.jsonValue["Bios"] = {{"@odata.id", "/redfish/v1/Systems/system/Bios"}};
  ...
  void doPatch(crow::Response &res, const crow::Request &req,
    ...
    if (wdtTimerProps) {
      ...
      if (!json_util::readJson(*wdtTimerProps, asyncResp->res,
                               "FunctionEnabled", wdtEnable, "TimeoutAction",
                               wdtTimeOutAction)) {
        return;
      }
