...
inline void getPowerMode(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    dbus::utility::getSubTree(
        ...
        [asyncResp](const boost::system::error_code& ec,
            ...
            if (subtree.size() > 1)
            {
                // More than one PowerMode object is not supported and is an
                // error
                BMCWEB_LOG_DEBUG(
    ...
    dbus::utility::getSubTree(
        ...
        [asyncResp,
         powerMode](const boost::system::error_code& ec,
            ...
            if (subtree.size() > 1)
            {
                // More then one PowerMode object is not supported an d is an
                // error
                BMCWEB_LOG_DEBUG(
                    "Found more than 1 system D-Bus Power.Mode objects: {}",
                    subtree.size());
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    dbus::utility::getSubTree(
        ...
        [asyncResp](const boost::system::error_code& ec,
            ...
            if (subtree.size() > 1)
            {
                // More than one PowerIdlePowerSaver object is not supported and
                // is an error
