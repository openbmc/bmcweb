 ...
 * corresponding property "AttemptsLeft" that keeps track of the amount of automatic
 * retry attempts left are hosted in phosphor-state-manager through dbus. These
 * values only need to be retrieved if auto reboot is enabled on the system.
 *
 * @param[in] aResp     Shared pointer for generating response message.
 *
 * @return None.
