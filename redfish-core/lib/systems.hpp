...
inline void getProcessorSummary(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                                const std::string& service_cpu, const std::string& path)
{

    crow::connections::systemBus->async_method_call(
        [aResp, service{service_cpu}, path](const boost::system::error_code ec2)
        {
            if (ec2)
            {
            ...
            auto getCpuModel = [aResp](const boost::system::error_code ec3,
                                       const std::variant<std::string>& model) {
                if (ec3)
                {
                    BMCWEB_LOG_ERROR << "DBUS response error " << ec3;
        ...
        service_cpu, path, "org.freedesktop.DBus.Properties", "GetAll",
