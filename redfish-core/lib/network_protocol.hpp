                    ...
                    const crow::Request& req)
{
    ...
    for (const auto& protocol : protocolToDBusService)
    {
        const std::string& keyValue = protocol.first;
        getPortStatusAndPath(protocol.second, [asyncResp, keyValue](
                                                  const std::string& socketPath,
                                                  bool isProtocolEnabled) {
            asyncResp->res.jsonValue[keyValue]["ProtocolEnabled"] =
                isProtocolEnabled;
            if (isProtocolEnabled)
            {
                getPortNumber(
                    socketPath, [asyncResp, keyValue](int portNumber) {
                        asyncResp->res.jsonValue[keyValue]["Port"] = portNumber;
                    });
            }
        });
    }
