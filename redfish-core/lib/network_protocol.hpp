...
class NetworkProtocol : public Node
{
  ...
  private:
    ...
    void getData(const std::shared_ptr<AsyncResp>& asyncResp)
    {
        ...
        // preparation
        getEthernetIfaceData([this, asyncResp](
                                 const bool& success,
                                 const std::vector<std::string>& ntpServers,
                                 const std::vector<std::string>& domainNames) {
            if (!success)
            ...
            if ((asyncResp->res.jsonValue["HostName"]
                     .get<std::string>()
                     .empty() == false) &&
                (domainNames.empty() == false))
            {
                asyncResp->res.jsonValue["FQDN"] =
                    asyncResp->res.jsonValue["HostName"].get<std::string>() +
                    "." + domainNames[0];
            }
        });
