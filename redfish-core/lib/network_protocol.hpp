                                        ...
                                        std::vector<std::string>& dnData)
{
    ...
    ntpData.erase(stl_utils::hasDuplicates(ntpData.begin(), ntpData.end()),
                  ntpData.end());
    dnData.erase(stl_utils::hasDuplicates(dnData.begin(), dnData.end()),
                 dnData.end());
}
...
inline void
    handleNTPServersPatch(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                          std::vector<std::string>& ntpServers)
{
    auto iter = stl_utils::hasDuplicates(ntpServers.begin(), ntpServers.end());
    if (iter != ntpServers.end())
    {
        messages::propertyValueIncorrect(asyncResp->res, "NTPServers", *iter);
        return;
    }
            ...
            [](const crow::Request& req,
               const std::shared_ptr<bmcweb::AsyncResp>& asyncResp) {
                ...
                if (ntp)
                {
                    ...
                    if (ntpServers)
                    {
                        std::vector<std::string> ntpServersVec = *ntpServers;
                        ntpServersVec.erase(
                            stl_utils::hasDuplicates(ntpServersVec.begin(),
                                                     ntpServersVec.end()),
                            ntpServersVec.end());
                        handleNTPServersPatch(asyncResp, ntpServersVec);
                    }
                }
