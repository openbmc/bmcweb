                          ...
                          std::vector<std::string> currentNtpServers)
{
    ...
    for (const auto& server : currentNtpServers)
    {
        BMCWEB_LOG_DEBUG << "Setting ntp server " << server;
    }
    BMCWEB_LOG_DEBUG << "Setting " << currentNtpServers.size() << " servers";
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Managers/bmc/NetworkProtocol/")
        ...
        if (ntpServerObjects)
        {
            getEthernetIfaceData(
                ...
                stl_utils::removeDuplicate(currentNtpServers);
