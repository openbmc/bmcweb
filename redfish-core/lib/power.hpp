...
struct PowerMetrics : std::enable_shared_from_this<PowerMetrics>
{
    ...
    void getAverage(const std::string service,
        ...
        crow::connections::systemBus->async_method_call(
            [self](const boost::system::error_code ec,
                   const std::variant<std::vector<
                       std::tuple<uint64_t, int64_t>>>& history)
            {
                if (ec)
                {
    ...
    void getMaximum(const std::string service,
                    const std::string path)
    {
        std::shared_ptr<PowerMetrics> self = shared_from_this();
        // Get aggregation history
        crow::connections::systemBus->async_method_call(
                       ...
                       std::tuple<uint64_t, int64_t>>>& history)
            {
                ...
                auto maxCompare = [](const auto& l, const auto& r)
                {
                    return (std::get<int64_t>(l) < std::get<int64_t>(r));
                };

                auto max = std::get<int64_t>(*std::max_element(
                    maxHistory->begin(), maxHistory->end(), maxCompare));
                ...
                {
                    self->maxTotal = max;
                    self->maxMetric = &self->maxTotal;
                }
                else
                {
                    self->maxTotal = self->maxTotal + max;
                }
        ...
        crow::connections::systemBus->async_method_call(
                            ...
                            std::vector<std::string>>>>>& subtree)
            {
                ...
                for (const auto& object : subtree)
                {
                    ...
                    auto objectName = path.substr(objectPos + 1,
                                                  (metricPos - 1) - objectPos);
