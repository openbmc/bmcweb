            ...
            auto chassisHandler = [sensorAsyncResp](
                ...
                auto valueHandler = [sensorAsyncResp](
                    ...
                    auto powerHandler = [sensorAsyncResp](
                        ...
                        if (ec)
                        {
                            BMCWEB_LOG_DEBUG << "DBUS response error on "
                                                "Power.PowerMetric GetSubTree "
                                             << ec;
                            messages::internalError(
                                sensorAsyncResp->asyncResp->res);
                            return;
                        ...
                        if (subtree.size() > 1)
                        {
                            // More then one Power.PowerMetric object is not
                            // supported and is an error
                            BMCWEB_LOG_DEBUG
                                << "Found more than 1 system D-Bus "
                        ...
                        {
                            BMCWEB_LOG_DEBUG
                                << "Power.PowerMetric mapper error!";
                            messages::internalError(
                                sensorAsyncResp->asyncResp->res);
                            return;
                        }
                        ...
                        auto metricHandler = [sensorAsyncResp](
                            ...
                            if (ec)
                            {
                                messages::internalError(
                                    sensorAsyncResp->asyncResp->res);
                                BMCWEB_LOG_ERROR << "Power Metric GetAll "
                                                    "handler: Dbus error "
                                                 << ec;
                                return;
                            }
                                ...
                                if (!property.first.compare("IntervalInMin"))
                                {
                                    ...
                                    if (!iIntervalInMin)
                                    {
                                        messages::internalError(
                                            sensorAsyncResp->asyncResp->res);
                                        BMCWEB_LOG_DEBUG
                                            << "Power.PowerMetric get "
                                               "IntervalInMin property failed";
                                        return;
                                    }
                                    nlohmann::json& value =
