                ...
                auto chassisHandler = [sensorAsyncResp](
                    ...
                    auto valueHandler = [sensorAsyncResp](
                        ...
                        [sensorAsyncResp](
                            ...
                            {
                                BMCWEB_LOG_DEBUG
                                    << "DBUS response error on Power.PowerMetric GetSubTree "
                                    << ec;
                            ...
                            if (subtree.size() > 1)
                            {
                                // More then one Power.PowerMetric object is not supported and
                                // is an error
                                BMCWEB_LOG_DEBUG << "Found more than 1 system D-Bus "
                                                    "Power.PowerMetric objects: "
                                                 << subtree.size();
                                messages::internalError(sensorAsyncResp->asyncResp->res);
                                return;
                            }
                            ...
                            {
                                BMCWEB_LOG_DEBUG << "Power.PowerMetric mapper error!";
                                messages::internalError(sensorAsyncResp->asyncResp->res);
                                return;
                            }
                            ...
                            {
                                BMCWEB_LOG_DEBUG
                                    << "Power.PowerMetric service mapper error!";
                                messages::internalError(sensorAsyncResp->asyncResp->res);
                                return;
                            }
                            ...
                            [sensorAsyncResp](
                                    ...
                                    properties)
                                {
                                    ...
                                    {
                                        const uint64_t* iIntervalInMin = std::get_if<uint64_t>(&property.second);
                                        if (!iIntervalInMin)
                                        {
                                            messages::internalError(sensorAsyncResp->asyncResp->res);
                                            BMCWEB_LOG_DEBUG
                                                << "Power.PowerMetric get IntervalInMin property failed";
                                            return;
                                        }
                                        nlohmann::json& value = sensorJson["PowerMetrics"]["IntervalInMin"];
                                        value = *iIntervalInMin;
                                    }
                                    else if (!property.first.compare("MinConsumedWatts"))
                                    {
                                        const uint16_t* iMinConsumedWatts = std::get_if<uint16_t>(&property.second);
                                        if (!iMinConsumedWatts)
                                        {
                                            messages::internalError(sensorAsyncResp->asyncResp->res);
                                            BMCWEB_LOG_DEBUG
                                                << "Power.PowerMetric get MinConsumedWatts property failed";
                                            return;
                                        }
                                        nlohmann::json& value = sensorJson["PowerMetrics"]["MinConsumedWatts"];
                                        value = *iMinConsumedWatts;
                                    }
                                    else if (!property.first.compare("MaxConsumedWatts"))
                                    {
                                        const uint16_t* iMaxConsumedWatts = std::get_if<uint16_t>(&property.second);
                                        if (!iMaxConsumedWatts)
                                        {
                                            messages::internalError(sensorAsyncResp->asyncResp->res);
                                            BMCWEB_LOG_DEBUG
                                                << "Power.PowerMetric get MaxConsumedWatts property failed";
                                            return;
                                        }
                                        nlohmann::json& value = sensorJson["PowerMetrics"]["MaxConsumedWatts"];
                                        value = *iMaxConsumedWatts;
                                    }
                                    else if (!property.first.compare("AverageConsumedWatts"))
                                    {
                                        const uint16_t* iAverageConsumedWatts = std::get_if<uint16_t>(&property.second);
                                        if (!iAverageConsumedWatts)
                                        {
                                            messages::internalError(sensorAsyncResp->asyncResp->res);
                                            BMCWEB_LOG_DEBUG
                                                << "Power.PowerMetric get AverageConsumedWatts property failed";
                                            return;
                                        }
                                        nlohmann::json& value = sensorJson["PowerMetrics"]["AverageConsumedWatts"];
                        ...
                        crow::connections::systemBus->async_method_call(
                            ...
                            "xyz.openbmc_project.ObjectMapper", "GetSubTree", "/xyz/openbmc_project/Power", int32_t(0),
