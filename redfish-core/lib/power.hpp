        ...
        std::vector<nlohmann::json>& powerControlCollections)
    {
        auto getChassisPath =
            [asyncResp, powerControlCollections](
                ...
                // powerControlCollections in lambda function is const, but the
                // first parameter of readJson should not be const. So do a
                // copy.
                auto collections = powerControlCollections;
                // Only host0 exists at present. Report an error if the size of
                // collections doesn't equal to 1
