...
<<<<<<< PATCH SET (982863 Use generic service to set/get values from PowerCap interfac)
    uint32_t valueToSet, std::string powerCapService,
    const boost::system::error_code& ec, bool powerCapEnable)
=======
    uint32_t valueToSet, const boost::system::error_code& ec,
    bool powerCapEnable)
>>>>>>> BASE      (e8fb0a Clean up power schema)
{
    if (ec)
    {
...
<<<<<<< PATCH SET (982863 Use generic service to set/get values from PowerCap interfac)
inline void afterDbusObjectGet(
    const std::shared_ptr<SensorsAsyncResp>& sensorsAsyncResp,
    uint32_t valueToSet, const boost::system::error_code& ec,
    ...
    const std::optional<std::string>& chassisPath)
{
    ...
    dbus::utility::getDbusObject(
        ...
        std::bind_front(afterDbusObjectGet, sensorsAsyncResp, *value));
}
...
inline void afterDbusObjectPatch(
    const std::shared_ptr<SensorsAsyncResp>& sensorsAsyncResp, const boost::system::error_code& ec,
    const dbus::utility::MapperGetObject& objects)
{
    std::string powerCapService;
    if (ec)
                    ...
                    const Mapper& chassisPaths)
{
    ...
    dbus::utility::getDbusObject(
        "/xyz/openbmc_project/control/host0/power_cap", interfaces,
        std::bind_front(afterDbusObjectPatch, sensorAsyncResp));
=======
inline void afterGetChassisPath(
    const std::shared_ptr<SensorsAsyncResp>& sensorsAsyncResp,
    std::vector<nlohmann::json>& powerControlCollections,
