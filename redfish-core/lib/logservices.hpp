...
using GetAllPropertiesType = PropertiesType;
 ...
 private:
  ...
  void extractLogEntryInterfaceData(const std::string &entry_id,
    ...
    if (entry_properties != nullptr) {
      ...
      entry_data.severity =
          translateSeverityBetweenDBusAndRedfish(severity, true);
      // Retrieve Message property
      entry_data.message =
          extractProperty<std::string>(*entry_properties, "Message");
      // Retrieve Resolved property
      entry_data.resolved =
          extractProperty<bool>(*entry_properties, "Resolved");
    }
 ...
 public:
  ...
  void getLogEntryIfaceData(const std::string &entry_id,
    ...
    auto resp_handler = [&, entry_id, callback{std::move(callback)} ](
      ...
      BMCWEB_LOG_DEBUG << "getLogEntriesIfaceData resp_handler callback Done";
      LogEntryInterfaceData entry_data{};
   ...
   */
  template <typename CallbackFunc>
  void getLogEntriesIfaceList(const std::shared_ptr<AsyncResp> &asyncResp,
                              CallbackFunc &&callback) {
    // Process response for Logging service and extract interface list
    auto resp_handler = [ asyncResp, callback{std::move(callback)} ](
        const boost::system::error_code ec, GetManagedObjectsType &resp) {
      BMCWEB_LOG_DEBUG << "getLogEntriesIfaceList resp_handler callback Done";
      ...
      if (ec) {
        // TODO Handle for specific error code
        BMCWEB_LOG_ERROR << "getLogEntriesIfaceList resp_handler got error "
                         << ec;
      ...
      for (auto &objpath : resp) {
        ...
        for (auto &interface : objpath.second) {
          ...
          if (interface.first == "xyz.openbmc_project.Logging.Entry") {
            ...
            BMCWEB_LOG_DEBUG << "Found iface: " << iface_id;
...
class LogEntry : public Node {
 public:
  ...
  LogEntry(CrowApp &app)
    ...
    Node::json["@odata.context"] = "/redfish/v1/$metadata#LogEntry.LogEntry";
 ...
 private:
  ...
  void doGet(crow::Response &res, const crow::Request &req,
    ...
    Node::json["@odata.id"] =
        "/redfish/v1/Systems/1/LogServices/SEL/Entries/" + entry_id;
...
class LogEntryCollection : public Node {
 ...
 private:
  ...
  void doGet(crow::Response &res, const crow::Request &req,
    ...
    // Process callback to prepare JSON payload
    auto callback = [asyncResp](const std::vector<std::string> &iface_list) {
      nlohmann::json iface_array = nlohmann::json::array();
      for (const std::string &iface_item : iface_list) {
        iface_array.push_back(
            {{"@odata.id",
              "/redfish/v1/Systems/1/LogServices/SEL/Entries/" + iface_item}});
      }
      asyncResp->res.jsonValue["Members"] = iface_array;
      asyncResp->res.jsonValue["Member@odata.count"] = iface_array.size();
    };
...
class LogService : public Node {
 ...
 private:
  ...
  void doGet(crow::Response &res, const crow::Request &req,
    ...
    // TODO Logging service has not supported get MaxNumberOfRecords property
    // yet hardcode to ERROR_CAP (200) from phosphor-logging.
    ...
    Node::json["DateTimeLocalOffset"] = redfishDateTime.substr(
        redfishDateTime.length() - 6);  // Time Zone position
