              ...
              for (auto &propertyMap : interfaceMap.second) {
                ...
                if (propertyMap.first == "Id") {
                  ...
                  const uint64_t *millisTimeStamp =
                      mapbox::getPtr<const uint64_t>(propertyMap.second);
                  ...
                  const std::string *severity =
                      mapbox::getPtr<const std::string>(propertyMap.second);
                  if (severity != nullptr) {
                    thisEntry["Severity"] =
                  ...
                  const bool *resolved =
                      mapbox::getPtr<const bool>(propertyMap.second);
                  if (resolved != nullptr) {
                    // No place to put this for now
                  }
                } else {
                  BMCWEB_LOG_WARNING
                      << "Got extra property in log entry interface "
                      << propertyMap.first;
                }
              }
              entriesArray.push_back(std::move(thisEntry));
              // Not sure if this is neccesary, but technically move could leave
              // thisEntry in any state it wants
              thisEntry.clear();
            }
          }
        },
...
class LogServiceActionsClear : public Node {
 public:
  LogServiceActionsClear(CrowApp &app)
      : Node(app,
             "/redfish/v1/Systems/1/LogServices/SEL/Actions/"
             "LogService.Reset") {
    entityPrivileges = {
        {boost::beast::http::verb::get, {{"Login"}}},
        {boost::beast::http::verb::head, {{"Login"}}},
