...
constexpr const char *defaultGlobalPath = "-";

// default to illegal dbus path
static std::string globalInventoryPath = defaultGlobalPath;
...
struct HealthPopulate : std::enable_shared_from_this<HealthPopulate>
{
    ...
    void getGlobalPath()
    {
        if (globalInventoryPath != defaultGlobalPath)
        {
            return;
        }
        ...
        crow::connections::systemBus->async_method_call(
            [self](const boost::system::error_code ec,
                ...
                globalInventoryPath = std::move(resp[0]);
            },
