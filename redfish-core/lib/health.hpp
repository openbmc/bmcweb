...
class HealthRollup
{
    ...
    {
        if (state == "xyz.openbmc_project.State.Decorator.Health.State.OK")
        {
            return "OK";
        }
        if (state == "xyz.openbmc_project.State.Decorator.Health.State.Warning")
        {
            return "Warning";
        }
        if (state ==
            "xyz.openbmc_project.State.Decorator.Health.State.Critical")
        {
            return "Critical";
        }
        return "";
  ...
  public:
    ...
    bool setStatus(const std::shared_ptr<bmcweb::AsyncResp>& aResp)
    {
        if (valid)
        {
