...
constexpr std::string_view bmcRedfishHostIntfUriName = "rhi";
...
{
    std::unordered_map<std::string, std::string> credentialRoleIdToUserRoleId = {
    ...
    if (roleId != nullptr)
    {
        std::string userRole = "Administrator";
        if (credentialRoleIdToUserRoleId.contains(*roleId))
        {
            userRole = credentialRoleIdToUserRoleId[*roleId];
        }
...
inline void handleHostInterfacePatch(
    ...
    const std::string& managerId, const std::string& hostInterfaceId)
{
    std::unordered_map<std::string, std::string> credentialRoleIdFromUserRoleId = {
        {"Administrator",
         "xyz.openbmc_project.HostInterface.CredentialBootstrapping.Role.Administrator"},
        {"Operator",
         "xyz.openbmc_project.HostInterface.CredentialBootstrapping.Role.Operator"},
    ...
    if (enabled)
    {
        crow::connections::systemBus->async_method_call(
            ...
            "xyz.openbmc_project.HostInterface.CredentialBootstrapping",
            "CredentialEnabled", dbus::utility::DbusVariantType(*enabled));
    }
