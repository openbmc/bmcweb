    ...
    const dbus::utility::DBusPropertiesMap& properties)
{
    ...
    const bool success = sdbusplus::unpackPropertiesNoThrow(
        ...
        enabledAfterReset, "CredentialBootstrappingEnabled", enabled,
        "CredentialBootstrappingRole", roleId);
    if (!success)
    {
        messages::internalError(asyncResp->res);
        return;
    ...
    const std::string& managerId, const std::string& hostInterfaceId)
{
    ...
    asyncResp->res.jsonValue["HostInterfaceType"] =
        host_interface::HostInterfaceType::NetworkHostInterface;
