...
class SubscriptionCollection : public Node
{
  ...
  private:
    ...
    {
        res.jsonValue = {
            {"@odata.type", "#EventDestinationCollection.v1_0_0.EventDestinationCollection"},
...
class ManagerSubscription : public Node
{
  public:
    ManagerSubscription(CrowApp& app) :
        Node(app, "/redfish/v1/EventService/Subscriptions/<str>/", std::string())
    {
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"Login"}}},
            {boost::beast::http::verb::head, {{"Login"}}},
  ...
  private:
               ...
               const std::vector<std::string>& params) override
    {
        ...
        res.jsonValue = {
            ...
            {"MessageIds", {"DCPowerOff", "DCPowerOn" , "InventoryAdded", "InventoryRemoved"}},
