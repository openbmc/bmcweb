    ...
    const std::string& reqFeature, const std::string& licenseString)
{
    ...
    crow::connections::systemBus->async_method_call(
        std::move(createCPULicenseTaskCallback), sdsiService, objectPath,
         ...
         oemAuthFeatureType, licenseString, authDevice](
            ...
            for (const auto& [objectPath, serviceMap] : subtree)
            {
                ...
                createCPULicense(std::move(payload), asyncResp, objectPath,
                                 processorId, oemAuthFeatureType,
                                 licenseString);
                return;
            }
                            ...
                            const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    asyncResp->res.jsonValue["LicenseExpirationWarningDays"] = 0;
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    licenseService::addCPUProvisionDynamicLicenseCollection(asyncResp);
}
