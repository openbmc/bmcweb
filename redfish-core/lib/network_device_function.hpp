    ...
    const dbus::utility::MapperGetSubTreeResponse& object)
{
    if (ec)
    {
        ...
        {
            BMCWEB_LOG_ERROR("D-Bus response error on assigned_to: {}",
                             ec.message());
            messages::internalError(asyncResp->res);
        }
        return;
    ...
    // A successful query that returns no assigned ports (distinct from the
    // EBADR "no association" error handled above) still lands here; return
    // early so the *object.begin() dereference below is not on an empty range.
    if (object.empty())
    {
        return;
    }
    ...
    for (const auto& [portPath, services] : object)
    {
        if (services.size() != 1)
        {
            BMCWEB_LOG_ERROR(
                "Expected exactly one service for port {}, found {}", portPath,
                services.size());
            messages::internalError(asyncResp->res);
            return;
        }
        const std::string portId = sdbusplus::object_path(portPath).filename();
        BMCWEB_LOG_DEBUG("Adding assignable physical network port {}", portId);
    ...
    // PortProtocol is shared by every assignable port under this NDF, so
    // query it from the first port only.
                         ...
                         const std::string& ndfId)
{
    ...
    getNetworkAdapterPath(
        ...
        std::bind_front(
            getAssociatedSubResourceById, asyncResp,
...
inline void afterGetNetworkAdapterPathForNDFCollection(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& chassisId, const std::string& networkAdapterId,
    ...
    const std::string& chassisId, const std::string& networkAdapterId)
{
    ...
    getNetworkAdapterPath(
        asyncResp, chassisId, networkAdapterId,
        std::bind_front(afterGetNetworkAdapterPathForNDFCollection, asyncResp,
                        chassisId, networkAdapterId));
}
