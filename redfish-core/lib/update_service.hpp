    ...
    const dbus::utility::MapperEndPoints& chassisEndpoints)
{
    if (ec)
    {
        ...
        {
            BMCWEB_LOG_DEBUG(
                "No contained_by association for NetworkAdapter {}: {}",
                adapterPath, ec);
            return;
        }
    ...
    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    // every returned object matches one of the branches below. Each object is
    // handled independently. Interfaces are aggregated across the object's
    // services first so the priority does not depend on service iteration
    // order: a NetworkAdapter is nested under its chassis and needs a
    // different link, so it takes precedence over a board/chassis link for the
    // rare object that exposes both.
        ...
        for (const auto& [service, interfaces] : serviceMap)
        {
            ...
            if (std::ranges::find(
                ...
                std::ranges::find(interfaces,
                    ...
                    interfaces.end())
            {
                isChassisOrBoard = true;
            }
        }

        if (isNetworkAdapter)
        {
            // A NetworkAdapter is nested under its chassis, so resolve the
            // owning chassis via the "contained_by" association before
            // building the link.
            sdbusplus::object_path containedByPath(objectPath);
