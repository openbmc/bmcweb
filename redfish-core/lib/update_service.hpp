    ...
    const dbus::utility::MapperGetSubTreePathsResponse& associatedPaths)
{
    if (ec)
    {
        BMCWEB_LOG_DEBUG("Could not get associated managed hosts, ec = {}", ec);
        return;
    }

    // for any managed host which is also associated to our software, we can add
    // a related item
    for (const auto& assocPath : associatedPaths)
    {
    ...
    const dbus::utility::MapperGetSubTreePathsResponse& chassisPaths)
{
    ...
    {
        BMCWEB_LOG_DEBUG(
            "Could not find related Chassis items for {} by association, ec = {}",
            swId, ec);
        return;
    }
    ...
    for (const auto& assocPath : chassisPaths)
    {
        ...
        addRelatedItemChassis(asyncResp, chassisPath.filename());
    }
}
                            ...
                            const std::string& purpose)
{
    ...
    const sdbusplus::message::object_path openbmcPath("/xyz/openbmc_project");

    const std::array<std::string_view, 1> managedHostInterfaces{
        "xyz.openbmc_project.Inventory.Decorator.ManagedHost"};

    const std::array<std::string_view, 1> swInterfaces{
        "xyz.openbmc_project.Software.Version"};

    if constexpr (BMCWEB_EXPERIMENTAL_REDFISH_MULTI_COMPUTER_SYSTEM)
    {
        dbus::utility::getAssociatedSubTreePathsById(
            swId, openbmcPath, swInterfaces, "running", managedHostInterfaces,
    ...
    const std::string associatedPath = std::format("{}/running", path);
