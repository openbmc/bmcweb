    ...
    crow::connections::systemBus->async_method_call(
        [](const boost::system::error_code errorCode,
           const std::variant<std::string>& purpose) {
            ...
            if (*swInvPurpose == fw_util::biosPurpose)
            {
                return "BIOS";
            }
            BMCWEB_LOG_ERROR << "Unknown software purpose " << *swInvPurpose;
            return "UnKnown";
        },
            ...
            crow::connections::systemBus->async_method_call(
                [objPath, asyncResp,
                 req](const boost::system::error_code errorCode,
                    ...
                    if (asyncResp)
                    {
                        auto sendEventcallback =
                            [swID, componentName](const std::string_view state,
                                                  size_t index) {
                                std::string origin =
                                    "/redfish/v1/TaskService/Tasks/" +
                                    std::to_string(index);
                                std::string resType = "Task";
                                // TaskState enums which should send out an
                                // event are: "Starting" = taskResumed "Running"
                                         ...
                                         (state == "Pending"))
                                {
                                    redfish::EventServiceManager::getInstance()
                                        .sendEvent(
                                            redfish::messages::awaitToUpdate(
                                                componentName, swID),
                                            origin, resType);
                                }
                        ...
                        auto messageCallback =
                            [swID,
                             componentName](const std::string_view state,
                                            [[maybe_unused]] size_t index) {
                                if (state == "Started")
                                {
                                    return messages::targetDetermined(
                                        componentName, swID);
                                }
                                if (state == "Aborted")
                                {
                                    return messages::applyFailed(swID,
                                                                 componentName);
                                }
