...
inline void handleProcessUpdateRequest(
    ...
    const std::string& applyTime, const std::set<std::string>& targetSet,
    const boost::system::error_code& ec,
                         ...
                         std::vector<boost::urls::url>& targets)
{
    ...
    {
        sdbusplus::message::object_path path(target.buffer());
    ...
    dbus::utility::getSubTree(
        ...
        [asyncResp, payload, memfd, applyTimeNewVal,
         targetSet](const boost::system::error_code& ec,
                    const dbus::utility::MapperGetSubTreeResponse& subtree) {
        handleProcessUpdateRequest(asyncResp, std::move(payload), memfd,
