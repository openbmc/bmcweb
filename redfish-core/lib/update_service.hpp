    ...
    const dbus::utility::DBusPropertiesMap& propertiesList)
{
    ...
    // "Purpose" is deprecated in the Software.Version interface and may be
    // absent (e.g. device firmware that only publishes a Version). It is
    // used solely to derive the optional Description and RelatedItem links,
    // so skip those when it is missing instead of failing the resource.
