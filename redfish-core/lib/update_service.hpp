...
static constexpr const char *redundancyIntf =
...
static void setActivations(const std::string &service,
                           ...
                           const std::string &activate)
{
    crow::connections::systemBus->async_method_call(
        ...
        std::variant<std::string>(activate));
}
                          ...
                          const std::vector<std::string> &imgUriTargets)
{
    ...
    {
        setActivations(service, objPath, reqActivationsActive);
        return;
    }

    // TODO: Now we support only one target because software-manager
                ...
                crow::connections::systemBus->async_method_call(
                    [objPath,
                     service](const boost::system::error_code error_code,
                              const std::variant<uint8_t> value) {
                        if (error_code)
                        {
                            BMCWEB_LOG_DEBUG << "Error in querying Redundancy "
                                                "Priority value";
                            // not all fwtypes are updateable,
                            // this is ok
                            return;
                        }
                        BMCWEB_LOG_DEBUG << "Priority Value: "
                        ...
                        crow::connections::systemBus->async_method_call(
                            [objPath, service](
                                ...
                                {
                                    BMCWEB_LOG_DEBUG
                                        << "Priority set failed: ec = "
                                        << error_code;
                                }

                                BMCWEB_LOG_DEBUG
                                    << "Setting RequestedActivation for "
                                    << service << " " << objPath;
                                setActivations(service, objPath,
                                               reqActivationsActive);
                                return;
                            },
                            service, objPath, "org.freedesktop.DBus.Properties",
                            "Set", redundancyIntf, "Priority", value);
                    },
                    invDict[0].first,
                    "/xyz/openbmc_project/software/" + imgTarget,
                    "org.freedesktop.DBus.Properties", "Get", redundancyIntf,
                    "Priority");
            }
        },
