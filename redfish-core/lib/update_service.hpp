  ...
  private:
        ...
        boost::container::flat_map<std::string, std::string>& multipartMaps)
    {
        ...
        if (!key.empty())
        {
            ...
            {
                populateMultipartFiles(res, *BodyItr, multipartMaps);
            }
        }
    }
                ...
                const std::vector<std::string>& params) override
    {
        ...
        boost::container::flat_map<std::string, std::string> multipartMaps;
        populateMultipartFiles(res, mimeticObj, multipartMaps);
        ...
        if (multipartMaps.find("UpdateParameters") != multipartMaps.end())
        {
            ...
            if (applyTime == "")
            {
                BMCWEB_LOG_DEBUG << "the applyTime property is missing!!";
                asyncResp->res.clear();
        ...
        {
            BMCWEB_LOG_DEBUG << "file upload failed, not found UpdateFile!!";
        }
    }
};
