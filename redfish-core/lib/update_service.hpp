...
static void uploadImageFile(const std::string& body)
{
...
class UpdateServiceMultipart : public Node
{
  ...
  private:
    std::string getContentValue(const std::string& line, const std::string& key)
    {
        std::vector<std::string> content =
            redfish::string_util::split(line, ";");
        for (auto& value : content)
        {
            std::vector<std::string> vec =
                redfish::string_util::split(value, "=");
            if (vec.size() != 2)
            {
                continue;
            }
            ...
            if (vec[0] == key)
            {
                vec[1].erase(0, vec[1].find_first_not_of("\""));
                vec[1].erase(vec[1].find_last_not_of("\"") + 1);
                return vec[1];
            }
        }
                ...
                const std::vector<std::string>& /*params*/) override
    {
        ...
        std::optional<std::string> uploadData = std::nullopt;
        ...
        for (const crow::FormPart& formpart : req.mime_fields)
        {
            ...
            std::string contentName =
                getContentValue(std::string{it->value()}, "name");
            if (contentName == "UpdateParameters")
            {
                std::vector<std::string> targets;
                std::optional<nlohmann::json> oem = std::nullopt;
            ...
            else if (contentName == "UpdateFile")
            {
                uploadData = formpart.content;
            }
            else
            {
                BMCWEB_LOG_ERROR << "Content-Disposition attribute value error";
                messages::internalError(asyncResp->res);
                return;
            }
        }
