    ...
    const dbus::utility::MapperGetSubTreeResponse& res)
{
    ...
    const std::string managedHostIntf =
    ...
    const std::string& swId, const sdbusplus::object_path& associatedPath)
{
    ...
    std::vector<std::string_view> combinedInterfaces(chassisInterfaces.begin(),
                                                     chassisInterfaces.end());

    combinedInterfaces.emplace_back(
        "xyz.openbmc_project.Inventory.Decorator.ManagedHost");

    dbus::utility::getAssociatedSubTree(
        associatedPath, hwInventoryPath, 0, combinedInterfaces,
                            ...
                            const std::string& purpose)
{
    ...
    getAssociatedRelatedItems(asyncResp, swId, associatedRunningPath);
