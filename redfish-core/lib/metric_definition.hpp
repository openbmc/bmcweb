...
template <typename F>
inline void getChassisNames(F&& callback)
{
        ...
        chassis::getChassisNames(
            [asyncResp,
             collectionReduce](const std::vector<std::string>& chassisNames) {
                ...
                {
                    for (const auto& [sensorNode, _] : sensors::dbus::types)
                    {
...
class MetricDefinition : public Node
{
  ...
  private:
               ...
               const std::vector<std::string>& params) override
    {
        ...
        for (const auto& name : sensors::dbus::names)
        {
            if (name == id)
            {
                break;
    ...
    class DefinitionGather
    {
      public:
        ...
        void insert(
            const boost::container::flat_map<std::string, std::string>& el)
        {
            for (auto& [redfishSensor, dbusSensor] : el)
            {
