...
inline void getChassisNames(F&& cb, const std::shared_ptr<AsyncResp>& asyncResp)
{
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, callback = std::move(cb)](
            ...
            std::vector<sdbusplus::message::object_path>& chassises) {
            if (ec)
            {
            ...
            for (const sdbusplus::message::object_path& chassis : chassises)
            {
                std::string name = chassis.filename();
                if (name.empty())
                {
                    messages::internalError(asyncResp->res);
                    BMCWEB_LOG_ERROR << "Invalid chassis, path: "
                                     << chassis.str;
                    return;
                }
...
class DefinitionCollectionReduce
{
  public:
    ...
    ~DefinitionCollectionReduce()
    {
        ...
        telemetry::dbusReportsToMembers(asyncResp, dbusTypes,
    ...
    void insert(const boost::container::flat_map<std::string, std::string>& el)
    {
        for (const auto& [_, dbusSensor] : el)
        {
            ...
            dbusTypes.emplace_back(std::move(parentPath));
        }
    }

  private:
    const std::shared_ptr<AsyncResp> asyncResp;
    std::vector<sdbusplus::message::object_path> dbusTypes;
};
