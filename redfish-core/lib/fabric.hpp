    ...
    const std::string& powerState)
{
    if (ec)
    {
        ...
        {
            BMCWEB_LOG_DEBUG("PowerState property absent on the interface");
            return;
        }
    ...
    const dbus::utility::MapperGetSubTreePathsResponse& chassisPaths)
{
    if (ec)
    {
        if (ec.value() == EBADR)
        {
            BMCWEB_LOG_DEBUG("No contained_by association for switch");
            return;
        }
        BMCWEB_LOG_ERROR("DBus error getting switch chassis: {}", ec);
        return;
    }
    ...
    {
        BMCWEB_LOG_ERROR("Switch is contained by more than one chassis");
        messages::internalError(asyncResp->res);
        return;
    }
    ...
    if (chassisId.empty())
    {
        BMCWEB_LOG_WARNING("Malformed chassis path {} for switch",
                           chassisPaths[0]);
        return;
    }
    ...
    const dbus::utility::MapperGetObject& object)
{
    if (ec)
    {
        if (ec.value() == EBADR || ec.value() == boost::system::errc::io_error)
        {
            BMCWEB_LOG_DEBUG("PCIeDevice interface absent on {}", switchPath);
            return;
        }
        BMCWEB_LOG_ERROR("DBus error checking PCIeDevice interface on {}: {}",
                         switchPath, ec);
        return;
    }
    ...
    {
        BMCWEB_LOG_WARNING("Malformed switch path {}", switchPath);
        return;
    }
