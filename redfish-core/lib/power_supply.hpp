    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, powerSupplyID](
            ...
            for (const auto& object : subtree)
            {
                ...
                const std::string& objectPowerSupplyID = path.filename();
                                 ...
                                 const std::string& powerSupplyID)
{
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, powerSupplyID](
            ...
            // Currently we only support 1 power supply EfficiencyPercent, use
        ...
        [asyncResp, chassisID](
            ...
            for (const auto& object : subtree)
            {
                ...
                powerSupplyList.push_back(
                    {{"@odata.id", powerSuppliesPath + objectPowerSupplyID}});
            }
    ...
    auto respHandler =
        [callback{std::move(callback)}, asyncResp,
         powerSupplyID](const boost::system::error_code ec,
                        const std::vector<std::string>& powerSupplyPaths) {
            ...
            std::optional<std::string> validPowerSupplyID;
            std::string powerSupplyName;
            for (const std::string& powerSupply : powerSupplyPaths)
            {
                sdbusplus::message::object_path path(powerSupply);
                powerSupplyName = path.filename();
                ...
                {
                    validPowerSupplyID = powerSupplyID;
                    break;
                }
            }
            callback(validPowerSupplyID);
        };
        ...
        auto getChassisID =
            [asyncResp, chassisID,
             powerSupplyID](const std::optional<std::string>& validChassisID) {
                    ...
                    [asyncResp, chassisID, powerSupplyID](
                        ...
                        if (!validChassisID)
                        {
                            ...
                            messages::resourceNotFound(
                                asyncResp->res, "Chassis", powerSupplyID);
                            return;
                        }
                ...
                getValidPowerSupplyID(asyncResp, powerSupplyID,
