...
inline void processPcieSlot(const dbus::utility::DBusInterfacesMap& interfacesList,
                            nlohmann::json& slots)
{
    ...
    for (const auto& [interface, properties] : interfacesList)
    {
        if (interface == "xyz.openbmc_project.Inventory.Item.PCIeSlot")
        {
            ...
            {
                const std::string* generation = std::get_if<std::string>(&it->second);
                if (generation)
                {
            ...
            it = std::find_if(properties.begin(), properties.end(),
                [](const auto& pair) { return pair.first == "Lanes"; });
            if (it != properties.end())
            {
            ...
            it = std::find_if(properties.begin(), properties.end(),
                [](const auto& pair) { return pair.first == "SlotType"; });
            if (it != properties.end())
            {
                const std::string* slotType = std::get_if<std::string>(&it->second);
                if (slotType)
                {
                    std::optional<pcie_slots::SlotTypes> redfishSlotType =
                        pcie_util::dbusSlotTypeToRf(*slotType);
                    if (redfishSlotType && *redfishSlotType != pcie_slots::SlotTypes::Invalid)
                    {
        ...
        else if (interface == "xyz.openbmc_project.Inventory.Decorator.LocationCode")
        {
            ...
            if (it != properties.end())
            {
                const std::string* locationCode = std::get_if<std::string>(&it->second);
                if (locationCode)
                {
                    slot["Location"]["PartLocation"]["ServiceLabel"] = *locationCode;
                    slotHasData = true;
                }
            }
        }
            ...
            const auto pcieSlotIt = std::ranges::find_if(
                ...
                [pcieSlotPath](
                    const std::pair<sdbusplus::message::object_path,
                                    dbus::utility::DBusInterfacesMap>& pcieSlot) {
                    return pcieSlotPath == pcieSlot.first;
                });
