    ...
    const std::string& service)
{
    if (!isPCIeDevice)
    {
        // If it is a PCIeSlot, getValidPCIePath has already returned the
        // PCIeSlot path and its associated service. There is no need to
        // retrieve it again.
...
// getValidPCIePath returns the path, service to the device and a boolean
// indicating if it is a PCIeDevice
                        ...
                        const std::string& pcieDeviceId)
{
    ...
    getValidPCIePath(asyncResp, pcieDeviceId,
                     [asyncResp, pcieDeviceId](
                         const std::string& pcieDevicePath,
                         const std::string& service, const bool isPCIeDevice) {
        addPCIeDeviceCommonProperties(asyncResp, pcieDeviceId, isPCIeDevice);
        if (isPCIeDevice)
        {
        ...
        getSlotLocationAndProperties(asyncResp, pcieDevicePath, isPCIeDevice,
                                     service);
    });
}
