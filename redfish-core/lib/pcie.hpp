    ...
    const boost::system::error_code& ec, const std::string& uuid)
{
    ...
    if (uuid.empty())
    {
        BMCWEB_LOG_DEBUG("PCIeDevice UUID is empty, reporting null");
        asyncResp->res.jsonValue["UUID"] = nullptr;
        return;
    }
    // A canonical UUID string is 36 characters (8-4-4-4-12 with hyphens).
    if (uuid.size() != 36)
    {
        BMCWEB_LOG_DEBUG("PCIeDevice UUID {} is not canonical, reporting null",
                         uuid);
        asyncResp->res.jsonValue["UUID"] = nullptr;
        return;
    }
