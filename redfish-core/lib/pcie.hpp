...
inline void getPCIeSlotProperties(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& pcieDeviceSlot, const std::string& service)
{
...
inline void addPCIeSlotLocation(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
...
inline void getPCIeSlotLocation(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& pcieDeviceSlot, const std::string& service)
{
    addPCIeSlotLocation(asyncResp, pcieDeviceSlot, service);
}
    ...
    const dbus::utility::MapperGetObject& object)
{
    ...
    dbus::utility::getAllProperties(
        service, pcieDeviceSlot, "xyz.openbmc_project.Inventory.Item.PCIeSlot",
    ...
    const std::string& service)
{
    ...
    addPCIeSlotLocation(asyncResp, pcieSlotPath, service);
}
                             ...
                             const bool isPCIeDevice)>& callback)
{
    ...
    {
        sdbusplus::object_path path(rawPath);
                             ...
                             const bool isPCIeDevice)>& callback)
{
    ...
    dbus::utility::getSubTree(
        "/xyz/openbmc_project/inventory", 0, pcieInterface,
        [asyncResp, pcieId,
         callback](const boost::system::error_code& ec,
                   const dbus::utility::MapperGetSubTreeResponse& subtree) {
            if (ec)
    ...
    const std::string& systemName, const std::string& pcieDeviceId)
{
    ...
    getValidPCIePath(asyncResp, pcieDeviceId,
                     [asyncResp, pcieDeviceId](const std::string& pciePath,
                         ...
                         if (isPCIeDevice)
                         {
                             afterGetValidPcieDevicePath(
                                 asyncResp, pcieDeviceId, pciePath, service);
                         }
                         else
                         {
                             afterGetValidPCieSlotPath(asyncResp, pcieDeviceId,
                                                       pciePath, service);
                         }
                     });
}
