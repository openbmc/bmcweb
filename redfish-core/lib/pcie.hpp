                ...
                dbus::utility::getAllProperties(
                    ...
                    [asyncResp](const boost::system::error_code& ec2,
                                const dbus::utility::DBusPropertiesMap&
                                    pcieSlotProperties) {
                        addPCIeSlotProperties(asyncResp->res, ec2,
                                              pcieSlotProperties);
                    });
            }
            else if (interface ==
...
inline void afterGetPCIeDeviceUUID(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const boost::system::error_code& ec, const std::string& uuid)
{
    if (ec)
    {
        if (ec.value() != EBADR)
    ...
    const dbus::utility::MapperGetSubTreePathsResponse& paths)
{
    ...
    {
        std::string filename = sdbusplus::object_path(path).filename();
        if (filename.empty())
        {
    ...
    const dbus::utility::MapperGetSubTreePathsResponse& paths)
{
    ...
    for (const std::string& path : paths)
    {
        if (sdbusplus::object_path(path).filename() == pcieFunctionId)
        {
            dbus::utility::getDbusObject(
