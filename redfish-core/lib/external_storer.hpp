...
inline const std::filesystem::path metadataFile{"index.json"};
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    std::filesystem::path outerFilename = safeAppend(outerDir, metadataFile);
    std::filesystem::path innerFilename = safeAppend(innerDir, metadataFile);
...
std::vector<std::filesystem::path>
    listFilenames(const std::filesystem::path& dir)
{
    ...
    const std::string& idInstance)
{
    ...
    auto outerFilename = safeAppend(outerDir, metadataFile);
                           ...
                           const std::string& keywordMiddle)
{
    ...
    auto innerFilename = safeAppend(innerDir, metadataFile);
    ...
    // Omit content["Id"] intentionally, would always be pathMiddle constant
    content["@odata.id"] = innerUrl;

    auto files = listFilenames(innerDir);
