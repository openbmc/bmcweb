...
{
    std::string topDir = pathPrefix + '/' + urlBase;
    std::error_code ec;

    auto count = std::filesystem::remove_all(topDir, ec);
    ...
    BMCWEB_LOG_INFO << "Deleted all " << count << " files/dirs from " << topDir;
}
...
inline void requestRoutesExternalStorerLogServices(App& app)
{
    ...
    auto hook =
        std::make_shared<external_storer::Hook>(urlBase, midWord, denyList);
