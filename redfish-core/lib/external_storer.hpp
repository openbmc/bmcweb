...
// File /run/bmcweb/redfish/v1/HOOK/INSTANCE/MIDDLE/ENTRY
...
inline const std::filesystem::path metadataStem{"index"};
inline const std::filesystem::path jsonExtension{".json"};
...
class Hook
{
  ...
  public:
    ...
    std::filesystem::path locToFilename(const std::filesystem::path& loc) const;
...
std::filesystem::path
    Hook::locToFilename(const std::filesystem::path& loc) const
{
...
{
    // NOTE: Once refactor approved, consider reusing existing validation
    // https://gerrit.openbmc-project.xyz/c/openbmc/bmcweb/+/52462
    auto name = proposedName.string();

    if (name == metadataStem)
    {
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    std::filesystem::path outerDir = locToFilename(outerUrl);
    ...
    std::filesystem::path innerDir = locToFilename(innerUrl);
    ...
    std::filesystem::path outerFilename = safeAppend(outerDir, metadataStem);
    outerFilename += jsonExtension;
    ...
    std::filesystem::path innerFilename = safeAppend(innerDir, metadataStem);
    innerFilename += jsonExtension;
    ...
    const std::string& idInstance)
{
    ...
    std::filesystem::path entryFilename = locToFilename(entryUrl);
    entryFilename += jsonExtension;
...
std::vector<std::filesystem::path>
    listJsonFilenames(const std::filesystem::path& dir)
{
         ...
         entries != std::filesystem::end(entries); entries.increment(ec))
    {
        ...
        auto fileBasename = entry.path().filename();

        // Only match files that have our recognized file extension
        if (fileBasename.extension() != jsonExtension)
        {
            continue;
        }

        auto file = fileBasename.stem();

        // Skip over the reserved filename
        if (file == metadataStem)
        {
            continue;
        }

        // The denylist is only needed for instance names, not regular files
...
std::vector<std::string> Hook::listInstances(void) const
{
    ...
    auto baseDir = locToFilename(locBase());

    // If containing directory not found, there can be no instances
    if (!(std::filesystem::exists(baseDir, ec)))
    {
        BMCWEB_LOG_INFO << "Location " << baseDir << " nonexistent";
        return files;
    }
    if (ec)
    {
        BMCWEB_LOG_ERROR << "Problem checking for " << baseDir
                         << " existence: " << ec.message();
        return files;
    }

    for (auto entries = std::filesystem::directory_iterator{baseDir};
         entries != std::filesystem::end(entries); entries.increment(ec))
    {
        if (ec)
        {
            BMCWEB_LOG_ERROR << "Problem with " << baseDir
                             << " iterating: " << ec.message();
            break;
        }
    ...
    const std::string& idInstance)
{
    ...
    auto outerDir = locToFilename(outerUrl);
    auto outerFilename = safeAppend(outerDir, metadataStem);
    outerFilename += jsonExtension;
                           ...
                           const std::string& keywordMiddle)
{
    ...
    auto innerUrl = locMiddle(idInstance);
    auto innerDir = locToFilename(innerUrl);
    auto innerFilename = safeAppend(innerDir, metadataStem);
    innerFilename += jsonExtension;
    ...
    for (const auto& file : files)
    {
        ...
        if (!(expandList.empty()))
        {
            std::filesystem::path entryFilename = safeAppend(innerDir, file);
            entryFilename += jsonExtension;
                          ...
                          const std::string& idEntry)
{
    ...
    auto entryFilename = locToFilename(entryUrl);
    entryFilename += jsonExtension;
...
{
    constexpr auto pathBase{"Systems/system/LogServices"};
    ...
    auto hook = std::make_shared<external_storer::Hook>(pathBase, midWord,
                                                        denyList, expandList);
