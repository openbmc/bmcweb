...
// Delegates Expand if there is an efficient expand available
inline query_param::Query delegateExpand(query_param::Query query)
{
    if (isEfficientExpandAvailable(query))
    {
        query.delegateExpand = true;
    }
    return query;
}
} // namespace sensors
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/Sensors/")
        ...
        .methods(
            ...
            if (!redfish::setUpRedfishRoute(app, req, aResp->res,
                                            &sensors::delegateExpand))
            {
                return;
            }

            std::optional<query_param::Query> query =
                query_param::parseParameters(req.urlView.params(), aResp->res);

            if (query.has_value() &&
                sensors::isEfficientExpandAvailable(*query))
            {
            ...
            BMCWEB_LOG_DEBUG
                << "SensorCollection doGet exit via default expand handler";
            return;
        });
}
