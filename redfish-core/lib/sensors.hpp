 ...
 */
static std::string getState(InventoryItem* inventoryItem)
{
        ...
        interfacesDict)
{
    ...
    {
        auto propertyIt = interfaceIt->second.find("PrettyName");
        ...
        propertyIt = interfaceIt->second.find("Present");
        if (propertyIt != interfaceIt->second.end())
        {
    ...
    Callback&& callback)
{
    ...
    auto respHandler = [callback{std::move(callback)}, sensorsAsyncResp,
        ...
        for (const auto& objDictEntry : resp)
        {
            ...
            for (auto& inventoryItem : *(inventoryItems.get()))
            {
...
/**
 * @brief Gets LEDs associated with given inventory items, if any.
 *
 * Finds the LEDs that are associated with the specified inventory items.
 * Then gets D-Bus State property data for the LEDS.
 *
 * This data is later used to provide sensor property values in the JSON
 * response.
 *
