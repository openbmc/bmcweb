...
nlohmann::json getRelatedItems(const InventoryItem& inventoryItem)
{
    ...
    nlohmann::json& sensorJson, InventoryItem* inventoryItem)
{
    ...
    if (sensorsAsyncResp->chassisSubNode == sensors::node::sensors)
    {
        sensorJson["@odata.type"] = "#Sensor.v1_5_0.Sensor";
        ...
        {
            sensorJson["RelatedItem"] = getRelatedItems(*inventoryItem);
        }
    }
    ...
    Callback&& callback)
{
    ...
    auto interfaces =
        std::to_array({"xyz.openbmc_project.Inventory.Item",
