    ...
    Callback&& callback)
{
    ...
    auto respHandler = [callback{std::move(callback)}, sensorsAsyncResp](
        ...
        if (chassisPath == nullptr)
        {
            messages::resourceNotFound(sensorsAsyncResp->asyncResp->res, "ThermalMetrics",
                                       sensorsAsyncResp->chassisId);
            return;
        }
        ...
        sensorsAsyncResp->asyncResp->res.jsonValue["Name"] = "Chassis Thermal Metrics";
        sensorsAsyncResp->asyncResp->res.jsonValue["TemperatureReadingsCelsius"] =
            nlohmann::json::array();
