...
using DbusVariantType = std::variant<int64_t, double, uint16_t, uint32_t,
...
inline void
    handlePowerMetricGet(std::shared_ptr<SensorsAsyncResp>& sensorsAsyncResp,
                         const boost::system::error_code& ec,
                         const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    if (ec)
    {
        messages::internalError(sensorsAsyncResp->asyncResp->res);
        BMCWEB_LOG_ERROR << "PowerMetric GetSubTree handler: Dbus error" << ec;
        return;
    }
    if (subtree.empty())
    {
    ...
    sdbusplus::asio::getAllProperties(
        *crow::connections::systemBus, service, path, powerMetricIface,
        [path, service, sensorsAsyncResp](
            ...
            const std::vector<std::pair<std::string, DbusVariantType>>&
                properties) {
        if (ec2)
        {
            messages::internalError(sensorsAsyncResp->asyncResp->res);
            BMCWEB_LOG_ERROR << "PowerMetric GetAll handler: Dbus error" << ec2;
            return;
        }
    ...
    const std::shared_ptr<SensorsAsyncResp>& sensorsAsyncResp)
{
    ...
    dbus::utility::getSubTree(
        ...
        std::bind_front(handlePowerMetricGet, sensorsAsyncResp));
