    ...
    nlohmann::json& sensor_json, InventoryItem* inventoryItem)
{
    ...
    // sensors can be stored in the same JSON object. We also want to set them
    // here for sensors in SensorCollection, even if it is a power sensor.
    ...
    if(sensorSchema == "Sensors")
    {
        sensor_json["@odata.type"] =
            "#Sensor.v1_0_0.Sensor";
        sensor_json["@odata.context"] =
    ...
    for (const std::string& connection : connections)
    {
        ...
        auto getManagedObjectsCb = [SensorsAsyncResp, sensorNames,
            ...
            for (const auto& objDictEntry : resp)
            {
                ...
                if(sensorSchema == "Sensors")
                {
                    // nullptr is an invalid field and to avoid duplication
                    // elsewhere we we will create this empty property. It will
                    // be overridden.
                    fieldName = "";
                }
                else if (sensorType == "temperature")
                ...
                if(sensorSchema == "Sensors")
                {
                    // Create a new json object to use for our Sensor json to
                    // avoid setting the "" field. These sensors are queried
                    // individually unlike the other types here, so we won't
                    // be creating an array object to put the sensor in.
                    SensorsAsyncResp->res.jsonValue = nlohmann::json::object();
                    SensorsAsyncResp->res.jsonValue["@odata.id"] =
                        "/redfish/v1/Chassis/" + SensorsAsyncResp->chassisId +
                        "/" + SensorsAsyncResp->chassisSubNode + "/" +
                        sensorName;
