...
static constexpr std::string_view all_sensors = "Sensors";
                ...
                Callback&& callback)
{
    ...
    auto respHandler = [callback{std::move(callback)}, sensorsAsyncResp](
        ...
        else if (chassisSubNode == sensors::node::all_sensors)
        {
            sensorsAsyncResp->res.jsonValue["@odata.type"] =
        ...
        if (chassisSubNode != sensors::node::all_sensors)
        {
    ...
    nlohmann::json& sensorJson, InventoryItem* inventoryItem)
{
    ...
    if (sensorsAsyncResp->chassisSubNode == sensors::node::all_sensors)
    {
                     ...
                     sensorsAsyncResp](const boost::system::error_code e,
                        ...
                        crow::connections::systemBus->async_method_call(
                            [path, sensorsAsyncResp](
                                ...
                                sdbusplus::message::object_path path(path);
    ...
    for (const std::string& connection : connections)
    {
        ...
        auto getManagedObjectsCb = [sensorsAsyncResp, sensorNames,
            ...
            for (const auto& objDictEntry : resp)
            {
                ...
                if (sensorSchema == sensors::node::all_sensors)
                {
                    sensorsAsyncResp->res.jsonValue["@odata.id"] =
                        "/redfish/v1/Chassis/" + sensorsAsyncResp->chassisId +
                        "/" + sensorsAsyncResp->chassisSubNode + "/" +
...
{
    for (std::string chassisSensor : sensorsList)
    {
               ...
               const std::vector<std::string>& params) override
    {
        ...
        std::shared_ptr<SensorsAsyncResp> asyncResp =
            std::make_shared<SensorsAsyncResp>(
                res, chassisId,
                sensors::dbus::types.at(sensors::node::all_sensors),
                sensors::node::all_sensors);
