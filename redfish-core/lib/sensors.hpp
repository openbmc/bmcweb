 ...
 * @param available Boolean representing if D-Bus sensor is marked as available.
    ...
    nlohmann::json& sensorJson, InventoryItem* inventoryItem)
{
    ...
    if (success)
    {
        sensorJson["Status"]["State"] = getState(inventoryItem, *checkAvailable);
    }
    else
    {
        sensorJson["Status"]["State"] = getState(inventoryItem, true);
    }
