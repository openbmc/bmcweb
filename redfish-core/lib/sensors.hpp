    ...
    const std::string& sensorName)
{
    ...
    if (criticalThresholdIt != interfacesDict.end())
    {
        ...
        if (health == "OK" &&
            thresholdLowIt != criticalThresholdIt->second.end())
        {
    ...
    if (health == "OK")
    {
        ...
        if (warningThresholdIt != interfacesDict.end())
        {
            ...
            {
                const bool* asserted =
                    std::get_if<bool>(&thresholdHighIt->second);
                if (asserted == nullptr)
                {
            ...
            if (health == "OK" &&
                thresholdLowIt != warningThresholdIt->second.end())
            {
                const bool* asserted =
                    std::get_if<bool>(&thresholdLowIt->second);
                if (asserted == nullptr)
                {
                    BMCWEB_LOG_ERROR << "Illegal sensor threshold"
                                     << sensorName;
    ...
    nlohmann::json& sensor_json)
{
    ...
    sensor_json["Status"]["Health"] = getHealth(interfacesDict, sensorName);
