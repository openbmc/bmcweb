             ...
             excludeSensors](const boost::system::error_code ec,
                ...
                crow::connections::systemBus->async_method_call(
                    [asyncResp, excludeSensors](
                        ...
                        if (ec)
                        {
                            // Chassis are not required to have the all_sensors
                            // association. If the association is missing, we
                            // assume there are no sensors in this chassis. It
                            // should not be an error. This should catch that
                            // case and allow us to continue without error.
                            if (ec.value() != EBADR)
                            {
                                BMCWEB_LOG_ERROR
