...
inline void verifyTotpDbusUtil(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const bool googleAuthEnabled, std::function<void(bool)>&& callback)
{
    crow::connections::systemBus->async_method_call(
    ...
    std::function<void(bool)>&& callback)
{
    dbus::utility::getProperty<std::string>(
        ...
        [asyncResp, totp, userPath, callback = std::move(callback)](
            ...
            verifyTotpDbusUtil(asyncResp, googleAuthEnabled,
                               std::move(callback));
        });
}
    ...
    const std::string& username)
{
    ...
    handleVerifyTotpAction(
        asyncResp, totp, userPath, [username, req{req.copy()}](bool success) {
            if (success)
            {
