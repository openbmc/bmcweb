  ...
  private:
                ...
                const std::vector<std::string> &params) override
    {
                     ...
                     certURI, "/redfish/v1/AccountService/LDAP/Certificates/"))
        {
            ...
            asyncResp->res.jsonValue = {{"Name", "LDAP Certificate"},
        ...
        crow::connections::systemBus->async_method_call(
            [asyncResp, objectPath, certFile, certId, certURI](
                ...
                crow::connections::systemBus->async_method_call(
                    [asyncResp, certFile, objectPath, certURI,
  ...
  private:
                ...
                const std::vector<std::string> &params) override
    {
        ...
        if (!json_util::readJson(
                ...
                optUnstructuredName, "CertificateUri/@odata.id", certURI))
        {
        ...
        if (city.empty())
        {
            messages::actionParameterMissing(asyncResp->res, "GenerateCSR", "City");
            return;
        }
        if (commonName.empty())
        {
            messages::actionParameterMissing(asyncResp->res, "GenerateCSR", "CommonName");
            return;
        }
        if (country.empty())
        {
            messages::actionParameterMissing(asyncResp->res, "GenerateCSR", "Country");
            return;
        }
        ...
        if (state.empty())
        {
            messages::actionParameterMissing(asyncResp->res, "GenerateCSR", "State");
            return;
        }
        ...
        if (optKeyPairAlgorithm)
        {
            ...
            {
                messages::actionParameterValueFormatError(
                    asyncResp->res, keyPairAlgorithm, "KeyPairAlgorithm", "GenerateCSR");
                return;
            }
        }
    ...
    const std::string &certId, const std::string &certURL)
{
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, objectPath, certURL, certId](
            ...
            const std::string &service = resp.begin()->first;
            crow::connections::systemBus->async_method_call(
                [asyncResp, certURL, certId](const boost::system::error_code ec,
