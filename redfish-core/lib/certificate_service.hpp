                       ...
                       const nlohmann::json::json_pointer& countPtr)
{
    crow::connections::systemBus->async_method_call(
        ...
        {
            std::string certURL;
            if (certPath.starts_with(certs::httpsObjectPath))
            {
                certURL =
                    "/redfish/v1/Managers/bmc/NetworkProtocol/HTTPS/Certificates/";
            }
            else if (certPath.starts_with(certs::ldapObjectPath))
            {
                certURL = "/redfish/v1/AccountService/LDAP/Certificates/";
            }
            else if (certPath.starts_with(certs::authorityObjectPath))
            {
                certURL = "/redfish/v1/Managers/bmc/Truststore/Certificates/";
            }
            ...
            std::string certId =
                sdbusplus::message::object_path(certPath).filename();
            link["@odata.id"] = certURL + certId;
