...
class CertificateActionsReplaceCertificate : public Node
{
  ...
  private:
                ...
                const std::vector<std::string> &params) override
    {
        ...
        std::string objecPath;
        if (boost::starts_with(
                certificateUri,
                "/redfish/v1/Managers/bmc/NetworkProtocol/HTTPS/Certificates/"))
        {
            objecPath = std::string(httpsObjectPath) + "/" + std::to_string(id);
        }
        else if (boost::starts_with(
                     certificateUri,
                     "/redfish/v1/AccountService/LDAP/Certificates/"))
        {
            objecPath = std::string(ldapObjectPath) + "/" + std::to_string(id);
        }
        else
        {
        ...
        if (!objecPath.empty())
        {
            auto getServiceName = [asyncResp, replaceCertificate, objecPath,
                                     ...
                                     const std::string &path)
{
    ...
    BMCWEB_LOG_DEBUG << "Update certificate properties service= Path=" << path;
    ...
    auto getServiceName = [asyncResp, getAllProperties,
        ...
        crow::connections::systemBus->async_method_call(
            std::move(getAllProperties), service, path, dbusPropIntf, "GetAll",
            certPropIntf);
    };
    crow::connections::systemBus->async_method_call(
...
class HTTPSCertificateCollection : public Node
{
  public:
               ...
               const std::vector<std::string> &params) override
    {
        ...
        auto getServiceName =
            [asyncResp, getCertificateList](const boost::system::error_code ec,
                                            const GetObjectType &resp) {
                if (ec || resp.empty())
                {
                    BMCWEB_LOG_ERROR << "DBUS response error: " << ec;
                ...
                BMCWEB_LOG_DEBUG << "getServiceName service: " << service;
                crow::connections::systemBus->async_method_call(
                    std::move(getCertificateList), service, httpsObjectPath,
                ...
                const std::vector<std::string> &params) override
    {
        ...
        auto getServiceName = [asyncResp, installCertificate,
            ...
            BMCWEB_LOG_DEBUG << "getServiceName service: " << service;
            crow::connections::systemBus->async_method_call(
                std::move(installCertificate), service, httpsObjectPath,
                certInstallIntf, "Install", filepath);
        };
        crow::connections::systemBus->async_method_call(
            std::move(getServiceName), mapperBusName, mapperObjectPath,
                             ...
                             const std::string &path)
{
    ...
    auto getServiceName = [asyncResp, getCertLocations,
                           path](const boost::system::error_code ec,
                                 const GetObjectType &resp) {
        if (ec || resp.empty())
        {
