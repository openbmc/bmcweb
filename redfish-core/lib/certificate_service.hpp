    ...
    const std::string& name)
{
    ...
    crow::connections::systemBus->async_method_call(
        ...
        asyncResp->res.addHeader("Location", certURL.c_str());
        },
...
inline void requestRoutesCertificateActionsReplaceCertificate(App& app)
{
    BMCWEB_ROUTE(
        ...
        crow::connections::systemBus->async_method_call(
            [asyncResp, certFile, objectPath, service, parsed, id,
             name](const boost::system::error_code ec) {
            if (ec)
            {
                BMCWEB_LOG_ERROR << "DBUS response error: " << ec;
    ...
    BMCWEB_ROUTE(
        ...
        .methods(boost::beast::http::verb::get)(
            [&app](const crow::Request& req,
                ...
                auto certURL = crow::utility::urlFromPieces(
                    "redfish", "v1", "Managers", "bmc", "NetworkProtocol",
