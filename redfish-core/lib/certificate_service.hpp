...
inline bool isValidEmail(const std::string& emailAddress)
{
    ...
    // Ensuring at least one of letter, digit, dot, underscore, plus or hyphen
    // is present in the user name.
    // Allowing any letter, digit or hyphen in the first part of the domain
    // name and allowing multiple sub-domains, and ending with the top level
    // domain match.
