...
class CertificateActionGenerateCSR : public Node
{
  ...
  private:
                ...
                const std::vector<std::string> &params) override
    {
        ...
        else if (certURI == "/redfish/v1/Managers/bmc/Truststore/Certificates/")
        {
            objectPath = certs::authorityObjectPath;
        }
        else
        {
            messages::actionParameterValueFormatError(asyncResp->res, certURI,
        ...
        crow::connections::systemBus->async_method_call(
            [asyncResp, objectPath,
             ...
             optChallengePassword, city(std::move(city)),
             commonName(std::move(commonName)), optContactPerson,
             country(std::move(country)), optEmail, optGivenName, optInitials,
             optKeyBitLength, optKeyCurveId, optKeyPairAlgorithm,
  ...
  public:
                ...
                const std::vector<std::string> &params) override
    {
        std::shared_ptr<CertificateFile> certFile =
            std::make_shared<CertificateFile>(req.body);
        auto asyncResp = std::make_shared<AsyncResp>(res);
        asyncResp->res.jsonValue = {{"Name", "TrustStore Certificate"},
                                    {"Description", "TrustStore Certificate"}};
        crow::connections::systemBus->async_method_call(
            [asyncResp, certFile](const boost::system::error_code ec,
                                  const GetObjectType &resp) {
