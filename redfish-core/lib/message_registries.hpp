    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    members.emplace_back(
        nlohmann::json::object_t{{"@odata.id", "/redfish/v1/Registries/PrivilegeRegistry"}});
    ...
    const std::string& registry)
{
    ...
    if (registry == "PrivilegeRegistry")
    {
        ...
        asyncResp->res.jsonValue["Name"] = "PrivilegeRegistry Message Registry File";
        asyncResp->res.jsonValue["Description"] =
        ...
        asyncResp->res.jsonValue["Registry"] =
            std::format("{}.{}.{}", privilegeHeader.registryPrefix,
                        privilegeHeader.versionMajor,
                        privilegeHeader.versionMinor);
