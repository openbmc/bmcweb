...
inline bool handleParamError(crow::Response& res, std::string_view errorMessage,
                             std::string_view key)
{
    if (errorMessage.starts_with(key))
    {
...
void afterAddReport(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
        ...
        if (!errorMessage.empty())
        {
            for (const auto& arg :
                 {"Id", "Name", "ReportingType", "AppendLimit", "ReportActions",
                  "Interval", "ReportUpdates", "MetricParams"})
            {
                if (!handleParamError(asyncResp->res, errorMessage, arg))
                {
                    return;
                }
            }
