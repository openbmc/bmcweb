...
inline bool getChassisSensorNodeFromMetrics(
    ...
    crow::flat_set<std::pair<std::string, std::string>>& matched)
{
...
class AddReport
{
  public:
    ...
    static void performAddReport(
        ...
        const crow::flat_map<std::string, std::string>& uriToDbus)
    {
  ...
  private:
    ...
    AddReportArgs args;
    crow::flat_map<std::string, std::string> uriToDbus{};
};
} // namespace telemetry
...
inline void requestRoutesMetricReportDefinitionCollection(App& app)
{
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/TelemetryService/MetricReportDefinitions/")
        ...
        crow::flat_set<std::pair<std::string, std::string>> chassisSensors;
