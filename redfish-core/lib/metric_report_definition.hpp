...
class UpdateMetrics
{
  public:
    ...
    void setReadingParams()
    {
        ...
        crow::connections::systemBus->async_method_call(
            [asyncResp(this->asyncResp), reportId = id](const boost::system::error_code& ec) {
                      ...
                      std::string_view id)
{
    ...
    if (!json_util::readJsonPatch(
            req, asyncResp->res, "Name", name, "Metrics", metrics,
            "MetricReportDefinitionType", reportingTypeStr, "ReportUpdates",
            reportUpdatesStr, "AppendLimit", appendLimit, "ReportActions",
            reportActionsStr, "Schedule", schedule,
            "MetricReportDefinitionEnabled", metricReportDefinitionEnabled))
                            ...
                            std::string_view id)
{
    ...
    boost::container::flat_set<std::pair<std::string, std::string>>
        chassisSensors;
    if (!getChassisSensorNodeFromMetrics(asyncResp, args.metrics,
                                         chassisSensors))
    {
        return;
    }
    ...
    crow::connections::systemBus->async_method_call(
        ...
        auto addReportReq = std::make_shared<AddReport>(args, asyncResp,
                                                        addReportMode);
        retrieveMetricPropertyToDbusPaths(
            chassisSensors,
...
inline void
    handleReportDelete(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                       std::string_view id)
{
    const std::string reportPath = getDbusReportPath(id);
