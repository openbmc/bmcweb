        ...
        [aResp, id, redfishActions = std::move(redfishActions),
         newDbusActions = std::move(newDbusActions)](
            ...
            crow::connections::systemBus->async_method_call(
                ...
                std::variant<std::vector<std::string>>(newDbusActions));
        },
        "xyz.openbmc_project.Telemetry",
        "/xyz/openbmc_project/Telemetry/Reports/TelemetryService/" + id,
        "org.freedesktop.DBus.Properties", "Get",
                              ...
                              const std::string& id)
{
    ...
    if (!json_util::readJson(req, aResp->res, "Status", status, "Schedule",
                             schedule, "ReportActions", actions, "Metrics",
                             metrics))
    {
        return;
    }
