...
inline bool verifyId(crow::Response& res, const boost::system::error_code& ec,
                     const sd_bus_error* dbusError,
                     const std::optional<std::string>& id)
{
    if (id && ec == boost::system::errc::invalid_argument &&
        getErrorMessage(dbusError) == "Id")
    {
...
inline bool verifyName(crow::Response& res, const boost::system::error_code& ec,
                       const sd_bus_error* dbusError,
                       const std::optional<std::string>& name)
{
    if (name && ec == boost::system::errc::invalid_argument &&
        getErrorMessage(dbusError) == "Name")
    {
...
inline bool verifyAppendLimit(crow::Response& res,
                              const boost::system::error_code& ec,
                              const sd_bus_error* dbusError,
                              const std::optional<uint64_t>& appendLimit)
{
    if (appendLimit && ec == boost::system::errc::invalid_argument &&
        getErrorMessage(dbusError) == "AppendLimit")
    {
...
inline bool verifyReportActions(
    ...
    const std::optional<std::vector<std::string>>& reportActions)
{
    if (reportActions && ec == boost::system::errc::invalid_argument &&
        getErrorMessage(dbusError) == "ReportActions")
    {
...
inline bool verifyRecurrenceInterval(
    crow::Response& res, const boost::system::error_code& ec,
    const sd_bus_error* dbusError,
    const std::optional<std::string>& recurrenceIntervalStr)
{
    if (recurrenceIntervalStr && ec == boost::system::errc::invalid_argument &&
        getErrorMessage(dbusError) == "Interval")
    {
...
inline bool verifyReportUpdates(crow::Response& res,
                                const boost::system::error_code& ec,
                                const sd_bus_error* dbusError,
                                const std::optional<std::string>& reportUpdates)
{
    if (reportUpdates && ec == boost::system::errc::invalid_argument &&
        getErrorMessage(dbusError) == "ReportUpdates")
    {
        messages::propertyValueIncorrect(res, *reportUpdates, "ReportUpdates");
        return false;
    }
  ...
  public:
        ...
        const boost::container::flat_map<std::string, std::string>& uriToDbus)
    {
        ...
        crow::connections::systemBus->async_method_call(
            ...
            if (!verifyId(asyncResp->res, ec, msg.get_error(), userArgs.id) ||
                !verifyName(asyncResp->res, ec, msg.get_error(),
                            userArgs.name) ||
                !verifyReportingType(asyncResp->res, ec, msg.get_error(),
                                     userArgs.reportingTypeStr) ||
                !verifyAppendLimit(asyncResp->res, ec, msg.get_error(),
                                   userArgs.appendLimit) ||
                !verifyReportActions(asyncResp->res, ec, msg.get_error(),
                                     userArgs.reportActionsStr) ||
                !verifyRecurrenceInterval(asyncResp->res, ec, msg.get_error(),
                                          userArgs.durationStr) ||
                !verifyReportUpdates(asyncResp->res, ec, msg.get_error(),
                                     userArgs.reportUpdatesStr) ||
                !verifyReadingParameters(asyncResp->res, ec, msg.get_error(),
                                         userArgs.metrics) ||
                !verify(asyncResp->res, ec, userArgs.id))
            {
                return;
            }
  ...
  public:
        ...
        const std::vector<nlohmann::json>& redfishMetrics)
    {
        ...
        crow::connections::systemBus->async_method_call(
            ...
            if (!verifyId(asyncResp->res, ec, msg.get_error(), reportId) ||
                !verifyReadingParameters(asyncResp->res, ec, msg.get_error(),
                                         redfishMetrics) ||
                !verify(asyncResp->res, ec, reportId))
            {
                return;
            }
            },
                     ...
                     std::string_view id, bool enabled)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, id = std::string(id)](const boost::system::error_code& ec,
                                          const sdbusplus::message_t& msg) {
        if (!verifyId(asyncResp->res, ec, msg.get_error(), id) ||
            !verify(asyncResp->res, ec, id))
        {
            return;
        }
        },
    ...
    sdbusplus::asio::getAllProperties(
        *crow::connections::systemBus, telemetry::service,
        ...
        [asyncResp, id](const boost::system::error_code& ec,
                        const dbus::utility::DBusPropertiesMap& properties) {
        if (!telemetry::verifyId(asyncResp->res, ec, nullptr, id) ||
            !telemetry::verify(asyncResp->res, ec, id))
        {
            return;
        }
