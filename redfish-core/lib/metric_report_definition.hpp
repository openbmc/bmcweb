    ...
    const std::optional<std::vector<nlohmann::json>>& redfishMetrics)
{
    if (redfishMetrics && ec == boost::system::errc::invalid_argument &&
        errorMessage.starts_with("ReadingParameters"))
    {
  ...
  public:
        ...
        const boost::container::flat_map<std::string, std::string>& uriToDbus)
    {
        ...
        crow::connections::systemBus->async_method_call(
            ...
            if (!verifyId(asyncResp->res, ec, errorMessage, userArgs.id) ||
                !verifyName(asyncResp->res, ec, errorMessage, userArgs.name) ||
                !verifyReportingType(asyncResp->res, ec, errorMessage,
                                     userArgs.reportingTypeStr) ||
                !verifyAppendLimit(asyncResp->res, ec, errorMessage,
                                   userArgs.appendLimit) ||
                !verifyReportActions(asyncResp->res, ec, errorMessage,
                                     userArgs.reportActionsStr) ||
                !verifyRecurrenceInterval(asyncResp->res, ec, errorMessage,
                                          userArgs.durationStr) ||
                !verifyReportUpdates(asyncResp->res, ec, errorMessage,
                                     userArgs.reportUpdatesStr) ||
                !verifyReadingParameters(asyncResp->res, ec, errorMessage,
                                         userArgs.metrics) ||
  ...
  public:
        ...
        const std::vector<nlohmann::json>& redfishMetrics)
    {
        ...
        crow::connections::systemBus->async_method_call(
            ...
            std::string_view errorMessage = getErrorMessage(msg.get_error());
            if (!verifyId(asyncResp->res, ec, errorMessage, reportId) ||
                !verifyReadingParameters(asyncResp->res, ec, errorMessage,
                                         redfishMetrics) ||
                     ...
                     std::string_view id, bool enabled)
{
    crow::connections::systemBus->async_method_call(
        [asyncResp, id = std::string(id)](const boost::system::error_code& ec,
                                          const sdbusplus::message_t& msg) {
        if (!verifyId(asyncResp->res, ec, getErrorMessage(msg.get_error()),
                      id) ||
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, id = std::string(id), reportingType,
         durationStr](const boost::system::error_code& ec,
                      const sdbusplus::message_t& msg) {
        std::string_view errorMessage = getErrorMessage(msg.get_error());
        if (!verifyId(asyncResp->res, ec, errorMessage, id) ||
            !verifyReportingType(asyncResp->res, ec, errorMessage,
                                 reportingType) ||
            !verifyRecurrenceInterval(asyncResp->res, ec, errorMessage,
                                      durationStr) ||
    ...
    sdbusplus::asio::getAllProperties(
        *crow::connections::systemBus, telemetry::service,
        ...
        [asyncResp, id](const boost::system::error_code& ec,
                        const dbus::utility::DBusPropertiesMap& properties) {
        if (!telemetry::verifyId(asyncResp->res, ec, "", id) ||
