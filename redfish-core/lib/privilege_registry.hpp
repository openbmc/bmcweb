...
#include <query.hpp>
#include <registries/privilege_map_registry.hpp>
#include <verb.hpp>
    ...
    fillInPrivilegeRegistry(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
         ...
         ++entityTag)
    {
        ...
        for (HttpVerb verb : registryMethods)
        {
            ...
            // If the privilege registry has no declared privileges, then this
            // action must not be possible. e.g. DELETE on ManagerDiagnosticData
            if (privilegeSetJson.empty())
            {
                continue;
            }
