    ...
    const dbus::utility::MapperEndPoints& endpoints)
{
    ...
    sdbusplus::message::object_path targetPath(endpoints[0]);
    ...
    else
    {
        ...
        BMCWEB_LOG_WARNING(
            "Cannot map inventory path {} to Redfish URI", endpoints[0]);
        return;
    }
            ...
            for (const auto& iface : interfaces)
            {
                ...
                crow::connections::systemBus->async_method_call(
                    [asyncResp](
                        ...
                        const sdbusplus::message::object_path& certificate,
                        ...
                        {
                            asyncResp->res
                                .jsonValue["Certificate"]["@odata.id"] =
                                boost::urls::format(
    ...
    const std::string& componentIntegrityId)
{
    ...
    // Check null bytes first so the error message isn't truncated
    if (nonceStr.find('\0') != std::string::npos)
    {
