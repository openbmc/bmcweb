...
inline void getLedGroupPath(
    ...
    const std::function<void(const std::string& ledGroupPath,
                             const std::string& service)>& callback)
{
    static constexpr const char* ledObjectPath = "/xyz/openbmc_project/led/groups";
    std::string ledGroupAssociatedPath = objPath + "/identifying";
    dbus::utility::getAssociatedSubTreePaths(
        ledGroupAssociatedPath,
        sdbusplus::message::object_path(ledObjectPath), 0,
        ledGroupInterface,
                        ...
                        const std::string& service)
{
    sdbusplus::asio::getProperty<bool>(
        *crow::connections::systemBus, service, ledGroupPath,
        "xyz.openbmc_project.Led.Group", "Asserted",
        [asyncResp](const boost::system::error_code& ec, bool assert) {
        if (ec)
        {
            if (ec.value() != EBADR)
            {
            ...
            messages::resourceNotFound(asyncResp->res, "Get", "Asserted");
            return;
        }
    ...
    sdbusplus::asio::setProperty(
        *crow::connections::systemBus, service, ledGroupPath,
        ...
        if (ec)
        {
            ...
            messages::resourceNotFound(asyncResp->res, "Set", "Asserted");
        }
    });
}
