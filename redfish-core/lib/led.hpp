                                       ...
                                       const std::string& service)>&& callback)
{
    ...
    dbus::utility::getAssociatedSubTree(
        ...
        [asyncResp, objPath, callback{std::move(callback)}](
            ...
            const dbus::utility::MapperGetSubTreeResponse& subtree) {
            handleLedGroupSubtree(asyncResp, objPath, ec, subtree, callback);
        });
}

inline void
    afterGetLedState(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    const std::string& objPath, bool ledState)
{
    ...
    getLedGroupPath(asyncResp, objPath,
                    [asyncResp, ledState](const std::string& ledGroupPath,
                                          const std::string& service) {
                        setDbusProperty(asyncResp, "LocationIndicatorActive",
                                        service, ledGroupPath,
                                        "xyz.openbmc_project.Led.Group",
                                        "Asserted", ledState);
                    });
}
