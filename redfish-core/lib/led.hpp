                         ...
                         const std::string& ledState)
{
    ...
    auto callbackBlink = [aResp, &ledOn,
            ...
            [aResp, &ledOn, ledBlinkng,
             serviceName](const boost::system::error_code ec) mutable {
                if (ec)
                {
                    ...
                    // object so Lets set enclosure_identify state to true
                    // if Blinking is true.
                    if (ledBlinkng)
                    {
                        ledOn = true;
                               ...
                               const bool ledState)
{
    ...
    auto callbackBlink = [aResp, &ledState](const std::string& serviceName) {
        crow::connections::systemBus->async_method_call(
            [aResp, ledState,
             serviceName](const boost::system::error_code ec) mutable {
                if (ec)
