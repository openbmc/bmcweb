...
static constexpr std::string_view ledGroupIntf =
...
{
    crow::connections::systemBus->async_method_call(
        [aResp, callback](const boost::system::error_code ec,
        ...
        std::string service = object.begin()->first;
        callback(service);
        },
    ...
    auto respHanler = [aResp, ledGroup](const std::string& service) {
        sdbusplus::asio::getProperty<bool>(
            *crow::connections::systemBus, service, ledGroup,
            std::string{ledGroupIntf}, "Asserted",
