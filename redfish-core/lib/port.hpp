...
inline void getPortLocation(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    std::function<void(const std::string& portPath)>&& callback)
{
    redfish::port_utils::getPortList(
        ...
        for (const std::string& portPath : endpoints)
        {
                        ...
                        const dbus::utility::MapperEndPoints& portPaths)
{
    ...
    for (const std::string& path : portPaths)
    {
                            ...
                            const std::string& adapterId)
{
    ...
    getValidFabricAdapterPath(adapterId, systemName, asyncResp,
                              std::bind_front(afterHandlePortCollectionGet,
