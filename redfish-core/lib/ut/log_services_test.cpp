...
TEST(LogServicesTest, LogServicesCreateSystemDumpInvalidDataTypeFails)
{
    ...
    EXPECT_CALL(*sdbusMock, async_method_call(Matcher<handlerEcDumpId_t>(_), _,
                                              _, _, _))
        .Times(0);
...
TEST(LogServicesTest, LogServicesGetDumpEntryCollectionBMCBadECFails)
{
    ...
    EXPECT_CALL(*sdbusMock, async_method_call(Matcher<handlerEcResp_t>(_), _, _,
                                              _, _))
        .WillOnce(InvokeArgument<0>(
            boost::system::error_code(1, boost::system::system_category()), dbus::utility::ManagedObjectType{}));
