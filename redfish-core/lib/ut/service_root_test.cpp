...
TEST(ServiceRootTest, ServiceRootConstructor)
{
    ...
    crow::Response* response = &(shareAsyncResp->res);
    response->setCompleteRequestHandler([](crow::Response& response) mutable {
        assertServiceRootGet(response);
    });

    // const std::shared_ptr<bmcweb::AsyncResp> shareAsyncRespTemp =
