...
TEST_F(EventServiceInternalImplTest, ClassEventPosTest)
{
    ...
    nlohmann::json event2TargetLogEntry = {
        ...
        {"MessageArgs", std::vector<std::string>{
            "GPU1 PWR_GOOD status", "interrupt asserted"}}
    };
    Event event2(event2TargetLogEntry["MessageId"]);
    nlohmann::json event2LogEntry;

    EXPECT_EQ(
        event2.setRegistryMsg(event2TargetLogEntry["MessageArgs"]),
        0);
    ...
    EXPECT_EQ(
        event3.setCustomMsg(
            "The resource %1 has been created successfully.",
...
TEST_F(EventServiceInternalImplTest, ClassEventNegaTest)
{
    ...
    EXPECT_NE(
        event1.setRegistryMsg(event1LogEntry["MessageArgs"]),
        0);
    ...
    {
        Event event2("aaa");
        FAIL() << "no event construction error with invalid MessageId: aaa";
    }
    catch(const std::runtime_error& error)
    {
        EXPECT_STREQ(error.what(),
            "Message not found in registry with ID: aaa");
    }
    catch(...)
    {
        FAIL() << "incorrect error throwed";
    }
