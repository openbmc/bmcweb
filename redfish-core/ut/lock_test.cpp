...
TEST_F(LockTest, ValidateNewDirCreation)
{
   // char tmpPath[] = "/tmp/testDir";
    EXPECT_TRUE(crow::ibm_utils::createDirectory(dir.c_str()));//tmpPath));
   // std::filesystem::remove_all("/tmp/testDir");
}

TEST_F(LockTest, ValidateExistingDirCreation)
{
    EXPECT_TRUE(crow::ibm_utils::createDirectory(dir.c_str()));
}
