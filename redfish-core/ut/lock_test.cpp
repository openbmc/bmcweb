...
class locktest : public ::testing::Test
{
   ...
   /* bool validlockrecord(lockrecord* record1)
    ...
    locktest()
    {
        ...
        //record1 = std::make_tuple("xxxxx","hmc-id","read","234",segmentflags {{"DontLock", 2}, {"DontLock", 4} });
        
        // lockrecord with a single request with multiple segment
        record = { "xxxxx", "hmc-id", "read", 234,  { {"DontLock", 2}, {"DontLock", 4} }};

        // lockrequest with multiple lockrequests
        request ={ { "xxxxx", "hmc-id", "read", 234,  { {"DontLock", 2}, {"DontLock", 4} }},{ "xxxxx", "hmc-id", "read", 234,  { {"Dontlock", 2}, {"DontLock", 4} }} };
        request1 = {{ "xxxxx", "hmc-id", "read", 234,  { {"DontLock", 2}, {"DontLock", 4} }}};
        request2 = {{ "xxxxx", "hmc-id", "write", 234,  { {"Lockall", 2}, {"DontLock", 4} }}};
        //lock lockobj;
    }
...
TEST_F(locktest, MultiRequestWithoutConflictduetoDifferentSegmentData)
{
    ...
    // Change the resource id(2nd byte) of first record, so the locks are different
    // so no conflict
...
TEST_F(locktest, MultiRequestWithConflictduetoSameSegmentData)
{
    ...
    // Dont Change the resource id(1st & 2nd byte) at all, so that the conflict occurs from 
    // the second segment which is trying to lock all the resources.
