...
TEST(PriorityTest, RuleConstructor)
{
    ...
    {
        constexpr char url1[]{
            "redfish/3/<int>/<uint>/<double>/<string>/<path>"};
        constexpr char url2[]{"redfish/3/1/1/1/test/path/to/test"};
        ...
        r.method(boost::beast::http::verb::get);
        Request req{r};
        req.url = "redfish/3/1/1/1/test/path/to/test";
        Response res;
        router.handle(req, res);
    ...
    // dynamic rule and tagged rule
    {
        ...
        rule1.name("rule1").priority(rule1.PriorityMax()).
