...
redfish, mostly specific to the **Systems** redfish resource tree as well as the
respective properties and which backends / mechanisms we use to built them.

Redfish terms are written in PascalCase throughout the document as in the
...
To ease implementation and review, here are some of the current understandings
and design decisions, we currently follow to support multi-host in bmcweb.
...
we want to use dbus associations to link a given ComputerSystem in a multi-host
setup with its specific resources. Recently, the topology mechanism in
entity-manager has been reworked in
...
request to create the necessary object paths and dbus service names, to form the
response. The proposal for the interface came from the
[mailing list](https://lists.ozlabs.org/pipermail/openbmc/2024-June/035442.html).

Beyond that, the interface now helps to determine the correct redfish identifier
in the ComputerSystem tree, for any association on multi-host.
...
introduced the `redfish-system-uri-name` option, which allows for customization
of the identifier. This **does not** apply to multi-host platforms. Currently,
...
In accordance to the redfish specification we want to respond with EventLog
entries specific to the ComputerSystem upon request. However, this requires the
EventLog to be filtered by host, so that each host has a collection of EventLog
entries specific to it. This type of filtering on multi-host is currently not
supported.

To circumvent that, `redfish-eventlog-location=managers` meson option has been
introduced. This puts the EventLog tree under /redfish/v1/Managers/{managerId}
URI, so there is a way to access it on a multi-host platform.
