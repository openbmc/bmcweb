...
inline void afterAuthenticateUser(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    std::string_view username, const boost::asio::ip::address& ipAddress,
    int32_t pamrc)
{
                        ...
                        const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    pamAuthenticateUser(username, password, std::nullopt, std::move(callback));
}
