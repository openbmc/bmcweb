...
json dict. In the majority of uses, these will not cause a problems, but if a
type that isn't an object (0.0, "foo", or null) is already present in the
object, these methods will [throw an
exception](https://json.nlohmann.me/api/basic_json/operator%5B%5D/#exceptions),
which generally goes uncaught, and can lead to security issues. Care should be
taken when operating with a nlohmann::json object, and should generally prefer
to use an nlohmann::json::object_t or nlohmann::json::array_t where
appropriate.  There is quite a large amount of code in bmcweb that does not
handle the distinction between nlohmann::json and nlohmann::json::object_t
correctly.  These will be corrected over time.
