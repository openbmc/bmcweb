...
dbus::utility::getSubTree("/", interfaces,
                         [asyncResp](boost::system::error_code& ec, MapperGetSubTreeResult& res){
                            <many lines of code>
                         })
```
...
Prefer to either use std::bind\_front, and a normal method to handle the return,
or a lambda that is less than 5 lines of code to handle an error inline.  In
cases where std::bind\_front cannot be used, such as in
sdbusplus::bus::async\_method\_call, keep the lambda length less than 5 lines,
and call the appropriate function for handling non-trivial transforms.


```cpp
inline void afterGetSubTree(std::shared_ptr<bmcweb::AsyncResp>& asyncResp, boost::system::error_code& ec,
...
for generalized guidelines on when lambdas are appropriate.
