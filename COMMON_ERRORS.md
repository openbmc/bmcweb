...
`nlohmann::json::[parse](https://json.nlohmann.me/api/basic_json/parse/)` by
default throws on failure, but also accepts an optional argument that causes
it to not throw: set the 3rd argument to `false`.

`nlohmann::json::[dump](https://json.nlohmann.me/api/basic_json/dump/)` by
default throws on failure, but also accepts an optional argument that causes
it to not throw: set the 4th argument to `replace`.  Although `ignore`
preserves content 1:1, `replace` is preferred from a security point of view.

Please consult the other examples in the code for how to handle errors.
