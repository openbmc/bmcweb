        ...
        boost::beast::http::async_read_header(
            ...
            [this,
             self(shared_from_this())](const boost::system::error_code& ec,
                ...
                boost::beast::http::verb method = parser->get().method(); // for auth
                readClientIp(); // for auth
                try // for auth
                {
                    //  auth uses url, but this is not working
                ...
                BMCWEB_LOG_DEBUG  << "JEBR doReadHeaders calling auth";
                this->userSession = crow::authorization::authenticate(*req, res, method, /* clientIp, */ userSession);
                BMCWEB_LOG_DEBUG  << "JEBR doReadHeaders ret from auth";
            ...
            [this,
             self(shared_from_this())](const boost::system::error_code& ec,
                ...
                else
                {
                    ...
                    {
                        cancelDeadlineTimer();
                        BMCWEB_LOG_DEBUG << "JEBR do read: no EC and Alive";
                        if (req)
                        {
                            startDeadline(loggedInAttempts);
                        }
                ...
                BMCWEB_LOG_DEBUG << "do read: calling handle";
                handle();
            });
    }
