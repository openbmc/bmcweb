    ...
    public std::enable_shared_from_this<Connection<Adaptor, Handler>>
{
  public:
        ...
        getCachedDateStr(getCachedDateStrF)
    {
        ...
        BMCWEB_LOG_DEBUG("{} Connection open, total {}", fmt::ptr(this),
                         connectionCount);
    }
    ...
    ~Connection()
    {
        ...
        connectionCount--;
        BMCWEB_LOG_DEBUG("{} Connection closed, total {}", fmt::ptr(this),
                         connectionCount);
    }
    ...
    void prepareMutualTls()
    {
                               ...
                               .tls)
        {
            ...
            if (ret == 0)
            {
                BMCWEB_LOG_ERROR("{} failed to set SSL id", fmt::ptr(this));
            }
        }
        ...
        adaptor.set_verify_callback([this](
            ...
            if (!preverified)
            {
                BMCWEB_LOG_DEBUG("{} TLS preverification failed.",
                                 fmt::ptr(this));
                return true;
            }
            ...
            if (error != X509_V_OK)
            {
                BMCWEB_LOG_INFO("{} Last TLS error is: {}", fmt::ptr(this),
                                error);
                return true;
            }
            ...
            if (usage == nullptr)
            {
                BMCWEB_LOG_DEBUG("{} TLS usage is null", fmt::ptr(this));
                return true;
            }
            ...
            if (extUsage == nullptr)
            {
                BMCWEB_LOG_DEBUG("{} TLS extUsage is null", fmt::ptr(this));
                return true;
            }
            ...
            {
                BMCWEB_LOG_DEBUG("{} Invalid TLS user name", fmt::ptr(this));
                return true;
            }
            ...
            {
                BMCWEB_LOG_DEBUG("{} Generating TLS session: {}",
                                 fmt::ptr(this), userSession->uniqueId);
            }
            return true;
        });
    }
    ...
    void handle()
    {
        ...
        BMCWEB_LOG_INFO("Request:  {} HTTP/{}.{} {} {} {}", fmt::ptr(this),
    ...
    void close()
    {
                                         ...
                                         boost::asio::ip::tcp::socket>>)
        {
            ...
            if (userSession != nullptr)
            {
                BMCWEB_LOG_DEBUG("{} Removing TLS session: {}", fmt::ptr(this),
    ...
    void completeRequest()
    {
        ...
        BMCWEB_LOG_INFO("Response: {}{}{}{}{} keepalive={}", fmt::ptr(this),
                        ' ', req->url, ' ', res.resultInt(), req->keepAlive());
  ...
  private:
    void doReadHeaders()
    {
        BMCWEB_LOG_DEBUG("{} doReadHeaders", fmt::ptr(this));
        ...
        boost::beast::http::async_read_header(
            ...
            [this,
             self(shared_from_this())](const boost::system::error_code& ec,
                ...
                BMCWEB_LOG_DEBUG("{} async_read_header {} Bytes",
                                 fmt::ptr(this), bytesTransferred);
                ...
                {
                    close();
                    BMCWEB_LOG_DEBUG("{} from read(1)", fmt::ptr(this));
                    return;
                }
    ...
    void doRead()
    {
        BMCWEB_LOG_DEBUG("{} doRead", fmt::ptr(this));
        startDeadline();
        boost::beast::http::async_read(
            ...
            [this,
             self(shared_from_this())](const boost::system::error_code& ec,
                                       std::size_t bytesTransferred) {
                BMCWEB_LOG_DEBUG("{} async_read {} Bytes", fmt::ptr(this),
                                 bytesTransferred);
                cancelDeadlineTimer();
                if (ec)
                {
    ...
    void doWrite()
    {
        BMCWEB_LOG_DEBUG("{} doWrite", fmt::ptr(this));
        res.preparePayload();
        ...
        boost::beast::http::async_write(
            ...
            [this,
             self(shared_from_this())](const boost::system::error_code& ec,
                                       std::size_t bytesTransferred) {
                BMCWEB_LOG_DEBUG("{} async_write {} bytes", fmt::ptr(this),
                                 bytesTransferred);
                ...
                if (ec)
                {
                    BMCWEB_LOG_DEBUG("{} from write(2)", fmt::ptr(this));
                    return;
                }
                if (!res.keepAlive())
                {
                    close();
                    BMCWEB_LOG_DEBUG("{} from write(1)", fmt::ptr(this));
                    return;
                }

                serializer.reset();
                BMCWEB_LOG_DEBUG("{} Clearing response", fmt::ptr(this));
                res.clear();
                parser.emplace(std::piecewise_construct, std::make_tuple());
    ...
    void startDeadline()
    {
        ...
        timer.async_wait([weakSelf](const boost::system::error_code ec) {
            ...
            if (ec)
            {
                BMCWEB_LOG_CRITICAL("{} timer failed {}", fmt::ptr(self.get()),
                                    ec);
            }
        ...
        BMCWEB_LOG_DEBUG("{} timer started", fmt::ptr(this));
    }
