    ...
    public std::enable_shared_from_this<Connection<Adaptor, Handler>>
{
        ...
        boost::beast::http::async_read_header(
            ...
            [this,
             self(shared_from_this())](const boost::system::error_code& ec,
                ...
                else
                {
                    ...
                    if (!isAlive() &&
                        !boost::beast::websocket::is_upgrade(
                            parser
                                ->get())) // parser->get()[boost::beast::http::field::host])
                    {
                        errorWhileReading = true;
                    }
                }
                ...
                boost::asio::ip::address ip;
                (void)getClientIp(ip);
                auto sp = userSession.lock();
                tmpSession = crow::authorization::authenticate(
                    req->url, ip, res, method, parser->get().base(), sp);
                bool loggedIn = req && tmpSession;
                if (loggedIn)
                {
  ...
  private:
    Adaptor adaptor;
    Handler* handler;
    std::shared_ptr<persistent_data::UserSession> tmpSession;
