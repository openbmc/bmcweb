    ...
    public std::enable_shared_from_this<Connection<Adaptor, Handler>>
{
  public:
    ...
    {
        BMCWEB_LOG_INFO << "Response: " << ' ' << req.url << ' '
                        << res.resultInt() << " keepalive=" << req.keepAlive();
        BMCWEB_LOG_INFO << "Body size: " << res.body().size();
  ...
  private:
                                      ...
                                      nghttp2_data_source* source, void*)
    {
        ...
        http2_stream_data* str =
            reinterpret_cast<http2_stream_data*>(source->ptr);
    ...
    int send_response(nghttp2_session* session, int32_t stream_id)
    {
        ...
        http2_stream_data* stream_ptr = it->second.get();
        stream_ptr->sent_sofar = 0;

        nghttp2_data_provider data_prd;

        data_prd.source.ptr = stream_ptr;
    ...
    {
        size_t readSofar = 0;
        auto begin = boost::asio::buffer_sequence_begin(inBuffer.data());
        auto end = boost::asio::buffer_sequence_end(inBuffer.data());

        for (auto it = begin; it != end; ++it)
        {
            size_t bytesToRead = boost::asio::buffer_size(*it);
            ...
            ssize_t readLen = nghttp2_session_mem_recv(
                ngSession->get(), reinterpret_cast<const uint8_t*>(it->data()),
                bytesToRead);
    ...
    int on_request_recv(nghttp2_session* session, int32_t stream_id)
    {
        ...
        res.completeRequestHandler = [this, session, stream_id]() {
  ...
  private:
    ...
    boost::beast::flat_static_buffer<8096> inBuffer;
