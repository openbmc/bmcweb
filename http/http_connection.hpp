    ...
    public std::enable_shared_from_this<Connection<Adaptor, Handler>>
{
  public:
    ...
    void prepareMutualTls(boost::beast::ssl_stream<Adaptor>& sslAdap)
    {
        ...
        if (caAvailable && persistent_data::SessionStore::getInstance()
                               ...
                               .tls)
        {
            sslAdap.set_verify_mode(boost::asio::ssl::verify_peer);
            ...
            sslAdap.set_verify_callback([this](bool preverified,
    ...
    void handle()
    {
        ...
        // If this isn't an SSL connection, and this isn't explicitly an HTTP
    ...
    void completeRequest()
    {
        ...
        if (!isAlive())
        {
            BMCWEB_LOG_DEBUG << this << "Connection was dead.  Ignoring completeRequest()";
            // delete lambda with self shared_ptr
            // to enable connection destruction
            res.completeRequestHandler = nullptr;
            return;
