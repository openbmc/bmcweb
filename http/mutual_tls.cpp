...
bool isUPNMatch(const std::string& upn, std::string hostname)
{
    size_t upnDomainPos = upn.find('@');
    if (upnDomainPos == std::string::npos)
    {
        return false;
    }

    std::string upnDomain = upn.substr(upnDomainPos + 1);
    while (1)
    {
        std::string upnDomainMatching = upnDomain;
        size_t dotUPNPos = upnDomain.find_last_of('.');
        if (dotUPNPos != std::string::npos)
        {
            upnDomainMatching = upnDomain.substr(dotUPNPos + 1);
        }

        std::string hostDomainMatching = hostname;
        size_t dotHostPos = hostname.find_last_of('.');
        if (dotHostPos != std::string::npos)
        {
...
std::string getUPNFromCert(X509* peerCert, const std::string& hostname)
{
    ...
    for (int i = 0; i < sk_GENERAL_NAME_num(gs); i++)
    {
        ...
        char* upnChar = std::bit_cast<char*>(
            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-union-access)
...
std::string getUsernameFromCert(X509* cert)
{
    ...
    switch (authMethodsConfig.mTLSCommonNameParsingMode)
    {
        ...
        {
            return getUPNFromCert(cert, redfish::getHostName());
        }
