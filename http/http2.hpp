...
struct nghttp2_session_ptr
{
    ...
    int submitSettings(uint8_t flags, std::span<nghttp2_settings_entry> iv)
    {
        return nghttp2_submit_settings(ptr, flags, iv.data(), iv.size());
    }
    ...
    int send()
    {
        int ret = nghttp2_session_send(ptr);
        if (ret != 0)
        {
            BMCWEB_LOG_ERROR << "Fatal error: " << nghttp2_strerror(ret);
        }
        return ret;
    }
