...
class HttpBody::writer
{
  ...
  public:
           ...
           value_type& bodyIn) : body(bodyIn)
    {
        ...
        BMCWEB_LOG_ERROR("Body compression type is {}",
                         static_cast<int>(body.compressionType));
        BMCWEB_LOG_ERROR("Body client compression type is {}",
                         static_cast<int>(body.clientCompressionType));
        if (body.compressionType == CompressionType::Raw &&
            body.clientCompressionType == CompressionType::Zstd)
        {
            BMCWEB_LOG_ERROR(
                "Body is raw and client supports zstd.  Compressing.");
        ...
        boost::beast::error_code& ec, size_t maxSize)
    {
        ...
        if (zstdCompressor)
        {
            BMCWEB_LOG_ERROR("Compressing body more={}", ret.second);
            std::optional<const_buffers_type> compressed =
                zstdCompressor->compress(ret.first, ret.second);
            if (!compressed)
            {
                return boost::none;
            }
