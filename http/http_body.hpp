...
class HttpBody::writer
{
  ...
  public:
        ...
        getWithMaxSize(boost::beast::error_code& ec, size_t maxSize)
    {
        ...
        if (zstd)
        {
            std::optional<std::span<char>> decompressed =
                zstd->decompress(std::span<const char>(
                    static_cast<const char*>(ret.first.data()),
                    ret.first.size()));
            if (!decompressed)
            {
                return boost::none;
            }
            ret.first = const_buffers_type(decompressed->data(),
                                           decompressed->size());
        }
