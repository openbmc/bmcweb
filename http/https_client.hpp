...
{
  private:
    boost::asio::ssl::stream<boost::asio::ip::tcp::socket> conn;
    ...
    void doConnect()
    {
        ...
        boost::asio::async_connect(
            conn.lowest_layer(), endpoint,
            [this](const boost::system::error_code& error,
                   const tcp::endpoint&) {
                if (!error)
                {
                    BMCWEB_LOG_DEBUG << "doConnect done. Calling doHandshake";
                    doHandshake();
                }
                else
                {
                    BMCWEB_LOG_DEBUG << "Connect failed: " << error.message();
                }
            });
    }
    ...
    void doHandshake()
    {
        ...
        conn.async_handshake(
            ...
            [this, data](const boost::system::error_code& error) {
                if (!error)
                {
                    BMCWEB_LOG_DEBUG << "doHandshake done. Calling sendMessage";
                    sendMessage(data);
                }
                else
                {
                    BMCWEB_LOG_DEBUG << "Handshake failed: " << error.message();
                }
            });
    }
    ...
    void sendMessage(const std::string& data)
    {
        ...
        // Send the HTTPs request to the remote host
    ...
    void doClose()
    {
        ...
        BMCWEB_LOG_DEBUG << "Connection is being closed";
        conn.lowest_layer().shutdown(
    ...
    void connStateCheck()
    {
        switch (state)
        {
            ...
            case ConnState::recvFailed:
            {
                ...
                BMCWEB_LOG_DEBUG << "calling doConnect";
                doConnect();
                break;
            }
  ...
  public:
    explicit HttpsClient(boost::asio::io_context& ioc, const std::string& id,
