...
TEST(Utility, readUrlSegments)
{
    ...
    EXPECT_TRUE(readUrlSegments(*parsed, "redfish", "v1", out1));
    EXPECT_EQ(out1, "Chassis");

    out1 = out2 = out3 = "";
    EXPECT_TRUE(readUrlSegments(*parsed, out1, out2, out3));
    EXPECT_EQ(out1, "redfish");
    EXPECT_EQ(out2, "v1");
    EXPECT_EQ(out3, "Chassis");

    out1 = out2 = out3 = "";
    EXPECT_TRUE(readUrlSegments(*parsed, "redfish", out1, "Chassis"));
    EXPECT_EQ(out1, "v1");

    out1 = out2 = out3 = "";
    EXPECT_TRUE(readUrlSegments(*parsed, out1, "v1", out2));
    EXPECT_EQ(out1, "redfish");
    EXPECT_EQ(out2, "Chassis");

    EXPECT_FALSE(readUrlSegments(*parsed, out1, "v2", out2));

    EXPECT_FALSE(readUrlSegments(*parsed, "redfish", out1, out2, out3));

    EXPECT_TRUE(readUrlSegments(*parsed, anySegment, anySegment, anySegment));

    EXPECT_FALSE(readUrlSegments(*parsed, anySegment, anySegment, anySegment,
                                 anySegment));

    EXPECT_FALSE(readUrlSegments(*parsed, anySegment, anySegment));

    EXPECT_TRUE(readUrlSegments(*parsed, "redfish", anySegment, "Chassis"));

    out1 = out2 = out3 = "";
    EXPECT_TRUE(readUrlSegments(*parsed, anySegment, out1, anySegment));
    EXPECT_EQ(out1, "v1");
}
