    ...
    public std::enable_shared_from_this<Connection<Adaptor, Handler>>
{
  public:
    ...
    void handle()
    {
        ...
        res.writeHandler =
            [self(shared_from_this())](const std::string_view body) {
                self->write(body);
                return true;
            };
    ...
    size_t write(const std::string_view content)
    {
        if (!isAlive())
        {
            return 0;
        }
        size_t copied = boost::asio::buffer_copy(buffer.prepare(content.size()),
                                                 boost::asio::buffer(content));

        if (copied != content.size())
        {
            return copied;
        }
