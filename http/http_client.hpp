...
class ConnectionInfo : public std::enable_shared_from_this<ConnectionInfo>
{
  private:
        ...
        const std::vector<boost::asio::ip::tcp::endpoint>& endpointList)
    {
        ...
        timer.async_wait(std::bind_front(onConnectTimeout, weak_from_this()));
    ...
    static void onConnectTimeout(const std::weak_ptr<ConnectionInfo>& weakSelf,
                                 const boost::system::error_code ec)
    {
        if (ec == boost::asio::error::operation_aborted)
        {
        ...
        // Connection timed out.  We need to set the isUnreachable bool in
        // ConnectionPool (if it's not already true) and kick off another
        // retry attempt
    ...
    void waitAndRetry()
    {
            ...
            (state == ConnState::sslInitFailed))
        {
            ...
            // If the connection was what failed then future messages can just
            // immediately return 504 and in the background we'll poll until we
            // are able to connect
