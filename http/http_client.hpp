...
class ConnectionInfo : public std::enable_shared_from_this<ConnectionInfo>
{
  ...
  public:
    explicit ConnectionInfo(boost::asio::io_context& iocIn,
        ...
        subId(idIn),
        reqSource(reqSourceIn), host(destIPIn), port(destPortIn),
...
class ConnectionPool : public std::enable_shared_from_this<ConnectionPool>
{
  ...
  public:
    explicit ConnectionPool(boost::asio::io_context& iocIn,
                            ...
                            bool useSSLIn) :
        ioc(iocIn),
        id(idIn), reqSource(reqSourceIn), destIP(destIPIn),
...
class HttpClient
{
  ...
  public:
    ...
    void sendData(std::string& data, const std::string& id,
                  RequestSource reqSource, const std::string& destIP,
                  ...
                  const std::string& retryPolicyName)
    {
        ...
        sendDataWithCallback(data, id, reqSource, destIP, destPort, destUri,
                             useSSL, httpHeader, verb, retryPolicyName, cb);
    }
                              ...
                              const std::function<void(Response&)>& resHandler)
    {
        ...
        if (conn == nullptr)
        {
            ...
            // not already exist
            conn = std::make_shared<ConnectionPool>(ioc, id, reqSource, destIP,
                                                    destPort, useSSL);
