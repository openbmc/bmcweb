  ...
  private:
    ...
    void doBeginResolveUnreachable()
    {
        ...
        resolver.asyncResolve(
            host, port,
            std::bind_front(&ConnectionInfo::afterBeginResolveUnreachable, this,
                            shared_from_this()));
    }

    void afterBeginResolveUnreachable(
        const std::shared_ptr<ConnectionInfo>& /*self*/,
        const boost::beast::error_code ec,
        const std::vector<boost::asio::ip::tcp::endpoint>& endpointList)
    {
        if (ec || (endpointList.empty()))
        {
            BMCWEB_LOG_ERROR << "Unreachable resolve failed: " << ec.message();
            state = ConnState::resolveFailed;
            doBeginResolveUnreachable();
            return;
        }
        state = ConnState::connectInProgress;
        doConnectUnreachable(endpointList);
    }
