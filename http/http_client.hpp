...
//class ConnectionInfo : public std::enable_shared_from_this<ConnectionInfo>
{
  private:
    ...
    void doConnect(const std::string& data,
...
*/
    ...
    {
//        boost::beast::http::request<boost::beast::http::string_body> req = connections[0]->req;
...
//  public:
    explicit ConnectionInfo(boost::asio::io_context& ioc, const std::string& id,
        ...
        subId(id), host(destIP), port(destPort), conn_id(conn_id)
    {
...
//    boost::asio::io_context& ioc;
    ...
    void sendData(const std::string& data)
    {
        for (unsigned int i=0; i<connections.size(); i++)
        {
            ...
            else if ((conn->state == ConnState::suspended) ||
          ...
          // TODO ME: In this loop should I handle terminated conns? Restart or close?
        }
    ...
    void addConnection(){
        ...
        ConnectionInfo* ci = new ConnectionInfo(
                        connections::systemBus->get_io_context(),
                        id,destIP,destPort,
                        destUri,httpHeader,connections.size());
        connections.push_back(ci);
        BMCWEB_LOG_DEBUG << "Added connection " <<
                         std::to_string(connections.size()-1) << " to pool " <<
                         destIP << ":" << destPort;
    }

//  public:
    //explicit ConnectionPool(boost::asio::io_context& ioc, const std::string& id,
    explicit ConnectionPool(const std::string& id,
                        const std::string& destIP, const std::string& destPort,
                        const std::string& destUri,
                        const boost::beast::http::fields& httpHeader) :
        //ioc(ioc),
...
*/
  ...
  public:
    ...
    //void createConnection(boost::asio::io_context& ioc, const std::string& id,
    void createConnection(const std::string& id,
                        const std::string& destIP, const std::string& destPort,
                        const std::string& destUri,
                        const boost::beast::http::fields& httpHeader)
    {
                        ...
                        const std::string& destPort)
    {
        ...
        BMCWEB_LOG_ERROR << "Requested connection " << destIP << ":"
                         << destPort << " does not exist.";
    }
