...
class ConnectionInfo : public std::enable_shared_from_this<ConnectionInfo>
{
  ...
  public:
    explicit ConnectionInfo(
        ...
        const boost::urls::url_view_base& hostIn, unsigned int connIdIn, const bool verifyCertIn) :
        subId(idIn),
        connPolicy(connPolicyIn), host(hostIn), connId(connIdIn), ioc(iocIn),
        resolver(iocIn), conn(iocIn), timer(iocIn), verifyCert(verifyCertIn)
    {
        initializeConnection(host.scheme() == "https");
    }
};
...
class ConnectionPool : public std::enable_shared_from_this<ConnectionPool>
{
  private:
    ...
    std::shared_ptr<ConnectionInfo>& addConnection()
    {
        ...
        auto& ret = connections.emplace_back(std::make_shared<ConnectionInfo>(
            ioc, id, connPolicy, destIP, newId, verifyCert));
  ...
  public:
    explicit ConnectionPool(
        boost::asio::io_context& iocIn, const std::string& idIn,
        const std::shared_ptr<ConnectionPolicy>& connPolicyIn,
        const boost::urls::url_view_base& destIPIn, const bool verifyCertIn) :
        ioc(iocIn),
        id(idIn), connPolicy(connPolicyIn), destIP(destIPIn), verifyCert(verifyCertIn)
    {
...
class HttpClient
{
  ...
  public:
                  ...
                  const boost::beast::http::verb verb)
    {
        ...
        sendDataWithCallback(std::move(data), destUri, verifyCert, httpHeader, verb, cb);
    }
