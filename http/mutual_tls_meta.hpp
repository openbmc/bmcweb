...
// Regex matching the Subject CommonName format used by internal Meta client certs
...
inline int mtlsMetaParseSslUser(std::string &sslUser) {
    ...
    // '<type>:<username or service name>[/<hostname>]' format and writes the resulting POSIX-compatible
    // user name in sslUser (followed by zero-byte so it's similar to X509_get_subject_name() behaviour).
    // Returns 0 on success, -1 otherwise.
    //
    // Examples:
    // "user:a_username/hostname" -> "user_a_username"
    // "svc:an_internal_service_name" -> "svc_an_internal_service_name"
    // "host:/hostname.facebook.com" -> "host_hostname" (note the stripped hostname suffix)
    const static std::regex sslUserRegex(sslUserRegexStr);

    std::smatch sm;
    if(!std::regex_match(sslUser, sm, sslUserRegex))
    {
        BMCWEB_LOG_DEBUG("Invalid Meta TLS user name: '{}', expected it to match: /{}/", sslUser, sslUserRegexStr);
        return -1;
    }
    ...
    if (userType == "host") userName = userHost;
