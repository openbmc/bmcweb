...
class StreamSocketRule : public BaseRule
{
    using self_t = StreamSocketRule;

  public:
    StreamSocketRule(std::string ruleIn) : BaseRule(std::move(ruleIn))
    {}
    ...
    {
        std::shared_ptr<
            crow::streamsocket::ConnectionImpl<boost::asio::ip::tcp::socket>>
            myConnection = std::make_shared<crow::streamsocket::ConnectionImpl<
    ...
    void handleUpgrade(const Request& req, Response&,
                       boost::beast::ssl_stream<boost::asio::ip::tcp::socket>&&
                           adaptor) override
    {
        std::shared_ptr<crow::streamsocket::ConnectionImpl<
            boost::beast::ssl_stream<boost::asio::ip::tcp::socket>>>
            myConnection = std::make_shared<crow::streamsocket::ConnectionImpl<
                boost::beast::ssl_stream<boost::asio::ip::tcp::socket>>>(
                req, std::move(adaptor), openHandler, messageHandler,
  ...
  protected:
    std::function<void(crow::streamsocket::Connection&)> openHandler;
    std::function<void(crow::streamsocket::Connection&, const std::string&,
                       bool)>
        messageHandler;
    std::function<void(crow::streamsocket::Connection&)> closeHandler;
    std::function<void(crow::streamsocket::Connection&)> errorHandler;
};
...
struct RuleParameterTraits
{
    ...
    StreamSocketRule& streamsocket()
    {
        BMCWEB_LOG_DEBUG << "Invoking stream socket rule";
        self_t* self = static_cast<self_t*>(this);
        StreamSocketRule* p = new StreamSocketRule(self->rule);
        self->ruleToUpgrade.reset(p);
        return *p;
    }
