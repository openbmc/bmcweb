...
// public PrivilegeParameterTraits<SseSocketRule>
{
  ...
  public:
    ...
    {
        std::shared_ptr<
            crow::sseSocket::ConnectionImpl<boost::asio::ip::tcp::socket>>
            myConnection = std::make_shared<
                crow::sseSocket::ConnectionImpl<boost::asio::ip::tcp::socket>>(
    ...
    void handleUpgrade(const Request& req,
                       ...
                       boost::beast::ssl_stream<boost::asio::ip::tcp::socket>&&
                           adaptor) override
    {
        std::shared_ptr<crow::sseSocket::ConnectionImpl<
            boost::beast::ssl_stream<boost::asio::ip::tcp::socket>>>
            myConnection = std::make_shared<crow::sseSocket::ConnectionImpl<
                boost::beast::ssl_stream<boost::asio::ip::tcp::socket>>>(
                req, std::move(adaptor), openHandler, closeHandler);
        myConnection->start();
    }
  ...
  private:
    std::function<void(std::shared_ptr<crow::sseSocket::Connection>&,
                       const crow::Request&,
                       const std::shared_ptr<bmcweb::AsyncResp>&)>
        openHandler;
    std::function<void(std::shared_ptr<crow::sseSocket::Connection>&)>
...
class Router
{
  public:
    ...
    {
        return std::any_of(sseSocket::sseRoutes.begin(),
                           sseSocket::sseRoutes.end(),
