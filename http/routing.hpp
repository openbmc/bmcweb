...
class Trie
{
  ...
  private:
    void optimizeNode(Node* node)
    {
        ...
        for (const std::pair<std::string, unsigned>& childNode : node->children)
        {
        ...
        if (mergeWithChild)
        {
            ...
            for (const std::pair<std::string, unsigned>& childNode :
                 node->children)
            {
                Node* child = &nodes[childNode.second];
  ...
  public:
                          ...
                          const Node* node = nullptr, unsigned pos = 0) const
    {
        ...
        for (const std::pair<std::string, unsigned>& childIndex :
             node->children)
        {
             ...
             size_t pos = 0, RoutingParams* params = nullptr) const
    {
        ...
        for (const std::pair<std::string, unsigned>& child : node->children)
        {
  ...
  private:
    void debugNodePrint(Node* n, size_t level)
    {
        ...
        for (const std::pair<std::string, unsigned>& childIndex : n->children)
        {
            BMCWEB_LOG_DEBUG
