        ...
        nlohmann::json::object_t& payload, const crow::Response& resIn)
    {
        ...
        for (size_t i = 0; i < fragRules.size(); ++i)
        {
            ...
            if (jsonPointer)
            {
                ...
                std::string oemKey = "";
                if (key.rfind(oemPrefix, 0) == 0)
                {
                    oemKey = key.substr(oemPrefix.length());
                }
                if (!oemKey.empty())
                {
                    BMCWEB_LOG_DEBUG("Adding OEM fragment key: {}", oemKey);
                    jsonKeyValuePair[i] =
                        redfish::json_util::PerUnpack{oemKey, &jsonObjects[i]};
        ...
        for (size_t i = 0; i < fragRules.size(); ++i)
        {
            ...
            {
                BMCWEB_LOG_INFO("Patch object not found for key: {}",
