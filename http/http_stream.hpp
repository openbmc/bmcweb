...
{
static std::unique_ptr<boost::asio::steady_timer> waitTimer;
...
class ConnectionImpl : public Connection
{
  public:
    ConnectionImpl(const crow::Request& reqIn, Adaptor&& adaptorIn,
        ...
        Connection(reqIn),
        adaptor(std::move(adaptorIn)), openHandler(std::move(open_handler)),
        messageHandler(std::move(message_handler)),
        closeHandler(std::move(close_handler)),
        errorHandler(std::move(error_handler)), req(reqIn)
    {
        waitTimer = std::make_unique<boost::asio::steady_timer>(*req.ioService);
    }

    ~ConnectionImpl()
    {
        BMCWEB_LOG_DEBUG << "Connection destroyed ";
    }
                     ...
                     std::function<void()> handler) override
    {
        if (msg.size())
        {
            ...
            // streamres.bufferResponse->body().commit(msg.size());
            doWrite();
        }
        else
        {
