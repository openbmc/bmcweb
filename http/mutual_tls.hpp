                   ...
                   boost::asio::ssl::verify_context& ctx)
{
    ...
    // We always return nullptr to allow full auth flow
    if (!preverified)
    {
    ...
    // Get certificate
    X509* peerCert = X509_STORE_CTX_get_current_cert(cts);
    if (peerCert == nullptr)
    {
    ...
    if (error != X509_V_OK)
    {
        BMCWEB_LOG_INFO << "Last TLS error is: " << error;
        return nullptr;
    }
    ...
    {
        BMCWEB_LOG_DEBUG
            << "Certificate ExtendedKeyUsage does not allow provided certificate to be used for user authentication";
        return nullptr;
    }
