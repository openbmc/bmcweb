...
class RfOemBaseRule : public BaseRule
{
  public:
    ...
    virtual void handle(Request& /*req*/, const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    public RuleParameterTraits<RfOemRule<Args...>>
{
  public:
    ...
    void setGetHandler(std::function<void(crow::Request&, const std::shared_ptr<bmcweb::AsyncResp>&, Args...)> newHandler) {
        getHandler = std::move(newHandler);
    }

    void setPatchHandler(std::function<void(crow::Request&, const std::shared_ptr<bmcweb::AsyncResp>&, const nlohmann::json::object_t& payload, Args...)> newHandler) {
                ...
                const std::vector<std::string>& params) override
    {
        ...
        {
            getHandler(req, asyncResp, params[0], params[1], params[2], params[3]);
        }
    ...
    void handle(Request& req,
                const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                const std::vector<std::string>& params, const nlohmann::json& payload) override
    {
        ...
        {
            patchHandler(req, asyncResp, payload, params[0], params[1], params[2]);
        }
        else if constexpr (sizeof...(Args) == 4)
        {
            patchHandler(req, asyncResp, payload, params[0], params[1], params[2], params[3]);
        }
        else if constexpr (sizeof...(Args) == 5)
        {
            patchHandler(req, asyncResp, payload, params[0], params[1], params[2], params[3],
  ...
  private:
    std::function<void(crow::Request&,
        ...
        getHandler;
    std::function<void(crow::Request&,
                       const std::shared_ptr<bmcweb::AsyncResp>&, const nlohmann::json::object_t& payload, Args...)>
        patchHandler;   
};
} // namespace crow
