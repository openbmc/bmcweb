...
{
    std::array<char,BOOST_BEAST_FILE_BUFFER_SIZE * 2>  buf_{0}; // Small buffer for reading
  ...
  public:
        ...
        get(beast::error_code& ec)
    {
        ...
        auto enclength = beast::detail::base64::encode(
            buf_.data(), ret.get().first.data(), ret.get().first.size());
        return {{const_buffers_type{buf_.data(), enclength}, ret.get().second}};
    }
};
} // namespace crow
