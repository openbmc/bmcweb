  ...
  public:
    ConnectionImpl(
        ...
        std::function<void(crow::websocket::Connection&, std::string_view,
            ...
            messageExHandlerIn,
        std::function<void(Connection&, std::string_view)> closeHandlerIn,
  ...
  private:
                       ...
                       const boost::beast::error_code& ec, size_t bytesRead)
    {
        if (ec)
        {
            ...
            if (closeHandler)
            {
                std::string_view reason{ws.reason().reason.data(),
    ...
    std::function<void(crow::websocket::Connection&, std::string_view,
                       ...
                       std::function<void()>&& whenComplete)>
        messageExHandler;
    std::function<void(Connection&, std::string_view)> closeHandler;
