...
void from_json(Value& message, const nlohmann::json& json) noexcept
{
    ...
    {
        auto* int_value = json.get_ptr<const int64_t*>();
        if (int_value != nullptr)
        {
            message.set_number_value(static_cast<double>(*int_value));
        }
    }
    else if (json.is_number_unsigned())
    {
        auto* uint_value = json.get_ptr<const uint64_t*>();
        if (uint_value != nullptr)
        {
            message.set_number_value(static_cast<double>(*uint_value));
        }
    }
