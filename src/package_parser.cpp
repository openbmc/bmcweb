    ...
    std::span<const uint8_t> bytes, uint16_t length)
{
    ...
    return std::make_pair(VendorDefined{std::move(name), std::move(data)},
                          length);
}
...
{
    // go through the images and look for where there is an intersection
    // between the left or right side of the buffer we've received
    // and the left and right side of a given image
    // case 0: left side of buffer overhangs the right side of the image
    //     truncate the bytes that we've found to the end of the image or the
    //     end of the buffer
    // case 1: no intersection
    //     drop the packet on the floor
    // case 2: right side of buffer overhangs the left side of the image
    //     send only the intersection the image, report all of the bytes
    //     consumed
    // case 4:

    // find the intersection of bytes in the buffer (w.r.t bytesReceived)
    // and the current image, if there is no intersection,
    // drop the buffer on the floor and call it all consumed
    // if it isn't aligned
    // drop the aligned piece on the floor and have the caller call us again
    size_t leftImageBoundary = images[currentImage].info.offset;
    size_t rightImageBoundary =
        images[currentImage].info.length + leftImageBoundary;
    ...
    if (leftBufferBoundary < rightImageBoundary &&
        rightBufferBoundary > rightImageBoundary)
    {
        // only the first portion of the buffer applies to this image
    }

    if (leftBufferBoundary < leftImageBoundary &&
        rightBufferBoundary >= leftImageBoundary)
    {
        // the first portion of this buffer does not apply to this image
    }

    if (leftBufferBoundary < leftImageBoundary &&
        rightBufferBoundary <= leftImageBoundary)
    {
        // none of this buffer applies to this image
    }

    if (leftBufferBoundary >= leftImageBoundary &&
        rightBufferBoundary <= rightImageBoundary)
    {
        // the whole buffer applies
    }
    return 0;
}

bool PackageParser::setupComponents()
{
    ...
    struct DescriptorMatcher
    {
        ...
        std::vector<desc::Descriptor> desc;
    };

    std::vector<DescriptorMatcher> matchers;
    for (uint8_t idCtr = 0; idCtr < *idRecordCount; ++idCtr)
    {
        ...
        success = parser.get(matcher.components, componentBitMapLengthBytes);
        if (!success)
        {
            return false;
        }
    ...
    for (uint16_t imageCtr = 0; imageCtr < *componentImageCount; ++imageCtr)
    {
