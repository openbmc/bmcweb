#include <http/ut/app_in_test.hpp>
...
TEST(SecurityHeaders, TestHeadersExist)
{
...

    {
        ...
        for (int i = 0; i < 200; i++)
        {
            ...
            catch (const std::exception& e)
            {
                ...
                // starting up
                BMCWEB_LOG_DEBUG << "Server isn't up yet; iterations=" << i;
                sleep(1);
            }
        }
    }
    ...
    BMCWEB_LOG_DEBUG << "Sent the request";
    c.receive(boost::asio::buffer(buf));
    BMCWEB_LOG_DEBUG << "Received the request";
    c.close();
