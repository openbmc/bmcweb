...
static void setVerifyMode(const std::shared_ptr<boost::asio::ssl::context>& ctx,
...
static std::string mtls_SSL_get_servername(SSL* ssl, int type)
{
    ...
    size_t sniLen = 0;
    int result =
        SSL_client_hello_get0_ext(ssl, TLSEXT_TYPE_server_name, &sni, &sniLen);

    BMCWEB_LOG_DEBUG("SSL_client_hello_get0_ext result: {}, sniLen: {}", result,
                     sniLen);

    if (result != 1 || sniLen < SNI_MIN_LENGTH)
    {
        BMCWEB_LOG_DEBUG("No valid SNI extension");
        return "";
    }

    std::span<const unsigned char> sniSpan(sni, sniLen);
    size_t pos = 0;

    // Parse server_name_list_length (2 bytes)
    if (pos + 2 > sniLen)
    {
        BMCWEB_LOG_ERROR("Buffer too small for list length");
        return "";
    }
    uint16_t listLen = SNI_READ_UINT16(sniSpan, pos);
    pos += 2;

    BMCWEB_LOG_DEBUG("Server name list length: {}", listLen);

    // Parse server_name_type (1 byte)
    if (pos + 1 > sniLen)
    {
        BMCWEB_LOG_ERROR("Buffer too small for name type");
        return "";
    }
    uint8_t nameType = SNI_READ_UINT8(sniSpan, pos);
    pos += 1;

    BMCWEB_LOG_DEBUG("Server name type: {}", nameType);

    // Check if it's a hostname (type 0)
    if (nameType != 0)
    {
        BMCWEB_LOG_DEBUG("Not a hostname type: {}", nameType);
        return "";
    }

    // Parse server_name_length (2 bytes)
    if (pos + 2 > sniLen)
    {
        BMCWEB_LOG_ERROR("Buffer too small for name length");
        return "";
    }
    uint16_t nameLen = SNI_READ_UINT16(sniSpan, pos);
    pos += 2;
    BMCWEB_LOG_DEBUG("Server name length: {}", nameLen);

    // Parse server_name (nameLen bytes)
    if (pos + nameLen > sniLen)
    {
        BMCWEB_LOG_ERROR("Name length exceeds buffer: {} + {} > {}", pos,
                         nameLen, sniLen);
        return "";
    }

    std::string hostname(reinterpret_cast<const char*>(&sniSpan[pos]), nameLen);
    BMCWEB_LOG_DEBUG("Extracted SNI hostname: '{}'", hostname);
