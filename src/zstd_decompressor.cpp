    ...
    ZstdDecompressor::decompress(boost::asio::const_buffer buffIn)
{
    ...
    unsigned long long frameSize = ZSTD_getFrameContentSize(buffIn.data(),
                                                            buffIn.size());

    if (ZSTD_isError(frameSize) == 0)
    {
        compressionBuf.reserve(frameSize);
    }
    ...
    {
        constexpr size_t contentSize = 4096;
        auto buffer = compressionBuf.prepare(contentSize);
