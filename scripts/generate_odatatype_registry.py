...
def main():
    ...
    with open(path, "w") as registry:
        ...
        for entity in entity_tag_list:
            ...
            registry.write("const std::array<std::string_view, {}> {}Uris = ".format(
                len(uri_set), entity))
            registry.write("{{\n")
        ...
        registry.write("const static std::array<const std::span<const std::string_view>, {}> entityTypeToUriMap".format(
            len(entity_tag_list)))
...
# std::array<std::span<const std::string>
