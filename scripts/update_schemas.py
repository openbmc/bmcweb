...
with open(os.path.join(static_path, "meson.build"), "w") as meson_file:
    meson_file.write(
        "install_data('$metadata/index.xml', install_dir: 'share/www/redfish/v1/$metadata')\n".format(
            schema_file
        )
    )

    for csdl_file in csdl_filenames:
        csdl_file_split = csdl_file.split("_")
        if csdl_file_split[0] not in include_list:
            continue
        meson_file.write(
            "install_data('schema/{}', install_dir: 'share/www/redfish/v1/schema/')\n".format(
                csdl_file
            )
        )
    ...
    for schema_file in included_json_schema_files:
        ...
        meson_file.write(
            "install_data('JsonSchemas/{0}/{0}.json', install_dir: 'share/www/redfish/v1/JsonSchemas/{0}')\n".format(
                schema_file
            )
        )
