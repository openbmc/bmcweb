...
with open(args.file) as error_file:
    ...
    with open(args.base) as base_file:
        ...
        for message_name, message_data in base_json['Messages'].items():
            message_id = base_json['RegistryPrefix'] + '.' + base_json['RegistryVersion'] + '.' + message_name
            message_found = False
            index = 0
            ...
            error_str = re.sub('std::to_string\(arg(\d+)\)', 'arg\\1', error_str)
            error_str = re.sub('"\n\s*"', '', error_str, re.MULTILINE)
            error_str = re.sub('"\s*\+\s*arg(\d+)\s*\+\n?\s*"', '%\\1', error_str, re.MULTILINE)
            if message_data['Message'] not in error_str:
                print("{}: error in Message: {}".format(message_id, message_data['Message']))
                print(error_str)

            if message_data['Severity'] not in error_str:
                print("{}: error in Severity: {}".format(message_id, message_data['Severity']))
                print(error_str)

            if message_data['Resolution'] not in error_str:
                print("{}: error in Resolution: {}".format(message_id, message_data['Resolution']))
                print(error_str)
