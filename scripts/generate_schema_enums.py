...
OUTFILE = os.path.realpath(
    os.path.join(SCRIPT_DIR, "..", "redfish-core", "include", "redfish_defs.hpp")
)
...
def generate_enums(flat_list):
    ...
    with open(OUTFILE, "w") as redfish_defs:
        ...
        for element in enum_list:
            ...
            enum_name = "{}_{}".format(namespace_split, element.name)
            ...
            print(element.name)
        redfish_defs.write("// clang-format on\n")
...
def main():
    ...
    filepaths = [os.path.join(REDFISH_SCHEMA_DIR, filename) for filename in dir_list]
    if multithread:
