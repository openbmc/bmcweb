...
def parse_node(target_entitytype, path, top_collections, found_top, xml_file,
    ...
    parse_root(root, target_entitytype, path, top_collections, found_top,
               xml_map, depth)
...
def parse_root(root, target_entitytype, path, top_collections, found_top,
               xml_map, depth):
    ...
    for schema in ds.findall(EDM + "Schema"):
        for entity_type in schema.findall(EDM + "EntityType"):
            ...
            for nav_prop in entity_type.findall(EDM + "NavigationProperty"):
                parse_navigation_property(root, name, nav_prop, path,
                                          top_collections, found_top, xml_map,
                                          depth)
...
#                    and (name != target_entitytype)):
#            if ((name != "GenZ") and (name != target_entitytype)):
#                continue
            if (name != target_entitytype):
                continue
            print("ComplexType match to: ", name)
...
def parse_navigation_property(root, curr_entitytype, element, path,
                              top_collections, found_top, xml_map, depth):
    ...
    if ("Collection" in nav_type):
        ...
        # type is "<Namespace>.<TypeName>", both should end with "Collection"
            ...
            if ((len(nav_type_split) != 2) and (nav_type_split[0] !=
                                                nav_type_split[1])):
        ...
        # At this point we're ready to either reparse this file or parse a
        # different file
    ...
    if (new_seg == "AccountService"):
        parse_node("ExternalAccountProvider", new_path + "/ActiveDirectory",
                   top_collections, found_top, xml_map[file_key], depth+2)
        parse_node("ExternalAccountProvider", new_path + "/LDAP",
                   top_collections, found_top, xml_map[file_key], depth+2)
        ...
        parse_node("Ethernet", new_path + "/Ethernet", top_collections,
                   found_top, xml_map[file_key], depth+2)

    # Other Property treated this way are:
    ...
    if (new_seg == "Metrics"):
        if ((curr_entitytype == "Processor") or
                (curr_entitytype == "Memory") or
                (curr_entitytype == "Switch")):
    ...
    if (file_key in xml_map):
        parse_node(typename, new_path, top_collections, found_top, xml_map[file_key],
                   depth+1)
    else:
        parse_root(root, typename, new_path, top_collections, found_top, xml_map,
                   depth)
...
def main():
    ...
    print("Path is: ", curr_path)
    seen_paths.add(curr_path)

    # ServiceRoot_v1.xml also shows this as an example
    print("Path is: ", curr_path + "/")
    seen_paths.add(curr_path + "/")
    parse_node("ServiceRoot", curr_path, top_collections, False,
               "ServiceRoot_v1.xml", 2)
