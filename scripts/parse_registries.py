...
def make_privilege_registry() -> None:
    ...
    with open(path, "w") as registry:
        ...
        for mapping in json_file["Mappings"]:
            ...
            if "SubordinateOverrides" in mapping.keys() and len(mapping["SubordinateOverrides"]) > 0:
                for subordinateOverride in mapping["SubordinateOverrides"]:
                    for operation, privilege_list in subordinateOverride["OperationMap"].items():
                        privilege_dict[
                            get_privilege_string_from_list(privilege_list)
                        ] = (
                            privilege_list,
        ...
        for mapping in json_file["Mappings"]:
            ...
            if "SubordinateOverrides" in mapping.keys() and len(mapping["SubordinateOverrides"]) > 0:
                for subordinateOverride in mapping["SubordinateOverrides"]:
                    targetsStr = "Sub" + "Sub".join(subordinateOverride["Targets"][::-1])
                    for operation, privilege_list in subordinateOverride["OperationMap"].items():
                        privilege_string = get_privilege_string_from_list(
                            privilege_list
                        )
                        ...
                        registry.write(
                            "const static auto& {}{}{} = privilegeSet{};\n".format(
                                operation, entity, targetsStr, privilege_dict[privilege_string][1]
                            )
                        )
            registry.write("\n")
        registry.write(
