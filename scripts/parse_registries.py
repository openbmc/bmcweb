...
inline const std::vector<Privileges>
    getPrivilegesFromUrlAndMethod(std::string url,
                                  HttpVerb method)
{
...
def make_privilege_registry():
    ...
    with open(path, 'w') as registry:
        ...
        for mapping in json_file["Mappings"]:
            ...
            registry.write("// {}\n".format(entity))
            registry.write(
                "std::map<HttpVerb, std::vector<Privileges>> tag{}Map = ".format(entity))
            registry.write(" {\n")
            for operation, privilege_list in mapping["OperationMap"].items():
                privilege_string = get_privilege_string_from_list(
                    privilege_list)
                ...
                registry.write("\t{")
                registry.write(
                    "HttpVerb::{}, std::vector<Privileges>(privilegeSet{}.begin(),privilegeSet{}.end())".format(
                        operation,
                        privilege_dict[privilege_string][1], privilege_dict[privilege_string][1]))
                registry.write("},\n")
            registry.write("};\n")
            registry.write("\n")
