...
OPENBMC_REGISTRIES_BASE_URL = "https://raw.githubusercontent.com/openbmc/bmcweb/refs/heads/master/redfish-core/include/registries/"

DBUS_IFACES_BASE_PATH = "../phosphor-dbus-interfaces"

DBUS_NAMESPACE_PREFIX = "xyz.openbmc_project."

RegistryInfo: t.TypeAlias = t.Tuple[str, str, str, str]
...
def openbmc_name_to_snake_case(text):
...
def update_dbus_mapping(
    dbus_registries_map: OrderedDict[str, DBusToRedfish],
) -> None:
...
def parse_dbus_oem_file_messages(
    ...
    with open(full_path, "rb") as fd:
        ...
        target_json_path = f"{registry_name.lower()}.json"
        path = os.path.join(
        ...
        if registry_name not in registries:
            registries[registry_name] = (
                ...
                json_file,
                registry_name,
                os.path.join(OPENBMC_REGISTRIES_BASE_URL, target_json_path),
            )
        else:
            for message_id, message in messages.items():
                if message_id in registries[registry_name][1]["Messages"]:
                    raise ValueError(
                        ...
                        f"{file.registry_name}. Required by {file}"
                    )
                registries[file.registry_name][1]["Messages"][
                    message_id
                ] = message
    ...
    shutil.copy2(full_path, os.path.join(INCLUDE_PATH, target_json_path))
