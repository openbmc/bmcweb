...
def clang_format(filename):
    subprocess.check_call(["clang-format-10", "-i", filename])
...
def create_Subordinate_Overrides_target_list(target_list):
    target_list_create = []
        ...
        for mapping in json_file["Mappings"]:
            ...
            if 'SubordinateOverrides' in mapping:
                for subordinateOverrides in mapping["SubordinateOverrides"]:
                    target_list_list = create_Subordinate_Overrides_target_list(subordinateOverrides["Targets"])
                    for target_list in target_list_list:
                        ...
                        for operation, privilege_list in subordinateOverrides["OperationMap"].items():
                            ...
                            registry.write("const static auto& {}{}SubOver{} = privilegeSet{};\n".format(
                                operation, entity, concateVarName, privilege_dict[privilege_string][1]))
                        registry.write("\n")
                registry.write("\n")
        registry.write("} // namespace redfish::privileges\n")
    clang_format(path)
