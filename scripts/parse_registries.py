...
def get_response_code(entry_id, entry):
    ...
    if "Value" in entry_id:
        return 'bad_request'
    elif "Unrecognized" in entry_id:
        return 'bad_request'
    elif "Invalid" in entry_id or "Malformed" in entry_id or "NotSupported" in entry_id:
        return 'bad_request'
    elif entry_id in bad_request:
        return 'bad_request'
    elif internal_error in internal_error:
...
def make_error_function(entry_id, entry, is_header):
        ...
        if argtypes:
            ...
            for index, typename in enumerate(argtypes):
                ...
                if typename == 'const nlohmann::json&':
                    out += f"std::string arg{index}Str = arg{index}.dump(2, ' ', true, nlohmann::json::error_handler_t::replace);\n"
            for index, typename in enumerate(argtypes):
                index += 1
    ...
    if is_header:
        ...
        args_out = ", ".join([f"arg{x+1}" for x in range(len(args) - 1)])
