...
def make_error_function(entry_id, entry, is_header, registry_name, namespace_name):
    arg_nonstring_types = {
...
def create_error_registry(entry, registry_version, registry_name, namespace_name, filename):
...
"""
        ...
        for entry_id, entry in messages.items():
            ...
            out.write(make_error_function(entry_id, entry, True, registry_name, namespace_name))
        out.write("    }\n")
        out.write("}\n")
...
""")
        ...
        out.write(f'#include "registries/{reg_name_lower}_message_registry.hpp"')
        out.write(
"""
...
"""
            ...
            message = entry["Message"]
            out.write(make_error_function(entry_id, entry, False, registry_name, namespace_name))
...
def main():
    ...
    create_error_registry(files[0], dmtf_registries[0][1], 'Base', "base", "error")
    create_error_registry(files[5], dmtf_registries[5][1], 'HeartbeatEvent', 'heartbeat', "heartbeat")
    create_error_registry(files[12], dmtf_registries[12][1], 'ResourceEvent', 'resource_event', "resource")
    create_error_registry(files[15], dmtf_registries[15][1], 'TaskEvent', 'task_event', "task")
