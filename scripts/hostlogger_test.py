...
def main():
    ...
    while 1:
        ...
        if id == entryCount:
            # entryCount equals to ID means there has no change during the interval, sleep 5 seconds for next search.
            time.sleep(5)
        elif id < entryCount:
            # print new entries which created in this interval.
            for i in range(id + 1, entryCount):
                singleEntryUri = "https://{}/redfish/v1/Systems/system/LogServices/HostLogger/Entries/{}".format(
                    args.host, i
                )
