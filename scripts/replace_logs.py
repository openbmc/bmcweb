...
"""
This script is intended to aid in the replacement of bmcweb log lines from the form
...
ptrs need to be wrapped in fmt::ptr().  This script tries to do the common
cases, but can't handle every one.

types of boost::ip::tcp::endpoint/address needs .to_string() called on it in the
args section
...
for filename in filenames:
    ...
    for line in contents.splitlines():
        match = re.match("\s+BMCWEB_LOG_", line)
        if match:
            replacing = True
    ...
    for line in contents.splitlines():
        ...
        match = re.match("(\s+BMCWEB_LOG_[A-Z]+) <<", line)
        if match:
            logstring = ""
            line = line.lstrip()
            ...
            for log_piece in chevron_split[1:]:
                ...
                if (log_piece.startswith('"') and log_piece.endswith('"')) or (
                    ...
                    if log_piece == "this" or log_piece.startswith("&"):
                        log_piece = "fmt::ptr({})".format(log_piece)
                    if log_piece == "self":
                        log_piece = "fmt::ptr(self.get())"

                    arguments.append(log_piece)
                    logstring += "{}"
            arguments.insert(0, '"' + logstring + '"')
            line = match.group(1) + "(" + ", ".join(arguments) + ");"
        new_contents += line + "\n"
