...
def main():
    ...
    parser.add_argument(
        "--password",
        help="""Password for user in order to install certs over
        Redfish.""",
        default="0penBmc",
    )
    ...
    clientKey, clientCert = generateCert(
        username, clientExtensions, caKey, caCert
    )
    ...
    if response.reason == "Internal Server Error":
        ...
        raise Exception(
            """Unauthorized Redfish access. Check username,
        ...
        raise Exception(
            "Could not install certificate. Reason: " + response.reason
        )
    serverCertJSON = {}
    ...
    if response.reason == "OK":
        ...
        raise Exception(
            "Could not enable TLS authenticaton. Reason: " + response.reason
        )
    ...
    if response.reason == "OK":
        ...
        raise Exception(
            "Redfish TLS authentication error. Reason: " + response.reason
        )
