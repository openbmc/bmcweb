...
def generateIntermediateCA(ca_key, ca_cert, name_prefix):
    ...
    name = x509.Name(
        [
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "NVIDIA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "NBU"),
            x509.NameAttribute(NameOID.COMMON_NAME, f"{name_prefix}-ca.bluefield.nbu.nvidia"),
        ]
    )
    builder = builder.subject_name(name)
    builder = builder.issuer_name(ca_cert.subject)
    ...
    builder = builder.not_valid_after(
        datetime.datetime(2043, 1, 1, 0, 0, tzinfo=datetime.timezone.utc)
    )
    builder = builder.serial_number(x509.random_serial_number())
    builder = builder.public_key(public_key)
...
def generate_and_load_certs(url, username, password, use_intermediate=False):
    ...
    if use_intermediate:
        ...
        server_intermediate_key_dump = server_intermediate_key.private_bytes(
            encoding=serialization.Encoding.PEM,
            format=serialization.PrivateFormat.TraditionalOpenSSL,
            encryption_algorithm=serialization.NoEncryption(),
        )
        server_intermediate_cert_dump = server_intermediate_cert.public_bytes(
            encoding=serialization.Encoding.PEM
        )
        server_intermediate_cert_filename = os.path.join(certs_dir, "server-intermediate-cert.cer")
        server_intermediate_key_filename = os.path.join(certs_dir, "server-intermediate-key.pem")
    else:
