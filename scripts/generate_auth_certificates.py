...
def test_mtls_auth(client, certs_dir):
    cn_transport = httpx.HTTPTransport(
    ...
    client._transport = cn_transport
    ...
    upn_transport = httpx.HTTPTransport(
        verify=os.path.join(certs_dir, "CA-cert.cer"),
        cert=(
    ...
    client._transport = upn_transport
...
def generate_and_load_certs(url, username, password):
    ...
    with httpx.Client(
        ...
        del redfish_session.headers["X-Auth-Token"]

        print("Testing redfish TLS authentication with generated certs.")
        time.sleep(2)
        test_mtls_auth(redfish_session, certs_dir)
