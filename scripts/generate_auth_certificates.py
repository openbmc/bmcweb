...
def generateCert(commonName, extensions, caKey, caCert):
    ...
    cert.add_extensions([
        crypto.X509Extension(b"authorityKeyIdentifier", False, b"keyid",
                             issuer=caCert)
    ])
...
def main():
    ...
    parser.add_argument(
        "--password", help="""Password for user in order to install certs over
        Redfish.""", default="0penBmc"
    )
    ...
    clientKey, clientCert = generateCert(username, clientExtensions, caKey,
                                         caCert)
    ...
    response = requests.post("https://" + host + caCertPath, verify=False,
                             json=caCertJSON, auth=(username, password))
    if (response.reason == "Internal Server Error"):
        print("""An existing CA certificate is likely already installed.
                Replacing...""")
        caCertificateUri = {}
        caCertificateUri["@odata.id"] = caCertPath + "/1"
        caCertJSON["CertificateUri"] = caCertificateUri
        response = requests.post("https://" + host + replaceCertPath,
                                 verify=False, json=caCertJSON,
                                 auth=(username, password))
        if (response.reason == "OK"):
            print("Successfully replaced existing CA certificate.")
        else:
            raise Exception("""Could not install or replace CA certificate.
        ...
        raise Exception("""Unauthorized Redfish access. Check username,
                        password, and user priveleges.""")
    elif (response.reason == "OK"):
        print("Successfully installed CA certificate.")
    else:
        raise Exception("Could not install certificate. Reason: " +
                        response.reason)
    serverCertJSON = {}
    serverCertJSON["CertificateString"] = serverKeyDump + serverCertDump
    serverCertificateUri = {}
    serverCertificateUri["@odata.id"] = """/redfish/v1/Managers/bmc/
    ...
    response = requests.post("https://" + host + replaceCertPath, verify=False,
                             json=serverCertJSON, auth=(username, password))
    if (response.reason == "OK"):
        print("Successfully replaced server certificate.")
    else:
        raise Exception("Could not replace certificate. Reason: " +
                        response.reason)
    tlsPatchJSON = {"Oem": {"OpenBMC": {"AuthMethods": {"TLS": True}}}}
    print("Ensuring TLS authentication is enabled.")
    response = requests.patch("https://" + host + "/redfish/v1/AccountService",
                              verify=False, json=tlsPatchJSON,
                              auth=(username, password))
    if (response.reason == "OK"):
        print("Successfully enabled TLS authentication.")
    else:
        raise Exception("Could not enable TLS authenticaton. Reason: " +
                        response.reason)
    print("Testing redfish TLS authentication with generated certs.")
    response = requests.get("https://" + host + """/redfish/v1/SessionService/
                            Sessions""", verify=False, cert=(
                            "certs/client-cert.pem", "certs/client-key.pem"))
    if (response.reason == "OK"):
        print("Redfish TLS authentication success!")
    else:
        raise Exception("Redfish TLS authentication error. Reason: " +
                        response.reason)
    ...
    with open("certs/client.p12", "wb") as f:
        ...
        print("""Client p12 cert file generated and stored in
                ./certs/client.p12.""")
        print("""Copy this file to a system with a browser and install the
                cert into the browser.""")
        print("""You will then be able to test redfish and webui
                authentication using this certificate.""")
        print("""Note: this p12 file was generated without a password, so it
