#!/usr/bin/env python3
"""
Script to generate a certificates for a CA, server, and client allowing for
...
class RedfishSessionContext:
    ...
    def __exit__(self, type, value, traceback):
        # print("Cleaning up Redfish session")
...
def generateclient_keyAndCert(commonName, ca_cert, ca_key):
    private_key = ec.generate_private_key(ec.SECP256R1())
    public_key = private_key.public_key()
    builder = x509.CertificateBuilder()
...
def install_server_cert(redfish_session, manager_uri, serverCertDump):

    serverCertJSON = {
        "CertificateString": serverCertDump.decode(),
        "CertificateUri": {
    ...
    response = redfish_session.post(replaceCertPath, json=serverCertJSON)
    if response.status_code == 200:
    ...
    response = redfish_session.patch(
        "/redfish/v1/AccountService", json=tlsPatchJSON
    )
    if response.status_code == 200:
...
def setup_server_cert(
    ...
    serverCertDump = serverCert.public_bytes(
    ...
    install_server_cert(redfish_session, manager_uri, serverCertDump)
...
def generate_and_load_certs(url, username, password):
    ...
    ca_certFilename = os.path.join(certs_dir, "CA-cert.cer")
    ca_keyFilename = os.path.join(certs_dir, "CA-key.pem")
    if not os.path.exists(ca_certFilename):
        ca_key, ca_cert = generateCA()

        ca_keyDump = ca_key.private_bytes(
            encoding=serialization.Encoding.PEM,
        ...
        with open(ca_certFilename, "wb") as f:
            f.write(ca_cert_dump)
            print("CA cert generated.")
        with open(ca_keyFilename, "wb") as f:
            f.write(ca_keyDump)
            print("CA key generated.")

    with open(ca_certFilename, "rb") as ca_cert_file:
        ca_cert_dump = ca_cert_file.read()
    ca_cert = x509.load_pem_x509_certificate(ca_cert_dump)

    with open(ca_keyFilename, "rb") as ca_key_file:
        ca_keyDump = ca_key_file.read()
    ca_key = load_pem_private_key(ca_keyDump, None)

    client_key, client_cert = generateclient_keyAndCert(
        username, ca_cert, ca_key
    )
    client_keyDump = client_key.private_bytes(
        encoding=serialization.Encoding.PEM,
        format=serialization.PrivateFormat.TraditionalOpenSSL,
        encryption_algorithm=serialization.NoEncryption(),
    )
    ...
    with open(os.path.join(certs_dir, "client-key.pem"), "wb") as f:
        f.write(client_keyDump)
        print("Client key generated.")
    client_certDump = client_cert.public_bytes(
        encoding=serialization.Encoding.PEM
    )

    with open(os.path.join(certs_dir, "client-cert.pem"), "wb") as f:
        f.write(client_certDump)
